...
namespace
{
    ...
    auto RunTest(const StressConfig &config) -> LatencyStats
    {
        ...
        for (std::size_t i = 0; i < config.iterations; ++i)
        {
            ...
            const auto elapsed = t1.time_since_epoch() - t0.time_since_epoch();
            const auto ns_count = std::chrono::duration_cast<std::chrono::nanoseconds>(elapsed).count();
            std::uint64_t ns = static_cast<std::uint64_t>(ns_count);
