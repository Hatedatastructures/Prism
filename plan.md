...
> - 全项目源码深度审计（45 测试文件、12 基准套件、~9000 行源码）
> - 与 mihomo/Xray/sing-box/HAProxy/shadowsocks-rust 等同类项目性能对比
>
> **修订说明** (2026/05/06):
> - 反映 recognition 模块重构：`clienthello/` → `arrival/` → 整体移除，TLS 解析下沉至 `protocol/tls/`
> - 反映 stealth 模块新增 `registry.hpp`/`executor.hpp`（方案注册与执行管道）
> - 更新 TODO 状态（原 7 个缩减为 3 个活跃 TODO）
> - 更新 dynamic_cast 状态（3 处 → 4 处）
> - 更新测试/基准文件清单
> - 调整任务优先级以匹配当前架构
...
| Reality TLS 伪装 | 完成 | `src/prism/stealth/reality/` (7 个 cpp) |
| ShadowTLS v2 | 完成 | `src/prism/stealth/shadowtls/` (3 个 cpp) |
...
| TLS 共享层 | 完成 | `include/prism/protocol/tls/types.hpp`, `signal.hpp` |
| 协议识别 (Probe→Stealth) | 完成 | `src/prism/recognition/` + `src/prism/stealth/executor.cpp` |
...
| # | TODO 位置 | 问题描述 | 优先级 | 状态 |
|---|----------|---------|--------|------|
| 1 | `src/prism/stealth/restls/scheme.cpp:68` | Restls 握手为 TODO 桩，直接返回 `protocol_type::tls` | CRITICAL | 活跃 |
| 2 | `src/prism/resolve/router.cpp:52` | 正向代理 `async_positive()` 返回 `not_supported` | HIGH | 活跃 |
| 3 | `src/prism/stealth/reality/handshake.cpp:429` | 类型统一 TODO：`client_hello_features` 与 `client_hello_info` 消除转换 | MEDIUM | 活跃 |

> **已消除的旧 TODO**（随重构解决或代码移除）：
> - ~~`arrival/ech.hpp:50,55`~~ — `arrival/` 子模块整体移除（commit `1687fe0`）
> - ~~`arrival/anytls.hpp:47,54`~~ — 同上
> - ~~`eyeball/racer.hpp:28`~~ — 已随重构解决
> - ~~`upstream.cpp:19`~~ — 截断处理已实现并测试
> - ~~`multiplex/yamux.hpp:21`~~ — 窗口流控已完整实现
...
| 问题 | 位置 | 影响 |
|------|------|------|
| `dynamic_cast` 4 处 | `restls/scheme.cpp:59`, `shadowtls/scheme.cpp:67`, `shadowtls/handshake.cpp:334` | 依赖 RTTI，包装传输时失败 |
...
### 1.5 最近重大重构摘要（2026/05/01 — 2026/05/06）

以下重构已在计划编制后完成，影响后续任务的前置条件：

| 重构 | 提交 | 变更内容 | 对计划的影响 |
|------|------|---------|-------------|
| recognition 瘦身为纯调度 | `1687fe0` | `arrival/` 子模块移除；`handshake/` 提升至 stealth 层；TLS 解析下沉到 `protocol/tls/` | 任务 3.2/3.3 的 ECH/AnyTLS 分析器已随代码删除，需重新设计接入点 |
| stealth 承接方案注册与执行 | `1687fe0` | 新增 `registry.hpp`（方案注册表单例）+ `executor.hpp`（执行器管道） | 任务 3.1 的 Restls 实现需适配新的 registry/executor 架构 |
| protocol/tls 共享层 | `1687fe0` | 新增 `types.hpp`（TLS 常量 + `client_hello_features`）和 `signal.hpp`/`signal.cpp`（ClientHello 解析器） | 任务 3.1 的 Restls 可直接复用 `protocol::tls::parse_client_hello()` |
| 基准测试重构 | `1466d1d` | 移除虚测试；新增 IOBench/LatencyBench/ProtocolBench 等 8 个基准文件 | 性能瓶颈数据可能已更新，需重新验证基线 |
| recognition 文档与重命名 | `65e1b73`, `3b33a2b` | `clienthello/` → `arrival/` → 移除；去死代码、解重复 | 测试文件结构已更新 |
...
**验证**: 运行 `build_release/benchmarks/MemoryBench.exe`
...
- `include/prism/channel/eyeball/racer.hpp` — Happy Eyeballs
...
**架构适配要点**（基于 2026/05/06 重构后的新架构）：

Restls 实现需适配新的 stealth 架构：
- 方案注册通过 `stealth::scheme_registry`（`include/prism/stealth/registry.hpp`）
- 方案执行通过 `stealth::scheme_executor`（`include/prism/stealth/executor.hpp`）
- TLS ClientHello 解析复用 `protocol::tls::parse_client_hello()`（`include/prism/protocol/tls/signal.hpp`）
- `scheme::execute()` 接收 `scheme_context`，返回 `scheme_result`
...
2. 在 `handshake.cpp` 中实现：
   ...
   - 通过 `channel::eyeball::racer` 连接到后端 TLS 服务器
   - 建立后端 TLS 连接后，进入认证循环
...
#### 任务 3.2: 统一 client_hello_features 类型（消除 reality/handshake.cpp:429 TODO）

**涉及文件**:
- `include/prism/protocol/tls/types.hpp` — `client_hello_features` 结构体（权威定义）
- `src/prism/stealth/reality/handshake.cpp:429` — 当前存在 `client_hello_info` 冗余类型

**当前问题**: reality/handshake.cpp:429 有 TODO：`统一 client_hello_features 与 client_hello_info 类型，消除此转换`。重构后 `protocol/tls/types.hpp` 中的 `client_hello_features` 已成为权威 TLS 特征结构，但 reality 模块仍有自己的 `client_hello_info`，需要手动转换。

**修改步骤**:

1. 审查 `client_hello_info` 与 `client_hello_features` 的字段差异
2. 将 reality 模块中的 `client_hello_info` 替换为 `protocol::tls::client_hello_features`
3. 消除 handshake.cpp:429 处的类型转换代码
4. 确认 `protocol::tls::client_hello_features` 的字段足以覆盖 reality 的所有需求（SNI、session_id、random、key_share 等）
5. 如有缺失字段，在 `types.hpp` 中补充

**验证**: `build_release/tests/Reality.exe` + `build_release/tests/Recognition.exe`

---

#### 任务 3.3: ECH/AnyTLS 方案重新接入【适配新架构】

> **注意**: 原计划的 ECH 分析器（`arrival/ech.hpp`）和 AnyTLS 分析器（`arrival/anytls.hpp`）已随 recognition 重构被移除。需要在新架构下重新设计接入点。

**涉及文件**:
- `include/prism/stealth/registry.hpp` — 方案注册表
- `include/prism/stealth/executor.hpp` — 方案执行器
- **新建**: `include/prism/stealth/ech/scheme.hpp`
- **新建**: `src/prism/stealth/ech/scheme.cpp`
- **新建**: `include/prism/stealth/anytls/scheme.hpp`
- **新建**: `src/prism/stealth/anytls/scheme.cpp`

**新架构下的接入方式**:

在旧架构中，ECH/AnyTLS 通过 `arrival::registry` 注册分析器，在 ClientHello 阶段被识别。新架构中，识别流程变为：

```
probe::detect() → protocol_type (HTTP/SOCKS5/TLS/SS2022)
       │ (仅 TLS)
       ▼
stealth::scheme_executor::execute():
  按候选顺序尝试 reality → shadowtls → restls → ... → native
  每个 scheme 的 execute() 返回 scheme_result：
    - detected = protocol_type::tls → "不是我，继续下一个"
    - detected = 其他 → "匹配成功，使用此方案"
```

**ECH 方案接入**:
- ECH 的 ClientHello 特征（`has_ech_extension`）可在 `protocol::tls::client_hello_features` 中检测
- 实现 `stealth::ech::scheme`，在 `execute()` 中检查 ClientHello 是否含 ECH 扩展
- 注册到 `scheme_registry`，优先级在 reality 之前

**AnyTLS 方案接入**:
- AnyTLS 无 ClientHello 层特征（外观为标准 TLS），正确策略是作为 fallback scheme
- 实现 `stealth::anytls::scheme`，在 `execute()` 中读取 TLS 应用数据，前 32 字节与 `SHA256(password)` 比对
- 注册到 `scheme_registry`，优先级在 restls 之后、native 之前

**实施步骤**:

1. 在 `config.hpp` 中添加 `stealth.ech` 和 `stealth.anytls` 配置
2. 实现 `stealth::ech::scheme` 和 `stealth::anytls::scheme`
3. 在 `register_all_schemes()` 中注册新方案
4. 更新 `include/prism/stealth.hpp` 聚合头文件

**验证**: 新建 `tests/EchScheme.cpp` 和 `tests/AnyTls.cpp`
...
**涉及文件**:
- `src/prism/stealth/restls/scheme.cpp:59` — `dynamic_cast<channel::transport::reliable *>`
- `src/prism/stealth/shadowtls/scheme.cpp:67` — `dynamic_cast<channel::transport::reliable *>`
- `src/prism/stealth/shadowtls/handshake.cpp:334` — `dynamic_cast<channel::transport::reliable *>`（新增）
- `include/prism/channel/transport/transmission.hpp` — 基类（当前有 `is_reliable()` 虚方法）
- `include/prism/channel/transport/reliable.hpp` — 派生类

**当前问题**（4 处 dynamic_cast）:
```cpp
// restls/scheme.cpp:59:
auto *rel = dynamic_cast<channel::transport::reliable *>(ctx.session->inbound.get());

// shadowtls/scheme.cpp:67:
auto *rel = dynamic_cast<channel::transport::reliable *>(ctx.session->inbound.get());

// shadowtls/handshake.cpp:334:
...
3. 重构后新增了第 4 处，问题比计划时更严重

**修改步骤**:

1. 在 `include/prism/channel/transport/transmission.hpp` 中添加虚方法（`is_reliable()` 已存在，补充 `raw_socket()`）：
   ```cpp
   /**
...
3. 替换全部 4 处 `dynamic_cast`:
   ```cpp
   // 替换前:
   auto *rel = dynamic_cast<channel::transport::reliable *>(ctx.inbound.get());
   // 替换后:
   auto *sock = ctx.inbound->raw_socket();
   if (!sock) { /* 错误处理 */ }
   ```

4. 在 `CMakeLists.txt` 中添加 `-fno-rtti`（Linux/Clang）或 `/GR-`（MSVC）编译选项。

5. 验证全量编译通过且 45 个测试全部通过。
...
**涉及文件**:
- `src/prism/resolve/router.cpp:52` — 当前返回 `not_supported`
...
**涉及文件**:
- `include/prism/channel/eyeball/racer.hpp` — Happy Eyeballs 核心
...
**涉及文件**:
- `src/prism/resolve/detail/upstream.cpp` — 截断处理已实现
- `tests/DnsPacket.cpp` — 添加测试用例

**当前代码分析**:
```cpp
// upstream.cpp — 截断处理已正确实现
if (resp->tc) {
    // 响应被截断，回退到 TCP
...
**涉及文件**:
- `src/prism/multiplex/yamux/craft.cpp` — 窗口流控已完整实现
...
| 重复模式 | trojan.cpp | vless.cpp | socks5.cpp | 可复用? |
|----------|-----------|-----------|-----------|--------|
| 验证器 lambda (account::try_acquire) | ✅ | ✅ | 无 | 是 |
| `wrap_with_preview` 调用 | ✅ | ✅ | ✅ | 是 |
| MUX 目标检测 + bootstrap | ✅ | ✅ | 无 | 是 |
| 目标地址构造 (host+port→target) | ✅ | ✅ | ✅ | 是 |
| UDP associate 逻辑 | ✅ | ✅ | ✅ | 是 |
| `primitives::forward()` 调用 | ✅ | ✅ | ✅ | 是 |
| 错误处理 (fault::code 检查) | ✅ | ✅ | ✅ | 是 |
...
2. 在 `handshake.cpp` 中添加 v3 路径（`if (cfg.version == 3)` 分支）：
   - 读取客户端 ClientHello（复用 `protocol::tls::read_tls_record()`）
...
| `docs/prism/stealth/` | 添加 Restls.md、AnyTLS.md、ECH.md |
| `docs/prism/channel.md` | 文档化 WebSocket 传输 |
| `docs/prism/recognition.md` | 反映新的纯调度架构 |
| `docs/prism/protocol.md` | 文档化 TLS 共享层 (`protocol/tls/`) |
...
第 3 周 (补全功能):
  ...
  ├─ 3.2 类型统一 (reality/handshake.cpp) ──────────┤  可并行
  ├─ 3.3 ECH/AnyTLS 方案接入 (新建 stealth/ech/, anytls/) ─┤
...
| 3.3 ECH/AnyTLS 重接入 | 中 | 原 arrival 分析器已删除，需在 stealth 架构下重新设计 | 先实现最小 scheme，再逐步完善特征检测 |
| 3.4 dynamic_cast 消除 | 低 | 4 处（比原计划多 1 处），需逐一替换 | `raw_socket()` 虚方法方案已验证可行 |
...
| `src/prism/stealth/reality/handshake.cpp:429` | 修改 | 3.2 | 消除 `client_hello_info` 冗余类型 |
| `include/prism/protocol/tls/types.hpp` | 修改 | 3.2 | 可能需补充 reality 所需字段 |
| `src/prism/resolve/router.cpp:52` | 重写 | 4.1 | `not_supported` → HTTP CONNECT |
| `include/prism/stealth/ech/scheme.cpp` | 新建 | 3.3 | ECH 方案实现 |
| `include/prism/stealth/ech/scheme.hpp` | 新建 | 3.3 | ECH 方案接口 |
| `src/prism/stealth/anytls/scheme.cpp` | 新建 | 3.3 | AnyTLS 认证逻辑 |
| `include/prism/stealth/anytls/scheme.hpp` | 新建 | 3.3 | AnyTLS 方案接口 |
...
| `src/prism/stealth/restls/scheme.cpp:59` | 修改 | 3.4 | `dynamic_cast` → `raw_socket()` |
| `src/prism/stealth/shadowtls/scheme.cpp:67` | 修改 | 3.4 | `dynamic_cast` → `raw_socket()` |
| `src/prism/stealth/shadowtls/handshake.cpp:334` | 修改 | 3.4 | `dynamic_cast` → `raw_socket()` |
...
| `tests/EchScheme.cpp` | 新建 | 3.3 | ECH 方案测试 |
...
本 8 周计划聚焦于 **性能修复 + TODO 清零 + 代码质量 + WebSocket/ShadowTLS v3**，不涉及以下更大规模的架构变更：
