...
namespace psm::runtime::front
{
    ...
    listener::listener(const psm::settings &cfg, balancer &dispatcher)
        : ioc_(1), acceptor_(ioc_), dispatcher_(dispatcher), buffer_size_(cfg.buffer.size),
          backpressure_delay_(2)
    {
        net::ip::address addr;
        boost::system::error_code ec;
        addr = net::ip::make_address(cfg.instance.addressable.host, ec);
        if (ec)
        { // 目前不支持 IPv6 地址，仅支持 IPv4 地址，仅支持 IPv4 地址
            if (cfg.instance.addressable.host == "localhost")
            { // 如果为 localhost，则使用回环地址 127.0.0.1
                addr = net::ip::address_v4::loopback();
            }
            else if (cfg.instance.addressable.host == "0.0.0.0" || cfg.instance.addressable.host.empty())
            ...
            else
            {
                throw std::system_error(
                    ec, "Invalid listen address: " + std::string(cfg.instance.addressable.host) +
