...
namespace psm::protocol::vless
{
    ...
    auto handler::run() -> net::awaitable<void>
    {
        ...
        const auto agent =
            make_conn(std::move(inbound), res_.worker->process->cfg->protocol.vless, std::move(verifier));
        ...
        if (fault::failed(vless_ec))
        {
            if (trace)
            {
                diagnose::warn(trace, "handshake failed: {}", fault::describe(vless_ec));
            }
            co_return;
        }
        ...
        case command::udp: {
            ...
            if (fault::failed(ec))
            {
                if (trace)
                {
                    diagnose::warn(trace, "UDP associate failed: {}", fault::describe(ec));
                }
            }
            else if (trace)
            {
