...
namespace psm::protocol::shadowsocks
{
        ...
        : next_layer_(std::move(next_layer)), config_(cfg), salt_pool_(std::move(salts)), psk_(memory::current_resource())
    {
        ...
        {
            trace::error(prefix_, "invalid PSK configuration: {}", fault::describe(ec));
            return;
        }
        ...
        -> net::awaitable<std::tuple<fault::code, std::uint16_t, std::int64_t>>
    {
        ...
        if (ec)
        {
            trace::warn(prefix_, "read fixed header failed: {}", ec.message());
        ...
        {
            trace::warn(prefix_, "decrypt fixed header failed: {} (expected {} plain bytes, got {} enc bytes)",
        ...
        {
            trace::warn(prefix_, "invalid request type: 0x{:02x}", header_plain[0]);
        ...
        {
            trace::warn(prefix_, "timestamp expired: client_ts={}, server_ts={}, diff={}s",
                        client_ts, now, diff);
        ...
        -> net::awaitable<fault::code>
    {
        ...
        if (ec)
        {
            trace::warn(prefix_, "read variable header failed: {}", ec.message());
        ...
        if (const auto r = decrypt_ctx_->open(var_header_plain, as_u8(var_header_enc));
            r != fault::code::success)
        {
            trace::warn(prefix_, "decrypt variable header failed: {}", fault::describe(r));
        ...
        {
            trace::warn(prefix_, "address parse failed: {}", fault::describe(addr_ec));
            co_return addr_ec;
        }
        ...
        if (offset + 2 <= var_header_plain.size())
        {
            ...
            {
                trace::error(prefix_, "padding_len ({}) exceeds remaining data ({})", padding_len, var_header_plain.size() - offset);
        ...
        -> net::awaitable<fault::code>
    {
        ...
        if (const auto r = encrypt_ctx_->seal(resp_fixed_enc, resp_fixed_plain);
            r != fault::code::success)
        {
            trace::error(prefix_, "encrypt response fixed header failed");
            co_return fault::code::crypto_error;
        }
        ...
        if (const auto r = encrypt_ctx_->seal(empty_payload_enc, {});
            r != fault::code::success)
        {
            trace::error(prefix_, "encrypt empty payload chunk failed");
            co_return fault::code::crypto_error;
        }
        ...
        if (ec)
        {
            trace::warn(prefix_, "write response failed: {}", ec.message());
            co_return fault::code::connection_reset;
        }
        ...
        -> net::awaitable<std::pair<fault::code, request>>
    {
        ...
        if (psk_.empty())
        {
            trace::error(prefix_, "PSK not configured");
        ...
        trace::debug(prefix_, "handshake start: method={}, key_salt_len={}, psk_size={}",
        ...
        {
            deadline.cancel();
            trace::warn(prefix_, "read client salt failed: {}", ec.message());
        ...
        {
            deadline.cancel();
            trace::warn(prefix_, "salt replay detected");
        ...
        trace::debug(prefix_, "derived decrypt context from salt");
        ...
        -> net::awaitable<std::size_t>
    {
        ...
        if (ec)
        {
            if (ec == fault::code::eof || ec == fault::code::canceled)
                trace::debug(prefix_, "async_read_some: client disconnected");
            else
                trace::warn(prefix_, "async_read_some: fetch_chunk failed: {}", ec.message());
            co_return 0;
        }
        ...
        -> net::awaitable<void>
    {
        ...
        if (const auto r = decrypt_ctx_->open(as_u8_mut(decrypted_), as_u8(chunk_buf_));
            r != fault::code::success)
        {
            trace::warn(prefix_, "decrypt payload block failed");
            ec = std::make_error_code(std::errc::protocol_error);
            decrypted_.clear();
            co_return;
        }
        ...
        -> net::awaitable<std::size_t>
    {
        ...
        trace::debug(prefix_, "send_chunk: chunk_len={}, data_size={}, conn={}",
        ...
        if (const auto r = encrypt_ctx_->seal(len_enc, len_plain);
            r != fault::code::success)
        {
            trace::warn(prefix_, "encrypt length block failed");
            ec = std::make_error_code(std::errc::protocol_error);
            co_return 0;
        }
        ...
        if (const auto r = encrypt_ctx_->seal(payload_enc_buf_, as_u8(data.first(chunk_len)));
            r != fault::code::success)
        {
            trace::warn(prefix_, "encrypt payload block failed");
            ec = std::make_error_code(std::errc::protocol_error);
            co_return 0;
        }

        trace::debug(prefix_, "send_chunk: len_enc_size={}, payload_enc_size={}, calling transport::async_write",
        ...
        trace::debug(prefix_, "send_chunk: transport::async_write completed, ec={}, returned {}",
                    ec_msg, logged_len);
