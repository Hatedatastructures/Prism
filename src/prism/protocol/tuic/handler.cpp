...
namespace psm::protocol::tuic
{
    ...
    auto handler::run() -> net::awaitable<void>
    {
        ...
        const auto [pe, pec] = std::to_chars(
            port_buf, port_buf + sizeof(port_buf), static_cast<std::uint32_t>(req.port));
    ...
    udp_handler::udp_handler(protocol::handler_params params)
        : res_(params.res)
        , data_(params.data)
    {
    }
        ...
        while (true)
        {
            ...
            while (offset + 2 <= n)
            {
                ...
                const auto port = static_cast<std::uint16_t>(
                    (fp[addr_start + addr_data_len] << 8) | fp[addr_start + addr_data_len + 1]);
                ...
                if (it == sockets.end())
                {
                    auto usock = std::make_shared<udp::socket>(
                        inbound->executor(), udp::endpoint(udp::v4(), 0));
                    // 回包协程：assoc_id 的 UDP 回包 → 组 Packet 帧 → 写流
                    auto sid = assoc_id;
                    auto chan = inbound;
                    net::co_spawn(inbound->executor(), [chan, usock, sid]() -> net::awaitable<void>
                    {
                        ...
                        while (true)
                        {
                            ...
                            std::error_code wr;
                            co_await chan->async_write_some(std::span<const std::byte>(hdr.data(), 13 + rdata), wr);
                            if (wr)
                                break;
                        }
                    }, net::detached);
                ...
                co_await it->second->async_send_to(
                    net::buffer(data, static_cast<std::size_t>(size)), target_ep,
