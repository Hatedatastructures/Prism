...
namespace psm::protocol::tuic
{
    ...
    namespace
    {
        /// 地址编码（TUIC v5：ATYP 0=domain 1=IPv4 2=IPv6）
        [[nodiscard]] auto parse_address(std::span<const std::uint8_t> in,
                                         psm::protocol::common::address &addr,
                                         std::uint16_t &port, std::size_t &consumed) -> bool
        {
            if (in.empty())
                return false;
    ...
    auto parse_connect(const std::span<const std::uint8_t> in, connect_frame &out,
