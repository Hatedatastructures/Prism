...
namespace psm::protocol::socks5
{
    ...
    auto handler::run() -> net::awaitable<void>
    {
        ...
        const auto agent = make_conn(std::move(inbound), res_.worker->process->cfg->protocol.socks5,
                                     &*res_.worker->process->accounts);
        ...
        if (fault::failed(ec))
        {
            if (trace)
            {
                diagnose::error(trace, "handshake failed: {}", fault::cached_message(ec));
            }
            co_return;
        }
        ...
        case command::connect: {
            ...
            char port_buf[8];
            const auto [pe, pec] =
                std::to_chars(port_buf, port_buf + sizeof(port_buf), request.destination_port);
            target.port.assign(port_buf, std::distance(port_buf, pe));
            target.positive = true;
            if (trace)
            {
                diagnose::access(trace, "CONNECT -> {}:{}", target.host, target.port);
            }
            ...
            if (fault::failed(dial_ec) || !outbound)
            {
                if (dial_ec == fault::code::ipv6_disabled)
                {
                    if (trace)
                    {
                        diagnose::debug(trace, "IPv6 disabled: {}:{}", target.host, target.port);
                    }
                ...
                else
                {
                    if (trace)
                    {
                        diagnose::warn(trace, "dial failed: {}, target: {}:{}", fault::describe(dial_ec),
        ...
        case command::udp_associate: {
            const auto target_host = to_string(request.destination_address, res_.arena.get());
            char udp_port_buf[8];
            const auto [upe, upec] =
                std::to_chars(udp_port_buf, udp_port_buf + sizeof(udp_port_buf), request.destination_port);
            const auto target_port = std::string_view(udp_port_buf, std::distance(udp_port_buf, upe));
            if (trace)
            {
                diagnose::access(trace, "UDP_ASSOCIATE -> {}:{}", target_host, target_port);
            }
            ...
            if (fault::failed(associate_ec))
            {
                if (trace)
                {
                    diagnose::warn(trace, "UDP_ASSOCIATE failed: {}", fault::describe(associate_ec));
                }
            }
            break;
        }
