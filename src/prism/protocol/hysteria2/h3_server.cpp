...
namespace psm::protocol::hysteria2::h3
{
    ...
    server::server(const memory::resource_pointer mr)
        : mr_(mr)
        , method_(mr)
        , path_(mr)
        , auth_(mr)
    ...
    auto server::init(std::function<std::int64_t()> open_uni_stream) -> bool
    {
        ...
        const auto rv = nghttp3_conn_server_new_versioned(
            &conn_, NGHTTP3_CALLBACKS_VERSION, &callbacks,
            NGHTTP3_SETTINGS_VERSION, &settings, nghttp3_mem_default(), this);
        if (rv != 0)
        {
            conn_ = nullptr;
            diagnose::warn("hysteria2: nghttp3_conn_server_new failed: {}",
                           nghttp3_strerror(rv));
            return false;
        }
    ...
    auto server::feed(const std::int64_t stream_id, const std::span<const std::byte> data,
                      const bool fin) -> fault::code
    {
        if (!conn_)
            return fault::code::protocol_error;

        const auto rc = nghttp3_conn_read_stream2(
            conn_, stream_id,
    ...
    auto server::pump_output(memory::vector<out_packet> &out) -> bool
    {
        ...
        for (;;)
        {
            ...
            nghttp3_ssize sveccnt = nghttp3_conn_writev_stream(
                conn_, &stream_id, &fin, vecs.data(), vecs.size());
            if (sveccnt < 0)
            {
            ...
            for (nghttp3_ssize i = 0; i < sveccnt; ++i)
            {
                pkt.data.insert(pkt.data.end(),
                                reinterpret_cast<const std::byte *>(vecs[i].base),
    ...
    auto server::submit_auth_response() -> fault::code
    {
        ...
        std::size_t n = 0;
        nva[n++] = nghttp3_nv{
            reinterpret_cast<const uint8_t *>(":status"), reinterpret_cast<const uint8_t *>("233"),
            7, 3, NGHTTP3_NV_FLAG_NONE};
        nva[n++] = nghttp3_nv{
            reinterpret_cast<const uint8_t *>("hysteria-udp"), reinterpret_cast<const uint8_t *>("true"),
            12, 4, NGHTTP3_NV_FLAG_NONE};
        nva[n++] = nghttp3_nv{
            reinterpret_cast<const uint8_t *>("hysteria-cc-rx"), reinterpret_cast<const uint8_t *>("0"),
            13, 1, NGHTTP3_NV_FLAG_NONE};
        nva[n++] = nghttp3_nv{
            reinterpret_cast<const uint8_t *>("hysteria-padding"), reinterpret_cast<const uint8_t *>("0"),
            15, 1, NGHTTP3_NV_FLAG_NONE};

        const auto rv = nghttp3_conn_submit_response(conn_, auth_stream_, nva.data(), n,
                                                     nullptr);
        if (rv != 0)
        {
            diagnose::warn("hysteria2: nghttp3 submit_response failed: {}",
                           nghttp3_strerror(rv));
            return fault::code::protocol_error;
        }
        return fault::code::success;
    ...
    auto server::cb_begin_headers(nghttp3_conn *conn, const int64_t stream_id,
    ...
    auto server::cb_recv_header(nghttp3_conn *conn, const int64_t stream_id,
                                const int32_t token, nghttp3_rcbuf *name,
                                nghttp3_rcbuf *value, const uint8_t flags,
                                void *user_data, void *stream_user_data) -> int
    {
        (void)conn;
        (void)flags;
        ...
        case NGHTTP3_QPACK_TOKEN__METHOD:
            self->method_.assign(v.data(), v.size());
            return 0;
        case NGHTTP3_QPACK_TOKEN__PATH:
            self->path_.assign(v.data(), v.size());
            return 0;
        default:
            break;
        }
    ...
    auto server::cb_end_headers(nghttp3_conn *conn, const int64_t stream_id,
                                const int fin, void *user_data,
    ...
    auto server::cb_recv_data(nghttp3_conn *conn, const int64_t stream_id,
    ...
    auto server::cb_stop_sending(nghttp3_conn *conn, const int64_t stream_id,
                                 const uint64_t app_error_code, void *user_data,
                                 void *stream_user_data) -> int
    {
        (void)conn;
        (void)stream_id;
    ...
    auto server::cb_end_stream(nghttp3_conn *conn, const int64_t stream_id,
                               void *user_data, void *stream_user_data) -> int
    {
        (void)conn;
        (void)stream_id;
