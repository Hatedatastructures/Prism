...
namespace psm::protocol::hysteria2::h3
{
    ...
    auto encode_auth_response(const std::uint16_t status, const bool udp_enabled,
                              const std::uint64_t rx, const std::span<std::byte> out)
        -> std::size_t
    {
        ...
        offset += qpack::encode_literal(":status", status_str,
                                        std::span<std::uint8_t>(block.data() + offset, block.size() - offset));

        // Hysteria-UDP: true
        offset += qpack::encode_literal("hysteria-udp", udp_enabled ? "true" : "false",
                                        std::span<std::uint8_t>(block.data() + offset, block.size() - offset));
        ...
        offset += qpack::encode_literal("hysteria-cc-rx", rx_str,
                                        std::span<std::uint8_t>(block.data() + offset, block.size() - offset));

        // Hysteria-Padding: 0（客户端解析用，填 0 表示无 padding）
        offset += qpack::encode_literal("hysteria-padding", "0",
                                        std::span<std::uint8_t>(block.data() + offset, block.size() - offset));
