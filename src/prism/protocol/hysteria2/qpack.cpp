...
namespace psm::protocol::hysteria2::qpack
{
    ...
    namespace
    {
        ...
        [[nodiscard]] auto write_varint(const std::span<std::uint8_t> out, const std::uint8_t prefix_bits,
                                        const std::uint64_t value, const std::uint8_t prefix_pattern) -> std::size_t
        {
            const std::uint64_t prefix_mask = (1ULL << prefix_bits) - 1;
            std::size_t n = 0;
        ...
        /// huffman 解码：位累积式查表（RFC 7541 附录 B，MSB 优先）
        ...
        /// huffman 编码：按符号输出码字（MSB 优先，RFC 7541 附录 B）
        [[nodiscard]] auto huffman_encode_impl(std::string_view in,
                                               memory::vector<std::uint8_t> &out) -> bool
        {
            std::uint64_t acc = 0;
            std::uint32_t acc_bits = 0;
        ...
        /// 解析字面量名称（3 位前缀 + H 位 0x08，RFC 9204 4.5.2）
        ...
        /// 解析一个字面量值（7 位前缀 + H 位 0x80，RFC 9204 4.5.1/4.5.2）
    ...
    auto decode_header_block(const std::span<const std::uint8_t> data,
    ...
    auto huffman_decode(const std::span<const std::uint8_t> in,
                        memory::vector<std::uint8_t> &out) -> bool
    {
