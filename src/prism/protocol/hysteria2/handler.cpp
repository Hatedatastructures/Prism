...
namespace psm::protocol::hysteria2
{
    ...
    handler::handler(protocol::handler_params params)
        : res_(params.res)
        , data_(params.data)
    {
    }
    ...
    auto handler::run() -> net::awaitable<void>
    {
        ...
        // UDP 消息流
        co_await handle_udp(std::move(inbound),
            std::span<const std::byte>(buf.data(), total));
    }

    auto handler::handle_udp(psm::transport::shared_transmission inbound, const std::span<const std::byte> head)
        -> net::awaitable<void>
    {
        auto trace = res_.trace;
        ...
        net::co_spawn(inbound->executor(), [inbound, usock, session_id]() -> net::awaitable<void>
        {
            ...
            while (true)
            {
                ...
                const auto n = co_await usock->async_receive_from(
        ...
        while (true)
        {
            ...
            if (!parse_udp_message(
                    std::span<const std::uint8_t>(reinterpret_cast<const std::uint8_t *>(pending.data()), pending.size()),
                    msg))
            {
                pending.clear();
                continue;
            ...
            pending.erase(pending.begin(), pending.begin() + static_cast<std::ptrdiff_t>(msg.data_offset + msg.data_len));
            ...
            co_await usock->async_send_to(
                net::buffer(pending.data() + msg.data_offset, msg.data_len), target_ep,
