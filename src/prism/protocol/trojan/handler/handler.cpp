    ...
    auto handler::run() -> net::awaitable<void>
    {
        ...
        auto verifier = [this](std::string_view credential) -> bool
        {
            ...
            if (!lease)
            {
                ...
                {
                    diagnose::warn(t, "credential verification failed");
                }
                return false;
            }
        ...
        const auto agent =
            make_conn(std::move(inbound), res_.worker->process->cfg->protocol.trojan, std::move(verifier));
        ...
        if (fault::failed(trojan_ec))
        {
            if (trace)
            {
                diagnose::warn(trace, "handshake failed: {}", fault::describe(trojan_ec));
            }
            co_return;
        }
        ...
        case command::udp_associate: {
            ...
            using route_fn = std::function<net::awaitable<std::pair<fault::code, net::ip::udp::endpoint>>(
            ...
            if (fault::failed(ec))
            {
                if (trace)
                {
                    diagnose::warn(trace, "UDP_ASSOCIATE failed: {}", fault::describe(ec));
                }
            }
            else if (trace)
            {
        ...
        case command::mux: {
            if (trace)
            {
                diagnose::debug(trace, "mux session started (cmd=0x7F)");
            }
        ...
        default:
            if (trace)
            {
                diagnose::warn(trace, "unknown command: {}", static_cast<int>(req.cmd));
            }
            break;
        }
    }
