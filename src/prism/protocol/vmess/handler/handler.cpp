...
namespace psm::protocol::vmess
{
                      ...
                      shared_transmission inbound) -> net::awaitable<void>
    {
        ...
        const auto agent =
            make_conn(std::move(inbound), res.worker->process->cfg->protocol.vmess, std::move(keys));
        ...
        if (fault::failed(vmess_ec))
        {
            if (trace)
            {
                diagnose::warn(trace, "handshake failed: {}", fault::describe(vmess_ec));
            }
            co_return;
        }
        ...
        default:
            if (trace)
            {
                diagnose::warn(trace, "unknown command: {}", static_cast<int>(req.command));
            }
            break;
        }
    }
