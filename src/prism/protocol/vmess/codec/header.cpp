...
namespace psm::protocol::vmess::codec
{
    ...
    namespace
    {
        ...
        /**
         * @brief 使用显式 key/nonce/AAD 执行 AES-128-GCM 加密
         * @param key 16 字节密钥
         ...
         * @return 是否加密成功
         */
        [[nodiscard]] auto gcm_seal(std::span<const std::uint8_t, 16> key,
                                    ...
                                    std::span<std::uint8_t> out) -> bool
        {
            ...
            if (ok)
            {
                ...
                ok = EVP_EncryptUpdate(ctx, out.data(), &len, plain.data(), static_cast<int>(plain.size())) &&
                     (total = len, true) && EVP_EncryptFinal_ex(ctx, out.data() + len, &len) &&
                     (total += len, true) &&
        ...
        /**
         * @brief 使用显式 key/nonce/AAD 执行 AES-128-GCM 解密
         * @param key 16 字节密钥
         * @param nonce 12 字节 nonce
         * @param aad 附加认证数据
         * @param cipher 密文（含尾部 16 字节认证标签）
         * @param out 明文输出
         * @return 是否解密成功
         */
        [[nodiscard]] auto gcm_open(std::span<const std::uint8_t, 16> key,
                                    std::span<const std::uint8_t, 12> nonce,
        ...
        /**
         * @brief 逐字节 AES-128-CFB 加密（BoringSSL 无 EVP CFB，手写实现）
         * @param key 16 字节密钥
         * @param iv 初始向量
         * @param in 明文
         * @param out 密文输出
         * @return 是否加密成功
         */
        [[nodiscard]] auto cfb128_encrypt(const std::span<const std::uint8_t, 16> key,
                                          const std::span<const std::uint8_t, 16> iv,
                                          const std::span<const std::uint8_t> in,
                                          const std::span<std::uint8_t> out) -> bool
        {
            AES_KEY aes_key;
        ...
        /**
         * @brief 序列化地址段（mihomo 顺序：port 2B + addrType 1B + addr），返回写入字节数
         * @param addr 目标地址
         * @param port 目标端口
         * @param out 输出缓冲区
         * @return 写入的字节数
         */
        [[nodiscard]] auto write_address(const psm::protocol::common::address &addr, const std::uint16_t port,
        ...
        /**
         * @brief 解析地址段（mihomo 顺序：port 2B + addrType 1B + addr），返回消耗字节数
         * @param in 输入字节序列
         * @param addr 解析出的地址
         * @param port 解析出的端口
         * @return 消耗的字节数
         */
        [[nodiscard]] auto read_address(const std::span<const std::uint8_t> in,
    ...
    auto seal_request(const std::span<const std::uint8_t, 16> cmd_key, const request_header &header,
                      const std::span<std::uint8_t> out) -> fault::code
    {
        ...
        const auto ts = std::chrono::duration_cast<std::chrono::seconds>(
                            ...
                            .count();
        auto auth_ec = seal_auth_header(cmd_key, ts, std::span<std::uint8_t, 16>(out.data(), 16));
        ...
        std::array<std::uint8_t, 2> len_plain{static_cast<std::uint8_t>(used >> 8),
                                              static_cast<std::uint8_t>(used & 0xFF)};
        std::span<std::uint8_t> len_out(out.data() + 16, 18);
        if (!gcm_seal(std::span<const std::uint8_t, 16>(len_key.data(), 16),
                      std::span<const std::uint8_t, 12>(len_nonce.data(), 12), auth_id, len_plain, len_out))
        {
            return fault::code::crypto_error;
        }
        ...
        if (!gcm_seal(std::span<const std::uint8_t, 16>(head_key.data(), 16),
                      std::span<const std::uint8_t, 12>(head_nonce.data(), 12), auth_id,
                      std::span<const std::uint8_t>(plain.data(), used), head_out))
        {
            return fault::code::crypto_error;
        }
    ...
    auto open_len_block(const std::span<const std::uint8_t, 16> cmd_key,
                        const std::span<const std::uint8_t, 16> auth_id,
                        const std::span<const std::uint8_t, 18> len_block,
                        const std::span<const std::uint8_t, 8> conn_nonce, std::size_t &header_len)
        -> fault::code
    {
        const auto len_key = kdf(cmd_key, kdf_header_len_key, auth_id, conn_nonce);
        const auto len_nonce = kdf(cmd_key, kdf_header_len_nonce, auth_id, conn_nonce);
        ...
        if (!gcm_open(std::span<const std::uint8_t, 16>(len_key.data(), 16),
                      std::span<const std::uint8_t, 12>(len_nonce.data(), 12), auth_id,
                      std::span<const std::uint8_t>(len_block.data(), 18), len_plain))
        {
            return fault::code::crypto_error;
        }
    ...
    auto open_payload(const std::span<const std::uint8_t, 16> cmd_key,
                      const std::span<const std::uint8_t, 16> auth_id,
                      const std::span<const std::uint8_t, 8> conn_nonce,
                      const std::span<const std::uint8_t> payload_block, request_header &header)
        -> fault::code
    {
        ...
        if (!gcm_open(std::span<const std::uint8_t, 16>(head_key.data(), 16),
                      std::span<const std::uint8_t, 12>(head_nonce.data(), 12), auth_id, payload_block,
        ...
        if (header.command != static_cast<std::uint8_t>(command::mux))
        {
            const auto n = read_address(
                std::span<const std::uint8_t>(plain.data() + header_base_len, header_len - header_base_len),
    ...
    auto open_request(const std::span<const std::uint8_t, 16> cmd_key,
                      ...
                      std::array<std::uint8_t, 8> &conn_nonce, request_header &header) -> fault::code
    {
        ...
        const auto len_ec =
            open_len_block(cmd_key, auth_id, std::span<const std::uint8_t, 18>(first_packet.data() + 16, 18),
                           std::span<const std::uint8_t, 8>(first_packet.data() + 34, 8), header_len);
        ...
        return open_payload(cmd_key, auth_id, std::span<const std::uint8_t, 8>(first_packet.data() + 34, 8),
                            first_packet.subspan(42, header_len + 16), header);
    }

    auto build_response(const std::span<const std::uint8_t, 16> request_key,
                        ...
                        const std::span<std::uint8_t> out) -> fault::code
    {
        ...
        if (legacy)
        {
            ...
            if (!cfb128_encrypt(key, iv, resp_plain, out.first(4)))
            {
                return fault::code::crypto_error;
            }
            return fault::code::success;
        ...
        if (!gcm_seal(std::span<const std::uint8_t, 16>(len_key.data(), 16),
                      std::span<const std::uint8_t, 12>(len_nonce.data(), 12), {}, len_plain, out.first(18)))
        {
            return fault::code::crypto_error;
        }
        ...
        if (!gcm_seal(std::span<const std::uint8_t, 16>(head_key.data(), 16),
                      std::span<const std::uint8_t, 12>(head_nonce.data(), 12), {}, resp_plain,
                      out.subspan(18, 20)))
        {
            return fault::code::crypto_error;
        }
