...
namespace psm::protocol::vmess::codec
{
    ...
    namespace
    {
        ...
        /// 单块 AES-128-ECB 加密（无填充）
        [[nodiscard]] auto aes_ecb_encrypt(
            std::span<const std::uint8_t, 16> key, std::span<const std::uint8_t, 16> in,
        ...
        /// 单块 AES-128-ECB 解密（无填充）
        [[nodiscard]] auto aes_ecb_decrypt(
            std::span<const std::uint8_t, 16> key, std::span<const std::uint8_t, 16> in,
            std::span<std::uint8_t, 16> out) -> bool
        {
            EVP_CIPHER_CTX *ctx = EVP_CIPHER_CTX_new();
            ...
            const bool ok = EVP_DecryptInit_ex(ctx, EVP_aes_128_ecb(), nullptr, key.data(), nullptr)
                && EVP_CIPHER_CTX_set_padding(ctx, 0) == 1
                && EVP_DecryptUpdate(ctx, out.data(), &len, in.data(), 16)
                && (total = len, true)
                && EVP_DecryptFinal_ex(ctx, out.data() + total, &len)
                && (total += len, true)
                && total == 16;
            EVP_CIPHER_CTX_free(ctx);
            return ok;
    ...
    auto seal_auth_header(
        const std::span<const std::uint8_t, 16> cmd_key, const std::int64_t timestamp,
