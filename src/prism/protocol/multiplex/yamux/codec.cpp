...
namespace psm::multiplex::yamux
{
    ...
    auto yamux_codec::header_size() const noexcept -> std::size_t
    {
        return frame_hdrsize;
    }

    auto yamux_codec::decode_header(const std::span<const std::byte> header) -> frame_meta
    {
        frame_meta meta;
        meta.raw_type = 0;
    ...
    auto yamux_codec::encode_data(const std::uint32_t stream_id, const std::span<const std::byte> payload)
    ...
    auto yamux_codec::encode_fin(const std::uint32_t stream_id) -> memory::vector<std::byte>
    {
    ...
    auto yamux_codec::type_name() const noexcept -> std::string_view
    {
        return "yamux";
    }
