...
namespace psm::multiplex
{
    ...
    namespace
    {
        auto negotiate(transmission &transport) -> net::awaitable<std::pair<std::error_code, protocol_type>>
        {
            std::error_code ec;
            ...
            if (n < 2)
            {
                co_return std::make_pair(std::make_error_code(std::errc::connection_reset),
                                         protocol_type::smux);
            }
    ...
    auto bootstrap(bootstrap_context ctx) -> net::awaitable<std::shared_ptr<multiplexer>>
    {
        auto &res = *ctx.res;
        ...
        if (ec)
        {
            if (prefix_)
            {
                diagnose::warn(prefix_, "sing-mux negotiate failed: {}", ec.message());
            }
            co_return nullptr;
        }
        ...
        catch (const std::exception &e)
        {
            if (prefix_)
            {
                diagnose::error(prefix_, "create_session exception: {}", e.what());
            }
            co_return nullptr;
        }
        catch (...)
        {
            if (prefix_)
            {
                diagnose::error(prefix_, "create_session unknown exception");
            }
            co_return nullptr;
        }
    }
