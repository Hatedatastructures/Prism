...
namespace psm::dns
{
    ...
    resolver::resolver(net::io_context &ioc, config cfg, memory::resource_pointer mr)
        : impl_(std::make_unique<impl>(ioc, std::move(cfg), mr))
    {
    }

    auto resolver::resolve(std::string_view host)
        -> net::awaitable<std::pair<fault::code, memory::vector<net::ip::address>>>
    {
        co_return co_await impl_->resolve(host);
    }

    auto resolver::resolve_tcp(std::string_view host, std::string_view port)
        -> net::awaitable<std::pair<fault::code, memory::vector<tcp::endpoint>>>
    {
        co_return co_await impl_->resolve_tcp(host, port);
    }

    auto resolver::resolve_udp(std::string_view host, std::string_view port)
        -> net::awaitable<std::pair<fault::code, net::ip::udp::endpoint>>
    {
