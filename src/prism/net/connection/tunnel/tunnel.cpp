...
namespace psm::connect
{
        ...
        -> net::awaitable<void>
    {
            ...
            [inbound, outbound, trace = opts.trace](const boost::system::error_code &ec)
        {
            if (!ec)
            {
                diagnose::debug(trace, "idle timeout, closing tunnel");
                inbound->cancel();
                outbound->cancel();
            }
        };
        ...
        if (opts.lease)
        {
            if (total_bytes[0] > 0)
            {
                user::accumulate_uplink(opts.lease->get(), total_bytes[0]);
            }
            if (total_bytes[1] > 0)
            {
                user::accumulate_downlink(opts.lease->get(), total_bytes[1]);
            }
        }
