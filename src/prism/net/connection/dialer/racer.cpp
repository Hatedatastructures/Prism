...
namespace psm::connect
{
        ...
        -> net::awaitable<shared_transmission>
    {
        ...
        if (endpoints.size() == 1)
        {
            auto [code, conn] = co_await dial_(endpoints[0]);
            co_return conn;
        }
        ...
        diagnose::debug(trace, "racing {} endpoints", count);
    ...
    // 清除父协程日志前缀，子协程不应该继承调用者的会话上下文
        ...
        try
        {
            ...
            auto [code, conn] = co_await dial_(ep);
        ...
        {
            diagnose::debug(trace, "endpoint {} error: {}", ep.address().to_string(), e.what());
            ctx->complete();
        }
        catch (...)
        {
            diagnose::error(trace, "endpoint {} unknown error", ep.address().to_string());
            ctx->complete();
        }
    }
