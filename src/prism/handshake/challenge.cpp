...
namespace psm::handshake
{
    ...
    {
        auto hasher = crypto::keyed_hasher(std::span<const std::uint8_t, 32>(input.server_secret));
        ...
        blake3_hasher_finalize(&hasher, reinterpret_cast<std::uint8_t *>(token.bytes.data()), 16);
        return token;
    }

    auto verify_challenge(const challenge_token &expected, std::span<const std::byte> response) noexcept
        -> bool
    {
        if (response.size() < 16)
        {
