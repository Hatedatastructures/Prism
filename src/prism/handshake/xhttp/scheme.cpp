...
namespace psm::handshake::xhttp
{
        ...
        -> net::awaitable<handshake::handshake_result>
    {
        ...
        const auto &cfg = ctx.session->worker->process->cfg->stealth.xhttp;
        ...
        auto xh_ssl_ctx = std::make_shared<ssl::context>(ssl::context::tlsv13);
        {
            auto &src_ctx = *ctx.session->worker->process->ssl;
            auto *src_native = src_ctx.native_handle();
            auto *dst_native = xh_ssl_ctx->native_handle();
        ...
        auto xh_session = make_session(encrypted_trans, cfg, memory::current_resource(), prefix_);
        xh_session->start();

        auto matched = co_await xh_session->wait_transport();
        if (!matched)
        {
            diagnose::debug(prefix_, "xhttp: no stream matched");
            result.detected = psm::connect::protocol_type::tls;
        ...
        diagnose::debug(prefix_, "xhttp: stream established");

        result.transport = std::move(matched);
