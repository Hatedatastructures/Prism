...
namespace psm::recognition
{
        ...
        -> net::awaitable<identify_result>
    {
        ...
        diagnose::debug(prefix_, "starting identify lifecycle");
        ...
        {
            diagnose::error(prefix_, "read_tls_record failed: {}", fault::describe(read_ec));
            result.error = read_ec;
            co_return result;
        }

        diagnose::debug(prefix_, "read {} bytes ClientHello", raw_record.size());

        // Phase 2: 解析 ClientHello 特征
        auto [parse_ec, features] = recognition::tls::parse_client_hello(raw_record);
        if (fault::failed(parse_ec))
        {
            diagnose::error(prefix_, "parse_client_hello failed: {}", fault::describe(parse_ec));
        ...
        auto &registry = handshake::scheme_registry::instance();
        std::vector<handshake::shared_scheme> matched_schemes;
        ...
        auto executor = handshake::scheme_executor(registry);

        // 确定性命中：直接执行
        if (pipeline_result.deterministic_hit)
        {
            diagnose::debug(prefix_, "deterministic hit: {}", pipeline_result.exclusive_scheme);
        ...
        -> net::awaitable<recognize_result>
    {
        ...
        if (!opts.transport)
        {
            diagnose::error(prefix_, "transport is null");
