...
namespace psm::handshake::anytls
{
    ...
    namespace
    {
        ...
        auto perform_tls_handshake(handshake::stealth_opts &ctx)
            -> net::awaitable<tls_hs_result>
        {
            ...
            diagnose::debug(ctx.session->trace, "TLS handshake succeeded");
            ...
            -> const memory::string *
        {
            ...
            if (it == user_map.end())
            {
                diagnose::warn(prefix_, "Authentication failed: unknown password hash");
                return nullptr;
            }
            diagnose::debug(prefix_, "Authenticated as user: {}", it->second);
            return &it->second;
        }
            ...
            -> net::awaitable<void>
        {
            ...
            if (psm::connect::is_mux(target.host, mux_sw))
            {
                ...
                try
                {
                    ...
                    {
                        diagnose::warn(prefix_, "anytls mux bootstrap failed, closing stream");
                        inbound->close();
                    }
                }
                catch (const std::exception &e)
                {
                    diagnose::error(prefix_, "anytls mux bootstrap exception: {}", e.what());
                }
                catch (...)
                {
                    diagnose::error(prefix_, "anytls mux bootstrap unknown exception");
                }
                co_return;
            }
            ...
            -> anytls_session::stream_callback
        {
                                  ...
                                  memory::vector<std::uint8_t> preread_data)
            {
                                         ...
                                         session_ptr, keepalive, prefix_]() -> net::awaitable<void>
                {
                    ...
                    {
                        diagnose::error(prefix_, "subsequent stream exception: {}", e.what());
                    }
                    catch (...)
                    {
                        diagnose::error(prefix_, "subsequent stream unknown exception");
                    }
                };
            ...
            -> net::awaitable<fault::code>
        {
                                  ...
                                  prefix_]() -> net::awaitable<void>
            {
                ...
                {
                    diagnose::error(prefix_, "first stream forward exception: {}", e.what());
                }
                catch (...)
                {
                    diagnose::error(prefix_, "first stream forward unknown exception");
                }
            };
            if (auto wr = session_ptr->worker)
            {
        ...
        -> net::awaitable<handshake::handshake_result>
    {
        ...
        const auto &cfg = ctx.session->worker->process->cfg->stealth.anytls;
