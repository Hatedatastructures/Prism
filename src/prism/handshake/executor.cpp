...
namespace psm::handshake
{
        ...
        -> net::awaitable<handshake_result>
    {
        ...
        for (const auto &name : order)
        {
            ...
            if (!scheme)
            {
                diagnose::warn(ctx.session->trace, "Scheme '{}' not found", name);
                continue;
            }

            if (!scheme->active(*ctx.session->worker->process->cfg))
            {
                diagnose::debug(ctx.session->trace, "Scheme '{}' disabled, skipping", name);
                continue;
            }

            ensure_snapshot(ctx);

            diagnose::debug(ctx.session->trace, "Executing scheme '{}'", name);
            ...
            if (scheme->category() == scheme_category::stack)
            {
                ...
                {
                    diagnose::debug(ctx.session->trace, "Stack scheme '{}' handled connection", name);
            ...
            {
                diagnose::debug(ctx.session->trace, "Scheme '{}' returned TLS, continuing to next", name);
            ...
            if (fault::failed(exec_result.error))
            {
                ...
                {
                    psm::rate::address_hash src_ip;
                    std::memcpy(src_ip.bytes.data(), ctx.session->meta->src_ip.data(), 16);
        ...
        -> net::awaitable<handshake_result>
    {
        ...
        {
            diagnose::debug(ctx.session->trace, "No candidates from analysis, executing by default priority");
            ...
            {
                diagnose::debug(ctx.session->trace, "All candidates failed, executing native fallback");
