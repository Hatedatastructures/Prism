...
namespace psm::handshake::shadowtls
{
        ...
        -> net::awaitable<std::size_t>
    {
        ...
        if (initial_offset_ < initial_buffer_.size())
        {
            ...
            initial_offset_ += n;
            diagnose::debug(prefix_, "returned {} from initial, off={}", n, initial_offset_);
            co_return n;
        }
        ...
        if (pending_offset_ < pending_buffer_.size())
        {
            ...
            diagnose::debug(prefix_, "returned {} from pending, rem={}", n, pending_buffer_.size());
            co_return n;
        }

        diagnose::debug(prefix_, "buffers empty, reading TLS frame");
        ...
        diagnose::debug(prefix_, "TLS frame payload: {} bytes", frame.size());
        ...
        if (frame.size() > n)
        {
            ...
            pending_offset_ = 0;
            diagnose::debug(prefix_, "stored {} pending", pending_buffer_.size());
        }

        diagnose::debug(prefix_, "returned {} to user", n);
        co_return n;
    }
    ...
    {
        diagnose::debug(prefix_, "read_tls_frame: starting to read TLS header");
    ...
    auto shadowtls_transport::async_write_some(std::span<const std::byte> buffer, std::error_code &ec)
        -> net::awaitable<std::size_t>
    {
        diagnose::debug(prefix_, "async_write_some: buffer_size={}, calling write_tls_frame", buffer.size());
    ...
    auto shadowtls_transport::async_write(std::span<const std::byte> data, std::error_code &ec)
        -> net::awaitable<std::size_t>
    {
        diagnose::debug(prefix_, "async_write: data_size={}, calling write_tls_frame directly", data.size());
        ...
        -> net::awaitable<std::size_t>
    {
        ...
        diagnose::debug(prefix_, "wrote {} bytes (TLS frame size={})", payload.size(), frame_bytes.size());
