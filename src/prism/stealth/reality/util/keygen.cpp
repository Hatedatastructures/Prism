...
namespace psm::stealth::reality
{
    ...
    static void copy_key(std::span<const std::uint8_t> src, auto &dst)
    {
        std::memcpy(dst.data(), src.data(), std::min(src.size(), dst.size()));
    }

    auto derive_handshake_keys(constspan shared_secret, constspan chello_msg, constspan shello_msg)
        -> std::pair<fault::code, key_material>
    {
        ...
        if (fault::failed(ec1))
        {
            ...
            return {fault::code::reality_kdferr, keys};
        }
    ...
    auto derive_application_keys(const std::span<const std::uint8_t> master_secret, const std::span<const std::uint8_t> server_finhash, key_material &keys)
        -> fault::code
    {
    ...
    auto compute_finished_verify(const std::span<const std::uint8_t> finished_key, const std::span<const std::uint8_t> transcript_hash)
