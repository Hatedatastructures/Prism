...
namespace psm::stealth::shadowtls
{
        ...
        , hmac_read_ctx_(std::move(handover.hmac_read_ctx))
    {
        ...
        if (hmac_read_ctx_)
            hr_str = "yes";
        trace::debug(prefix_, "shadowtls_transport created, initial_data_size={}, hmac_write_ctx={}, hmac_read_ctx={}",
        ...
        -> net::awaitable<std::size_t>
    {
        ...
        trace::debug(prefix_, "async_read_some: buf={}, init_off={}, init_sz={}, pend_off={}, pend_sz={}",
        ...
        if (initial_offset_ < initial_buffer_.size())
        {
            ...
            initial_offset_ += n;
            trace::debug(prefix_, "returned {} from initial, off={}", n, initial_offset_);
            co_return n;
        }
        ...
        if (pending_offset_ < pending_buffer_.size())
        {
            ...
            trace::debug(prefix_, "returned {} from pending, rem={}", n, pending_buffer_.size());
            co_return n;
        }

        trace::debug(prefix_, "buffers empty, reading TLS frame");
        auto frame_opt = co_await read_tls_frame(ec);
        if (ec || !frame_opt)
        {
            trace::warn(prefix_, "read_tls_frame failed: {}", ec.message());
            co_return 0;
        }

        auto &frame = *frame_opt;
        trace::debug(prefix_, "TLS frame payload: {} bytes", frame.size());
        ...
        if (frame.size() > n)
        {
            ...
            pending_offset_ = 0;
            trace::debug(prefix_, "stored {} pending", pending_buffer_.size());
        }

        trace::debug(prefix_, "returned {} to user", n);
        co_return n;
    }
    ...
    {
        trace::debug(prefix_, "read_tls_frame: starting to read TLS header");

        auto [read_ec, rec] = co_await ::psm::tls::record::read(*lower_);
        if (fault::failed(read_ec))
        {
            ec = std::make_error_code(std::errc::connection_reset);
            trace::warn(prefix_, "read TLS record failed");
            co_return std::nullopt;
        }

        if (rec.header().content_type != content_appdata)
        {
            trace::warn(prefix_, "unexpected TLS record type: 0x{:02x}", rec.header().content_type);
        ...
        {
            trace::warn(prefix_, "payload too small for HMAC: {}", payload.size());
            ec = std::make_error_code(std::errc::protocol_error);
            co_return std::nullopt;
        }
        ...
        if (!hmac_read_ctx_)
        {
            trace::warn(prefix_, "hmac_read_ctx is null, cannot verify HMAC");
            ec = std::make_error_code(std::errc::protocol_error);
            co_return std::nullopt;
        }
        ...
        {
            trace::warn(prefix_, "HMAC mismatch in transport read_tls_frame");
            ec = std::make_error_code(std::errc::protocol_error);
            co_return std::nullopt;
        }
        ...
        trace::debug(prefix_, "TLS frame verified (cumulative HMAC), payload_size={}, added HMAC to cumulative state",
                    actual_data.size());
    ...
    auto shadowtls_transport::async_write_some(std::span<const std::byte> buffer, std::error_code &ec)
        -> net::awaitable<std::size_t>
    {
        trace::debug(prefix_, "async_write_some: buffer_size={}, calling write_tls_frame", buffer.size());
    ...
    auto shadowtls_transport::async_write(std::span<const std::byte> data, std::error_code &ec)
        -> net::awaitable<std::size_t>
    {
        trace::debug(prefix_, "async_write: data_size={}, calling write_tls_frame directly", data.size());
    ...
    {
        ec.clear();
        trace::debug(prefix_, "write_tls_frame: payload_size={}", payload.size());
        ...
        {
            ec = write_ec;
            trace::warn(prefix_, "write TLS frame failed: {}", write_ec.message());
            co_return 0;
        }

        trace::debug(prefix_, "wrote {} bytes (TLS frame size={})", payload.size(), frame_bytes.size());
