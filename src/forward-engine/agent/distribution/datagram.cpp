...
{
    datagram_resolver::datagram_resolver(net::any_io_executor executor, const memory::resource_pointer mr,
    ...
    auto datagram_resolver::resolve(const std::string_view host, const std::string_view port) -> net::awaitable<result>
    {
        ...
        if (const auto hit = records_.find(lookup); hit != records_.end())
        {
            ...
            {
                co_return result{gist::code::success, hit->second.value};
            }
            records_.erase(hit);
        }
        ...
        if (!is_new)
        {
            ...
            coalescer::cleanup_flight(flight);
            co_return result{gist::code::host_unreachable, endpoint{}};
        }
        ...
        records_.insert_or_assign(flight->key, record{resolved, std::chrono::steady_clock::now() + ttl_});
        ...
        coalescer::cleanup_flight(flight);
        co_return result{gist::code::success, resolved};
    }
} // namespace ngx::agent::distribution
