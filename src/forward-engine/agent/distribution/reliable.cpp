...
{
    reliable_resolver::reliable_resolver(source &pool, const net::any_io_executor &executor, const memory::resource_pointer mr,
    ...
    auto reliable_resolver::resolve(std::string_view host, std::string_view port) -> net::awaitable<result>
    {   
        ...
        {
            co_return result{gist::code::success, std::move(cached)};
        }
        // 没找到查找或创建一个解析请求记录表
        ...
        {   // 不是第一条请求解析的等待解析完成
            ...
            // 连接失败
            coalescer::cleanup_flight(flight);
            co_return result{gist::code::host_unreachable, nullptr};
        }
        ...
        record cached{memory::vector<tcp::endpoint>(mr_), std::chrono::steady_clock::now() + ttl_};
        ...
        {
            coalescer::cleanup_flight(flight);
            co_return result{gist::code::success, std::move(socket)};
        }
        ...
        coalescer::cleanup_flight(flight);
        co_return result{gist::code::bad_gateway, nullptr};
    }

    auto reliable_resolver::async_connect(const lookup_key &lookup, const std::chrono::steady_clock::time_point now)
