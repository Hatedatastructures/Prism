...
namespace
{
     ...
     */
    TEST(ShadowTlsE2E, ClientHelloHmacVerify)
    {
        ...
        EXPECT_TRUE(ok) << "ClientHelloHmacVerify: correct password accepted";
    }

    /**
     * @brief 测试 ClientHello HMAC 认证失败（错误密码）
     */
    TEST(ShadowTlsE2E, ClientHelloHmacVerifyWrongPassword)
    {
        ...
        EXPECT_TRUE(!ok) << "ClientHelloHmacVerifyWrongPassword: wrong password rejected";
    }
    ...
    TEST(ShadowTlsE2E, FrameHmacProtocol)
    {
        ...
        EXPECT_TRUE(ok) << "FrameHmacProtocol: correct HMAC accepted";
    }
    ...
    TEST(ShadowTlsE2E, FrameHmacProtocolWrongHmac)
    {
        ...
        EXPECT_TRUE(!ok) << "FrameHmacProtocolWrongHmac: wrong HMAC rejected";
    }
