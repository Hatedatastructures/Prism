...
TEST(Reality, CertificateParsesAsEd25519)
{
    using namespace psm;
    ...
    ASSERT_FALSE(cert_der.empty()) << "failed to extract leaf certificate DER from handshake plaintext";

    const auto *p = cert_der.data();
    X509 *x509 = d2i_X509(nullptr, &p, static_cast<long>(cert_der.size()));
    ASSERT_TRUE(x509 != nullptr) << "OpenSSL failed to parse generated certificate DER";

    EVP_PKEY *pkey = X509_get_pubkey(x509);
    ASSERT_TRUE(pkey != nullptr) << "X509_get_pubkey returned null";

    const auto pkey_type = EVP_PKEY_id(pkey);
    EXPECT_EQ(pkey_type, EVP_PKEY_ED25519) << "leaf certificate public key is not Ed25519";

    EXPECT_EQ(X509_get_signature_nid(x509), NID_ED25519) << "certificate signature algorithm is not Ed25519";
