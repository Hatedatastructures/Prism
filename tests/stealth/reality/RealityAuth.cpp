...
namespace
{
    TEST(RealityAuth, MatchSniExactMatch)
    {
        psm::memory::vector<psm::memory::string> names;
        names.emplace_back("example.com");
        names.emplace_back("test.local");

        EXPECT_TRUE(psm::stealth::reality::match_sni("example.com", names))
            << "match_sni exact match";
        EXPECT_TRUE(psm::stealth::reality::match_sni("test.local", names))
            << "match_sni second entry match";
    }

    TEST(RealityAuth, MatchSniNoMatch)
    {
        psm::memory::vector<psm::memory::string> names;
        names.emplace_back("example.com");

        EXPECT_FALSE(psm::stealth::reality::match_sni("other.com", names))
            << "match_sni no match";
    }
    ...
    TEST(RealityAuth, MatchSniEmpty)
    {
        ...
        EXPECT_FALSE(psm::stealth::reality::match_sni("", names))
    ...
    TEST(RealityAuth, MatchSniEmptyList)
    {
        ...
        EXPECT_FALSE(psm::stealth::reality::match_sni("example.com", names))
            << "match_sni empty list returns false";
    }

    TEST(RealityAuth, MatchShortidWildcard)
    {
        ...
        EXPECT_TRUE(psm::stealth::reality::match_shortid(short_id, allowed))
            << "match_shortid wildcard (empty string) matches any";
    }

    TEST(RealityAuth, MatchShortidExactMatch)
    {
        psm::memory::vector<psm::memory::string> allowed;
    ...
    TEST(RealityAuth, MatchShortidNoMatch)
    {
        ...
        std::array<std::uint8_t, 8> short_id = {0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08};
        EXPECT_FALSE(psm::stealth::reality::match_shortid(short_id, allowed))
    ...
    TEST(RealityAuth, AuthenticateSniMismatch)
    {
    ...
    TEST(RealityAuth, MatchShortidInvalidHex)
    {
        psm::memory::vector<psm::memory::string> allowed;
    ...
    TEST(RealityAuth, MatchShortidPrefixMatch)
    {
        psm::memory::vector<psm::memory::string> allowed;
    ...
    TEST(RealityAuth, AuthenticateFullSuccess)
    {
        // 1. 生成服务端 X25519 密钥对
