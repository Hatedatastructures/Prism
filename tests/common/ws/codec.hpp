...
namespace psmtest::ws
{
    ...
    [[nodiscard]] inline auto compute_accept(std::string_view key) -> std::string
    {
        ...
        SHA1(reinterpret_cast<const std::uint8_t *>(combined.data()), combined.size(),
             digest.data());
     ...
     */
    [[nodiscard]] inline auto parse_frame_header(std::span<const std::byte> in,
     ...
     * @param mask 4 字节掩码键
     */
    inline auto apply_mask(std::span<std::byte> data, std::span<const std::uint8_t, mask_len> mask)
        -> void
    {
