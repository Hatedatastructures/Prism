...
#include <common/stealth/xhttp/types.hpp>
...
namespace psmtest::xhttp
{
    ...
    public:
        ...
        [[nodiscard]] auto run() -> net::awaitable<shared_transmission>
        {
            ...
            auto [code, stream, recovered] =
                co_await psmtest::transport::encrypted::ssl_handshake(std::move(raw_), ssl_ctx_);
            (void)code;
            (void)recovered;
            if (!stream)
            {
            ...
            encrypted_ = std::make_shared<psmtest::transport::encrypted>(std::move(stream));
                ...
                (std::int32_t sid, const h2::header_list &headers, bool)
            {
                ...
                const bool path_ok = base == "/" ? path.rfind('/', 0) == 0 : path.rfind(base, 0) == 0;
                if (is_post && path_ok)
                {
    ...
    private:
        ...
        config cfg_;
        psmtest::shared_transmission encrypted_;
