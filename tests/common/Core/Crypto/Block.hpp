...
namespace Preview::Crypto
{
     ...
     * @return 密文（16 字节）
     */
    [[nodiscard]] auto EcbEncrypt(std::span<const std::uint8_t, 16> input, std::span<const std::uint8_t> key)
     ...
     * @return 明文（16 字节）
     */
    [[nodiscard]] auto EcbDecrypt(std::span<const std::uint8_t, 16> input, std::span<const std::uint8_t> key)
        -> std::array<std::uint8_t, 16>;



    inline auto EcbEncrypt(std::span<const std::uint8_t, 16> input, std::span<const std::uint8_t> key)
        -> std::array<std::uint8_t, 16>
    {
        ...
        if (EVP_EncryptUpdate(ctx, out.data(), &OutLen, input.data(), 16) != 1)
        {
            EVP_CIPHER_CTX_free(ctx);
            return out;
        }
    ...
    inline auto EcbDecrypt(std::span<const std::uint8_t, 16> input, std::span<const std::uint8_t> key)
        -> std::array<std::uint8_t, 16>
    {
        std::array<std::uint8_t, 16> out{};
        ...
        int OutLen = 0; // EVP API 要求 int*
        if (EVP_DecryptUpdate(ctx, out.data(), &OutLen, input.data(), 16) != 1)
        {
            EVP_CIPHER_CTX_free(ctx);
            return out;
        }
