...
namespace preview::recognition
{
    ...
    struct address_hash
    {
         ...
         */
        [[nodiscard]] static auto from_v4(std::uint32_t ip) noexcept 
            -> address_hash;

        /**
         * 从 IPv6 地址构造(取前 16 字节作为 key)
         */
        [[nodiscard]] static auto from_v6(std::span<const std::byte, 16> addr) noexcept 
            -> address_hash;

        /**
         * 从 boost::asio endpoint 构造(IPv4/IPv6 自动判断)
         */
        [[nodiscard]] static auto from_endpoint(bool is_v6, std::uint8_t *addr_bytes, std::size_t addr_len) noexcept 
            -> address_hash;
    };
    ...
    {
    public:
        explicit probe_defense_tracker(std::uint32_t window_sec = 300, std::uint32_t threshold = 2,
         ...
         * @param tier 探测层级
         */
        auto record(const address_hash &src, std::uint16_t tier) 
            -> void;
         ...
         * @return 连续失败次数
         */
        [[nodiscard]] auto fail_count(const address_hash &src) const noexcept 
            -> std::uint16_t;
         ...
         * @return 是否应触发挑战
         */
        [[nodiscard]] auto should_challenge(const address_hash &src) const noexcept 
            -> bool;
    ...
    inline auto address_hash::from_v4(std::uint32_t ip) noexcept 
        -> address_hash
    {
        address_hash h{};
        // IPv4 地址写入前 4 字节(大端序)
    ...
    inline auto address_hash::from_v6(std::span<const std::byte, 16> addr) noexcept 
        -> address_hash
    {
        address_hash h{};
    ...
    inline auto address_hash::from_endpoint(bool is_v6, std::uint8_t *addr_bytes, std::size_t addr_len) noexcept 
        -> address_hash
    {
    ...
    inline auto address_hasher::operator()(const address_hash &key) const noexcept 
        -> std::size_t
    {
        // FNV-1a hash,简单高效
    ...
    inline auto probe_defense_tracker::record(const address_hash &src, std::uint16_t tier) -> void
    {
    ...
    inline auto probe_defense_tracker::fail_count(const address_hash &src) const noexcept 
        -> std::uint16_t
    {
        auto it = records_.find(src);
        if (it == records_.end())
    ...
    inline auto probe_defense_tracker::should_challenge(const address_hash &src) const noexcept -> bool
    {
        if (threshold_ == 0)
        {
            return false;
    ...
    inline auto probe_defense_tracker::reset(const address_hash &src)
        -> void
    {
        records_.erase(src);
    }

    inline auto probe_defense_tracker::expire() -> void
    {
