...
namespace psmtest::transport {
    ...
    inline preview::preview(shared_transmission inner, std::span<const std::byte> preread)
        : inner_(std::move(inner)), preread_buffer_(preread.begin(), preread.end())
    {
    }

    inline auto preview::executor() const -> executor_type
    {
        if (!inner_)
        {
    ...
    inline auto preview::async_read_some(std::span<std::byte> buffer, std::error_code &ec)
        -> net::awaitable<std::size_t>
    {
    ...
    inline void preview::async_read_some(
        std::span<std::byte> buffer,
    ...
    inline void preview::async_write_some(
        std::span<const std::byte> buffer,
        net::any_completion_handler<void(boost::system::error_code, std::size_t)> handler)
    {
        if (!inner_)
