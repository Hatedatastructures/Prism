...
namespace preview::crypto
{
    ...
    inline void derive_key(std::string_view context, std::span<const std::uint8_t> material,
    ...
    inline auto derive_key(std::string_view context, std::span<const std::uint8_t> material,
    ...
    inline auto keyed_hasher(std::span<const std::uint8_t> key) -> blake3_hasher
    {
        blake3_hasher hasher;
    ...
    inline auto keyed_hash(std::span<const std::uint8_t> key, std::span<const std::uint8_t> data)
        -> std::array<std::uint8_t, 32>
    {
        blake3_hasher hasher;
        blake3_hasher_init_keyed(&hasher, key.data());
