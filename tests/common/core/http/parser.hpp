...
namespace psmtest::http11
{
    ...
    namespace detail
    {
        ...
        /// @brief 大小写不敏感比较
        [[nodiscard]] inline auto iequals(const std::string_view l, const std::string_view r) noexcept
            -> bool
        {
            if (l.size() != r.size())
            {
        ...
        /// @brief 去除首尾空白
        [[nodiscard]] inline auto trim(const std::string_view v) noexcept -> std::string_view
        {
            auto s = v;
     ...
     */
    [[nodiscard]] inline auto parse_request(const std::string_view raw, http_request &out) -> fault::code
    {
     ...
     * @return 完整请求字节串
     */
    [[nodiscard]] inline auto make_connect_request(const std::string_view host, const std::uint16_t port,
     ...
     * @return 状态码（解析失败返回 0）
     */
    [[nodiscard]] inline auto parse_status_code(const std::string_view raw) -> int
    {
