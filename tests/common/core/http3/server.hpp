...
#include <common/core/http3/session.hpp>
...
namespace psm::http3
{
     ...
     * @details 薄封装 hysteria2::h3::server：持有构造选项，
    ...
    public:
        ...
        explicit server(server_options options)
            : options_(std::move(options)),
              inner_(options_.mr ? options_.mr : memory::current_resource())
        {
        }
     ...
     * @param options 构造选项
     * @return 服务端共享指针
     * @details 内部包装 hysteria2::h3::server；mr 为 nullptr 时
     * 自动回退到当前默认内存资源。
     */
