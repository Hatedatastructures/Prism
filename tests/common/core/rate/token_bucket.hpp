...
namespace preview::rate
{
    ...
    public:
        ...
        explicit token_bucket(std::size_t capacity, std::chrono::milliseconds refill_interval,
            ...
            : capacity_(std::min<std::size_t>(capacity, k_tokens_max))
        {
            ...
            state_.store(static_cast<std::uint64_t>(capacity_) << k_tokens_shift,
        ...
        [[nodiscard]] auto try_take(std::size_t n, std::uint64_t now) -> bool
        {
            ...
            const auto rel_now = (now + 1) > base ? (now + 1) - base : 0; // 时钟回拨钳制为 0

            auto state = state_.load(std::memory_order_relaxed);
            while (true)
            {
                const auto tokens = state >> k_tokens_shift;
                const auto rel_last = state & k_rel_mask;

                // 惰性补发：按经过的完整间隔补令牌（rel_last==0 = 创建时刻 0）
                auto next_tokens = tokens;
                auto next_rel = rel_last;
                if (rel_now > rel_last)
                {
                    const auto elapsed = rel_now - rel_last;
                    ...
                    if (intervals > 0)
                    {
                        ...
                        const auto delta = intervals * refill_interval_ms_;
                        next_rel = delta >= k_rel_mask
                                       ? k_rel_mask
                                       : (rel_last <= k_rel_mask - delta ? rel_last + delta
                                                                         : k_rel_mask);
                ...
                if (next_tokens < n)
                {
                    ...
                    if (next_tokens != tokens || next_rel != rel_last)
                    {
                        const auto packed = ((next_tokens & k_tokens_max) << k_tokens_shift) |
                                            (next_rel & k_rel_mask);
                ...
                const auto new_state =
                    (((next_tokens - n) & k_tokens_max) << k_tokens_shift) |
                    (next_rel & k_rel_mask);
                if (state_.compare_exchange_weak(state, new_state, std::memory_order_relaxed,
                                                 std::memory_order_relaxed))
                {
                    return true;
                }
        ...
        {
            return static_cast<std::size_t>(state_.load(std::memory_order_relaxed) >> k_tokens_shift);
        }
    ...
    private:
        static constexpr std::uint64_t k_tokens_shift{32};               ///< 令牌字段位移
        static constexpr std::uint64_t k_tokens_max{0xFFFFFFFFULL};      ///< 令牌字段上限（2^32-1）
        static constexpr std::uint64_t k_rel_mask{0xFFFFFFFFULL};        ///< 相对补发时刻掩码（低 32 位）
        ...
        std::atomic<std::uint64_t> base_{0};        ///< 时间基准（0 = 未初始化；存 now+1，首次 try_take 确定）
    };
