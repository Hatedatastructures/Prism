...
namespace psmtest::coroutine
{
    ...
    inline task_token::~task_token() noexcept
    {
        if (!released_)
        {
            release();
    ...
    inline auto task_token::release() noexcept -> void
    {
        if (released_)
        {
            return;
    ...
    inline auto task_registry::cancel_and_wait(const std::chrono::milliseconds /*timeout*/) -> bool
    {
        cancelling_ = true;
    ...
    inline auto task_registry::stats() const noexcept -> task_stats
    {
        return task_stats{tokens_.size(), total_spawned_, total_released_, total_cancelled_};
    }

    inline auto task_registry::release_internal(const task_token &token) noexcept -> void
    {
        if (cancelling_)
        {
            return;
