...
namespace preview::mux::h2mux
{
     ...
     * @return 9 字节帧
     */
    [[nodiscard]] inline auto build_close(std::uint32_t stream_id) -> std::array<std::uint8_t, frame_hdrsize>
    {
    ...
    struct codec
    {
        ...
        [[nodiscard]] static auto frame_event(const frame_type &frame) noexcept -> mux::stream_event
        {
            switch (frame.type)
            {
            case h2mux::frame_type::data: return mux::stream_event::data;
            case h2mux::frame_type::close: return mux::stream_event::fin;
            default: return mux::stream_event::rst; // window_update/ping 忽略
            }
        }
        ...
        {
            return frame.type != h2mux::frame_type::data && frame.type != h2mux::frame_type::close;
        }
