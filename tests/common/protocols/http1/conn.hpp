...
namespace preview::http11
{
     ...
     */
    [[nodiscard]] inline auto check_basic(std::string_view authorization,
                                          const preview::authenticator &auth) -> preview::auth_result
    {
        ...
        const auto decoded = preview::crypto::base64_decode(authorization.substr(basic_prefix.size()));
    ...
    public:
        ...
        [[nodiscard]] auto read_request(http_request &out) -> net::awaitable<fault::code>
        {
            ...
            while (true)
            {
                ...
                const auto n = co_await transport_->async_read_some(
                    as_bytes_span(std::span(buffer_.data() + used, buffer_.size() - used)), ec);
                if (ec || n == 0)
                {
         ...
         * @return 成功或 io_error
         */
        [[nodiscard]] auto send_response(std::string_view body) -> net::awaitable<fault::code>
        {
            std::error_code ec;
            const auto span = as_bytes_span(body);
     ...
     * @return 成功或 io_error
     */
    [[nodiscard]] inline auto send_connect(shared_transmission transport, std::string_view host,
        ...
        -> net::awaitable<fault::code>
    {
        ...
        std::error_code ec;
        const auto span = as_bytes_span(req);
        ...
        -> net::awaitable<int>
    {
        ...
        while (true)
        {
            ...
            const auto n = co_await transport->async_read_some(
                    as_bytes_span(std::span(buf.data() + used, buf.size() - used)), ec);
            if (ec || n == 0)
            {
                co_return 0;
            }
