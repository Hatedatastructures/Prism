...
namespace preview::socks5
{
    ...
    public:
            ...
            -> net::awaitable<error>
        {
            ...
            if (enable_auth)
            {
                g.methods = std::vector<std::uint8_t>{static_cast<std::uint8_t>(auth_method::user_pass)};
            }
            else
            {
                g.methods = std::vector<std::uint8_t>{static_cast<std::uint8_t>(auth_method::no_auth)};
            }
            ...
            -> net::awaitable<std::pair<error, request>>
        {
            ...
            if (enable_auth)
            {
                want = static_cast<std::uint8_t>(auth_method::user_pass);
            }
            else
            {
                want = static_cast<std::uint8_t>(auth_method::no_auth);
            }
