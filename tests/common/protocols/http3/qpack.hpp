...
namespace preview::http3::qpack
{
    ...
    namespace
    {
         ...
         * @return 是否解码成功
         */
        [[nodiscard]] auto read_varint(std::span<const std::uint8_t> in, std::uint8_t prefix_bits,
         ...
         */
        [[nodiscard]] auto write_varint(std::span<std::uint8_t> out, std::uint8_t prefix_bits,
         ...
         */
        [[nodiscard]] auto lookup_encoder_name(std::string_view name) -> const encoder_name_entry *
        {
            for (const auto &entry : encoder_names)
            {
                if (entry.name == name)
         ...
         */
        [[nodiscard]] auto huffman_decode_to(std::span<const std::uint8_t> in,
         ...
         *       字节即可保证不失败（热路径栈缓冲零分配）
         */
        [[nodiscard]] auto huffman_encode_to(std::string_view in, std::span<std::uint8_t> out)
            -> std::size_t
        {
            std::uint64_t acc = 0;
            std::uint32_t acc_bits = 0;
                                      ...
                                      std::string &out) -> bool
        {
            ...
            memory::vector<std::uint8_t> heap(preview::memory::current_resource());
            if (huffman)
            {
         ...
         */
        [[nodiscard]] auto write_string_prefixed(std::span<std::uint8_t> out, std::size_t &offset,
    ...
    inline auto decode_header_block(std::span<const std::uint8_t> data, const memory::resource_pointer mr)
        -> memory::vector<header_field>
    {
    ...
    inline auto encode_prefix(std::span<std::uint8_t> out) -> std::size_t
    {
        if (out.size() < 2)
        {
            return 0;
    ...
    inline auto encode_literal(std::string_view name, std::string_view value,
    ...
    inline auto huffman_decode(std::span<const std::uint8_t> in, memory::vector<std::uint8_t> &out) -> bool
    {
        return huffman_decode_impl(in, out);
    }

    inline auto huffman_encode(std::string_view in, memory::vector<std::uint8_t> &out) -> bool
    {
