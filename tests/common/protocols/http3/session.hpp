...
namespace preview::http3 {
     ...
     */
    class auth_server : public std::enable_shared_from_this<auth_server>
    {
    public:
        /**
         * @brief 构造函数
         * @param mr 内存资源
         */
        explicit auth_server(memory::resource_pointer mr);

        /**
         * @brief 析构函数，释放 nghttp3 连接状态
         */
        ~auth_server() noexcept;
    ...
    namespace
    {
        ...
        [[nodiscard]] auto rcbuf_view(nghttp3_rcbuf *rc) -> std::string_view
        {
            ...
            return as_str_view(std::span<const std::uint8_t>(buf.base, buf.len));
        }
    } // namespace

    inline auth_server::auth_server(const memory::resource_pointer mr) : mr_(mr), method_(mr), path_(mr), auth_(mr)
    {
    }

    inline auth_server::~auth_server() noexcept
    {
        close();
    }

    inline auto auth_server::now_tstamp() -> std::uint64_t
    {
    ...
    inline auto auth_server::init(std::function<std::int64_t()> open_uni_stream) -> bool
    {
        if (conn_)
        {
            return true;
    ...
    inline auto auth_server::feed(const std::int64_t stream_id, std::span<const std::byte> data, const bool fin)
        -> fault::code
    {
        if (!conn_)
        {
        ...
        const auto rc =
            nghttp3_conn_read_stream2(conn_, stream_id, as_u8(data).data(), data.size(), fin_flag,
                                      now_tstamp());
        if (rc < 0)
        {
    ...
    inline auto auth_server::pump_output(memory::vector<out_packet> &out) -> bool
    {
        if (!conn_)
        {
            return false;
        ...
        for (;;)
        {
            ...
            {
                vecs[i].base = as_u8(std::span(bufs[i])).data();
            ...
            {
                const auto bytes = as_bytes(std::span<const std::uint8_t>(vecs[i].base, vecs[i].len));
    ...
    inline void auth_server::add_write_offset(const std::int64_t stream_id, const std::size_t len)
    {
        if (conn_ && len > 0)
        {
    ...
    inline auto auth_server::auth_headers_complete() const noexcept -> bool
    {
        return headers_done_;
    }

    inline auto auth_server::method() const noexcept -> std::string_view
    {
        return std::string_view(method_.data(), method_.size());
    }

    inline auto auth_server::path() const noexcept -> std::string_view
    {
        return std::string_view(path_.data(), path_.size());
    }

    inline auto auth_server::auth() const noexcept -> std::string_view
    {
        return std::string_view(auth_.data(), auth_.size());
    }

    inline auto auth_server::rx() const noexcept -> std::uint64_t
    {
        return rx_;
    }

    inline auto auth_server::auth_stream_id() const noexcept -> std::int64_t
    {
        return auth_stream_;
    }

    inline auto auth_server::submit_auth_response() -> fault::code
    {
    ...
    inline auto auth_server::cb_begin_headers(nghttp3_conn *conn, const int64_t stream_id, void *user_data,
                                  void *stream_user_data) -> int
    {
        (void)conn;
        (void)stream_user_data;
        auto *self = static_cast<auth_server *>(user_data);
    ...
    inline auto auth_server::cb_recv_header(nghttp3_conn *conn, const int64_t stream_id, const int32_t token,
    ...
    inline auto auth_server::cb_end_headers(nghttp3_conn *conn, const int64_t stream_id, const int fin, void *user_data,
                                void *stream_user_data) -> int
    {
        (void)conn;
        (void)fin;
    ...
    inline auto auth_server::cb_recv_data(nghttp3_conn *conn, const int64_t stream_id, const uint8_t *data,
    ...
    inline auto auth_server::cb_stop_sending(nghttp3_conn *conn, const int64_t stream_id, const uint64_t app_error_code,
                                 void *user_data, void *stream_user_data) -> int
    {
        (void)conn;
        (void)stream_id;
    ...
    inline auto auth_server::cb_end_stream(nghttp3_conn *conn, const int64_t stream_id, void *user_data,
                               void *stream_user_data) -> int
    {
        (void)conn;
        (void)stream_id;
    ...
    inline void auth_server::cb_rand(uint8_t *dest, const size_t destlen)
    {
