 ...
 *          - ComputeServerMask：BLAKE3 keyed(Secret, server_random) → 16B
 *          - ComputeAuthMac：BLAKE3 keyed(Secret, server_random + dir +
 *            counter + [ClientFinished] + TlsHeader + payload) → 8B
 *          - ComputeMask：BLAKE3 keyed(Secret, server_random + dir +
...
namespace Preview::Restls
{
    ...
    struct AuthMacInput
    {
        ...
        FlowDirection direction{FlowDirection::to_client}; ///< 数据流方向
        ...
        -> std::array<std::uint8_t, AppdataMaclen>
    {
        ...
        std::string_view dir;
        if (in.direction == FlowDirection::to_client)
        {
            dir = DirToclient;
        }
        else
