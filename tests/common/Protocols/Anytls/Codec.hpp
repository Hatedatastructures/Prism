...
namespace Preview::Anytls
{
     ...
     */
    [[nodiscard]] inline auto PasswordHashLen(std::string_view password)
     ...
     * @return 错误码
     */
    [[nodiscard]] inline auto BuildAuthFrame(std::string_view password, std::uint16_t pad_len, std::string &out)
        -> Error
    {
        const auto Hash = PasswordHashLen(password);
        out.clear();
        out.reserve(AuthFrameHdrlen + pad_len);
                                               ...
                                               std::uint16_t &pad_len) -> Error
    {
        ...
        pad_len = static_cast<std::uint16_t>(Data[PasswordHashLen]) << 8 | Data[PasswordHashLen + 1];
        if (Data.size() < AuthFrameHdrlen + pad_len)
        {
            return Error::bad_length;
        }
        return Error::none;
    ...
    {
        const auto expected = PasswordHashLen(password);
