...
namespace
{
    ...
    void TestReadU16Basic(TestRunner &runner)
    {
        ...
        runner.Check(val == 0x0123, "read_u16: 0x0123");
    }
    ...
    void TestReadU16Max(TestRunner &runner)
    {
        ...
        auto val = psm::recognition::tls::read_u16({data, 2}, 0);
        runner.Check(val == 0xFFFF, "read_u16: max");
    }

    void TestReadU16Offset(TestRunner &runner)
    {
        std::uint8_t data[] = {0x00, 0xAB, 0xCD};
        auto val = psm::recognition::tls::read_u16({data, 3}, 1);
        runner.Check(val == 0xABCD, "read_u16: offset=1");
    }
    ...
    void TestReadU24Basic(TestRunner &runner)
    {
        ...
        runner.Check(val == 0x012345, "read_u24: 0x012345");
    }
    ...
    void TestReadU24Max(TestRunner &runner)
    {
        ...
        auto val = psm::recognition::tls::read_u24({data, 3}, 0);
        runner.Check(val == 0xFFFFFF, "read_u24: max");
    }

    void TestReadU24Offset(TestRunner &runner)
    {
        std::uint8_t data[] = {0x00, 0x00, 0x01, 0x02};
        auto val = psm::recognition::tls::read_u24({data, 4}, 1);
        runner.Check(val == 0x000102, "read_u24: offset=1");
    }

    // ─── parse_sni ──────────────────────────────

    void TestParseSniTooShort(TestRunner &runner)
    {
        tls_proto::hello_features features;
        std::uint8_t data[] = {0x00}; // < 2 bytes
        psm::recognition::tls::parse_sni({data, 1}, features);
        runner.Check(features.server_name.empty(), "parse_sni: too short -> no SNI");
    }

    void TestParseSniEmptyList(TestRunner &runner)
    {
        tls_proto::hello_features features;
        std::uint8_t data[] = {0x00, 0x00}; // list_len=0
        psm::recognition::tls::parse_sni({data, 2}, features);
        runner.Check(features.server_name.empty(), "parse_sni: empty list");
    }
    ...
    void TestParseSniHostname(TestRunner &runner)
    {
        ...
        runner.Check(features.server_name == "example.com", "parse_sni: hostname found");
    }
    ...
    void TestParseSniNonHostnameType(TestRunner &runner)
    {
        ...
        psm::recognition::tls::parse_sni({buf.data(), buf.size()}, features);
        runner.Check(features.server_name == "test.org", "parse_sni: skips non-hostname, finds hostname");
    }

    void TestParseSniTruncatedName(TestRunner &runner)
    {
        tls_proto::hello_features features;
        ...
        psm::recognition::tls::parse_sni({buf.data(), buf.size()}, features);
        runner.Check(features.server_name.empty(), "parse_sni: truncated name -> no SNI");
    }

    // ─── parse_key_share ────────────────────────

    void TestParseKeyShareTooShort(TestRunner &runner)
    {
        tls_proto::hello_features features;
        std::uint8_t data[] = {0x00};
        psm::recognition::tls::parse_key_share({data, 1}, features);
        runner.Check(!features.has_x25519, "parse_key_share: too short -> no key");
    }
    ...
    void TestParseKeyShareX25519(TestRunner &runner)
    {
        ...
        runner.Check(features.has_x25519, "parse_key_share: X25519 found");
        runner.Check(features.x25519_key[0] == 0xCA, "parse_key_share: key[0]=0xCA");
        runner.Check(features.x25519_key[31] == 0xFE, "parse_key_share: key[31]=0xFE");
    }

    void TestParseKeyShareX25519Mlkem768(TestRunner &runner)
    {
        tls_proto::hello_features features;
        ...
        psm::recognition::tls::parse_key_share({buf.data(), buf.size()}, features);
        runner.Check(features.has_x25519, "parse_key_share: X25519MLKEM768 found");
        runner.Check(features.x25519_key[0] == 0xDE, "parse_key_share: hybrid key[0]=0xDE");
        runner.Check(features.x25519_key[31] == 0xAD, "parse_key_share: hybrid key[31]=0xAD");
    }

    void TestParseKeyShareWrongGroup(TestRunner &runner)
    {
        tls_proto::hello_features features;
        psm::memory::vector<std::uint8_t> buf;
        auto entry_len = static_cast<std::uint16_t>(2 + 2 + 32);
        ...
        psm::recognition::tls::parse_key_share({buf.data(), buf.size()}, features);
        runner.Check(!features.has_x25519, "parse_key_share: wrong group -> no key");
    }
    ...
    void TestParseKeyShareTruncatedKey(TestRunner &runner)
    {
        ...
        psm::recognition::tls::parse_key_share({buf.data(), buf.size()}, features);
        runner.Check(!features.has_x25519, "parse_key_share: truncated key -> no key");
    }
    ...
    void TestParseVersionsEmpty(TestRunner &runner)
    {
        ...
        runner.Check(features.versions.empty(), "parse_versions: empty -> no versions");
    }

    void TestParseVersionsSingle(TestRunner &runner)
    {
        tls_proto::hello_features features;
        std::uint8_t data[] = {0x02, 0x03, 0x03}; // list_len=2, version=0x0303
        psm::recognition::tls::parse_versions({data, 3}, features);
        runner.Check(features.versions.size() == 1, "parse_versions: single version");
        runner.Check(features.versions[0] == 0x0303, "parse_versions: TLS 1.2");
    }

    void TestParseVersionsMultiple(TestRunner &runner)
    {
        tls_proto::hello_features features;
        ...
        runner.Check(features.versions.size() == 3, "parse_versions: 3 versions");
        runner.Check(features.versions[0] == 0x0303, "parse_versions: v[0]=TLS1.2");
        runner.Check(features.versions[1] == 0x0304, "parse_versions: v[1]=TLS1.3");
        runner.Check(features.versions[2] == 0x0301, "parse_versions: v[2]=TLS1.0");
    }

    void TestParseVersionsOddLength(TestRunner &runner)
    {
        tls_proto::hello_features features;
        // list_len=3 (odd), but only 2 bytes available → parse 1 version
        std::uint8_t data[] = {0x03, 0x03, 0x03};
        psm::recognition::tls::parse_versions({data, 3}, features);
        runner.Check(features.versions.size() == 1, "parse_versions: odd list -> 1 version");
    }

    // ─── parse_extensions ───────────────────────

    void TestParseExtensionsTooShort(TestRunner &runner)
    {
        tls_proto::hello_features features;
        std::uint8_t data[] = {0x00};
        psm::recognition::tls::parse_extensions({data, 1}, features);
        runner.Check(features.server_name.empty(), "parse_extensions: too short -> nothing");
    }

    void TestParseExtensionsEmpty(TestRunner &runner)
    {
        tls_proto::hello_features features;
        std::uint8_t data[] = {0x00, 0x00}; // ext_total_len=0
        psm::recognition::tls::parse_extensions({data, 2}, features);
        runner.Check(features.server_name.empty(), "parse_extensions: empty -> nothing");
    }

    void TestParseExtensionsUnknownType(TestRunner &runner)
    {
        tls_proto::hello_features features;
        psm::memory::vector<std::uint8_t> buf;
        ...
        runner.Check(features.server_name.empty(), "parse_extensions: unknown type ignored");
        runner.Check(!features.has_ech, "parse_extensions: unknown type -> no ech");
    }

    void TestParseExtensionsTruncatedExt(TestRunner &runner)
    {
        tls_proto::hello_features features;
        psm::memory::vector<std::uint8_t> buf;
        ...
        psm::recognition::tls::parse_extensions({buf.data(), buf.size()}, features);
        runner.Check(features.server_name.empty(), "parse_extensions: truncated ext -> break");
    }

    void TestParseExtensionsEch(TestRunner &runner)
    {
        tls_proto::hello_features features;
        psm::memory::vector<std::uint8_t> buf;
        ...
        psm::recognition::tls::parse_extensions({buf.data(), buf.size()}, features);
        runner.Check(features.has_ech, "parse_extensions: ECH found");
    }
} // namespace
...
auto main() -> int
{
    ...
    TestRunner runner("TlsSignalParseExt");

    TestReadU16Basic(runner);
    TestReadU16Zero(runner);
    TestReadU16Max(runner);
    TestReadU16Offset(runner);
    TestReadU24Basic(runner);
    TestReadU24Zero(runner);
    TestReadU24Max(runner);
    TestReadU24Offset(runner);

    TestParseSniTooShort(runner);
    TestParseSniEmptyList(runner);
    TestParseSniHostname(runner);
    TestParseSniNonHostnameType(runner);
    TestParseSniTruncatedName(runner);

    TestParseKeyShareTooShort(runner);
    TestParseKeyShareX25519(runner);
    TestParseKeyShareX25519Mlkem768(runner);
    TestParseKeyShareWrongGroup(runner);
    TestParseKeyShareTruncatedKey(runner);

    TestParseVersionsEmpty(runner);
    TestParseVersionsSingle(runner);
    TestParseVersionsMultiple(runner);
    TestParseVersionsOddLength(runner);

    TestParseExtensionsTooShort(runner);
    TestParseExtensionsEmpty(runner);
    TestParseExtensionsUnknownType(runner);
    TestParseExtensionsTruncatedExt(runner);
    TestParseExtensionsEch(runner);
