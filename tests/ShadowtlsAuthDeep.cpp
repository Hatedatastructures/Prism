...
namespace
{
    ...
    void TestComputeHmacBasic(TestRunner &runner)
    {
        ...
        runner.Check(result.size() == 4, "compute_hmac: size=4");
        // 确定性：相同输入产生相同输出
        auto result2 = compute_hmac(key, data.data(), data.size());
        runner.Check(result == result2, "compute_hmac: deterministic");
    }

    void TestComputeHmacEmptyData(TestRunner &runner)
    {
        const char *key = "password";
        auto result = compute_hmac(key, nullptr, 0);
        runner.Check(result.size() == 4, "compute_hmac: empty data -> size=4");
    }

    void TestComputeHmacDifferentKey(TestRunner &runner)
    {
        ...
        runner.Check(r1 != r2, "compute_hmac: different key -> different result");
    }

    void TestComputeHmacDifferentData(TestRunner &runner)
    {
        const char *key = "password";
        ...
        runner.Check(r1 != r2, "compute_hmac: different data -> different result");
    }
    ...
    void TestVerifyClientHelloValid(TestRunner &runner)
    {
        auto buf = make_valid_client_hello("mypassword");
        auto span = std::span<const std::byte>(buf.data(), buf.size());
        runner.Check(verify_client_hello(span, "mypassword"), "verify_client_hello: valid -> true");
    }

    void TestVerifyClientHelloWrongPassword(TestRunner &runner)
    {
        auto buf = make_valid_client_hello("mypassword");
        auto span = std::span<const std::byte>(buf.data(), buf.size());
        runner.Check(!verify_client_hello(span, "wrongpassword"), "verify_client_hello: wrong password -> false");
    }

    void TestVerifyClientHelloTooShort(TestRunner &runner)
    {
        psm::memory::vector<std::byte> buf(10, std::byte{}, psm::memory::current_resource());
        auto span = std::span<const std::byte>(buf.data(), buf.size());
        runner.Check(!verify_client_hello(span, "password"), "verify_client_hello: too short -> false");
    }

    void TestVerifyClientHelloBadContentType(TestRunner &runner)
    {
        auto buf = make_valid_client_hello("password");
        // 修改 content type 为非 handshake
        buf[0] = std::byte{0x17}; // Application Data
        auto span = std::span<const std::byte>(buf.data(), buf.size());
        runner.Check(!verify_client_hello(span, "password"), "verify_client_hello: bad content type -> false");
    }

    void TestVerifyClientHelloBadHandshakeType(TestRunner &runner)
    {
        auto buf = make_valid_client_hello("password");
        buf[5] = std::byte{0x02}; // ServerHello
        auto span = std::span<const std::byte>(buf.data(), buf.size());
        runner.Check(!verify_client_hello(span, "password"), "verify_client_hello: bad handshake type -> false");
    }

    void TestVerifyClientHelloBadSessionIdLen(TestRunner &runner)
    {
        auto buf = make_valid_client_hello("password");
        auto *raw = reinterpret_cast<std::uint8_t *>(buf.data());
        raw[session_id_len_idx] = 16; // 非 32
        auto span = std::span<const std::byte>(buf.data(), buf.size());
        runner.Check(!verify_client_hello(span, "password"), "verify_client_hello: bad session_id len -> false");
    }

    // ─── verify_frame_hmac ─────────────────────────

    void TestVerifyFrameHmacValid(TestRunner &runner)
    {
        ...
        verify_input in{
            ...
            payload,
            client_hmac};
        runner.Check(verify_frame_hmac(in), "verify_frame_hmac: valid -> true");

        // 篡改一个字节应失败
        std::array<std::uint8_t, 4> bad_hmac = client_hmac;
        bad_hmac[0] ^= 0xFF;
        verify_input in_bad{password, server_random, payload, bad_hmac};
        runner.Check(!verify_frame_hmac(in_bad), "verify_frame_hmac: tampered -> false");
    }

    // ─── compute_write_hmac ────────────────────────

    void TestComputeWriteHmacBasic(TestRunner &runner)
    {
        const char *password = "password";
        std::array<std::byte, 32> server_random{};
        std::array<std::byte, 16> payload{};
        ...
        runner.Check(result.size() == 4, "compute_write_hmac: size=4");

        // 确定性
        auto result2 = compute_write_hmac(password, server_random, payload);
        runner.Check(result == result2, "compute_write_hmac: deterministic");
    }

    void TestComputeWriteHmacDifferentPayload(TestRunner &runner)
    {
        const char *password = "password";
        std::array<std::byte, 32> server_random{};
        ...
        runner.Check(r1 != r2, "compute_write_hmac: different payload -> different");
    }
    ...
    void TestComputeWriteKeyBasic(TestRunner &runner)
    {
        ...
        runner.Check(key.size() == SHA256_DIGEST_LENGTH, "compute_write_key: size=32");
    }
    ...
    void TestComputeWriteKeyDeterministic(TestRunner &runner)
    {
        ...
        runner.Check(k1 == k2, "compute_write_key: deterministic");
    }

    void TestComputeWriteKeyDifferentInputs(TestRunner &runner)
    {
...
int main()
{
    ...
    TestRunner runner("ShadowtlsAuthDeep");

    // compute_hmac
    TestComputeHmacBasic(runner);
    TestComputeHmacEmptyData(runner);
    TestComputeHmacDifferentKey(runner);
    TestComputeHmacDifferentData(runner);

    // verify_client_hello
    TestVerifyClientHelloValid(runner);
    TestVerifyClientHelloWrongPassword(runner);
    TestVerifyClientHelloTooShort(runner);
    TestVerifyClientHelloBadContentType(runner);
    TestVerifyClientHelloBadHandshakeType(runner);
    TestVerifyClientHelloBadSessionIdLen(runner);

    // verify_frame_hmac
    TestVerifyFrameHmacValid(runner);

    // compute_write_hmac
    TestComputeWriteHmacBasic(runner);
    TestComputeWriteHmacDifferentPayload(runner);

    // compute_write_key
    TestComputeWriteKeyBasic(runner);
    TestComputeWriteKeyDeterministic(runner);
    TestComputeWriteKeyDifferentInputs(runner);
