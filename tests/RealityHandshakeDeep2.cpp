...
namespace
{
    ...
    void TestDeriveAndEncryptFinishedSuccess(TestRunner &runner)
    {
        ...
        runner.Check(ks_ec == psm::fault::code::success, "derive_encrypt_fin: key derivation ok");
        ...
        runner.Check(sh_result.enc_hs_record.size() > 0,
                     "derive_encrypt_fin: encrypted record produced");
        runner.Check(sh_result.enc_hs_plain.size() >= 36,
                     "derive_encrypt_fin: plaintext updated");
    }

    void TestDeriveAndEncryptFinishedPlaintextTooShort(TestRunner &runner)
    {
        auto chello_raw = make_chello_raw();
        ...
        auto ec = derive_and_encrypt_finished(keys, sh_result, chello_raw);
        runner.Check(ec == psm::fault::code::kdferr,
                     "derive_encrypt_fin: too short -> kdferr");
    }

    void TestDeriveAndEncryptFinishedExactMinSize(TestRunner &runner)
    {
        auto chello_raw = make_chello_raw();
    ...
    void TestDeriveAndEncryptFinished35Bytes(TestRunner &runner)
    {
        auto chello_raw = make_chello_raw();
    ...
    void TestDeriveAndEncryptFinishedEmptyPlaintext(TestRunner &runner)
    {
        auto chello_raw = make_chello_raw();
    ...
    void TestNegotiateTlsSuccess(TestRunner &runner)
    {
        // 构造 hello_features
        auto chello_raw = make_chello_raw();
        auto keypair = psm::crypto::generate_keypair();
        ...
        runner.Check(result.done, "negotiate_tls: success -> done=true");
        runner.Check(result.result.error == psm::fault::code::success,
                     "negotiate_tls: no error");
        runner.Check(result.keys.master_secret.size() == 32,
                     "negotiate_tls: master_secret 32 bytes");
        runner.Check(!result.sh_result.shello_record.empty(),
                     "negotiate_tls: shello_record produced");
        runner.Check(!result.sh_result.enc_hs_record.empty(),
                     "negotiate_tls: enc_hs_record produced");
        runner.Check(!result.shared_secret.empty(),
                     "negotiate_tls: shared_secret produced");
    }

    void TestNegotiateTlsX25519Failure(TestRunner &runner)
    {
        ...
        runner.Check(!result.done || result.done,
                     "negotiate_tls: zero key handled without crash");
    }

    void TestNegotiateTlsGenerateShelloFailure(TestRunner &runner)
    {
        ...
        runner.Check(!result.done || result.result.error == psm::fault::code::success,
                     "negotiate_tls: short raw_msg handled");
    }

    void TestNegotiateTlsWithSessionId(TestRunner &runner)
    {
        auto chello_raw = make_chello_raw();
        auto keypair = psm::crypto::generate_keypair();
        ...
        auto result = negotiate_tls(features, auth_res, timer);
        runner.Check(result.done, "negotiate_tls: with session_id success");
    }
...
int main()
{
    ...
    TestRunner runner("RealityHandshakeDeep2");

    // derive_and_encrypt_finished
    TestDeriveAndEncryptFinishedSuccess(runner);
    TestDeriveAndEncryptFinishedPlaintextTooShort(runner);
    TestDeriveAndEncryptFinishedExactMinSize(runner);
    TestDeriveAndEncryptFinished35Bytes(runner);
    TestDeriveAndEncryptFinishedEmptyPlaintext(runner);

    // negotiate_tls
    TestNegotiateTlsSuccess(runner);
    TestNegotiateTlsX25519Failure(runner);
    TestNegotiateTlsGenerateShelloFailure(runner);
    TestNegotiateTlsWithSessionId(runner);
