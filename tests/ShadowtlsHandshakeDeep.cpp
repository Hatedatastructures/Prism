...
namespace
{
    ...
    void TestVerifyClientV3MatchFirstUser(TestRunner &runner)
    {
        config cfg;
        cfg.version = 3;
        ...
        runner.Check(result.has_value(), "verify_client v3: first user match");
        if (result)
        {
            runner.Check(result->matched_user == "alice", "verify_client v3: matched_user=alice");
            runner.Check(result->password == "pass_alice", "verify_client v3: password=pass_alice");
        }
    }

    void TestVerifyClientV3MatchSecondUser(TestRunner &runner)
    {
        config cfg;
        cfg.version = 3;
        cfg.users.push_back(user{psm::memory::string("alice"), psm::memory::string("pass_alice")});
        ...
        auto result = verify_client(cfg, span);
        runner.Check(result.has_value(), "verify_client v3: second user match");
        if (result)
        {
            runner.Check(result->matched_user == "bob", "verify_client v3: matched_user=bob");
            runner.Check(result->password == "pass_bob", "verify_client v3: password=pass_bob");
        }
    }

    void TestVerifyClientV3NoMatch(TestRunner &runner)
    {
        config cfg;
        cfg.version = 3;
        cfg.users.push_back(user{psm::memory::string("alice"), psm::memory::string("pass_alice")});
        ...
        auto result = verify_client(cfg, span);
        runner.Check(!result.has_value(), "verify_client v3: no match -> nullopt");
    }

    void TestVerifyClientV3EmptyPasswordSkipped(TestRunner &runner)
    {
        config cfg;
        cfg.version = 3;
        ...
        auto result = verify_client(cfg, span);
        runner.Check(result.has_value(), "verify_client v3: skip empty password user");
        if (result)
        {
            runner.Check(result->matched_user == "alice", "verify_client v3: skipped empty user");
        }
    }

    void TestVerifyClientV3AllEmptyPasswords(TestRunner &runner)
    {
        config cfg;
        cfg.version = 3;
        ...
        auto result = verify_client(cfg, span);
        runner.Check(!result.has_value(), "verify_client v3: all empty passwords -> nullopt");
    }

    void TestVerifyClientV3EmptyUsers(TestRunner &runner)
    {
        config cfg;
        cfg.version = 3;
        ...
        auto result = verify_client(cfg, span);
        runner.Check(!result.has_value(), "verify_client v3: empty user list -> nullopt");
    }

    // ─── verify_client v1/v2 单密码 ────────────────────

    void TestVerifyClientV2Match(TestRunner &runner)
    {
        config cfg;
        cfg.version = 2;
        ...
        auto result = verify_client(cfg, span);
        runner.Check(result.has_value(), "verify_client v2: password match");
        if (result)
        {
            runner.Check(result->matched_user == "default", "verify_client v2: matched_user=default");
            runner.Check(result->password == "shared_secret", "verify_client v2: password=shared_secret");
        }
    }

    void TestVerifyClientV2NoMatch(TestRunner &runner)
    {
        config cfg;
        cfg.version = 2;
        cfg.password = psm::memory::string("shared_secret");
        ...
        auto result = verify_client(cfg, span);
        runner.Check(!result.has_value(), "verify_client v2: wrong password -> nullopt");
    }

    void TestVerifyClientV2EmptyPassword(TestRunner &runner)
    {
        config cfg;
        cfg.version = 2;
        ...
        auto result = verify_client(cfg, span);
        runner.Check(!result.has_value(), "verify_client v2: empty password -> nullopt");
    }

    void TestVerifyClientV1Match(TestRunner &runner)
    {
        config cfg;
        cfg.version = 1;
        ...
        auto result = verify_client(cfg, span);
        runner.Check(result.has_value(), "verify_client v1: password match");
        if (result)
        {
            runner.Check(result->matched_user == "default", "verify_client v1: matched_user=default");
        }
    }
...
int main()
{
    ...
    TestRunner runner("ShadowtlsHandshakeDeep");

    // verify_client v3 多用户
    TestVerifyClientV3MatchFirstUser(runner);
    TestVerifyClientV3MatchSecondUser(runner);
    TestVerifyClientV3NoMatch(runner);
    TestVerifyClientV3EmptyPasswordSkipped(runner);
    TestVerifyClientV3AllEmptyPasswords(runner);
    TestVerifyClientV3EmptyUsers(runner);

    // verify_client v1/v2 单密码
    TestVerifyClientV2Match(runner);
    TestVerifyClientV2NoMatch(runner);
    TestVerifyClientV2EmptyPassword(runner);
    TestVerifyClientV1Match(runner);
