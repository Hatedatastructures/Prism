...
namespace
{
    psm::testing::TestRunner runner("MuxParcel");
} // namespace
    ...
    std::span<const std::byte> incoming) -> bool
{
    ...
    return parse_udp_datagram(accumulated, psm::memory::current_resource()).has_value();
}
 ...
 */
void TestCrossFrameSplitPacketAddrIPv4()
{
    runner.LogInfo("=== TestCrossFrameSplitPacketAddrIPv4 ===");
    ...
    auto full = build_udp_datagram("127.0.0.1", 9090, payload, psm::memory::current_resource());
    ...
    runner.Check(!ok1, "cross-frame IPv4: frame 1 incomplete");

    // 帧 2：地址+端口完整，但缺少 Length+Payload
    const bool ok2 = accumulate_and_try_parse_packet_addr(buf, frag2);
    runner.Check(!ok2, "cross-frame IPv4: frame 2 incomplete");

    // 帧 3：Length+Payload 到达，数据报完整
    const bool ok3 = accumulate_and_try_parse_packet_addr(buf, frag3);
    runner.Check(ok3, "cross-frame IPv4: frame 3 complete");

    if (ok3)
    {
        auto result = parse_udp_datagram(buf, psm::memory::current_resource());
        runner.Check(result.has_value(), "cross-frame IPv4: final parse valid");
        if (result)
        {
            runner.Check(result->host == "127.0.0.1", "cross-frame IPv4: host matches");
            runner.Check(result->port == 9090, "cross-frame IPv4: port matches");
            runner.Check(result->payload.size() == 4, "cross-frame IPv4: payload size matches");
            runner.Check(result->consumed == full.size(), "cross-frame IPv4: consumed equals full size");
        }
    }
}
 ...
 */
void TestCrossFrameSplitPacketAddrDomain()
{
    runner.LogInfo("=== TestCrossFrameSplitPacketAddrDomain ===");

    const std::byte payload[] = {std::byte{0x42}};
    auto full = build_udp_datagram("example.com", 443, payload, psm::memory::current_resource());
    ...
    const bool ok1 = accumulate_and_try_parse_packet_addr(buf, frag1);
    runner.Check(!ok1, "cross-frame domain: frame 1 incomplete");

    const bool ok2 = accumulate_and_try_parse_packet_addr(buf, frag2);
    runner.Check(ok2, "cross-frame domain: frame 2 complete");
    ...
    if (ok2)
    {
        ...
        if (result)
        {
            runner.Check(result->host == "example.com", "cross-frame domain: host matches");
            runner.Check(result->port == 443, "cross-frame domain: port matches");
            runner.Check(result->payload.size() == 1, "cross-frame domain: payload size matches");
            runner.Check(result->consumed == full.size(), "cross-frame domain: consumed equals full size");
        }
    }
}
 ...
 */
void TestCrossFrameSplitLengthPrefixed()
{
    ...
    runner.Check(!ok1, "cross-frame LP: header only incomplete");

    const bool ok2 = accumulate_and_try_parse_length_prefixed(buf, frag2);
    runner.Check(ok2, "cross-frame LP: payload arrived complete");
    ...
    if (ok2)
    {
        ...
        if (result)
        {
            runner.Check(result->payload.size() == 3, "cross-frame LP: payload size matches");
            runner.Check(result->consumed == full.size(), "cross-frame LP: consumed equals full size");
            runner.Check(result->payload[0] == std::byte{0x11}, "cross-frame LP: payload content byte 0");
            runner.Check(result->payload[2] == std::byte{0x33}, "cross-frame LP: payload content byte 2");
        }
    }
}
 ...
 */
void TestSingleFrameMultipleDatagrams()
{
    runner.LogInfo("=== TestSingleFrameMultipleDatagrams ===");
    ...
    auto enc1 = build_udp_datagram("10.0.0.1", 1001, payload1, psm::memory::current_resource());
    auto enc2 = build_udp_datagram("10.0.0.2", 1002, payload2, psm::memory::current_resource());
    auto enc3 = build_udp_datagram("10.0.0.3", 1003, payload3, psm::memory::current_resource());
    ...
    auto r1 = parse_udp_datagram(span1, psm::memory::current_resource());
    runner.Check(r1.has_value(), "multi-dgram: datagram 1 parsed");
    if (r1)
    {
        runner.Check(r1->host == "10.0.0.1", "multi-dgram: datagram 1 host");
        runner.Check(r1->port == 1001, "multi-dgram: datagram 1 port");
        runner.Check(r1->payload.size() == 1, "multi-dgram: datagram 1 payload size");
        offset += r1->consumed;
    }

    // 数据报 2
    auto span2 = std::span<const std::byte>(combined.data() + offset, combined.size() - offset);
    auto r2 = parse_udp_datagram(span2, psm::memory::current_resource());
    runner.Check(r2.has_value(), "multi-dgram: datagram 2 parsed");
    if (r2)
    {
        runner.Check(r2->host == "10.0.0.2", "multi-dgram: datagram 2 host");
        runner.Check(r2->port == 1002, "multi-dgram: datagram 2 port");
        runner.Check(r2->payload.size() == 2, "multi-dgram: datagram 2 payload size");
        offset += r2->consumed;
    }

    // 数据报 3
    auto span3 = std::span<const std::byte>(combined.data() + offset, combined.size() - offset);
    auto r3 = parse_udp_datagram(span3, psm::memory::current_resource());
    runner.Check(r3.has_value(), "multi-dgram: datagram 3 parsed");
    if (r3)
    {
        runner.Check(r3->host == "10.0.0.3", "multi-dgram: datagram 3 host");
        runner.Check(r3->port == 1003, "multi-dgram: datagram 3 port");
        runner.Check(r3->payload.size() == 3, "multi-dgram: datagram 3 payload size");
        offset += r3->consumed;
    }

    // 所有字节已消费
    runner.Check(offset == combined.size(), "multi-dgram: all bytes consumed");
}
 ...
 */
void TestSingleFrameMultipleLengthPrefixed()
{
    runner.LogInfo("=== TestSingleFrameMultipleLengthPrefixed ===");
    ...
    if (r1)
    {
        runner.Check(r1->payload.size() == 1, "multi-LP: datagram 1 payload size");
        runner.Check(r1->payload[0] == std::byte{0x01}, "multi-LP: datagram 1 payload content");
        offset += r1->consumed;
    }
    ...
    if (r2)
    {
        runner.Check(r2->payload.size() == 2, "multi-LP: datagram 2 payload size");
        offset += r2->consumed;
    }
    ...
    if (r3)
    {
        runner.Check(r3->payload.size() == 4, "multi-LP: datagram 3 payload size");
        offset += r3->consumed;
    }

    runner.Check(offset == combined.size(), "multi-LP: all bytes consumed");
}
 ...
 * 新数据到达后第二个数据报可解析。
 */
void TestPartialThenComplete()
{
    runner.LogInfo("=== TestPartialThenComplete ===");

    const std::byte p1[] = {std::byte{0x01}, std::byte{0x02}};
    const std::byte p2[] = {std::byte{0x03}, std::byte{0x04}, std::byte{0x05}};

    auto enc1 = build_udp_datagram("10.0.0.1", 1111, p1, psm::memory::current_resource());
    auto enc2 = build_udp_datagram("10.0.0.2", 2222, p2, psm::memory::current_resource());
    ...
    auto r1 = parse_udp_datagram(span, psm::memory::current_resource());
    runner.Check(r1.has_value(), "partial: datagram 1 complete");

    if (r1)
    {
        const auto offset = r1->consumed;
        runner.Check(r1->host == "10.0.0.1", "partial: datagram 1 host");
        runner.Check(r1->port == 1111, "partial: datagram 1 port");
        runner.Check(r1->payload.size() == 2, "partial: datagram 1 payload size");

        // 数据报 2 不完整
        auto remaining = std::span<const std::byte>(buf.data() + offset, buf.size() - offset);
        auto r2 = parse_udp_datagram(remaining, psm::memory::current_resource());
        ...
        auto r3 = parse_udp_datagram(full_remaining, psm::memory::current_resource());
        runner.Check(r3.has_value(), "partial: datagram 2 now complete");
        if (r3)
        {
            runner.Check(r3->host == "10.0.0.2", "partial: datagram 2 host");
            runner.Check(r3->port == 2222, "partial: datagram 2 port");
            runner.Check(r3->payload.size() == 3, "partial: datagram 2 payload size");
            runner.Check(r3->consumed == enc2.size(), "partial: datagram 2 consumed correct");
        }
    }
}
...
{
    runner.LogInfo("=== TestOverflowProtection ===");
    ...
    auto enc_small = build_udp_datagram("127.0.0.1", 80, small_payload, psm::memory::current_resource());
    ...
    auto enc_large = build_udp_datagram("127.0.0.1", 80, large_payload, psm::memory::current_resource());
    ...
    runner.Check(overflow_after, "overflow: buffer exceeds max after accumulation");

    // 解析层：溢出后第一个数据报仍然可正确解析
    // （在 parcel 实际逻辑中，溢出时直接 close 不会到达 parse）
    auto result = parse_udp_datagram(mux_buffer, psm::memory::current_resource());
    runner.Check(result.has_value(), "overflow: first datagram still parseable");
    if (result)
    {
        runner.Check(result->payload.size() == 10, "overflow: first datagram payload correct");
    }
    ...
    runner.Check(lp_buf.size() > udp_max_dg, "overflow: LP buffer exceeds max");
}
 ...
 */
void TestEmptyAndZeroPayload()
{
    runner.LogInfo("=== TestEmptyAndZeroPayload ===");

    // 空数据
    psm::memory::vector<std::byte> empty_buf;
    auto r1 = parse_udp_datagram(empty_buf, psm::memory::current_resource());
    ...
    auto enc_zero = build_udp_datagram("127.0.0.1", 1234, zero_span, psm::memory::current_resource());
    runner.Check(enc_zero.size() == 9, "zero-payload: IPv4 header is 9 bytes");

    auto r3 = parse_udp_datagram(enc_zero, psm::memory::current_resource());
    runner.Check(r3.has_value(), "zero-payload: parse succeeds");
    if (r3)
    {
        runner.Check(r3->payload.empty(), "zero-payload: payload is empty");
        runner.Check(r3->consumed == 9, "zero-payload: consumed is header size only");
    }
    ...
    if (r4)
    {
        runner.Check(r4->payload.empty(), "zero-payload: LP payload is empty");
        runner.Check(r4->consumed == 2, "zero-payload: LP consumed is header size only");
    }
}
...
int main()
{
    ...
    TestCrossFrameSplitPacketAddrIPv4();
    TestCrossFrameSplitPacketAddrDomain();
    TestCrossFrameSplitLengthPrefixed();
    TestSingleFrameMultipleDatagrams();
    TestSingleFrameMultipleLengthPrefixed();
    TestPartialThenComplete();
    TestOverflowProtection();
    TestEmptyAndZeroPayload();
