...
namespace
{
    ...
    void TestIsTls13Yes(TestRunner &runner)
    {
        auto hello = make_tls13_hello();
        runner.Check(is_tls13_local(hello), "is_tls13: TLS 1.3 hello = true");
    }

    void TestIsTls13No(TestRunner &runner)
    {
        auto hello = make_tls12_hello();
        runner.Check(!is_tls13_local(hello), "is_tls13: TLS 1.2 hello = false");
    }

    void TestIsTls13TooShort(TestRunner &runner)
    {
        std::vector<std::byte> short_hello(10, std::byte{0});
        runner.Check(!is_tls13_local(short_hello), "is_tls13: too short = false");
    }

    void TestIsTls13NoExtensions(TestRunner &runner)
    {
        // 无扩展列表
        ...
        raw[ext_offset] = 0;
        raw[ext_offset + 1] = 0;
        runner.Check(!is_tls13_local(hello), "is_tls13: no extensions = false");
    }

    void TestIsTls13WithSessionId(TestRunner &runner)
    {
        auto hello = make_tls13_hello();
        auto *raw = reinterpret_cast<std::uint8_t *>(hello.data());
        // 设置 session_id_len = 4
        ...
        runner.Check(is_tls13_local(hello), "is_tls13: with session_id = true");
    }
...
int main()
{
    ...
    TestRunner runner("RestlsHandshakeParse2");

    TestIsTls13Yes(runner);
    TestIsTls13No(runner);
    TestIsTls13TooShort(runner);
    TestIsTls13NoExtensions(runner);
    TestIsTls13WithSessionId(runner);
