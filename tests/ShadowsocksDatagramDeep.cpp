...
namespace
{
    ...
    void TestParseBodyBadRequestType(TestRunner &runner)
    {
        ...
        runner.Check(ec == psm::fault::code::bad_message, "parse_body: bad request type -> bad_message");
    }

    void TestParseBodyTimestampExpired(TestRunner &runner)
    {
        ...
        udp_dec_pkt result;
        auto ec = parse_body_after_timestamp(body, result);
        runner.Check(ec == psm::fault::code::timestamp_expired, "parse_body: expired ts -> timestamp_expired");
    }

    void TestParseBodyBadAddress(TestRunner &runner)
    {
        const auto now = std::chrono::duration_cast<std::chrono::seconds>(
                             std::chrono::system_clock::now().time_since_epoch())
                             .count();
        ...
        udp_dec_pkt result;
        auto ec = parse_body_after_timestamp(body, result);
        runner.Check(ec != psm::fault::code::success, "parse_body: bad address -> error");
    }

    void TestParseBodyIpv4Success(TestRunner &runner)
    {
        const auto now = std::chrono::duration_cast<std::chrono::seconds>(
                             std::chrono::system_clock::now().time_since_epoch())
                             .count();
        ...
        udp_dec_pkt result;
        auto ec = parse_body_after_timestamp(body, result);
        runner.Check(ec == psm::fault::code::success, "parse_body: IPv4 success");
        runner.Check(result.destination_port == 8080, "parse_body: IPv4 port=8080");
        runner.Check(result.payload.size() == 3, "parse_body: IPv4 payload size=3");
    }

    void TestParseBodyDomainSuccess(TestRunner &runner)
    {
        const auto now = std::chrono::duration_cast<std::chrono::seconds>(
                             std::chrono::system_clock::now().time_since_epoch())
                             .count();
        ...
        udp_dec_pkt result;
        auto ec = parse_body_after_timestamp(body, result);
        runner.Check(ec == psm::fault::code::success, "parse_body: domain success");
        runner.Check(result.destination_port == 443, "parse_body: domain port=443");
        runner.Check(result.payload.size() == 2, "parse_body: domain payload size=2");
    }

    void TestParseBodyWithPadding(TestRunner &runner)
    {
        const auto now = std::chrono::duration_cast<std::chrono::seconds>(
                             std::chrono::system_clock::now().time_since_epoch())
                             .count();
        ...
        udp_dec_pkt result;
        auto ec = parse_body_after_timestamp(body, result);
        runner.Check(ec == psm::fault::code::success, "parse_body: padding success");
        runner.Check(result.payload.size() == 2, "parse_body: padding payload size=2");
        runner.Check(result.payload[0] == 0x11, "parse_body: padding payload[0]");
    }

    void TestParseBodyNoPayload(TestRunner &runner)
    {
        const auto now = std::chrono::duration_cast<std::chrono::seconds>(
                             std::chrono::system_clock::now().time_since_epoch())
                             .count();
        ...
        udp_dec_pkt result;
        auto ec = parse_body_after_timestamp(body, result);
        runner.Check(ec == psm::fault::code::success, "parse_body: no payload success");
        runner.Check(result.payload.empty(), "parse_body: no payload -> empty span");
    }

    void TestParseBodyNoPaddingField(TestRunner &runner)
    {
        const auto now = std::chrono::duration_cast<std::chrono::seconds>(
                             std::chrono::system_clock::now().time_since_epoch())
                             .count();
        ...
        udp_dec_pkt result;
        auto ec = parse_body_after_timestamp(body, result);
        runner.Check(ec == psm::fault::code::success, "parse_body: no padding field success");
        runner.Check(result.payload.empty(), "parse_body: no padding field -> empty payload");
    }

    // ─── 完整 AES-GCM 解密路径 ────────────────────────

    void TestAesDecryptIpv4Success(TestRunner &runner)
    {
        ...
        runner.Check(dec_ec == psm::fault::code::success, "aes ipv4: decode psk");
        ...
        runner.Check(ec == psm::fault::code::success, "aes ipv4 decrypt: success");
        runner.Check(result.destination_port == 80, "aes ipv4 decrypt: port=80");
        runner.Check(result.payload.size() == 1, "aes ipv4 decrypt: payload size=1");
        runner.Check(result.payload[0] == 0xAB, "aes ipv4 decrypt: payload[0]=0xAB");
        runner.Check(result.session_id == session_id, "aes ipv4 decrypt: session_id");
    }

    void TestAesDecryptDomainSuccess(TestRunner &runner)
    {
        auto cfg = make_aes128_config();
        auto tracker = std::make_shared<session_tracker>();
        auto relay = make_udp_relay(cfg, tracker);
        ...
        auto [dec_ec, psk_bytes] = format::decode_psk(cfg.psk);
        runner.Check(dec_ec == psm::fault::code::success, "aes domain: decode psk");
        ...
        auto [ec, result] = relay->decrypt_inbound(packet, sender);
        runner.Check(ec == psm::fault::code::success, "aes domain decrypt: success");
        runner.Check(result.destination_port == 443, "aes domain decrypt: port=443");
        runner.Check(result.payload.size() == 1, "aes domain decrypt: payload size=1");
    }

    void TestAesDecryptWithPadding(TestRunner &runner)
    {
        auto cfg = make_aes128_config();
        auto tracker = std::make_shared<session_tracker>();
        auto relay = make_udp_relay(cfg, tracker);
        ...
        auto [dec_ec, psk_bytes] = format::decode_psk(cfg.psk);
        runner.Check(dec_ec == psm::fault::code::success, "aes padding: decode psk");
        ...
        auto [ec, result] = relay->decrypt_inbound(packet, sender);
        runner.Check(ec == psm::fault::code::success, "aes padding decrypt: success");
        runner.Check(result.payload.size() == 2, "aes padding decrypt: payload size=2");
        runner.Check(result.payload[0] == 0xDD, "aes padding decrypt: payload[0]=0xDD");
        runner.Check(result.payload[1] == 0xEE, "aes padding decrypt: payload[1]=0xEE");
    }

    void TestAesDecryptReplay(TestRunner &runner)
    {
        auto cfg = make_aes128_config();
        auto tracker = std::make_shared<session_tracker>();
        auto relay = make_udp_relay(cfg, tracker);
        ...
        auto [dec_ec, psk_bytes] = format::decode_psk(cfg.psk);
        runner.Check(dec_ec == psm::fault::code::success, "aes replay: decode psk");
        ...
        runner.Check(ec1 == psm::fault::code::success, "aes replay: first decrypt success");

        // 重放同一包 → AEAD nonce 自增导致解密失败（crypto_error，replay 检查在解密之后）
        auto [ec2, res2] = relay->decrypt_inbound(packet, sender);
        runner.Check(ec2 == psm::fault::code::crypto_error, "aes replay: crypto_error (nonce mismatch)");
    }

    // ─── 完整 ChaCha20 解密路径 ────────────────────────

    void TestChachaDecryptIpv6Success(TestRunner &runner)
    {
        ...
        auto [dec_ec, psk_bytes] = format::decode_psk(cfg.psk);
        runner.Check(dec_ec == psm::fault::code::success, "chacha ipv6: decode psk");
        ...
        auto [ec, result] = relay->decrypt_inbound(packet, sender);
        runner.Check(ec == psm::fault::code::success, "chacha ipv6 decrypt: success");
        runner.Check(result.destination_port == 80, "chacha ipv6 decrypt: port=80");
        runner.Check(result.payload.size() == 1, "chacha ipv6 decrypt: payload size=1");
        runner.Check(result.payload[0] == 0xFF, "chacha ipv6 decrypt: payload[0]=0xFF");
        runner.Check(result.session_id == session_id, "chacha ipv6 decrypt: session_id");
    }

    void TestChachaDecryptDomainSuccess(TestRunner &runner)
    {
        auto cfg = make_chacha_config();
        auto tracker = std::make_shared<session_tracker>();
        auto relay = make_udp_relay(cfg, tracker);
        ...
        auto [dec_ec, psk_bytes] = format::decode_psk(cfg.psk);
        runner.Check(dec_ec == psm::fault::code::success, "chacha domain: decode psk");
        ...
        auto [ec, result] = relay->decrypt_inbound(packet, sender);
        runner.Check(ec == psm::fault::code::success, "chacha domain decrypt: success");
        runner.Check(result.destination_port == 443, "chacha domain decrypt: port=443");
        runner.Check(result.payload.size() == 1, "chacha domain decrypt: payload size=1");
        runner.Check(result.payload[0] == 0x77, "chacha domain decrypt: payload[0]=0x77");
    }

    void TestChachaDecryptReplay(TestRunner &runner)
    {
        auto cfg = make_chacha_config();
        auto tracker = std::make_shared<session_tracker>();
        auto relay = make_udp_relay(cfg, tracker);
        ...
        auto [dec_ec, psk_bytes] = format::decode_psk(cfg.psk);
        runner.Check(dec_ec == psm::fault::code::success, "chacha replay: decode psk");
        ...
        auto [ec1, res1] = relay->decrypt_inbound(packet, sender);
        runner.Check(ec1 == psm::fault::code::success, "chacha replay: first success");

        auto [ec2, res2] = relay->decrypt_inbound(packet, sender);
        runner.Check(ec2 == psm::fault::code::crypto_error, "chacha replay: crypto_error (nonce mismatch)");
    }

    // ─── encrypt_out / send_chacha null entry ─────────────

    void TestChachaEncryptNullEntry(TestRunner &runner)
    {
        auto cfg = make_chacha_config();
        auto tracker = std::make_shared<session_tracker>();
        auto relay = make_udp_relay(cfg, tracker);
        ...
        runner.Check(ec == psm::fault::code::crypto_error, "chacha encrypt null: crypto_error");
    }

    void TestAesEncryptNoAeadCtx(TestRunner &runner)
    {
        auto cfg = make_aes128_config();
        auto tracker = std::make_shared<session_tracker>();
        auto relay = make_udp_relay(cfg, tracker);
        ...
        runner.Check(ec == psm::fault::code::crypto_error, "aes encrypt no ctx: crypto_error");
    }

    // ─── decrypt_inbound 密文篡改 ─────────────────────

    void TestAesDecryptCorruptedBody(TestRunner &runner)
    {
        auto cfg = make_aes128_config();
        auto tracker = std::make_shared<session_tracker>();
        auto relay = make_udp_relay(cfg, tracker);
        ...
        auto [dec_ec, psk_bytes] = format::decode_psk(cfg.psk);
        runner.Check(dec_ec == psm::fault::code::success, "aes corrupt: decode psk");
        ...
        net::ip::udp::endpoint sender(net::ip::make_address("127.0.0.1"), 9999);
        auto [ec, result] = relay->decrypt_inbound(packet, sender);
        runner.Check(ec == psm::fault::code::crypto_error, "aes corrupt body: crypto_error");
    }

    void TestChachaDecryptCorruptedBody(TestRunner &runner)
    {
        auto cfg = make_chacha_config();
        auto tracker = std::make_shared<session_tracker>();
        auto relay = make_udp_relay(cfg, tracker);
        ...
        auto [dec_ec, psk_bytes] = format::decode_psk(cfg.psk);
        runner.Check(dec_ec == psm::fault::code::success, "chacha corrupt: decode psk");
        ...
        net::ip::udp::endpoint sender(net::ip::make_address("::1"), 9999);
        auto [ec, result] = relay->decrypt_inbound(packet, sender);
        runner.Check(ec == psm::fault::code::crypto_error, "chacha corrupt body: crypto_error");
    }
...
int main()
{
    ...
    TestRunner runner("ShadowsocksDatagramDeep");

    // parse_body_after_timestamp 直接测试
    TestParseBodyTooShort(runner);
    TestParseBodyBadRequestType(runner);
    TestParseBodyTimestampExpired(runner);
    TestParseBodyBadAddress(runner);
    TestParseBodyIpv4Success(runner);
    TestParseBodyDomainSuccess(runner);
    TestParseBodyWithPadding(runner);
    TestParseBodyNoPayload(runner);
    TestParseBodyNoPaddingField(runner);

    // AES-GCM 完整解密路径
    TestAesDecryptIpv4Success(runner);
    TestAesDecryptDomainSuccess(runner);
    TestAesDecryptWithPadding(runner);
    TestAesDecryptReplay(runner);

    // ChaCha20 完整解密路径
    TestChachaDecryptIpv6Success(runner);
    TestChachaDecryptDomainSuccess(runner);
    TestChachaDecryptReplay(runner);

    // encrypt 边界
    TestChachaEncryptNullEntry(runner);
    TestAesEncryptNoAeadCtx(runner);

    // 密文篡改
    TestAesDecryptCorruptedBody(runner);
    TestChachaDecryptCorruptedBody(runner);
