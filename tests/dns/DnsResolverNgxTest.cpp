...
{
    net::io_context ioc;
    net_dns::resolver r(ioc.get_executor());
    std::error_code ec;
...
{
    net::io_context ioc;
    net_dns::resolver r(ioc.get_executor(), 64, std::chrono::seconds(60), std::chrono::seconds(10));
    std::error_code ec;
    run_coro(ioc,
             [&]() -> net::awaitable<void>
             {
...
TEST(DnsResolver, CacheExpiry)
{
    net::io_context ioc;
    // 极短 TTL（1 秒）
    net_dns::resolver r(ioc.get_executor(), 64, std::chrono::seconds(1));
    std::error_code ec;
    run_coro(ioc,
             [&]() -> net::awaitable<void>
             {
...
TEST(DnsResolver, LruEviction)
{
    net::io_context ioc;
    // 容量 2
    net_dns::resolver r(ioc.get_executor(), 2);
    std::error_code ec;
    run_coro(ioc,
             [&]() -> net::awaitable<void>
             {
