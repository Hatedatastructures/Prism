...
namespace
{
    ...
    TEST(RouterDeep, StringHashStringView)
    {
        ...
        EXPECT_EQ(v1, v2) << "string_hash: same string_view -> same hash";
    }
    ...
    TEST(RouterDeep, StringHashDifferentStrings)
    {
        ...
        EXPECT_NE(v1, v2) << "string_hash: different strings -> different hash";
    }
    ...
    TEST(RouterDeep, StringHashMemoryString)
    {
        ...
        auto v_mem = h(s);
        EXPECT_EQ(v_view, v_mem) << "string_hash: string_view == memory::string hash";
    }
    ...
    TEST(RouterDeep, StringHashEmpty)
    {
        ...
        // 恒真断言无验证价值；不崩溃即通过（hash 函数对空串返回合法值）
        static_cast<void>(v);
    }
    ...
    TEST(RouterDeep, Constructor)
    {
        ...
        EXPECT_EQ(r.positive_port(), 0) << "constructor: positive_port == 0";
    }
    ...
    TEST(RouterDeep, DnsAccessor)
    {
        ...
        EXPECT_NE(&r.dns(), nullptr) << "dns: accessor returns non-null";
    }
    ...
    TEST(RouterDeep, DnsConstAccessor)
    {
        ...
        EXPECT_NE(&r.dns(), nullptr) << "dns: const accessor returns non-null";
    }
    ...
    TEST(RouterDeep, SetEndpointEmptyHost)
    {
        ...
        EXPECT_EQ(r.positive_port(), 0) << "set_endpoint: empty host -> port 0";
    }
    ...
    TEST(RouterDeep, SetEndpointZeroPort)
    {
        ...
        EXPECT_EQ(r.positive_port(), 0) << "set_endpoint: zero port -> port 0";
    }
    ...
    TEST(RouterDeep, SetEndpointValid)
    {
        ...
        EXPECT_EQ(r.positive_port(), 3128) << "set_endpoint: port == 3128";
    }
    ...
    TEST(RouterDeep, SetEndpointOverwrite)
    {
        ...
        EXPECT_EQ(r.positive_port(), 443) << "set_endpoint: overwrite -> port 443";
    }
    ...
    TEST(RouterDeep, SetEndpointClearAfterSet)
    {
        ...
        EXPECT_EQ(r.positive_port(), 0) << "set_endpoint: cleared -> port 0";
    }
    ...
    TEST(RouterDeep, SetEndpointAndReadback)
    {
        ...
        EXPECT_EQ(r.positive_port(), 100) << "endpoint: port 100";

        r.set_endpoint("host2.com", 200);
        EXPECT_EQ(r.positive_port(), 200) << "endpoint: overwritten to 200";
    ...
    TEST(RouterDeep, SetEndpointBothEmpty)
    {
        ...
        EXPECT_EQ(r.positive_port(), 0) << "endpoint: both empty -> port 0";
    }
