...
namespace
{
    ...
    TEST(DnsUpstreamDeep, TransportContextConstruction)
    {
        ...
        EXPECT_EQ(ctx.timeout_ms, 5000) << "transport_context: timeout_ms=5000";
    }
    ...
    TEST(DnsUpstreamDeep, TransportResultDefault)
    {
        ...
        EXPECT_EQ(tr.result.error, psm::fault::code::success) << "transport_result: default error=success";
    }
    ...
    TEST(DnsUpstreamDeep, QueryContextConstruction)
    {
        ...
        EXPECT_EQ(qctx.default_timeout, 4000) << "query_context: timeout=4000";
        EXPECT_EQ(qctx.server.address, "8.8.8.8") << "query_context: server address";
    }
    ...
    TEST(DnsUpstreamDeep, ResolveEmptyServers)
    {
        ...
        EXPECT_EQ(result.error, psm::fault::code::dns_failed) << "resolve: empty servers -> dns_failed";
    ...
    TEST(DnsUpstreamDeep, DialTargetConstruction)
    {
        ...
        EXPECT_EQ(dt.endpoint, ep) << "dial_target: endpoint matches";
    }
    ...
    TEST(DnsUpstreamDeep, QueryResultDefaults)
    {
        ...
        EXPECT_EQ(qr.error, psm::fault::code::success) << "query_result: default error=success";
        EXPECT_TRUE(qr.ips.empty()) << "query_result: default ips empty";
        EXPECT_EQ(qr.rtt_ms, 0) << "query_result: default rtt_ms=0";
