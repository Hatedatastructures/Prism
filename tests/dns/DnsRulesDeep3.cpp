...
namespace
{
    ...
    TEST(DnsRulesDeep3, SplitLabelsLeadingTrailingDots)
    {
        ...
        EXPECT_EQ(labels.size(), 2) << "split_labels: .example.com. -> 2 labels";
        EXPECT_EQ(labels[0], "com") << "split_labels: first label is com";
        EXPECT_EQ(labels[1], "example") << "split_labels: second label is example";
    }

    TEST(DnsRulesDeep3, SplitLabelsSingleLabel)
    {
        auto labels = dns::domain_trie::split_labels("localhost");
        EXPECT_EQ(labels.size(), 1) << "split_labels: single label -> 1";
        EXPECT_EQ(labels[0], "localhost") << "split_labels: single label value";
    }

    TEST(DnsRulesDeep3, SplitLabelsMultiLabel)
    {
        auto labels = dns::domain_trie::split_labels("www.example.com");
        EXPECT_EQ(labels.size(), 3) << "split_labels: www.example.com -> 3";
        EXPECT_EQ(labels[0], "com") << "split_labels: reversed [0]=com";
        EXPECT_EQ(labels[1], "example") << "split_labels: reversed [1]=example";
        EXPECT_EQ(labels[2], "www") << "split_labels: reversed [2]=www";
    }
