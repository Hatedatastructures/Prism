...
namespace
{
    ...
    TEST(UpstreamDeep, ConstructDefault)
    {
        ...
        EXPECT_EQ(ups.timeout_ms_, 4000) << "construct: default timeout=4000";
        EXPECT_EQ(ups.mode_, dns::mode::fastest) << "construct: default mode=fastest";
    }
    ...
    TEST(UpstreamDeep, ConstructWithMr)
    {
        ...
        EXPECT_NE(ups.mr_, nullptr) << "construct with mr: mr set";
    }
    ...
    TEST(UpstreamDeep, SetServers)
    {
        ...
        EXPECT_EQ(ups.servers_.size(), 1) << "set_servers: size=1";
        EXPECT_EQ(std::string(ups.servers_[0].address), "8.8.8.8") << "set_servers: addr=8.8.8.8";
    }
    ...
    TEST(UpstreamDeep, SetMode)
    {
        ...
        EXPECT_EQ(ups.mode_, dns::mode::first) << "set_mode: first";
        ups.set_mode(dns::mode::fallback);
        EXPECT_EQ(ups.mode_, dns::mode::fallback) << "set_mode: fallback";
    }
    ...
    TEST(UpstreamDeep, SetTimeout)
    {
        ...
        EXPECT_EQ(ups.timeout_ms_, 2000) << "set_timeout: 2000";
    }
    ...
    TEST(UpstreamDeep, SelectBestLowestRtt)
    {
        ...
        EXPECT_EQ(best.rtt_ms, 50) << "best: lowest rtt=50";
        EXPECT_EQ(std::string(best.server_addr), "server2") << "best: server2";
    }
    ...
    TEST(UpstreamDeep, SelectBestPartialSuccess)
    {
        ...
        auto best = ups.select_best_result(results);
        EXPECT_EQ(best.rtt_ms, 80) << "partial: selects the only successful rtt=80";
        EXPECT_EQ(best.error, psm::fault::code::success) << "partial: success error";
    }
    ...
    TEST(UpstreamDeep, SelectBestSuccessNoIps)
    {
        ...
        auto best = ups.select_best_result(results);
        EXPECT_EQ(best.rtt_ms, 100) << "no-ips: skips empty-ips, picks rtt=100";
    ...
    TEST(UpstreamDeep, SelectBestAllFailed)
    {
        ...
        auto best = ups.select_best_result(results);
        EXPECT_EQ(std::string(best.server_addr), "first") << "all-failed: returns first";
    }
    ...
    TEST(UpstreamDeep, SelectBestEmpty)
    {
        ...
        auto best = ups.select_best_result(results);
        EXPECT_EQ(best.error, psm::fault::code::dns_failed) << "empty: dns_failed";
    ...
    TEST(UpstreamDeep, SelectBestSingle)
    {
        ...
        auto best = ups.select_best_result(results);
        EXPECT_EQ(best.rtt_ms, 42) << "single: rtt=42";
        EXPECT_EQ(best.error, psm::fault::code::success) << "single: success";
    }
    ...
    TEST(UpstreamDeep, SslKeyEquality)
    {
        ...
        EXPECT_EQ(k1, k2) << "ssl_key: same equals";
    ...
    TEST(UpstreamDeep, SslKeyHash)
    {
        ...
        EXPECT_EQ(hasher(k1), hasher(k2)) << "ssl_hash: same key same hash";
        EXPECT_NE(hasher(k1), hasher(k3)) << "ssl_hash: diff key diff hash";
    }
    ...
    {
        server srv;
        EXPECT_EQ(srv.protocol, dns::protocol::udp) << "server: default udp";
        EXPECT_EQ(srv.port, 53) << "server: default port=53";
        EXPECT_EQ(srv.timeout_ms, 5000) << "server: default timeout=5000";
        EXPECT_EQ(srv.skip_cert_check, false) << "server: default verify";
        EXPECT_EQ(std::string(srv.http_path), "/dns-query") << "server: default path";
    }
    ...
    {
        query_result qr;
        EXPECT_EQ(qr.error, psm::fault::code::success) << "qr: default success";
        EXPECT_EQ(qr.rtt_ms, 0) << "qr: default rtt=0";
