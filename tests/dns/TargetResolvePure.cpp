...
namespace
{
    ...
    TEST(TargetResolvePure, ParseHostPort)
    {
        ...
        EXPECT_EQ(host, "example.com") << "parse: host=example.com";
        EXPECT_EQ(port, "443") << "parse: port=443";
    }
    ...
    TEST(TargetResolvePure, ParseHostOnly)
    {
        ...
        EXPECT_EQ(host, "example.com") << "parse: host-only host";
    ...
    TEST(TargetResolvePure, ParseIPv6WithPort)
    {
        ...
        EXPECT_EQ(host, "::1") << "parse: ipv6 host=::1";
        EXPECT_EQ(port, "8080") << "parse: ipv6 port=8080";
    }
    ...
    TEST(TargetResolvePure, ParseIPv6NoPort)
    {
        ...
        EXPECT_EQ(host, "::1") << "parse: ipv6 no-port host=::1";
    ...
    TEST(TargetResolvePure, ParseIPv6NoBracket)
    {
        psm::memory::string host, port;
        psm::recognition::parse("[::1", host, port);
        EXPECT_EQ(host, "[::1") << "parse: unclosed bracket host";
    }
    ...
    TEST(TargetResolvePure, ParseMultipleColons)
    {
        psm::memory::string host, port;
        psm::recognition::parse("::1", host, port);
        EXPECT_EQ(host, "::1") << "parse: bare ipv6 host";
        EXPECT_EQ(port, "80") << "parse: bare ipv6 default port=80";
    }
    ...
    TEST(TargetResolvePure, ParseHostEmptyPort)
    {
        psm::memory::string host, port;
        psm::recognition::parse("example.com:", host, port);
        EXPECT_EQ(host, "example.com") << "parse: empty port host";
        EXPECT_EQ(port, "80") << "parse: empty port default=80";
    }
    ...
    TEST(TargetResolvePure, ParseIpv4WithPort)
    {
        ...
        EXPECT_EQ(port, "443") << "parse: IPv4 port";
    }
    ...
    TEST(TargetResolvePure, ParseIpv6Scoped)
    {
        ...
        EXPECT_EQ(host, "fe80::1%eth0") << "parse: IPv6 scoped host";
        EXPECT_EQ(port, "8080") << "parse: IPv6 scoped port";
    }
    ...
    TEST(TargetResolvePure, ResolveHostPort)
    {
        ...
        EXPECT_EQ(t.host, "example.com") << "resolve sv: host=example.com";
        EXPECT_EQ(t.port, "8080") << "resolve sv: port=8080";
    }
    ...
    TEST(TargetResolvePure, ResolveHostOnly)
    {
        ...
        EXPECT_EQ(t.host, "example.com") << "resolve sv: host-only host";
    }

    TEST(TargetResolvePure, ResolveIpv6)
    {
        auto t = psm::recognition::resolve("[::1]:443");
        EXPECT_EQ(t.host, "::1") << "resolve sv: IPv6 host";
        EXPECT_EQ(t.port, "443") << "resolve sv: IPv6 port";
    }
    ...
    TEST(TargetResolvePure, ResolveConnect)
    {
        ...
        EXPECT_EQ(t.port, "443") << "resolve: CONNECT port=443";
    }
    ...
    TEST(TargetResolvePure, ResolveConnectIPv6)
    {
        ...
        auto t = psm::recognition::resolve(req);
        EXPECT_EQ(t.host, "::1") << "resolve: CONNECT ipv6 host";
        EXPECT_EQ(t.port, "443") << "resolve: CONNECT ipv6 port";
    }
    ...
    TEST(TargetResolvePure, ResolveConnectNoPort)
    {
        ...
        auto t = psm::recognition::resolve(req);
        EXPECT_EQ(t.host, "example.com") << "resolve: CONNECT no-port host";
        EXPECT_EQ(t.port, "443") << "resolve: CONNECT no-port default=443";
    }
    ...
    TEST(TargetResolvePure, ResolveConnectIPv6NoPort)
    {
        ...
        auto t = psm::recognition::resolve(req);
        EXPECT_EQ(t.host, "::1") << "resolve: CONNECT IPv6 no-port host";
        EXPECT_EQ(t.port, "443") << "resolve: CONNECT IPv6 no-port default=443";
    }
    ...
    TEST(TargetResolvePure, ResolveAbsoluteHttp)
    {
        ...
        EXPECT_EQ(t.host, "example.com") << "resolve: http:// host";
        EXPECT_EQ(t.port, "80") << "resolve: http:// default port=80";
    }
    ...
    TEST(TargetResolvePure, ResolveAbsoluteHttps)
    {
        ...
        auto t = psm::recognition::resolve(req);
        EXPECT_EQ(t.host, "example.com") << "resolve: https:// host";
        EXPECT_EQ(t.port, "8443") << "resolve: https:// port=8443";
    }
    ...
    TEST(TargetResolvePure, ResolveAbsoluteHttpsDefaultPort)
    {
        ...
        auto t = psm::recognition::resolve(req);
        EXPECT_EQ(t.host, "example.com") << "resolve: https default port host";
        EXPECT_EQ(t.port, "443") << "resolve: https default port=443";
    }
    ...
    TEST(TargetResolvePure, ResolveAbsoluteNoPath)
    {
        ...
        auto t = psm::recognition::resolve(req);
        EXPECT_EQ(t.host, "example.com") << "resolve: http no path host";
        EXPECT_EQ(t.port, "80") << "resolve: http no path port=80";
    }
    ...
    TEST(TargetResolvePure, ResolveRelativePath)
    {
        ...
        EXPECT_EQ(t.host, "backend.example.com") << "resolve: relative host";
        EXPECT_EQ(t.port, "8080") << "resolve: relative port";
    }
    ...
    TEST(TargetResolvePure, ResolveRelativeHostOnly)
    {
        ...
        EXPECT_EQ(t.host, "backend.local") << "resolve: relative host only host";
        EXPECT_EQ(t.port, "80") << "resolve: relative host only default port";
    }
    ...
    TEST(TargetResolvePure, ResolveProxyBadScheme)
    {
        ...
        EXPECT_EQ(t.host, "fallback.com") << "resolve: bad scheme fallback host";
        EXPECT_EQ(t.port, "9090") << "resolve: bad scheme fallback port";
    }
} // namespace
