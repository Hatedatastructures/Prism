...
namespace
{
    ...
    TEST(DnsRules, TrieExactMatch)
    {
...

        {
            ...
            EXPECT_EQ(val, 42) << "search(\"example.com\") should return 42";
        }
    ...
    TEST(DnsRules, TrieWildcardMatch)
    {
...

        {
            ...
            auto val = std::any_cast<int>(result.value());
            EXPECT_EQ(val, 100) << "search(\"www.example.com\") should return 100";
        }
...

        {
            ...
            auto val = std::any_cast<int>(result.value());
            EXPECT_EQ(val, 100) << "search(\"sub.example.com\") should return 100";
        }
    }
    ...
    TEST(DnsRules, TrieCaseInsensitive)
    {
        ...
        auto val = std::any_cast<int>(result.value());
        EXPECT_EQ(val, 77) << "search(\"example.com\") should return 77";
    }
    ...
    TEST(DnsRules, RulesAddressRule)
    {
...

        {
            ...
            EXPECT_EQ(addr, "1.2.3.4") << "addresses[0] should be \"1.2.3.4\"";
        }
    }
    ...
    TEST(DnsRules, TransparentHashDeterminism)
    {
        ...
        EXPECT_EQ(v1, v2) << "hash(string_view) should be deterministic across calls";

        psm::memory::string ms("test");
        auto v3 = h(ms);

        EXPECT_EQ(v1, v3) << "hash(string_view) should equal hash(memory::string) for same content";
    }
