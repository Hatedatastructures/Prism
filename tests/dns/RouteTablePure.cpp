...
namespace
{
    ...
    TEST(RouteTablePure, AddSingleRoute)
    {
        ...
        EXPECT_EQ(result.size(), 1) << "route: lookup returns 1";
        EXPECT_EQ(result[0], "reality") << "route: lookup returns reality";
    }
    ...
    TEST(RouteTablePure, MultiSchemeSameSNI)
    {
        ...
        EXPECT_EQ(result.size(), 2) << "route: multi scheme -> 2 results";
        EXPECT_EQ(result[0], "reality") << "route: first = reality";
        EXPECT_EQ(result[1], "shadowtls") << "route: second = shadowtls";
    }
    ...
    TEST(RouteTablePure, DuplicateSchemeIgnored)
    {
        ...
        EXPECT_EQ(result.size(), 1) << "route: duplicate scheme not added";
    }
    ...
    TEST(RouteTablePure, RegisteredSnis)
    {
        ...
        EXPECT_EQ(snis.size(), 2) << "route: 2 registered SNIs";
    }
