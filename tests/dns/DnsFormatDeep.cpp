...
namespace
{
    ...
    {
        dns::message msg;
        EXPECT_EQ(msg.id, 0) << "message: default id = 0";
        EXPECT_TRUE(!msg.qr) << "message: default qr = false";
        EXPECT_EQ(msg.opcode, 0) << "message: default opcode = 0";
    ...
    TEST(DnsFormatDeep, MessageConstructorWithMr)
    {
        ...
        dns::message msg{&mr};
        EXPECT_EQ(msg.id, 0) << "message: with mr -> id = 0";
    ...
    TEST(DnsFormatDeep, MakeQueryBasic)
    {
        ...
        EXPECT_EQ(msg.opcode, 0) << "make_query: opcode = 0";
        EXPECT_EQ(msg.questions.size(), 1) << "make_query: 1 question";
        EXPECT_EQ(msg.questions[0].name, "example.com") << "make_query: domain lowercase";
        EXPECT_EQ(msg.questions[0].query_type, dns::qtype::a) << "make_query: qtype = A";
        EXPECT_EQ(msg.questions[0].qclass, 1) << "make_query: qclass = IN";
    }

    TEST(DnsFormatDeep, MakeQueryUppercase)
    {
        auto msg = dns::message::make_query("EXAMPLE.COM", dns::qtype::aaaa);
        EXPECT_EQ(msg.questions[0].name, "example.com") << "make_query: uppercase -> lowercase";
    }

    TEST(DnsFormatDeep, MakeQueryTrailingDot)
    {
        auto msg = dns::message::make_query("example.com.", dns::qtype::a);
        EXPECT_EQ(msg.questions[0].name, "example.com") << "make_query: trailing dot removed";
    }

    TEST(DnsFormatDeep, MakeQuerySubdomain)
    {
        auto msg = dns::message::make_query("www.example.com", dns::qtype::a);
        EXPECT_EQ(msg.questions[0].name, "www.example.com") << "make_query: subdomain preserved";
    }
    ...
    TEST(DnsFormatDeep, MakeQueryWithMr)
    {
        ...
        EXPECT_EQ(msg.questions.size(), 1) << "make_query: with mr -> ok";
    }

    TEST(DnsFormatDeep, MakeQueryNullMr)
    {
        auto msg = dns::message::make_query("test.com", dns::qtype::a, nullptr);
        EXPECT_EQ(msg.questions.size(), 1) << "make_query: null mr -> ok";
    }
    ...
    TEST(DnsFormatDeep, ExtractIpsARecords)
    {
        ...
        EXPECT_EQ(ips.size(), 2) << "extract_ips: 2 A records -> 2 ips";
    }
    ...
    TEST(DnsFormatDeep, ExtractIpsAaaaRecords)
    {
        ...
        auto ips = msg.extract_ips();
        EXPECT_EQ(ips.size(), 2) << "extract_ips: 2 AAAA records -> 2 ips";
    }
    ...
    TEST(DnsFormatDeep, ExtractIpsMixed)
    {
        ...
        auto ips = msg.extract_ips();
        EXPECT_EQ(ips.size(), 2) << "extract_ips: mixed A+AAAA -> 2 ips";
    }
    ...
    TEST(DnsFormatDeep, ExtractIpsFromAllSections)
    {
        ...
        auto ips = msg.extract_ips();
        EXPECT_EQ(ips.size(), 3) << "extract_ips: from all 3 sections -> 3 ips";
    }
    ...
    TEST(DnsFormatDeep, ExtractIpsSkipsCname)
    {
        ...
        auto ips = msg.extract_ips();
        EXPECT_EQ(ips.size(), 1) << "extract_ips: skips CNAME, keeps A";
    }
    ...
    TEST(DnsFormatDeep, MinTtlBasic)
    {
        ...
        EXPECT_EQ(msg.min_ttl(), 100) << "min_ttl: min of 300,100 = 100";
    }
    ...
    TEST(DnsFormatDeep, MinTtlAllSections)
    {
        ...
        EXPECT_EQ(msg.min_ttl(), 50) << "min_ttl: across all sections = 50";
    }

    TEST(DnsFormatDeep, MinTtlEmpty)
    {
        dns::message msg;
        EXPECT_EQ(msg.min_ttl(), 0) << "min_ttl: no records -> 0";
    }
    ...
    TEST(DnsFormatDeep, MinTtlSingle)
    {
        ...
        EXPECT_EQ(msg.min_ttl(), 3600) << "min_ttl: single record -> 3600";
    }
    ...
    TEST(DnsFormatDeep, MinTtlZero)
    {
        ...
        EXPECT_EQ(msg.min_ttl(), 0) << "min_ttl: includes 0 -> 0";
    }
    ...
    TEST(DnsFormatDeep, PackEmptyMessage)
    {
        ...
        msg.id = 42;
        auto packed = msg.pack();
        EXPECT_EQ(packed.size(), 12) << "pack: empty message -> 12 bytes header";
        auto result = dns::message::unpack(packed);
