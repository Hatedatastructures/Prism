...
namespace
{
    ...
    auto CredentialUuid(const std::array<std::uint8_t, 16> &Uuid)
        -> std::array<std::byte, 16>
    {
        std::array<std::byte, 16> Result{};
        for (std::size_t Index = 0; Index < Result.size(); ++Index)
        {
            Result[Index] = static_cast<std::byte>(Uuid[Index]);
        }
        return Result;
    }
    ...
    TEST(VmessConnSession, DirectoryUuidAuthTransfersAccountLease)
    {
        ...
        const auto Record = std::make_shared<const Preview::Account::AccountRecord>(
            Preview::Account::AccountRecord::CreateRequest{
                .AccountId = Preview::AccountId{7301},
                .CredentialValue = Preview::Account::Credential::Uuid(CredentialUuid(Uuid)),
        ...
        RunCoroutine(IoContext,
                     [&]() -> Net::awaitable<void>
                     {
                         ...
                         auto ServerCoroutine = [&]() -> Net::awaitable<void>
                         {
                             ...
                             auto [ErrorValue, Request, Connection] = co_await Vmess::Accept(
                             ...
                             EXPECT_EQ(Connection->AccountId(), Preview::AccountId{7301});
                         ...
                         auto [ErrorValue, Client] = co_await Vmess::Connect(
                             std::make_shared<MemoryStream>(std::move(ClientMemory)),
                             ClientConfig,
                             MakeAddress(Vmess::AddressType::Domain, "account.test", 443));
                         EXPECT_EQ(ErrorValue, Error::None);
    ...
    TEST(VmessConnSession, UdpCommandDisabledBeforeResponse)
    {
        Net::io_context Io;
        auto [ClientMemory, ServerMemory] = MakeMemoryPair(Io.get_executor());
        const auto Uuid = TestUuid();
        ...
        RunCoroutine(
            ...
            [&]() -> Net::awaitable<void>
            {
                ...
                auto ServerTask = Net::co_spawn(
                    Io.get_executor(),
                    [&]() -> Net::awaitable<std::tuple<Error, Vmess::Message, Vmess::SharedConn>>
                    {
                        co_return co_await Vmess::Accept(
                            std::make_shared<MemoryStream>(std::move(ServerMemory)), Config);
                    },
                    Net::use_awaitable);
                ...
                Message.RequestKey.fill(0x52);
                Message.Cmd = Vmess::CmdUdp;
                Message.dst = MakeAddress(Vmess::AddressType::Domain, "udp.example", 443);
                ...
                const auto WireSize = Serializer.Get(
                    Net::mutable_buffer(Wire.data(), Wire.size()), WriteError);
                if (WriteError || !Serializer.IsDone())
                {
                    ADD_FAILURE() << "failed to serialize VMess UDP wire";
                    co_return;
                }
                co_await ClientMemory.async_write_some(
                    AsBytes(std::span<const std::uint8_t>(Wire).first(WireSize)), WriteError);
                if (WriteError)
                {
                    ADD_FAILURE() << WriteError.message();
                    co_return;
                }

                auto [ErrorValue, Request, Connection] = co_await std::move(ServerTask);
    ...
    TEST(VmessConnSession, MuxCommandAcceptedByTcpHandoff)
    {
        Net::io_context Io;
        auto [ClientMemory, ServerMemory] = MakeMemoryPair(Io.get_executor());
        const auto Uuid = TestUuid();
        ...
        RunCoroutine(
            ...
            [&]() -> Net::awaitable<void>
            {
                ...
                Vmess::Message Message;
                Message.uuid = Uuid;
                Message.RequestNonce.fill(0x61);
                Message.RequestKey.fill(0x72);
                Message.Cmd = Vmess::CmdMux;
                Message.dst = MakeAddress(Vmess::AddressType::Domain, "mux.example", 443);
                Vmess::Serializer Serializer(Uuid);
                Serializer.Reset(
                    Message,
                    static_cast<std::uint64_t>(std::chrono::duration_cast<std::chrono::seconds>(
                ...
                if (WriteError || !Serializer.IsDone())
                {
                    ADD_FAILURE() << "failed to serialize VMess Mux wire";
                    co_return;
                }
                co_await ClientMemory.async_write_some(
                    AsBytes(std::span<const std::uint8_t>(Wire).first(WireSize)), WriteError);
                ...
                EXPECT_EQ(Request.Cmd, Vmess::CmdMux);
                if (Connection)
                {
                    Connection->Close();
                }
