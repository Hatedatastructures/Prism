...
namespace
{
    ...
    TEST(TrojanConnSession, MuxCommandAcceptedByTcpHandoff)
    {
        Net::io_context Ioc;
        ...
        RunCoroutine(
            ...
            [&]() -> Net::awaitable<void>
            {
                auto ServerTask = Net::co_spawn(
                    Ioc.get_executor(),
                    [&]() -> Net::awaitable<std::tuple<Error, Trojan::RequestHeader, Trojan::SharedConn>>
                    {
                        co_return co_await Trojan::Accept(
                            std::make_shared<MemoryStream>(std::move(Server)),
                            Trojan::ServerConfig{"pw"});
                    },
                    Net::use_awaitable);
                const auto Wire = Trojan::BuildRequest(
                    Trojan::Credential("pw"),
                    Trojan::Command::Mux,
                    MakeAddress(Trojan::AddressType::Domain, "example.com", 443));
                ...
                EXPECT_EQ(Request.Cmd, Trojan::Command::Mux);
                if (Connection)
                {
                    Connection->Close();
                }
