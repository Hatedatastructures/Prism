...
namespace
{
    ...
    public:
        ...
        Net::any_io_executor provider_executor{};
    ...
    TEST(QuicStreamAdapter, ProviderExecutorWinsOverRequestedExecutor)
    {
        Net::io_context Requested;
        Net::io_context ProviderIo;
        auto Provider = std::make_shared<mock_stream_provider>();
        Provider->provider_executor = ProviderIo.get_executor();

        const auto Adapter = std::make_shared<Preview::Quic::StreamAdapter>(
            Requested.get_executor(),
            Provider);

        EXPECT_EQ(Adapter->Executor(), ProviderIo.get_executor());
    }
