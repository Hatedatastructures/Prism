...
namespace
{
    ...
    void TestFromRecord(TestRunner &runner)
    {
        ...
        runner.Check(psm::fault::succeeded(ec), "from(record): success");
        runner.Check(ch.sni() == "fromrecord.test", "from(record): sni");
        runner.Check(ch.has_x25519(), "from(record): has_x25519");
        runner.Check(!ch.versions().empty(), "from(record): versions");
        runner.Check(!ch.raw_record().empty(), "from(record): raw_record");
        runner.Check(!ch.raw_msg().empty(), "from(record): raw_msg");
    }

    void TestFromBytesExactly44(TestRunner &runner)
    {
        std::vector<std::uint8_t> hs_body;
        hs_body.push_back(0x03);
        hs_body.push_back(0x03);
        ...
        runner.Check(psm::fault::succeeded(ec), "exactly 44 bytes: success");
        runner.Check(ch.sni().empty(), "exactly 44 bytes: no extensions");
    }

    void TestFromBytes43Fails(TestRunner &runner)
    {
        std::vector<std::uint8_t> raw(43, 0x16);
        auto [ec, ch] = psm::tls::client_hello::from_bytes(raw);
        runner.Check(psm::fault::failed(ec), "43 bytes: fails");
    }

    void TestFromBytesCipherLenTruncated(TestRunner &runner)
    {
        std::vector<std::uint8_t> raw;
        raw.push_back(psm::protocol::tls::CT_HANDSHAKE);
        write_u16(raw, 0x0303);
        ...
        auto [ec, ch] = psm::tls::client_hello::from_bytes(raw);
        runner.Check(psm::fault::failed(ec), "cipher_len truncated: fails");
    }

    void TestFromBytesSessionIdBounds(TestRunner &runner)
    {
        std::vector<std::uint8_t> raw;
        raw.push_back(psm::protocol::tls::CT_HANDSHAKE);
        write_u16(raw, 0x0303);
        ...
        auto [ec, ch] = psm::tls::client_hello::from_bytes(raw);
        runner.Check(psm::fault::failed(ec), "session_id claims 20 but only 10 bytes: fails");
    }

    void TestSniOnlyNonHostnameExits(TestRunner &runner)
    {
        std::vector<std::uint8_t> ext_block;
        ...
        runner.Check(psm::fault::succeeded(ec), "SNI only non-hostname: parses");
        runner.Check(ch.sni().empty(), "SNI only non-hostname: sni empty");
    }

    void TestToFeaturesSessionIdLen(TestRunner &runner)
    {
        auto raw = build_valid_hello();
        auto [ec, ch] = psm::tls::client_hello::from_bytes(raw);
        runner.Check(psm::fault::succeeded(ec), "to_features setup: success");

        auto feat = ch.to_features();
        runner.Check(feat.session_id_len == 0, "to_features: session_id_len=0");
        runner.Check(feat.random[0] == 0x00, "to_features: random[0]=0");
        runner.Check(feat.random[31] == 0x00, "to_features: random[31]=0");
        runner.Check(feat.has_alpn == false, "to_features: has_alpn=false");
        runner.Check(feat.has_psk == false, "to_features: has_psk=false");
        runner.Check(feat.has_ech == false, "to_features: has_ech=false");
    }

    void TestFromRecordBadPayload(TestRunner &runner)
    {
        ...
        auto [ec, ch] = psm::tls::client_hello::from(rec);
        runner.Check(psm::fault::failed(ec), "from(record) bad payload: fails");
    }
} // namespace
...
auto main() -> int
{
    ...
    TestRunner runner("TlsHelloPure");

    TestFromRecord(runner);
    TestFromBytesExactly44(runner);
    TestFromBytes43Fails(runner);
    TestFromBytesCipherLenTruncated(runner);
    TestFromBytesSessionIdBounds(runner);
    TestSniOnlyNonHostnameExits(runner);
    TestToFeaturesSessionIdLen(runner);
    TestFromRecordBadPayload(runner);
