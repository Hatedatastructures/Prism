...
namespace
{
    ...
    void TestExtractServerRandomValid(TestRunner &runner)
    {
        ...
        runner.Check(result.has_value(), "extract_server_random: 合法 ServerHello 返回非空");
        ...
        if (result)
        {
            ...
            runner.Check(all_42, "extract_server_random: 随机数字节全部为 0x42");
        }
    }
    ...
    void TestExtractServerRandomTooShort(TestRunner &runner)
    {
        ...
        runner.Check(!result.has_value(), "extract_server_random: 过短缓冲区返回 nullopt");
    }

    /**
     * @brief 测试 extract_server_random 对空缓冲区返回 nullopt
     */
    void TestExtractServerRandomEmpty(TestRunner &runner)
    {
        std::vector<std::byte> empty_buf;
        auto result = extract_server_random(empty_buf);
        runner.Check(!result.has_value(), "extract_server_random: 空缓冲区返回 nullopt");
    }
    ...
    void TestIsTls13ServerHelloTrue(TestRunner &runner)
    {
        ...
        runner.Check(result, "is_tls13_server_hello: TLS 1.3 ServerHello 返回 true");
    }
    ...
    void TestIsTls13ServerHelloFalse(TestRunner &runner)
    {
        ...
        runner.Check(!result, "is_tls13_server_hello: 无扩展 ServerHello 返回 false");
    }
     ...
     */
    void TestIsTls13ServerHelloShort(TestRunner &runner)
    {
        // 缓冲区短于所需的最小长度
        std::vector<std::byte> short_buf(20, std::byte{0x00});
        auto result = is_tls13_server_hello(short_buf);

        runner.Check(!result, "is_tls13_server_hello: 截断缓冲区返回 false");
    }
    ...
    void TestParseHostPortDefault(TestRunner &runner)
    {
        ...
        runner.Check(host == "example.com", "parse_host_port: 无端口时 host 正确");
        runner.Check(port == 443, "parse_host_port: 无端口时默认 port=443");
    }
     ...
     */
    void TestParseHostPortExplicit(TestRunner &runner)
    {
        auto [host, port] = parse_host_port("example.com:8443");

        runner.Check(host == "example.com", "parse_host_port: 显式端口时 host 正确");
        runner.Check(port == 8443, "parse_host_port: 显式端口时 port=8443");
    }
