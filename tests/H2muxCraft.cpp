...
namespace
{
    psm::testing::TestRunner runner("H2muxCraft");
...
void TestDataFrame()
{
    ...
    runner.Check(NGHTTP2_DATA == 0x00, "NGHTTP2_DATA == 0x00");
    runner.Check(NGHTTP2_HEADERS == 0x01, "NGHTTP2_HEADERS == 0x01");
    runner.Check(NGHTTP2_RST_STREAM == 0x03, "NGHTTP2_RST_STREAM == 0x03");
    runner.Check(NGHTTP2_SETTINGS == 0x04, "NGHTTP2_SETTINGS == 0x04");
    runner.Check(NGHTTP2_PING == 0x06, "NGHTTP2_PING == 0x06");
    runner.Check(NGHTTP2_GOAWAY == 0x07, "NGHTTP2_GOAWAY == 0x07");
    runner.Check(NGHTTP2_WINDOW_UPDATE == 0x08, "NGHTTP2_WINDOW_UPDATE == 0x08");

    // 验证标志位常量
    runner.Check(NGHTTP2_FLAG_NONE == 0x00, "NGHTTP2_FLAG_NONE == 0x00");
    runner.Check(NGHTTP2_FLAG_ACK == 0x01, "NGHTTP2_FLAG_ACK == 0x01");

    // 验证错误码常量
    runner.Check(NGHTTP2_NO_ERROR == 0x00, "NGHTTP2_NO_ERROR == 0x00");
    runner.Check(NGHTTP2_PROTOCOL_ERROR == 0x01, "NGHTTP2_PROTOCOL_ERROR == 0x01");
    runner.Check(NGHTTP2_INTERNAL_ERROR == 0x02, "NGHTTP2_INTERNAL_ERROR == 0x02");

    runner.LogPass("DataFrame constants verified");
}
...
{
    runner.LogInfo("=== TestSettingsFrame ===");
    ...
    runner.Check(!output.empty(), "SETTINGS frame: output not empty");

    // SETTINGS 帧：stream_id=0, type=0x04
    if (output.size() >= 9)
    {
        ...
        runner.Check(length == 0, "SETTINGS frame: length == 0 (empty settings)");

        // 验证类型
        runner.Check(raw[3] == NGHTTP2_SETTINGS, "SETTINGS frame: type == 0x04");

        // 验证标志位
        runner.Check(raw[4] == NGHTTP2_FLAG_NONE, "SETTINGS frame: flags == 0x00");

        // 验证 stream_id == 0
        const auto sid = parse_stream_id(raw);
        runner.Check(sid == 0, "SETTINGS frame: stream_id == 0");

        runner.Check(output.size() == 9, "SETTINGS frame: total size == 9 (header only)");
    }

    nghttp2_session_del(session);
    runner.LogPass("SettingsFrame");
}
 ...
 */
void TestSettingsFrameWithParams()
{
    runner.LogInfo("=== TestSettingsFrameWithParams ===");
    ...
    runner.Check(!output.empty(), "SettingsWithParams: output not empty");
    ...
    if (output.size() >= 9)
    {
        ...
        const auto length = parse_length(raw);
        runner.Check(length == 12,
                     "SettingsWithParams: length == 12 (2 entries x 6 bytes)");

        runner.Check(raw[3] == NGHTTP2_SETTINGS, "SettingsWithParams: type == 0x04");
        runner.Check(raw[4] == NGHTTP2_FLAG_NONE, "SettingsWithParams: flags == 0x00");

        const auto sid = parse_stream_id(raw);
        runner.Check(sid == 0, "SettingsWithParams: stream_id == 0");

        // 验证载荷长度
        runner.Check(output.size() == 9 + 12,
        ...
        runner.Check(id1 == NGHTTP2_SETTINGS_MAX_CONCURRENT_STREAMS,
        ...
        runner.Check(val1 == 100, "SettingsWithParams: first entry value == 100");
    }

    nghttp2_session_del(session);
    runner.LogPass("SettingsFrameWithParams");
}
...
void TestPingFrame()
{
    ...
    runner.Check(!output.empty(), "PING frame: output not empty");
    ...
    if (output.size() >= 9)
    {
        ...
        // PING 帧固定 8 字节载荷
        const auto length = parse_length(raw);
        runner.Check(length == 8, "PING frame: length == 8");

        // 类型 = PING (0x06)
        runner.Check(raw[3] == NGHTTP2_PING, "PING frame: type == 0x06");

        // 标志位 = 0 (非 ACK)
        runner.Check(raw[4] == NGHTTP2_FLAG_NONE, "PING frame: flags == 0x00");

        // stream_id = 0
        const auto sid = parse_stream_id(raw);
        runner.Check(sid == 0, "PING frame: stream_id == 0");

        // 验证载荷内容
        runner.Check(output.size() == 17, "PING frame: total size == 17 (9 + 8)");
        ...
        runner.Check(payload_match, "PING frame: opaque data matches");
    }

    nghttp2_session_del(session);
    runner.LogPass("PingFrame");
}
...
{
    runner.LogInfo("=== TestPingAckFrame ===");
    ...
    // 验证帧头字段
    runner.Check(parse_length(ping_ack_frame) == 8,
                 "PING ACK: length == 8");
    runner.Check(ping_ack_frame[3] == NGHTTP2_PING,
                 "PING ACK: type == 0x06");
    runner.Check(ping_ack_frame[4] == NGHTTP2_FLAG_ACK,
                 "PING ACK: flags == 0x01 (ACK)");
    runner.Check(parse_stream_id(ping_ack_frame) == 0,
    ...
    runner.Check(echo_match, "PING ACK: opaque data echoed from original PING");

    // 验证总帧大小
    constexpr std::size_t expected_size = 9 + 8; // header + 8 byte opaque
    runner.Check(sizeof(ping_ack_frame) == expected_size,
                 "PING ACK: total frame size == 17");

    runner.LogPass("PingAckFrame");
}
...
{
    runner.LogInfo("=== TestGoawayFrame ===");
    ...
    const int rv = nghttp2_submit_goaway(session, NGHTTP2_FLAG_NONE,
                                          ...
                                          NGHTTP2_NO_ERROR,
                                          nullptr, 0);
    runner.Check(rv == 0, "GOAWAY submit succeeded");

    auto output = collect_pending(session);

    runner.Check(!output.empty(), "GOAWAY frame: output not empty");
    ...
    if (output.size() >= 9)
    {
        ...
        const auto length = parse_length(raw);
        runner.Check(length == 8, "GOAWAY frame: length == 8 (no debug data)");

        // 类型 = GOAWAY (0x07)
        runner.Check(raw[3] == NGHTTP2_GOAWAY, "GOAWAY frame: type == 0x07");

        // 标志位 = 0
        runner.Check(raw[4] == NGHTTP2_FLAG_NONE, "GOAWAY frame: flags == 0x00");

        // stream_id = 0 (GOAWAY 始终在连接级别)
        const auto sid = parse_stream_id(raw);
        runner.Check(sid == 0, "GOAWAY frame: stream_id == 0");
        ...
        if (output.size() >= 13)
        {
            ...
            runner.Check(last_sid == 0, "GOAWAY frame: last_stream_id == 0");
            ...
            runner.Check(err_code == NGHTTP2_NO_ERROR,
                         "GOAWAY frame: error_code == NO_ERROR (0x00)");
        }

        runner.Check(output.size() == 17, "GOAWAY frame: total size == 17 (9 + 8)");
    }

    nghttp2_session_del(session);
    runner.LogPass("GoawayFrame");
}
 ...
 */
void TestGoawayFrameWithDebugData()
{
    runner.LogInfo("=== TestGoawayFrameWithDebugData ===");
    ...
    runner.Check(rv == 0, "GOAWAY with debug: submit succeeded");
    ...
    if (output.size() >= 9)
    {
        ...
        const auto length = parse_length(raw);
        runner.Check(length == 12, "GOAWAY debug: length == 12 (8 + 4 debug)");

        runner.Check(raw[3] == NGHTTP2_GOAWAY, "GOAWAY debug: type == 0x07");
        ...
        if (output.size() >= 13)
        {
            const auto last_sid = (static_cast<std::uint32_t>(raw[9]) << 24) |
                                   ...
                                   (static_cast<std::uint32_t>(raw[11]) << 8) |
                                   static_cast<std::uint32_t>(raw[12]);
            runner.Check(last_sid == 5, "GOAWAY debug: last_stream_id == 5");
        }
        ...
        if (output.size() >= 17)
        {
            const auto err_code = (static_cast<std::uint32_t>(raw[13]) << 24) |
                                   ...
                                   (static_cast<std::uint32_t>(raw[15]) << 8) |
                                   static_cast<std::uint32_t>(raw[16]);
            runner.Check(err_code == NGHTTP2_INTERNAL_ERROR,
        ...
        if (output.size() >= 21)
        {
            ...
            runner.Check(debug_match, "GOAWAY debug: debug_data matches");
        }

        runner.Check(output.size() == 21, "GOAWAY debug: total size == 21 (9 + 8 + 4)");
    }

    nghttp2_session_del(session);
    runner.LogPass("GoawayFrameWithDebugData");
}
...
{
    runner.LogInfo("=== TestRstStreamFrame ===");
    ...
    runner.Check(NGHTTP2_RST_STREAM == 0x03, "RST_STREAM type == 0x03");
    ...
    // 验证帧头字段
    runner.Check(parse_length(rst_frame) == 4, "RST_STREAM: payload length == 4");
    runner.Check(rst_frame[3] == NGHTTP2_RST_STREAM, "RST_STREAM: type byte == 0x03");
    runner.Check(rst_frame[4] == 0x00, "RST_STREAM: flags == 0x00");
    runner.Check(parse_stream_id(rst_frame) == 3, "RST_STREAM: stream_id == 3");
    ...
    runner.Check(err == NGHTTP2_PROTOCOL_ERROR,
                 "RST_STREAM: error_code == PROTOCOL_ERROR (0x01)");

    nghttp2_session_del(session);
    runner.LogPass("RstStreamFrame");
}
...
{
    runner.LogInfo("=== TestWindowUpdateFrame ===");
    ...
    const int rv = nghttp2_submit_window_update(session, NGHTTP2_FLAG_NONE,
                                                 ...
                                                 32768);
    runner.Check(rv == 0, "WINDOW_UPDATE submit succeeded");
    ...
    while (offset + 9 <= output.size())
    {
        ...
        if (raw[3] == NGHTTP2_WINDOW_UPDATE)
        {
            ...
            runner.Check(frame_len == 4, "WINDOW_UPDATE: length == 4");

            runner.Check(raw[4] == NGHTTP2_FLAG_NONE,
                         "WINDOW_UPDATE: flags == 0x00");

            const auto sid = parse_stream_id(raw);
            runner.Check(sid == 0, "WINDOW_UPDATE: stream_id == 0 (connection level)");
    ...
    runner.Check(found_wu, "WINDOW_UPDATE: found in output");

    // 验证 WINDOW_UPDATE 帧格式规范常量
    runner.Check(NGHTTP2_WINDOW_UPDATE == 0x08,
                 "WINDOW_UPDATE type == 0x08");

    nghttp2_session_del(session);
    runner.LogPass("WindowUpdateFrame");
}
...
{
    runner.LogInfo("=== TestHeadersFrame ===");
    ...
    // 验证帧头字段
    runner.Check(parse_length(headers_frame) == 1,
                 "HEADERS: length == 1 (minimal HPACK payload)");
    runner.Check(headers_frame[3] == NGHTTP2_HEADERS,
                 "HEADERS: type == 0x01");
    runner.Check(headers_frame[4] == NGHTTP2_FLAG_END_HEADERS,
                 "HEADERS: flags == END_HEADERS (0x04)");
    runner.Check(parse_stream_id(headers_frame) == 1,
                 "HEADERS: stream_id == 1");

    // 验证 HPACK 载荷
    runner.Check(headers_frame[9] == 0x88,
                 "HEADERS: HPACK :status 200 (indexed 0x88)");

    // 验证 END_HEADERS 标志位含义
    runner.Check(NGHTTP2_FLAG_END_HEADERS == 0x04,
                 "HEADERS: END_HEADERS constant == 0x04");
    runner.Check(NGHTTP2_FLAG_END_STREAM == 0x01,
    ...
    runner.Check(headers_frame_fin[4] == 0x05,
                 "HEADERS+FIN: flags == 0x05 (END_STREAM | END_HEADERS)");
    runner.Check(parse_stream_id(headers_frame_fin) == 3,
                 "HEADERS+FIN: stream_id == 3");

    runner.LogPass("HeadersFrame");
}
 ...
 */
void TestFrameHeaderLengthEncoding()
{
    runner.LogInfo("=== TestFrameHeaderLengthEncoding ===");
    ...
    runner.Check(parse_length(frame_len_256) == 256,
    ...
    runner.Check(parse_length(frame_len_65535) == 65535,
    ...
    runner.Check(parse_length(frame_len_16384) == 16384,
                 "Frame length 16384: 3-byte big-endian correct");

    runner.LogPass("FrameHeaderLengthEncoding");
}
 ...
 */
void TestFrameHeaderStreamIdEncoding()
{
    runner.LogInfo("=== TestFrameHeaderStreamIdEncoding ===");
    ...
    runner.Check(parse_stream_id(frame_sid_1) == 1,
    ...
    runner.Check(parse_stream_id(frame_sid_max) == 0x7FFFFFFF,
    ...
    runner.Check(masked_sid == 1,
                 "Stream ID with reserved bit: masked value == 1");

    runner.LogPass("FrameHeaderStreamIdEncoding");
}

/**
 * @brief 测试入口
 * @details 初始化全局内存池和日志系统，运行 HTTP/2 帧构造的全部测试，
 * 验证 DATA/HEADERS/RST_STREAM/SETTINGS/GOAWAY/WINDOW_UPDATE/PING
 * 帧类型编码正确性。
...
int main()
{
    ...
    runner.LogInfo("========== H2muxCraft Tests ==========");

    TestDataFrame();
    TestSettingsFrame();
    TestSettingsFrameWithParams();
    TestPingFrame();
    TestPingAckFrame();
    TestGoawayFrame();
    TestGoawayFrameWithDebugData();
    TestRstStreamFrame();
    TestWindowUpdateFrame();
    TestHeadersFrame();
    TestFrameHeaderLengthEncoding();
    TestFrameHeaderStreamIdEncoding();
