...
namespace
{
    ...
    void TestReadU16Basic(TestRunner &runner)
    {
        psm::memory::vector<std::uint8_t> data(psm::memory::current_resource());
        data.push_back(0x01);
        data.push_back(0x23);
        auto val = read_u16(data, 0);
        runner.Check(val == 0x0123, "read_u16: basic value");
    }

    void TestReadU16Max(TestRunner &runner)
    {
        psm::memory::vector<std::uint8_t> data(psm::memory::current_resource());
        data.push_back(0xFF);
        data.push_back(0xFF);
        auto val = read_u16(data, 0);
        runner.Check(val == 0xFFFF, "read_u16: max value");
    }

    void TestReadU16Zero(TestRunner &runner)
    {
        psm::memory::vector<std::uint8_t> data(psm::memory::current_resource());
        data.push_back(0x00);
        data.push_back(0x00);
        auto val = read_u16(data, 0);
        runner.Check(val == 0, "read_u16: zero");
    }

    void TestReadU16AtOffset(TestRunner &runner)
    {
        psm::memory::vector<std::uint8_t> data(psm::memory::current_resource());
        data.push_back(0x00);
        data.push_back(0x00);
        ...
        runner.Check(val == 0xABCD, "read_u16: at offset 2");
    }

    // ─── read_u24 ──────────────────────────────────────

    void TestReadU24Basic(TestRunner &runner)
    {
        psm::memory::vector<std::uint8_t> data(psm::memory::current_resource());
        data.push_back(0x01);
        data.push_back(0x23);
        ...
        runner.Check(val == 0x012345, "read_u24: basic value");
    }

    void TestReadU24Max(TestRunner &runner)
    {
        psm::memory::vector<std::uint8_t> data(psm::memory::current_resource());
        data.push_back(0xFF);
        data.push_back(0xFF);
        data.push_back(0xFF);
        auto val = read_u24(data, 0);
        runner.Check(val == 0xFFFFFF, "read_u24: max value");
    }

    void TestReadU24Zero(TestRunner &runner)
    {
        psm::memory::vector<std::uint8_t> data(psm::memory::current_resource());
        data.push_back(0x00);
        data.push_back(0x00);
        data.push_back(0x00);
        auto val = read_u24(data, 0);
        runner.Check(val == 0, "read_u24: zero");
    }

    // ─── parse_sni ─────────────────────────────────────

    void TestParseSniHostname(TestRunner &runner)
    {
        ...
        parse_sni(span, sni);
        runner.Check(sni == "example.com", "parse_sni: hostname extracted");
    }

    void TestParseSniEmptyExt(TestRunner &runner)
    {
        psm::memory::vector<std::uint8_t> ext(psm::memory::current_resource());
        ...
        auto span = std::span<const std::uint8_t>(ext.data(), ext.size());
        parse_sni(span, sni);
        runner.Check(sni.empty(), "parse_sni: too short → empty sni");
    }

    void TestParseSniNonHostnameType(TestRunner &runner)
    {
        psm::memory::vector<std::uint8_t> ext(psm::memory::current_resource());
        ...
        auto span = std::span<const std::uint8_t>(ext.data(), ext.size());
        parse_sni(span, sni);
        runner.Check(sni.empty(), "parse_sni: non-hostname type → empty sni");
    }

    void TestParseSniMixedTypes(TestRunner &runner)
    {
        ...
        auto span = std::span<const std::uint8_t>(ext.data(), ext.size());
        parse_sni(span, sni);
        runner.Check(sni == "test.io", "parse_sni: mixed types → hostname found");
    }

    void TestParseSniTruncatedNameLen(TestRunner &runner)
    {
        psm::memory::vector<std::uint8_t> ext(psm::memory::current_resource());
        write_u16(ext, 5);
        ext.push_back(ptls::SNAME_TYPE_HOSTNAME);
        ...
        auto span = std::span<const std::uint8_t>(ext.data(), ext.size());
        parse_sni(span, sni);
        runner.Check(sni.empty(), "parse_sni: truncated name_len → break");
    }

    void TestParseSniTruncatedNameData(TestRunner &runner)
    {
        psm::memory::vector<std::uint8_t> ext(psm::memory::current_resource());
        write_u16(ext, 10);
        ext.push_back(ptls::SNAME_TYPE_HOSTNAME);
        ...
        auto span = std::span<const std::uint8_t>(ext.data(), ext.size());
        parse_sni(span, sni);
        runner.Check(sni.empty(), "parse_sni: truncated name data → break");
    }

    void TestParseSniNonHostTruncatedLen(TestRunner &runner)
    {
        psm::memory::vector<std::uint8_t> ext(psm::memory::current_resource());
        write_u16(ext, 3);
        ...
        auto span = std::span<const std::uint8_t>(ext.data(), ext.size());
        parse_sni(span, sni);
        runner.Check(sni.empty(), "parse_sni: non-host truncated len → break");
    }

    // ─── parse_keyshare ────────────────────────────────

    void TestParseKeyshareX25519(TestRunner &runner)
    {
        psm::memory::vector<std::uint8_t> ext(psm::memory::current_resource());
        ...
        runner.Check(has_key, "parse_keyshare: X25519 found");
        runner.Check(key[0] == 1, "parse_keyshare: X25519 key[0]=1");
        runner.Check(key[31] == 32, "parse_keyshare: X25519 key[31]=32");
    }

    void TestParseKeyshareMlKem(TestRunner &runner)
    {
        psm::memory::vector<std::uint8_t> ext(psm::memory::current_resource());
        ...
        auto span = std::span<const std::uint8_t>(ext.data(), ext.size());
        parse_keyshare(span, has_key, key);
        runner.Check(has_key, "parse_keyshare: ML-KEM found");
        runner.Check(key[0] == 0, "parse_keyshare: ML-KEM key[0]");
        runner.Check(key[31] == 31, "parse_keyshare: ML-KEM key[31]");
    }

    void TestParseKeyshareEmpty(TestRunner &runner)
    {
        psm::memory::vector<std::uint8_t> ext(psm::memory::current_resource());
        // 不足 2 字节
        ext.push_back(0x00);
        ...
        auto span = std::span<const std::uint8_t>(ext.data(), ext.size());
        parse_keyshare(span, has_key, key);
        runner.Check(!has_key, "parse_keyshare: too short → no key");
    }

    void TestParseKeyshareUnknownGroup(TestRunner &runner)
    {
        psm::memory::vector<std::uint8_t> ext(psm::memory::current_resource());
        write_u16(ext, 36);
        ...
        auto span = std::span<const std::uint8_t>(ext.data(), ext.size());
        parse_keyshare(span, has_key, key);
        runner.Check(!has_key, "parse_keyshare: unknown group → no key");
    }

    void TestParseKeyshareTruncatedKey(TestRunner &runner)
    {
        psm::memory::vector<std::uint8_t> ext(psm::memory::current_resource());
        write_u16(ext, 36);
        write_u16(ext, ptls::GROUP_X25519);
        ...
        auto span = std::span<const std::uint8_t>(ext.data(), ext.size());
        parse_keyshare(span, has_key, key);
        runner.Check(!has_key, "parse_keyshare: truncated key → break");
    }

    void TestParseKeyshareX25519WrongKeyLen(TestRunner &runner)
    {
        psm::memory::vector<std::uint8_t> ext(psm::memory::current_resource());
        write_u16(ext, 8);
        write_u16(ext, ptls::GROUP_X25519);
        ...
        auto span = std::span<const std::uint8_t>(ext.data(), ext.size());
        parse_keyshare(span, has_key, key);
        runner.Check(!has_key, "parse_keyshare: X25519 with wrong key_len → skip");
    }

    void TestParseKeyshareMultipleEntries(TestRunner &runner)
    {
        psm::memory::vector<std::uint8_t> ext(psm::memory::current_resource());
        ...
        auto span = std::span<const std::uint8_t>(ext.data(), ext.size());
        parse_keyshare(span, has_key, key);
        runner.Check(has_key, "parse_keyshare: multiple entries → X25519 found");
        runner.Check(key[0] == 10, "parse_keyshare: second entry key[0]");
    }

    // ─── parse_versions ────────────────────────────────

    void TestParseVersionsBasic(TestRunner &runner)
    {
        psm::memory::vector<std::uint8_t> ext(psm::memory::current_resource());
        ...
        runner.Check(versions.size() == 2, "parse_versions: 2 versions");
        runner.Check(versions[0] == ptls::VERSION_TLS12, "parse_versions: TLS 1.2");
        runner.Check(versions[1] == ptls::VERSION_TLS13, "parse_versions: TLS 1.3");
    }

    void TestParseVersionsEmpty(TestRunner &runner)
    {
        psm::memory::vector<std::uint8_t> ext(psm::memory::current_resource());
        psm::memory::vector<std::uint16_t> versions(psm::memory::current_resource());
        auto span = std::span<const std::uint8_t>(ext.data(), ext.size());
        parse_versions(span, versions);
        runner.Check(versions.empty(), "parse_versions: empty → no versions");
    }

    void TestParseVersionsOddListLen(TestRunner &runner)
    {
        psm::memory::vector<std::uint8_t> ext(psm::memory::current_resource());
        ...
        auto span = std::span<const std::uint8_t>(ext.data(), ext.size());
        parse_versions(span, versions);
        runner.Check(versions.size() == 1, "parse_versions: odd list → 1 version");
    }

    void TestParseVersionsSingleByteList(TestRunner &runner)
    {
        psm::memory::vector<std::uint8_t> ext(psm::memory::current_resource());
        ...
        auto span = std::span<const std::uint8_t>(ext.data(), ext.size());
        parse_versions(span, versions);
        runner.Check(versions.empty(), "parse_versions: list_len=1 → no versions");
    }

    // ─── parse_exts ────────────────────────────────────

    void TestParseExtsAllThree(TestRunner &runner)
    {
        ...
        runner.Check(sni == "ext.test", "parse_exts: SNI parsed");
        runner.Check(has_key, "parse_exts: key found");
        runner.Check(versions.size() == 1, "parse_exts: 1 version");
    }

    void TestParseExtsEmpty(TestRunner &runner)
    {
        psm::memory::vector<std::uint8_t> ext(psm::memory::current_resource());
        ...
        auto span = std::span<const std::uint8_t>(ext.data(), ext.size());
        parse_exts(span, state);
        runner.Check(sni.empty(), "parse_exts: empty → no sni");
        runner.Check(!has_key, "parse_exts: empty → no key");
    }

    void TestParseExtsUnknownExtType(TestRunner &runner)
    {
        psm::memory::vector<std::uint8_t> ext_block(psm::memory::current_resource());
        ...
        auto span = std::span<const std::uint8_t>(full_ext.data(), full_ext.size());
        parse_exts(span, state);
        runner.Check(sni.empty(), "parse_exts: unknown type → no sni");
        runner.Check(!has_key, "parse_exts: unknown type → no key");
    }

    void TestParseExtsTruncatedPayload(TestRunner &runner)
    {
        ...
        auto span = std::span<const std::uint8_t>(ext.data(), ext.size());
        parse_exts(span, state);
        runner.Check(sni.empty(), "parse_exts: truncated → break");
    }

    // ─── from_bytes 更多分支 ───────────────────────────

    void TestFromBytesBadContentType(TestRunner &runner)
    {
        ...
        runner.Check(ec == psm::fault::code::recorderr, "from_bytes: bad content type → error");
    }

    void TestFromBytesBodyTooLarge(TestRunner &runner)
    {
        ...
        raw[4] = 0xFF;
        auto [ec, ch] = client_hello::from_bytes(raw);
        runner.Check(ec == psm::fault::code::recorderr, "from_bytes: body too large → error");
    }

    void TestFromBytesBadHandshakeType(TestRunner &runner)
    {
        psm::memory::vector<std::uint8_t> raw(44, static_cast<std::uint8_t>(0x00));
        raw[0] = ptls::CT_HANDSHAKE;
        // body_len = 39 (44 - 5)
        ...
        auto [ec, ch] = client_hello::from_bytes(raw);
        runner.Check(ec == psm::fault::code::recorderr, "from_bytes: bad handshake type → error");
    }

    void TestFromBytesHandshakeLenOverflow(TestRunner &runner)
    {
        ...
        raw[8] = 0xFF;
        auto [ec, ch] = client_hello::from_bytes(raw);
        runner.Check(ec == psm::fault::code::recorderr, "from_bytes: handshake len overflow → error");
    }

    void TestFromBytesSessionIdOutOfRange(TestRunner &runner)
    {
        ...
        auto [ec, ch] = client_hello::from_bytes(raw);
        runner.Check(ec == psm::fault::code::recorderr, "from_bytes: sid out of range → error");
    }

    void TestFromBytesOddCipherLen(TestRunner &runner)
    {
        // cipher_len 为奇数
        psm::memory::vector<std::uint8_t> hs_body(psm::memory::current_resource());
        hs_body.push_back(0x03);
        ...
        auto [ec, ch] = client_hello::from_bytes(raw);
        runner.Check(ec == psm::fault::code::recorderr, "from_bytes: odd cipher_len → error");
    }

    void TestFromBytesCompLenOverflow(TestRunner &runner)
    {
        psm::memory::vector<std::uint8_t> hs_body(psm::memory::current_resource());
        hs_body.push_back(0x03);
        hs_body.push_back(0x03);
        ...
        auto [ec, ch] = client_hello::from_bytes(raw);
        runner.Check(ec == psm::fault::code::recorderr, "from_bytes: comp_len overflow → error");
    }

    void TestFromBytesWithExtensions(TestRunner &runner)
    {
        ...
        auto [ec, ch] = client_hello::from_bytes(raw);
        runner.Check(ec == psm::fault::code::success, "from_bytes: full hello → success");
        runner.Check(ch.sni() == "deep.test", "from_bytes: sni");
        runner.Check(ch.has_x25519(), "from_bytes: has_x25519");
        runner.Check(ch.x25519_key()[0] == 100, "from_bytes: key[0]=100");
        runner.Check(ch.versions().size() == 2, "from_bytes: 2 versions");
        runner.Check(ch.session_id().size() == 4, "from_bytes: session_id len=4");
        runner.Check(ch.random()[0] == 0, "from_bytes: random[0]=0");
        runner.Check(ch.random()[31] == 31, "from_bytes: random[31]=31");

        auto feat = ch.to_features();
        runner.Check(feat.server_name == "deep.test", "to_features: server_name");
        runner.Check(feat.session_id_len == 4, "to_features: session_id_len=4");
        runner.Check(feat.has_x25519, "to_features: has_x25519");
        runner.Check(feat.versions.size() == 2, "to_features: 2 versions");
    }

    void TestFromBytesRandomOverflow(TestRunner &runner)
    {
        ...
        auto [ec, ch] = client_hello::from_bytes(raw);
        runner.Check(ec == psm::fault::code::recorderr, "from_bytes: random overflow → error");
    }

    void TestFromBytesSidLenOverflow(TestRunner &runner)
    {
        ...
        auto [ec, ch] = client_hello::from_bytes(raw);
        runner.Check(ec == psm::fault::code::recorderr, "from_bytes: sid overflow → error");
    }
...
int main()
{
    ...
    TestRunner runner("TlsHelloDeep");

    // read_u16 / read_u24
    TestReadU16Basic(runner);
    TestReadU16Max(runner);
    TestReadU16Zero(runner);
    TestReadU16AtOffset(runner);
    TestReadU24Basic(runner);
    TestReadU24Max(runner);
    TestReadU24Zero(runner);

    // parse_sni
    TestParseSniHostname(runner);
    TestParseSniEmptyExt(runner);
    TestParseSniNonHostnameType(runner);
    TestParseSniMixedTypes(runner);
    TestParseSniTruncatedNameLen(runner);
    TestParseSniTruncatedNameData(runner);
    TestParseSniNonHostTruncatedLen(runner);

    // parse_keyshare
    TestParseKeyshareX25519(runner);
    TestParseKeyshareMlKem(runner);
    TestParseKeyshareEmpty(runner);
    TestParseKeyshareUnknownGroup(runner);
    TestParseKeyshareTruncatedKey(runner);
    TestParseKeyshareX25519WrongKeyLen(runner);
    TestParseKeyshareMultipleEntries(runner);

    // parse_versions
    TestParseVersionsBasic(runner);
    TestParseVersionsEmpty(runner);
    TestParseVersionsOddListLen(runner);
    TestParseVersionsSingleByteList(runner);

    // parse_exts
    TestParseExtsAllThree(runner);
    TestParseExtsEmpty(runner);
    TestParseExtsUnknownExtType(runner);
    TestParseExtsTruncatedPayload(runner);

    // from_bytes 更多分支
    TestFromBytesBadContentType(runner);
    TestFromBytesBodyTooLarge(runner);
    TestFromBytesBadHandshakeType(runner);
    TestFromBytesHandshakeLenOverflow(runner);
    TestFromBytesSessionIdOutOfRange(runner);
    TestFromBytesOddCipherLen(runner);
    TestFromBytesCompLenOverflow(runner);
    TestFromBytesWithExtensions(runner);
    TestFromBytesRandomOverflow(runner);
    TestFromBytesSidLenOverflow(runner);
