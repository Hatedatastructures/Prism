...
namespace
{
    ...
    void TestDecodePayloadTooShort(TestRunner &runner)
    {
        ...
        runner.Check(!result.has_value(), "decode: too short -> nullopt");
        runner.Check(ec == std::errc::protocol_error, "decode: too short -> protocol_error");
    }

    void TestDecodePayloadExactAuthHeaderLen(TestRunner &runner)
    {
        std::array<std::uint8_t, 32> secret{};
        ...
        auto result = decode_restls_payload(payload, opts, ec);
        runner.Check(result.has_value(), "decode: exact auth_header len -> success");
        runner.Check(!ec, "decode: exact auth_header len -> no error");
        runner.Check(result->data_len == 0, "decode: exact auth_header len -> data_len=0");
        runner.Check(result->cmd == 0, "decode: exact auth_header len -> cmd=0");
    }

    void TestDecodePayloadWithData(TestRunner &runner)
    {
        std::array<std::uint8_t, 32> secret{};
        for (std::size_t i = 0; i < 32; ++i)
        ...
        auto result = decode_restls_payload(payload, opts, ec);
        runner.Check(result.has_value(), "decode: with data -> success");
        runner.Check(!ec, "decode: with data -> no error");
        runner.Check(result->data_len == data_size, "decode: with data -> data_len=8");
        runner.Check(result->cmd == 0, "decode: with data -> cmd=0");
    }

    void TestDecodePayloadMacMismatch(TestRunner &runner)
    {
        ...
        auto result = decode_restls_payload(payload, opts, ec);
        runner.Check(!result.has_value(), "decode: bad MAC -> nullopt");
        runner.Check(ec == std::errc::permission_denied, "decode: bad MAC -> permission_denied");
    }

    void TestDecodePayloadDifferentCounter(TestRunner &runner)
    {
        std::array<std::uint8_t, 32> secret{};
        std::array<std::uint8_t, 32> server_random{};
        std::array<std::uint8_t, tls_hdrsize> tls_hdr{0x17, 0x03, 0x03, 0x00, 0x0C};
        ...
        runner.Check(r0.has_value(), "decode: counter=0 -> success");
        ...
        runner.Check(!r1.has_value(), "decode: wrong counter -> fail");
    }

    void TestDecodePayloadWithCloseCmd(TestRunner &runner)
    {
        std::array<std::uint8_t, 32> secret{};
        std::array<std::uint8_t, 32> server_random{};
        std::array<std::uint8_t, tls_hdrsize> tls_hdr{0x17, 0x03, 0x03, 0x00, 0x0C};
        ...
        runner.Check(r.has_value(), "decode: close cmd -> success");
        runner.Check(r->cmd == cmd_close, "decode: close cmd -> cmd=0x0001");
        runner.Check(r->data_len == 0, "decode: close cmd -> data_len=0");
    }

    void TestDecodePayloadWithRandrespCmd(TestRunner &runner)
    {
        std::array<std::uint8_t, 32> secret{};
        for (std::size_t i = 0; i < 32; ++i)
        ...
        std::error_code ec;
        auto r = decode_restls_payload(payload, opts, ec);
        runner.Check(r.has_value(), "decode: randresp cmd -> success");
        runner.Check(r->cmd == cmd_randresp, "decode: randresp cmd -> cmd=0x0002");
    }
...
int main()
{
    ...
    TestRunner runner("RestlsTransportPure");

    // decode_restls_payload
    TestDecodePayloadTooShort(runner);
    TestDecodePayloadExactAuthHeaderLen(runner);
    TestDecodePayloadWithData(runner);
    TestDecodePayloadMacMismatch(runner);
    TestDecodePayloadDifferentCounter(runner);
    TestDecodePayloadWithCloseCmd(runner);
    TestDecodePayloadWithRandrespCmd(runner);
