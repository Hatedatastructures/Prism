...
namespace
{
    ...
    void TestConstructDefault(TestRunner &runner)
    {
        net::io_context ioc;
        upstream ups(ioc);
        runner.Check(ups.servers_.empty(), "construct: empty servers");
        runner.Check(ups.timeout_ms_ == 4000, "construct: default timeout=4000");
        runner.Check(ups.mode_ == dns::resolve_mode::fastest, "construct: default mode=fastest");
    }

    void TestConstructWithMr(TestRunner &runner)
    {
        net::io_context ioc;
        upstream ups(ioc, psm::memory::current_resource());
        runner.Check(ups.mr_ != nullptr, "construct with mr: mr set");
    }
    ...
    void TestSetServers(TestRunner &runner)
    {
        ...
        runner.Check(ups.servers_.size() == 1, "set_servers: size=1");
        runner.Check(std::string(ups.servers_[0].address) == "8.8.8.8", "set_servers: addr=8.8.8.8");
    }
    ...
    void TestSetMode(TestRunner &runner)
    {
        ...
        runner.Check(ups.mode_ == dns::resolve_mode::first, "set_mode: first");
        ups.set_mode(dns::resolve_mode::fallback);
        runner.Check(ups.mode_ == dns::resolve_mode::fallback, "set_mode: fallback");
    }
    ...
    void TestSetTimeout(TestRunner &runner)
    {
        ...
        runner.Check(ups.timeout_ms_ == 2000, "set_timeout: 2000");
    }

    // ─── select_best_result: 全成功选最低 RTT ──────

    void TestSelectBestLowestRtt(TestRunner &runner)
    {
        net::io_context ioc;
        upstream ups(ioc);
        ...
        runner.Check(best.rtt_ms == 50, "best: lowest rtt=50");
        runner.Check(std::string(best.server_addr) == "server2", "best: server2");
    }

    // ─── select_best_result: 部分成功 ──────────────

    void TestSelectBestPartialSuccess(TestRunner &runner)
    {
        net::io_context ioc;
        upstream ups(ioc);
        ...
        auto best = ups.select_best_result(results);
        runner.Check(best.rtt_ms == 80, "partial: selects the only successful rtt=80");
        runner.Check(best.error == psm::fault::code::success, "partial: success error");
    }
    ...
    void TestSelectBestSuccessNoIps(TestRunner &runner)
    {
        ...
        auto best = ups.select_best_result(results);
        runner.Check(best.rtt_ms == 100, "no-ips: skips empty-ips, picks rtt=100");
        runner.Check(!best.ips.empty(), "no-ips: result has ips");
    }
    ...
    void TestSelectBestAllFailed(TestRunner &runner)
    {
        ...
        auto best = ups.select_best_result(results);
        runner.Check(std::string(best.server_addr) == "first", "all-failed: returns first");
    }
    ...
    void TestSelectBestEmpty(TestRunner &runner)
    {
        ...
        auto best = ups.select_best_result(results);
        runner.Check(best.error == psm::fault::code::dns_failed, "empty: dns_failed");
        runner.Check(best.ips.empty(), "empty: no ips");
    }

    // ─── select_best_result: 单个成功 ──────────────

    void TestSelectBestSingle(TestRunner &runner)
    {
        net::io_context ioc;
        upstream ups(ioc);
        ...
        auto best = ups.select_best_result(results);
        runner.Check(best.rtt_ms == 42, "single: rtt=42");
        runner.Check(best.error == psm::fault::code::success, "single: success");
    }

    // ─── ssl_key 相等和哈希 ────────────────────────

    void TestSslKeyEquality(TestRunner &runner)
    {
        net::io_context ioc;
        upstream ups(ioc);
        ...
        runner.Check(k1 == k2, "ssl_key: same equals");
        runner.Check(!(k1 == k3), "ssl_key: diff verify_peer not equal");
        runner.Check(!(k1 == k4), "ssl_key: diff hostname not equal");
    }

    void TestSslKeyHash(TestRunner &runner)
    {
        net::io_context ioc;
        upstream ups(ioc);
        ...
        runner.Check(hasher(k1) == hasher(k2), "ssl_hash: same key same hash");
        runner.Check(hasher(k1) != hasher(k3), "ssl_hash: diff key diff hash");
    }

    // ─── get_ssl_ctx: 缓存命中 ────────────────────

    void TestGetSslCtxCache(TestRunner &runner)
    {
        net::io_context ioc;
        upstream ups(ioc);
        ...
        runner.Check(ctx1 != nullptr, "ssl_ctx: first call not null");
        auto ctx2 = ups.get_ssl_ctx(srv);
        runner.Check(ctx1.get() == ctx2.get(), "ssl_ctx: cache hit same ptr");
        runner.Check(ups.ssl_cache_.size() == 1, "ssl_ctx: cache size=1");
    }

    // ─── get_ssl_ctx: 空 hostname 用 address ──────

    void TestGetSslCtxNoHostname(TestRunner &runner)
    {
        net::io_context ioc;
        upstream ups(ioc);
        ...
        runner.Check(ctx != nullptr, "ssl_ctx no host: not null");
        runner.Check(ups.ssl_cache_.size() == 1, "ssl_ctx no host: cached");
    }

    // ─── get_ssl_ctx: 不同配置不同缓存 ──────────────

    void TestGetSslCtxDifferentConfigs(TestRunner &runner)
    {
        net::io_context ioc;
        upstream ups(ioc);
        ...
        runner.Check(ctx1.get() != ctx2.get(), "ssl_ctx: diff config diff ctx");
        runner.Check(ups.ssl_cache_.size() == 2, "ssl_ctx: cache size=2");
    }

    // ─── dns_remote 默认值 ──────────────────────────

    void TestDnsRemoteDefaults(TestRunner &runner)
    {
        dns_remote srv;
        runner.Check(srv.protocol == dns::dns_protocol::udp, "dns_remote: default udp");
        runner.Check(srv.port == 53, "dns_remote: default port=53");
        runner.Check(srv.timeout_ms == 5000, "dns_remote: default timeout=5000");
        runner.Check(srv.skip_cert_check == false, "dns_remote: default verify");
        runner.Check(std::string(srv.http_path) == "/dns-query", "dns_remote: default path");
    }

    // ─── query_result 默认值 ──────────────────────────

    void TestQueryResultDefaults(TestRunner &runner)
    {
        query_result qr;
        runner.Check(qr.error == psm::fault::code::success, "qr: default success");
        runner.Check(qr.rtt_ms == 0, "qr: default rtt=0");
        runner.Check(qr.ips.empty(), "qr: default empty ips");
        runner.Check(std::string(qr.server_addr).empty(), "qr: default empty server");
    }
...
int main()
{
    ...
    TestRunner runner("UpstreamDeep");

    TestConstructDefault(runner);
    TestConstructWithMr(runner);
    TestSetServers(runner);
    TestSetMode(runner);
    TestSetTimeout(runner);

    TestSelectBestLowestRtt(runner);
    TestSelectBestPartialSuccess(runner);
    TestSelectBestSuccessNoIps(runner);
    TestSelectBestAllFailed(runner);
    TestSelectBestEmpty(runner);
    TestSelectBestSingle(runner);

    TestSslKeyEquality(runner);
    TestSslKeyHash(runner);
    TestGetSslCtxCache(runner);
    TestGetSslCtxNoHostname(runner);
    TestGetSslCtxDifferentConfigs(runner);

    TestDnsRemoteDefaults(runner);
    TestQueryResultDefaults(runner);
