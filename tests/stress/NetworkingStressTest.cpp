...
namespace
{
    ...
    TEST(NetworkStress, TcpConnectionStorm)
    {
        ...
        net::co_spawn(
            ...
            [&]() -> net::awaitable<void>
            {
                ...
                // 风暴：并发连接 × 多轮
                std::atomic<int> ok{0};
                psmtest::stress::gate g(ioc.get_executor(), total);
    ...
    TEST(NetworkStress, UdpRelayLongRun)
    {
        ...
        auto a = std::make_shared<psmtest::transport::unreliable>(ioc.get_executor());
        auto b = std::make_shared<psmtest::transport::unreliable>(ioc.get_executor());
        ...
        net::co_spawn(
            ...
            [&]() -> net::awaitable<void>
            {
                psmtest::net_udp::relay_options opts;
                opts.idle_timeout = std::chrono::milliseconds(5000);
                psmtest::net_udp::udp_relay relay(a, b, opts);
                co_await relay.run();
            },
            net::detached);
    ...
    TEST(NetworkStress, LeakTrackerDetectsRelease)
    {
        ...
        psmtest::stress::leak_tracker tracker;
        tracker.track(obj);
    ...
    {
        net::io_context ioc;
        psmtest::stress::gate g(ioc.get_executor(), 3);
