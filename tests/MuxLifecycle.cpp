...
namespace
{
    psm::testing::TestRunner runner("MuxLifecycle");

    /// 测试用标志：各子测试是否通过
    std::atomic<bool> g_smux_tcp_pass{false};
    std::atomic<bool> g_yamux_tcp_pass{false};
    std::atomic<bool> g_smux_udp_pass{false};
    std::atomic<bool> g_smux_disconnect_pass{false};
    std::atomic<bool> g_yamux_disconnect_pass{false};
    std::atomic<bool> g_smux_multi_pass{false};
    std::atomic<bool> g_yamux_rst_pass{false};
    std::atomic<bool> g_yamux_goaway_pass{false};
} // namespace
...
auto test_smux_tcp_lifecycle(net::any_io_executor ex,
                             psm::connect::router &router,
                             const psm::multiplex::config &mux_config) -> net::awaitable<void>
{
    // 启动 echo server
...
auto test_yamux_tcp_lifecycle(net::any_io_executor ex,
                              psm::connect::router &router,
                              const psm::multiplex::config &mux_config) -> net::awaitable<void>
{
    tcp::acceptor echo_acceptor(ex, tcp::endpoint(net::ip::address_v4::loopback(), 0));
...
auto test_smux_udp_lifecycle(net::any_io_executor ex,
                             psm::connect::router &router,
                             const psm::multiplex::config &mux_config) -> net::awaitable<void>
{
    auto [client_sock, server_sock] = co_await make_socket_pair(ex);
...
auto test_smux_abrupt_disconnect(net::any_io_executor ex,
                                 psm::connect::router &router,
                                 const psm::multiplex::config &mux_config) -> net::awaitable<void>
{
    tcp::acceptor echo_acceptor(ex, tcp::endpoint(net::ip::address_v4::loopback(), 0));
    ...
    g_smux_disconnect_pass = inactive;
}

// ── Test 5: yamux 异常断连 ──

auto test_yamux_abrupt_disconnect(net::any_io_executor ex,
                                  psm::connect::router &router,
                                  const psm::multiplex::config &mux_config) -> net::awaitable<void>
{
    tcp::acceptor echo_acceptor(ex, tcp::endpoint(net::ip::address_v4::loopback(), 0));
    ...
    // 仅发送 Data(SYN) 携带地址即可打开流（WindowUpdate SYN 会导致重复 SYN）
    const std::uint32_t stream_id = 1;
    ...
    g_yamux_disconnect_pass = inactive;
}

// ── Test 6: smux 多流并发 ──

auto test_smux_multi_stream(net::any_io_executor ex,
                            psm::connect::router &router,
                            const psm::multiplex::config &mux_config) -> net::awaitable<void>
{
    tcp::acceptor echo_acceptor(ex, tcp::endpoint(net::ip::address_v4::loopback(), 0));
    ...
    g_smux_multi_pass = responses_ok;
...
auto test_yamux_rst_stream(net::any_io_executor ex,
                           psm::connect::router &router,
                           const psm::multiplex::config &mux_config) -> net::awaitable<void>
{
    tcp::acceptor echo_acceptor(ex, tcp::endpoint(net::ip::address_v4::loopback(), 0));
...
auto test_yamux_goaway(net::any_io_executor ex,
                       psm::connect::router &router,
                       const psm::multiplex::config &mux_config) -> net::awaitable<void>
{
    auto [client_sock, server_sock] = co_await make_socket_pair(ex);
    ...
    g_yamux_goaway_pass = closed;
}

// ── 主测试协程：顺序执行所有测试 ──

auto run_all_tests(net::any_io_executor ex,
                   psm::connect::router &router,
                   const psm::multiplex::config &mux_config) -> net::awaitable<void>
{
    co_await test_smux_tcp_lifecycle(ex, router, mux_config);
    co_await test_yamux_tcp_lifecycle(ex, router, mux_config);
    co_await test_smux_udp_lifecycle(ex, router, mux_config);
    co_await test_smux_abrupt_disconnect(ex, router, mux_config);
    co_await test_yamux_abrupt_disconnect(ex, router, mux_config);
    co_await test_smux_multi_stream(ex, router, mux_config);
    co_await test_yamux_rst_stream(ex, router, mux_config);
    co_await test_yamux_goaway(ex, router, mux_config);
}

/**
 * @brief 测试入口
 * @details 初始化全局内存池和日志系统，在协程中顺序运行所有测试，
 * 最后收集结果输出汇总
...
int main()
{
    ...
    // 测试完成回调：设置标志并停止 io_context
    auto on_tests_complete = [&](std::exception_ptr ep)
    {
        if (ep)
        {
            try
            ...
            {
                runner.LogFail(std::string("test coroutine exception: ") + e.what());
            }
        }
        tests_done = true;
        ioc->stop();
    ...
    // 在 io_context 上启动所有测试协程
    net::co_spawn(*ioc, run_all_tests(ioc->get_executor(), *router_ptr, mux_config), std::move(on_tests_complete));

    // 运行 io_context 直到所有协程完成
    ioc->run();

    // 收集结果（不调用 Summary() 的 shutdown，手动控制生命周期）
    runner.Check(g_smux_tcp_pass, "smux TCP stream lifecycle (SYN/PSH/FIN)");
    runner.Check(g_yamux_tcp_pass, "yamux TCP stream lifecycle (Data SYN/FIN)");
    runner.Check(g_smux_udp_pass, "smux UDP stream lifecycle");
    runner.Check(g_smux_disconnect_pass, "smux abrupt disconnect cleanup");
    runner.Check(g_yamux_disconnect_pass, "yamux abrupt disconnect cleanup");
    runner.Check(g_smux_multi_pass, "smux multi-stream concurrent");
    runner.Check(g_yamux_rst_pass, "yamux RST stream reset");
    runner.Check(g_yamux_goaway_pass, "yamux GoAway session close");

    const int exit_code = (runner.FailedCount() > 0) ? 1 : 0;
    psm::trace::info("[MuxLifecycle] Results: {} passed, {} failed",
                     runner.PassedCount(), runner.FailedCount());
