...
namespace
{
    ...
    void TestNormalizeLowercase(TestRunner &runner)
    {
        ...
        runner.Check(r == "example.com", "normalize: 大写转小写");
    }
    ...
    {
        auto r = Normalize("example.com.", psm::memory::current_resource());
        runner.Check(r == "example.com", "normalize: 移除末尾点号");
    }
    ...
    {
        auto r = Normalize("example.com...", psm::memory::current_resource());
        runner.Check(r == "example.com", "normalize: 移除多个末尾点号");
    }

    /**
     * @brief 测试已经是规范形式
     */
    void TestNormalizeAlreadyCanonical(TestRunner &runner)
    {
        auto r = Normalize("example.com", psm::memory::current_resource());
        runner.Check(r == "example.com", "normalize: 已规范形式不变");
    }

    /**
     * @brief 测试混合大小写 + 末尾点号
     */
    void TestNormalizeMixedCaseTrailingDot(TestRunner &runner)
    {
        auto r = Normalize("ExAmPlE.CoM.", psm::memory::current_resource());
        runner.Check(r == "example.com", "normalize: 混合大小写 + 点号");
    }
    ...
    void TestNormalizeEmpty(TestRunner &runner)
    {
        ...
        runner.Check(r.empty(), "normalize: 空字符串保持空");
    }

    /**
     * @brief 测试纯点号字符串
     */
    void TestNormalizeOnlyDots(TestRunner &runner)
    {
        auto r = Normalize("...", psm::memory::current_resource());
        runner.Check(r.empty(), "normalize: 纯点号变空串");
    }

    /**
     * @brief 测试子域名
     */
    void TestNormalizeSubdomain(TestRunner &runner)
    {
        auto r = Normalize("WWW.Sub.EXAMPLE.COM.", psm::memory::current_resource());
        runner.Check(r == "www.sub.example.com", "normalize: 多级子域名规范化");
    }

    /**
     * @brief 测试单标签域名
     */
    void TestNormalizeSingleLabel(TestRunner &runner)
    {
        auto r = Normalize("LOCALHOST", psm::memory::current_resource());
        runner.Check(r == "localhost", "normalize: 单标签域名小写");
    }

    /**
     * @brief 测试确定性和幂等性
     */
    void TestNormalizeDeterministic(TestRunner &runner)
    {
        const auto mr = psm::memory::current_resource();
        auto r1 = Normalize("EXAMPLE.COM.", mr);
        auto r2 = Normalize("EXAMPLE.COM.", mr);
        runner.Check(r1 == r2, "normalize: 相同输入 → 相同输出（确定性）");
    }
     ...
     */
    void TestBlacklistV4Slash8(TestRunner &runner)
    {
     ...
     */
    void TestBlacklistV4Slash24(TestRunner &runner)
    {
        psm::memory::vector<net::ip::network_v4> bl_v4(psm::memory::current_resource());
        psm::memory::vector<net::ip::network_v6> bl_v6(psm::memory::current_resource());
     ...
     */
    void TestBlacklistV4Slash32(TestRunner &runner)
    {
        psm::memory::vector<net::ip::network_v4> bl_v4(psm::memory::current_resource());
        psm::memory::vector<net::ip::network_v6> bl_v6(psm::memory::current_resource());
     ...
     * @brief 测试空黑名单
     */
    void TestBlacklistEmpty(TestRunner &runner)
    {
        psm::memory::vector<net::ip::network_v4> bl_v4(psm::memory::current_resource());
        psm::memory::vector<net::ip::network_v6> bl_v6(psm::memory::current_resource());
     ...
     */
    void TestBlacklistV4MultipleRules(TestRunner &runner)
    {
        psm::memory::vector<net::ip::network_v4> bl_v4(psm::memory::current_resource());
        psm::memory::vector<net::ip::network_v6> bl_v6(psm::memory::current_resource());
        bl_v4.push_back(net::ip::make_network_v4("10.0.0.0/8"));
     ...
     */
    void TestBlacklistV6Slash64(TestRunner &runner)
    {
        psm::memory::vector<net::ip::network_v4> bl_v4(psm::memory::current_resource());
        psm::memory::vector<net::ip::network_v6> bl_v6(psm::memory::current_resource());
     ...
     */
    void TestBlacklistV6Slash128(TestRunner &runner)
    {
        psm::memory::vector<net::ip::network_v4> bl_v4(psm::memory::current_resource());
        psm::memory::vector<net::ip::network_v6> bl_v6(psm::memory::current_resource());
     ...
     */
    void TestBlacklistV6Slash48(TestRunner &runner)
    {
        psm::memory::vector<net::ip::network_v4> bl_v4(psm::memory::current_resource());
        psm::memory::vector<net::ip::network_v6> bl_v6(psm::memory::current_resource());
     ...
     */
    void TestBlacklistTypeMismatch(TestRunner &runner)
    {
        psm::memory::vector<net::ip::network_v4> bl_v4(psm::memory::current_resource());
        psm::memory::vector<net::ip::network_v6> bl_v6(psm::memory::current_resource());
        bl_v4.push_back(net::ip::make_network_v4("10.0.0.0/8"));
     ...
     */
    void TestFilterIpsOnlyV4(TestRunner &runner)
    {
        psm::memory::vector<net::ip::network_v4> bl_v4(psm::memory::current_resource());
        psm::memory::vector<net::ip::network_v6> bl_v6(psm::memory::current_resource());
        ...
        runner.Check(filtered.size() == 2, "filter: qtype=A 保留 2 个 IPv4");
        runner.Check(filtered[0].is_v4(), "filter: 第一个是 IPv4");
        runner.Check(filtered[1].is_v4(), "filter: 第二个是 IPv4");
    }

    /**
     * @brief 测试 qtype=AAAA 只保留 IPv6
     */
    void TestFilterIpsOnlyV6(TestRunner &runner)
    {
        psm::memory::vector<net::ip::network_v4> bl_v4(psm::memory::current_resource());
        psm::memory::vector<net::ip::network_v6> bl_v6(psm::memory::current_resource());
        ...
        runner.Check(filtered.size() == 2, "filter: qtype=AAAA 保留 2 个 IPv6");
        runner.Check(filtered[0].is_v6(), "filter: 第一个是 IPv6");
        runner.Check(filtered[1].is_v6(), "filter: 第二个是 IPv6");
    }

    /**
     * @brief 测试过滤黑名单 IP
     */
    void TestFilterIpsRemoveBlacklisted(TestRunner &runner)
    {
        psm::memory::vector<net::ip::network_v4> bl_v4(psm::memory::current_resource());
        psm::memory::vector<net::ip::network_v6> bl_v6(psm::memory::current_resource());
        bl_v4.push_back(net::ip::make_network_v4("10.0.0.0/8"));
        ...
        runner.Check(filtered.size() == 2, "filter: 黑名单移除 2 个，保留 2 个");
        runner.Check(filtered[0].to_string() == "1.1.1.1", "filter: 保留 1.1.1.1");
        runner.Check(filtered[1].to_string() == "8.8.8.8", "filter: 保留 8.8.8.8");
    }
    ...
    void TestFilterIpsAllBlacklisted(TestRunner &runner)
    {
        ...
        runner.Check(filtered.empty(), "filter: 0.0.0.0/0 黑名单移除全部 IPv4");
    }
    ...
    void TestFilterIpsEmptyInput(TestRunner &runner)
    {
        ...
        runner.Check(filtered.empty(), "filter: 空输入返回空");
    }

    /**
     * @brief 测试 qtype 不匹配导致空结果
     */
    void TestFilterIpsQtypeMismatch(TestRunner &runner)
    {
        psm::memory::vector<net::ip::network_v4> bl_v4(psm::memory::current_resource());
        psm::memory::vector<net::ip::network_v6> bl_v6(psm::memory::current_resource());
        ...
        runner.Check(filtered.empty(), "filter: 纯 IPv4 + qtype=AAAA 返回空");
    }

    /**
     * @brief 测试混合 IPv4/IPv6 + 黑名单 + qtype
     */
    void TestFilterIpsMixedComplex(TestRunner &runner)
    {
        psm::memory::vector<net::ip::network_v4> bl_v4(psm::memory::current_resource());
        psm::memory::vector<net::ip::network_v6> bl_v6(psm::memory::current_resource());
        bl_v4.push_back(net::ip::make_network_v4("10.0.0.0/8"));
        ...
        runner.Check(filtered_a.size() == 2, "filter complex: qtype=A 保留 2 个干净 IPv4");

        // qtype=AAAA 只保留干净 IPv6
        auto filtered_aaaa = FilterIps(ips, detail::qtype::aaaa, bl_v4, bl_v6);
        runner.Check(filtered_aaaa.size() == 2, "filter complex: qtype=AAAA 保留 2 个干净 IPv6");
    }

    /**
     * @brief 测试 IPv6 黑名单过滤
     */
    void TestFilterIpsV6Blacklist(TestRunner &runner)
    {
        psm::memory::vector<net::ip::network_v4> bl_v4(psm::memory::current_resource());
        psm::memory::vector<net::ip::network_v6> bl_v6(psm::memory::current_resource());
        ...
        runner.Check(filtered.size() == 2, "filter v6: fc00::/7 黑名单移除 2 个，保留 2 个");
    }
...
int main()
{
    ...
    TestRunner runner("DnsResolverPure");
    runner.LogInfo("Starting DnsResolverPure tests...");

    // normalize 测试
    TestNormalizeLowercase(runner);
    TestNormalizeTrailingDot(runner);
    TestNormalizeMultipleTrailingDots(runner);
    TestNormalizeAlreadyCanonical(runner);
    TestNormalizeMixedCaseTrailingDot(runner);
    TestNormalizeEmpty(runner);
    TestNormalizeOnlyDots(runner);
    TestNormalizeSubdomain(runner);
    TestNormalizeSingleLabel(runner);
    TestNormalizeDeterministic(runner);

    // is_blacklisted 测试
    TestBlacklistV4Slash8(runner);
    TestBlacklistV4Slash24(runner);
    TestBlacklistV4Slash32(runner);
    TestBlacklistEmpty(runner);
    TestBlacklistV4MultipleRules(runner);
    TestBlacklistV6Slash64(runner);
    TestBlacklistV6Slash128(runner);
    TestBlacklistV6Slash48(runner);
    TestBlacklistTypeMismatch(runner);

    // filter_ips 测试
    TestFilterIpsOnlyV4(runner);
    TestFilterIpsOnlyV6(runner);
    TestFilterIpsRemoveBlacklisted(runner);
    TestFilterIpsAllBlacklisted(runner);
    TestFilterIpsEmptyInput(runner);
    TestFilterIpsQtypeMismatch(runner);
    TestFilterIpsMixedComplex(runner);
    TestFilterIpsV6Blacklist(runner);
