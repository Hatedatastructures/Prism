...
TEST(QuicE2E, HandshakeAndStreamEcho)
{
    ...
    qserver->on_handshake_complete = [server_ok]()
    {
        *server_ok = true;
    };
    qserver->on_stream = [qserver, stream_data_ok](quic::shared_stream st)
    {
        *stream_data_ok = true;
        ...
        net::co_spawn(st->executor(), [qserver, st]() -> net::awaitable<void>
        {
            ...
            co_await st->async_write_some(std::span<const std::byte>(
                reinterpret_cast<const std::byte *>(first.data()), first.size()), ec);
            std::string second(34, 'B');
            co_await st->async_write_some(std::span<const std::byte>(
                reinterpret_cast<const std::byte *>(second.data()), second.size()), ec);
        }, net::detached);
    };
    ...
    qclient->on_handshake_complete = [client_ok]()
    {
        *client_ok = true;
    };
    ...
    net::co_spawn(ioc, [&]() -> net::awaitable<void>
    {
        ...
        if (*server_ok && *client_ok)
        {
            ...
            co_await qclient->write_stream_data(sid, std::span<const std::byte>(
