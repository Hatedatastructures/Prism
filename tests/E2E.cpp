...
namespace
{
    psm::testing::TestRunner runner("E2E");
} // namespace
                                         ...
                                         const std::string_view tag, const std::string_view payload)
{
    ...
    runner.LogInfo(std::format("{} client: connected to proxy", tag));
    ...
    runner.LogInfo(std::format("{} client: method negotiated", tag));
    ...
    runner.LogInfo(std::format("{} client: CONNECT success", tag));
    ...
    runner.LogInfo(std::format("{} client: echo verified", tag));
                                             ...
                                             const std::string_view tag, const std::string_view payload)
{
    ...
    runner.LogInfo(std::format("{} client: password auth selected", tag));
    ...
    runner.LogInfo(std::format("{} client: auth success", tag));
                                            ...
                                            const std::string_view tag)
{
    ...
    runner.LogInfo(std::format("{} client: sent CONNECT", tag));

    const std::string response = co_await ReadProxyConnectResponse(socket);
    runner.LogInfo(std::format("{} client: CONNECT response `{}`", tag, response.substr(0, response.find("\r\n"))));
...
net::awaitable<void> HttpAuth407Client(tcp::endpoint proxy_ep, const std::string_view tag)
{
    ...
    runner.LogInfo(std::format("{} client: response `{}`", tag, response.substr(0, response.find("\r\n"))));
    ...
    runner.LogInfo(std::format("{} client: 407 verified", tag));
...
net::awaitable<void> E2ESocks5Echo(psm::context::server &server_ctx, psm::context::worker &worker_ctx)
{
    ...
    try
    {
        ...
        runner.LogPass("E2ESocks5Echo");
    }
    catch (const std::exception &e)
    {
        runner.LogFail(std::format("E2ESocks5Echo: {}", e.what()));
    }
}
...
net::awaitable<void> E2EHttpConnectEcho(psm::context::server &server_ctx, psm::context::worker &worker_ctx)
{
    ...
    try
    {
        ...
        runner.LogPass("E2EHttpConnectEcho");
    }
    catch (const std::exception &e)
    {
        runner.LogFail(std::format("E2EHttpConnectEcho: {}", e.what()));
    }
}
...
net::awaitable<void> E2ESocks5Auth(psm::context::server &server_ctx, psm::context::worker &worker_ctx)
{
    ...
    try
    {
        ...
        runner.LogPass("E2ESocks5Auth");
    }
    catch (const std::exception &e)
    {
        runner.LogFail(std::format("E2ESocks5Auth: {}", e.what()));
    }
}
...
net::awaitable<void> E2EHttpAuth407(psm::context::server &server_ctx, psm::context::worker &worker_ctx)
{
    ...
    try
    {
        ...
        runner.LogPass("E2EHttpAuth407");
    }
    catch (const std::exception &e)
    {
        runner.LogFail(std::format("E2EHttpAuth407: {}", e.what()));
    }
}
    ...
    try
    {
        ...
        for (int i = 0; i < conn_count; ++i)
        {
            ...
            auto client_task = [proxy_ep, echo_ep, payload = std::move(payload), completed, succeeded, all_done, i]() -> net::awaitable<void>
            {
                              ...
                              {
                                  runner.LogFail(std::format("conc_{}: {}", i, e.what()));
                              }
        ...
        runner.Check(succeeded->load() == conn_count,
                     std::format("E2EConcurrency: {}/{} succeeded", succeeded->load(), conn_count));
    }
    catch (const std::exception &e)
    {
        runner.LogFail(std::format("E2EConcurrency: {}", e.what()));
    }
}
...
int main()
{
    try
    {
        ...
        {
            auto function = [&no_auth_server_ctx, &auth_server_ctx, &worker_ctx]() -> net::awaitable<void>
            {
                co_await E2ESocks5Echo(no_auth_server_ctx, worker_ctx);
                co_await E2EHttpConnectEcho(no_auth_server_ctx, worker_ctx);
                co_await E2ESocks5Auth(auth_server_ctx, worker_ctx);
                co_await E2EHttpAuth407(auth_server_ctx, worker_ctx);
                co_await E2EConcurrency(no_auth_server_ctx, worker_ctx);
    ...
    {
        psm::trace::shutdown();
        psm::trace::error("[E2E] fatal: {}", e.what());
        return 1;
    }
}
