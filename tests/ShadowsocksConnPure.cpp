...
namespace
{
    ...
    void TestDeriveKeyAes128(TestRunner &runner)
    {
        ...
        runner.Check(key.size() == 16, "derive aes-128: key length=16");
        ...
        runner.Check(seal_ec == psm::fault::code::success, "derive aes-128: seal success");
        runner.Check(sealed.size() == 20, "derive aes-128: sealed size=20");

        std::vector<std::uint8_t> opened(open_ctx.open_size(sealed.size()));
        auto open_ec = open_ctx.open(opened, sealed);
        runner.Check(open_ec == psm::fault::code::success, "derive aes-128: open success");
        runner.Check(opened.size() == 4, "derive aes-128: opened size=4");
        runner.Check(std::memcmp(opened.data(), pt, 4) == 0,
                     "derive aes-128: round-trip matches");
    }

    void TestDeriveKeyAes256(TestRunner &runner)
    {
        ...
        runner.Check(key.size() == 32, "derive aes-256: key length=32");
        ...
        runner.Check(ec == psm::fault::code::success, "derive aes-256: open success");
        runner.Check(std::memcmp(opened.data(), pt, 3) == 0,
                     "derive aes-256: round-trip matches");
    }

    void TestDeriveKeyChaCha20(TestRunner &runner)
    {
        std::array<std::uint8_t, 32> psk{};
        std::array<std::uint8_t, 32> salt{};
        for (int i = 0; i < 32; ++i)
        ...
        runner.Check(key.size() == 32, "derive chacha20: key length=32");
        ...
        std::vector<std::uint8_t> opened(open_ctx.open_size(sealed.size()));
        auto ec = open_ctx.open(opened, sealed);
        runner.Check(ec == psm::fault::code::success, "derive chacha20: open success");
        runner.Check(std::memcmp(opened.data(), pt, 4) == 0,
                     "derive chacha20: round-trip matches");
    }

    void TestDeriveKeyDeterministic(TestRunner &runner)
    {
        std::array<std::uint8_t, 32> material{};
        ...
        runner.Check(key1.size() == key2.size(), "derive: same size");
        runner.Check(std::memcmp(key1.data(), key2.data(), key1.size()) == 0,
                     "derive: deterministic output");
    }

    void TestDeriveKeyDifferentSalt(TestRunner &runner)
    {
        ...
        runner.Check(std::memcmp(key_a.data(), key_b.data(), 16) != 0,
                     "derive: different salt produces different key");
    }

    void TestFormatDecodePsk(TestRunner &runner)
    {
        // Base64 编码 32 个 0x00 字节
        auto [ec, bytes] = ss_fmt::decode_psk("//////////////////////////////////////////8=");
        runner.Check(ec == psm::fault::code::success, "decode_psk: valid base64 success");
        runner.Check(bytes.size() == 32, "decode_psk: 32-byte key");
    }

    void TestFormatDecodePskEmpty(TestRunner &runner)
    {
        auto [ec, bytes] = ss_fmt::decode_psk("");
        runner.Check(ec != psm::fault::code::success || bytes.empty(),
                     "decode_psk: empty string handled");
    }

    void TestFormatResolveMethod(TestRunner &runner)
    {
        auto m1 = ss_fmt::resolve_method("2022-blake3-aes-128-gcm", 16);
        runner.Check(m1 == cipher_method::aes_128_gcm, "resolve: aes-128");

        auto m2 = ss_fmt::resolve_method("2022-blake3-aes-256-gcm", 32);
        runner.Check(m2 == cipher_method::aes_256_gcm, "resolve: aes-256");

        auto m3 = ss_fmt::resolve_method("2022-blake3-chacha20-poly1305", 32);
        runner.Check(m3 == cipher_method::chacha20_poly1305, "resolve: chacha20");

        auto m4 = ss_fmt::resolve_method("", 16);
        runner.Check(m4 == cipher_method::aes_128_gcm, "resolve: default for 16-byte PSK");
    }

    void TestFormatKeysaltLen(TestRunner &runner)
    {
        runner.Check(ss_fmt::keysalt_len(cipher_method::aes_128_gcm) == 16,
                     "keysalt_len: aes-128 = 16");
        runner.Check(ss_fmt::keysalt_len(cipher_method::aes_256_gcm) == 32,
                     "keysalt_len: aes-256 = 32");
        runner.Check(ss_fmt::keysalt_len(cipher_method::chacha20_poly1305) == 32,
...
int main()
{
    ...
    TestRunner runner("ShadowsocksConnPure");

    TestDeriveKeyAes128(runner);
    TestDeriveKeyAes256(runner);
    TestDeriveKeyChaCha20(runner);
    TestDeriveKeyDeterministic(runner);
    TestDeriveKeyDifferentSalt(runner);
    TestFormatDecodePsk(runner);
    TestFormatDecodePskEmpty(runner);
    TestFormatResolveMethod(runner);
    TestFormatKeysaltLen(runner);
