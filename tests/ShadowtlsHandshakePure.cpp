...
namespace
{
    ...
    void TestExtractRandomValid(TestRunner &runner)
    {
        ...
        runner.Check(result.has_value(), "extract_random: 合法 ServerHello 返回非空");
        ...
        if (result)
        {
            ...
            runner.Check(all_42, "extract_random: 随机数字节全部为 0x42");
        }
    }
    ...
    void TestExtractRandomTooShort(TestRunner &runner)
    {
        ...
        runner.Check(!result.has_value(), "extract_random: 过短缓冲区返回 nullopt");
    }
     ...
     */
    void TestExtractRandomWrongType(TestRunner &runner)
    {
        auto hello = make_server_hello(false);
        ...
        auto result = extract_random(hello);
        runner.Check(!result.has_value(), "extract_random: 错误 content_type 返回 nullopt");
    }
     ...
     */
    void TestExtractRandomNotServerHello(TestRunner &runner)
    {
        auto hello = make_server_hello(false);
        ...
        auto result = extract_random(hello);
        runner.Check(!result.has_value(), "extract_random: 非 ServerHello 类型返回 nullopt");
    }
     ...
     */
    void TestIsTls13HelloWithSupportedVersions(TestRunner &runner)
    {
        auto hello = make_server_hello(true);
        auto result = is_tls13_hello(hello);

        runner.Check(result, "is_tls13_hello: TLS 1.3 ServerHello 返回 true");
    }
     ...
     */
    void TestIsTls13HelloTls12(TestRunner &runner)
    {
        auto hello = make_server_hello(false);
        auto result = is_tls13_hello(hello);

        runner.Check(!result, "is_tls13_hello: 无扩展 ServerHello 返回 false");
    }
    ...
    void TestIsTls13HelloTooShort(TestRunner &runner)
    {
        ...
        runner.Check(!result, "is_tls13_hello: 截断缓冲区返回 false");
    }
     ...
     */
    void TestIsTls13HelloWrongVersion(TestRunner &runner)
    {
        ...
        runner.Check(!result, "is_tls13_hello: supported_versions 值为 TLS 1.2 返回 false");
    }
     ...
     *          验证函数能正确遍历扩展列表。
     */
    void TestIsTls13HelloSkipOtherExt(TestRunner &runner)
    {
        std::vector<std::byte> buf;
        ...
        auto result = is_tls13_hello(buf);
        runner.Check(result, "is_tls13_hello: 跳过无关扩展后找到 TLS 1.3");
    }
    ...
    void TestIsTls13HelloWithSessionId(TestRunner &runner)
    {
        ...
        auto result = is_tls13_hello(buf);
        runner.Check(result, "is_tls13_hello: 带 SessionID 的 TLS 1.3 ServerHello 返回 true");
    }

    /**
     * @brief 测试 extract_random 对空缓冲区返回 nullopt
     */
    void TestExtractRandomEmpty(TestRunner &runner)
    {
        std::vector<std::byte> empty_buf;
        auto result = extract_random(empty_buf);
        runner.Check(!result.has_value(), "extract_random: 空缓冲区返回 nullopt");
    }
    ...
    void TestExtractRandomExactMinSize(TestRunner &runner)
    {
        ...
        runner.Check(result.has_value(), "extract_random: 恰好最小长度返回非空");
        ...
        if (result)
        {
            ...
            runner.Check(all_55, "extract_random: 恰好最小长度时随机数字节正确");
        }
    }
...
int main()
{
    ...
    TestRunner runner("ShadowtlsHandshakePure");

    // extract_random 测试
    TestExtractRandomValid(runner);
    TestExtractRandomTooShort(runner);
    TestExtractRandomWrongType(runner);
    TestExtractRandomNotServerHello(runner);
    TestExtractRandomEmpty(runner);
    TestExtractRandomExactMinSize(runner);

    // is_tls13_hello 测试
    TestIsTls13HelloWithSupportedVersions(runner);
    TestIsTls13HelloTls12(runner);
    TestIsTls13HelloTooShort(runner);
    TestIsTls13HelloWrongVersion(runner);
    TestIsTls13HelloSkipOtherExt(runner);
