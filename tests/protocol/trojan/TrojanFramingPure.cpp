...
namespace
{
    ...
    TEST(TrojanFramingPure, ParseCredentialTooShort)
    {
        ...
        EXPECT_EQ(ec, psm::fault::code::bad_message) << "credential: too short";
    }
    ...
    TEST(TrojanFramingPure, ParseCredentialInvalidChar)
    {
        ...
        buf[0] = 'Z'; // not hex
        auto [ec, cred] = parse_credential(buf);
        EXPECT_EQ(ec, psm::fault::code::protocol_error) << "credential: invalid char";
    }
    ...
    TEST(TrojanFramingPure, ParseCredentialValidHex)
    {
        ...
        auto [ec, cred] = parse_credential(buf);
        EXPECT_EQ(ec, psm::fault::code::success) << "credential: valid hex -> success";
        EXPECT_EQ(cred[0], 'a') << "credential: first char = 'a'";
    }
    ...
    TEST(TrojanFramingPure, ParseCredentialValidDigits)
    {
        ...
        auto [ec, cred] = parse_credential(buf);
        EXPECT_EQ(ec, psm::fault::code::success) << "credential: all digits -> success";
    }
    ...
    TEST(TrojanFramingPure, ParseCredentialValidUpperHex)
    {
        ...
        auto [ec, cred] = parse_credential(buf);
        EXPECT_EQ(ec, psm::fault::code::success) << "credential: upper hex -> success";
    }

    TEST(TrojanFramingPure, ParseCrlfTooShort)
    {
        std::array<std::uint8_t, 1> buf{};
        EXPECT_EQ(parse_crlf(buf), psm::fault::code::bad_message) << "crlf: too short";
    }

    TEST(TrojanFramingPure, ParseCrlfValid)
    {
        std::array<std::uint8_t, 2> buf{'\r', '\n'};
        EXPECT_EQ(parse_crlf(buf), psm::fault::code::success) << "crlf: valid CRLF";
    }

    TEST(TrojanFramingPure, ParseCrlfInvalid)
    {
        std::array<std::uint8_t, 2> buf{'\n', '\r'};
        EXPECT_EQ(parse_crlf(buf), psm::fault::code::protocol_error) << "crlf: wrong order";
    }
    ...
    TEST(TrojanFramingPure, ParseCmdAtypTooShort)
    {
        ...
        EXPECT_EQ(ec, psm::fault::code::bad_message) << "cmd_atyp: too short";
    }
    ...
    TEST(TrojanFramingPure, ParseCmdAtypValid)
    {
        ...
        auto [ec, result] = parse_cmd_atyp(buf);
        EXPECT_EQ(ec, psm::fault::code::success) << "cmd_atyp: success";
        EXPECT_EQ(result.cmd, command::connect) << "cmd_atyp: cmd=connect";
        EXPECT_EQ(result.atyp, address_type::ipv4) << "cmd_atyp: atyp=ipv4";
    }
    ...
    TEST(TrojanFramingPure, BuildUdpPktIPv4Roundtrip)
    {
        ...
        EXPECT_EQ(result.destination_port, 80) << "trojan roundtrip: port=80";
        EXPECT_EQ(result.payload_size, 3) << "trojan roundtrip: payload_size=3";
    }
    ...
    TEST(TrojanFramingPure, BuildUdpPktDomainRoundtrip)
    {
        ...
        EXPECT_EQ(ec, psm::fault::code::success) << "trojan build_udp domain: success";

        auto [pec, result] = parse_udp_pkt(out);
        EXPECT_EQ(pec, psm::fault::code::success) << "trojan domain roundtrip: parse success";
        EXPECT_EQ(result.destination_port, 443) << "trojan domain roundtrip: port=443";
        EXPECT_EQ(result.payload_size, 0) << "trojan domain roundtrip: payload_size=0";
    }
    ...
    TEST(TrojanFramingPure, BuildUdpPktIPv6Roundtrip)
    {
        ...
        auto ec = build_udp_pkt(frame, {}, out);
        EXPECT_EQ(ec, psm::fault::code::success) << "trojan build_udp IPv6: success";

        auto [pec, result] = parse_udp_pkt(out);
        EXPECT_EQ(pec, psm::fault::code::success) << "trojan IPv6 roundtrip: parse success";
        EXPECT_EQ(result.destination_port, 8443) << "trojan IPv6 roundtrip: port=8443";
    }
    ...
    TEST(TrojanFramingPure, ParseUdpPktTooShort)
    {
        ...
        EXPECT_EQ(ec, psm::fault::code::bad_message) << "trojan parse_udp: too short";
    }
    ...
    TEST(TrojanFramingPure, ParseUdpPktUnknownAtyp)
    {
        ...
        buf[0] = std::byte{0xFF};
        auto [ec, result] = parse_udp_pkt(buf);
        EXPECT_EQ(ec, psm::fault::code::unsupported_address) << "trojan parse_udp: unknown atyp";
    }
} // namespace
