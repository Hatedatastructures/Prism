...
namespace
{
    ...
    auto run_nested_vless(net::io_context &ioc, Factory factory, const char *name) -> void
    {
        ...
        run_coro(ioc, [&]() -> net::awaitable<void>
            ...
            {
                auto [serr, sconn] = co_await factory.server_accept(
                    std::make_shared<memory_stream>(std::move(b)));
                if (serr != error::none || !sconn)
                {
                    EXPECT_TRUE(false) << name << ": stealth accept failed err="
                                       << static_cast<int>(serr);
                    failed = true;
                    ioc.stop();
                    co_return;
                }
                auto [verr, req, vconn] = co_await vless::accept(std::move(sconn),
            ...
            auto [cerr, cconn] = co_await factory.client_connect(
                std::make_shared<memory_stream>(std::move(a)));
            if (cerr != error::none || !cconn)
            {
                EXPECT_TRUE(false) << name << ": stealth connect failed err="
                                   << static_cast<int>(cerr);
                failed = true;
                ioc.stop();
                co_return;
            }
            auto [herr, cli] = co_await vless::connect(std::move(cconn),
                                                       vless::client_config{make_uuid()}, make_dst());
            if (herr != error::none || !cli)
            {
                EXPECT_TRUE(false) << name << ": vless connect failed err="
        ...
        std::printf("%-12s 联通=%s  bytes=%zu(期望 %zu)  吞吐=%.1f MB/s  延迟(ms) avg=%.3f p50=%.3f p95=%.3f p99=%.3f\n",
                    name, (linked && !failed) ? "OK" : "FAIL", tp.bytes, kTotal, tp.mbps,
                    tp.latency_avg, tp.latency_p50, tp.latency_p95, tp.latency_p99);
    }
    ...
    struct shadowtls_factory
    {
        auto client_connect(shared_transmission up) -> net::awaitable<std::pair<error, shared_transmission>>
        {
            ...
            auto [err, c] = co_await shadowtls::connect(
                std::move(up), shadowtls::client_config{"st_password"}, sr, cr);
        ...
        {
            auto [err, c] = co_await shadowtls::accept(std::move(up),
                                                       shadowtls::server_config{"st_password"});
            co_return std::pair{err, err == error::none ? shared_transmission(std::move(c))
                                                        : shared_transmission{}};
        }
    };
    ...
    struct trusttunnel_factory
    {
        ...
        auto server_accept(shared_transmission up) -> net::awaitable<std::pair<error, shared_transmission>>
        {
            auto [err, target, c] = co_await trusttunnel::accept(
                std::move(up), trusttunnel::server_config{"tu_user", "tu_pass"});
            (void)target;
            co_return std::pair{err, err == error::none ? shared_transmission(std::move(c))
                                                        : shared_transmission{}};
        }
    ...
    TEST(TrustTunnelConn, HandshakeDirect)
    {
        ...
        run_coro(ioc, [&]() -> net::awaitable<void>
            ...
            auto server_coro = [&]() -> net::awaitable<void>
            {
                auto [err, t, c] = co_await trusttunnel::accept(
                    std::make_shared<memory_stream>(std::move(b)),
