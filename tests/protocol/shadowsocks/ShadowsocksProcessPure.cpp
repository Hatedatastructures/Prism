...
namespace
{
    ...
    TEST(ShadowsocksProcessPure, ConfigDefaults)
    {
        ...
        EXPECT_EQ(cfg.enable_tcp, true) << "config: 默认 enable_tcp=true";
        EXPECT_EQ(cfg.enable_udp, false) << "config: 默认 enable_udp=false";
        EXPECT_EQ(cfg.timestamp_window, 30) << "config: 默认 timestamp_window=30";
        EXPECT_EQ(cfg.salt_ttl, 60) << "config: 默认 salt_ttl=60";
    ...
    TEST(ShadowsocksProcessPure, ConfigCustomValues)
    {
        ...
        EXPECT_EQ(cfg.timestamp_window, 120) << "config: 自定义 timestamp_window=120";
        EXPECT_EQ(cfg.salt_ttl, 300) << "config: 自定义 salt_ttl=300";
        EXPECT_EQ(cfg.idle_timeout, 180) << "config: 自定义 idle_timeout=180";
    }
    ...
    TEST(ShadowsocksProcessPure, CipherMethodEnum)
    {
        ...
        EXPECT_EQ(m, cipher_method::aes_128_gcm) << "cipher_method: aes_128_gcm";

        m = cipher_method::aes_256_gcm;
        EXPECT_EQ(m, cipher_method::aes_256_gcm) << "cipher_method: aes_256_gcm";

        m = cipher_method::chacha20_poly1305;
        EXPECT_EQ(m, cipher_method::chacha20_poly1305) << "cipher_method: chacha20_poly1305";
    }
    ...
    {
        request req;
        EXPECT_EQ(req.port, 0) << "request: 默认 port=0";
    }
    ...
    TEST(ShadowsocksProcessPure, Constants)
    {
        ...
        EXPECT_EQ(aead_tag_len, 16) << "constant: aead_tag_len=16";
        EXPECT_EQ(fixed_hdr_plain, 11) << "constant: fixed_hdr_plain=11";
        EXPECT_EQ(fixed_hdr_size, 27) << "constant: fixed_hdr_size=27";
        EXPECT_EQ(len_block_size, 18) << "constant: len_block_size=18";
        EXPECT_EQ(max_chunk_size, 0x3FFF) << "constant: max_chunk_size=0x3FFF";
    }
    ...
    TEST(ShadowsocksProcessPure, AddressTypes)
    {
        ...
        EXPECT_EQ(v4.bytes[0], 10) << "address: ipv4 byte 0";
        EXPECT_EQ(v4.bytes[3], 1) << "address: ipv4 byte 3";

        // IPv6 地址构造
        ipv6_address v6{};
        v6.bytes[0] = 0xFF;
        EXPECT_EQ(v6.bytes[0], 0xFF) << "address: ipv6 byte 0";

        // 域名地址构造
        domain_address domain{};
        domain.length = 3;
        std::memcpy(domain.value.data(), "com", 3);
        EXPECT_EQ(domain.length, 3) << "address: domain length=3";
