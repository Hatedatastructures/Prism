...
namespace
{
    ...
    TEST(ShadowsocksFramingPure, ParseAddrPortEmpty)
    {
        ...
        EXPECT_EQ(ec, psm::fault::code::bad_message) << "parse_addr_port: empty buffer";
    }
    ...
    TEST(ShadowsocksFramingPure, ParseAddrPortIPv4)
    {
        ...
        EXPECT_EQ(ec, psm::fault::code::success) << "parse_addr_port IPv4: success";
        EXPECT_EQ(result.port, 80) << "parse_addr_port IPv4: port=80";
        EXPECT_EQ(result.offset, 7) << "parse_addr_port IPv4: offset=7";

        auto *ipv4 = std::get_if<psm::protocol::common::ipv4_address>(&result.addr);
        EXPECT_NE(ipv4, nullptr) << "parse_addr_port IPv4: variant";
    ...
    TEST(ShadowsocksFramingPure, ParseAddrPortIPv4Truncated)
    {
        ...
        auto [ec, result] = parse_addr_port(buf);
        EXPECT_EQ(ec, psm::fault::code::bad_message) << "parse_addr_port IPv4: truncated";
    }
    ...
    TEST(ShadowsocksFramingPure, ParseAddrPortDomain)
    {
        ...
        EXPECT_EQ(result.offset, 15) << "parse_addr_port domain: offset=15";

        auto *domain = std::get_if<psm::protocol::common::domain_address>(&result.addr);
        EXPECT_NE(domain, nullptr) << "parse_addr_port domain: variant";
    ...
    TEST(ShadowsocksFramingPure, ParseAddrPortDomainTruncated)
    {
        ...
        auto [ec, result] = parse_addr_port(buf);
        EXPECT_EQ(ec, psm::fault::code::bad_message) << "parse_addr_port domain: truncated";
    }
    ...
    TEST(ShadowsocksFramingPure, ParseAddrPortIPv6)
    {
        ...
        buf[18] = 0x51;
        auto [ec, result] = parse_addr_port(buf);
        EXPECT_EQ(ec, psm::fault::code::success) << "parse_addr_port IPv6: success";
        EXPECT_EQ(result.port, 4433) << "parse_addr_port IPv6: port=4433";
        EXPECT_EQ(result.offset, 19) << "parse_addr_port IPv6: offset=19";

        auto *ipv6 = std::get_if<psm::protocol::common::ipv6_address>(&result.addr);
        EXPECT_NE(ipv6, nullptr) << "parse_addr_port IPv6: variant";
    ...
    TEST(ShadowsocksFramingPure, ParseAddrPortIPv6Truncated)
    {
        ...
        buf[0] = 0x04;
        auto [ec, result] = parse_addr_port(buf);
        EXPECT_EQ(ec, psm::fault::code::bad_message) << "parse_addr_port IPv6: truncated";
    }
    ...
    TEST(ShadowsocksFramingPure, ParseAddrPortUnknownAtyp)
    {
        ...
        buf[0] = 0xFF;
        auto [ec, result] = parse_addr_port(buf);
        EXPECT_EQ(ec, psm::fault::code::unsupported_address) << "parse_addr_port: unknown atyp";
    }

    TEST(ShadowsocksFramingPure, DecodePskEmpty)
    {
        auto [ec, psk] = decode_psk("");
        EXPECT_EQ(ec, psm::fault::code::invalid_psk) << "decode_psk: empty -> invalid";
    ...
    TEST(ShadowsocksFramingPure, DecodePskInvalidLength)
    {
        ...
        EXPECT_EQ(ec, psm::fault::code::invalid_psk) << "decode_psk: 8 bytes -> invalid";
    }
    ...
    TEST(ShadowsocksFramingPure, DecodePsk16Bytes)
    {
        ...
        EXPECT_EQ(ec, psm::fault::code::success) << "decode_psk: 16 bytes -> success";
        EXPECT_EQ(psk.size(), 16) << "decode_psk: 16 bytes size";
    }
    ...
    TEST(ShadowsocksFramingPure, DecodePsk32Bytes)
    {
        ...
        EXPECT_EQ(ec, psm::fault::code::success) << "decode_psk: 32 bytes -> success";
        EXPECT_EQ(psk.size(), 32) << "decode_psk: 32 bytes size";
    }
    ...
    {
        EXPECT_EQ(resolve_method("", 16), cipher_method::aes_128_gcm) << "resolve: auto 16B -> aes-128";
        EXPECT_EQ(resolve_method("", 32), cipher_method::aes_256_gcm) << "resolve: auto 32B -> aes-256";
        EXPECT_EQ(resolve_method("", 0), cipher_method::aes_256_gcm) << "resolve: auto 0B -> aes-256";
    }

    TEST(ShadowsocksFramingPure, KeysaltLen)
    {
        EXPECT_EQ(keysalt_len(cipher_method::aes_128_gcm), 16) << "keysalt: aes-128 = 16";
        EXPECT_EQ(keysalt_len(cipher_method::aes_256_gcm), 32) << "keysalt: aes-256 = 32";
        EXPECT_EQ(keysalt_len(cipher_method::chacha20_poly1305), 32) << "keysalt: chacha20 = 32";
    }
} // namespace
