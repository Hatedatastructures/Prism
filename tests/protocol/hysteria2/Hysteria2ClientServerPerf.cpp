        ...
        run_coro(ioc, [&]() -> net::awaitable<void>
            ...
            auto server_coro = [&]() -> net::awaitable<void>
            {
                auto [err, req, conn] = co_await hysteria2::accept(
                    std::make_shared<memory_stream>(std::move(b)), hysteria2::server_config{"pw123456"});
                if (err != error::none)
                {
            ...
            auto [herr, cli] = co_await hysteria2::connect(
                std::make_shared<memory_stream>(std::move(a)), hysteria2::client_config{"pw123456"}, make_dst());
