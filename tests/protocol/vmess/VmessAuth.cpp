...
TEST(VmessAuth, SealOpenRoundtrip)
{
    ...
    const auto ec = seal_auth_header(std::span<const std::uint8_t, 16>(cmd_key.data(), 16), ts,
                                     std::span<std::uint8_t, 16>(auth_id.data(), 16));
    ASSERT_EQ(ec, psm::fault::code::success);

    const auto result = open_auth_header(std::span<const std::uint8_t, 16>(cmd_key.data(), 16),
...
TEST(VmessAuth, WrongKeyFails)
{
    ...
    wrong.fill(0x42);
    const auto result = open_auth_header(std::span<const std::uint8_t, 16>(wrong.data(), 16),
                                         std::span<const std::uint8_t, 16>(auth_id.data(), 16));
