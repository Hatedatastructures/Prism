...
TEST(Hkdf, HmacSha256)
{
    ...
    auto mac =
        psm::crypto::hmac_sha256(std::span{reinterpret_cast<const std::uint8_t *>(key.data()), key.size()},
                                 std::span{reinterpret_cast<const std::uint8_t *>(data.data()), data.size()});

    auto hex = ToHex(mac);
    EXPECT_EQ(hex, expected) << "HMAC-SHA256 mismatch";
}
...
TEST(Hkdf, HmacSha256EmptyKey)
{
    ...
    EXPECT_EQ(ToHex(mac).size(), 64u) << "HMAC-SHA256 with empty key should produce 32 bytes";
}
...
TEST(Hkdf, HmacSha256EmptyData)
{
    ...
    // 验证输出为 32 字节
    ASSERT_EQ(ToHex(mac).size(), 64u) << "HMAC-SHA256 with empty data should produce 32 bytes";
    ...
    auto hex = ToHex(mac);
    EXPECT_EQ(hex, expected) << "HMAC-SHA256('secret', '') mismatch";
}
...
TEST(Hkdf, HmacSha512)
{
    const std::string key = "Jefe";
    const std::string data = "what do ya want for nothing?";
    constexpr std::string_view expected = "164b7a7bfcf819e2e395fbe73b56e0a387bd64222e831fd610270cd7ea250554"
                                          "9758bf75c05a994a6d034f65f8f0e6fdcaeab1a34d4a6b4b636e070a38bce737";

    auto mac =
        psm::crypto::hmac_sha512(std::span{reinterpret_cast<const std::uint8_t *>(key.data()), key.size()},
                                 std::span{reinterpret_cast<const std::uint8_t *>(data.data()), data.size()});

    auto hex = ToHex(mac);
    EXPECT_EQ(hex, expected) << "HMAC-SHA512 mismatch";
}
...
TEST(Hkdf, HkdfExtract)
{
    ...
    const std::array<std::uint8_t, 22> ikm = {0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b,
                                              0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b,
                                              0x0b, 0x0b, 0x0b, 0x0b, 0x0b, 0x0b};

    // salt: 13 bytes 0x00..0x0c
    const std::array<std::uint8_t, 13> salt = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06,
    ...
    auto hex = ToHex(prk);
    EXPECT_EQ(hex, expected_prk) << "HKDF-Extract PRK mismatch";
}
...
TEST(Hkdf, HkdfExpand)
{
    ...
    const std::array<std::uint8_t, 10> info = {0xf0, 0xf1, 0xf2, 0xf3, 0xf4, 0xf5, 0xf6, 0xf7, 0xf8, 0xf9};
    ...
    EXPECT_EQ(okm.size(), 42u) << "HKDF-Expand output length should be 42";
}
...
TEST(Hkdf, Sha256Empty)
{
    ...
    auto hash =
        psm::crypto::sha256(std::span{reinterpret_cast<const std::uint8_t *>(empty.data()), empty.size()});
...
TEST(Hkdf, Sha256Known)
{
    ...
    auto hash =
        psm::crypto::sha256(std::span{reinterpret_cast<const std::uint8_t *>(data.data()), data.size()});
...
TEST(Hkdf, Sha256Span)
{
    ...
    // 单块 span
    auto hash_single =
        psm::crypto::sha256(std::span{reinterpret_cast<const std::uint8_t *>(abc.data()), abc.size()});

    // 两块 span 重载: SHA-256("a" || "bc")
    auto hash_two =
        psm::crypto::sha256(std::span{reinterpret_cast<const std::uint8_t *>(a.data()), a.size()},
...
TEST(Hkdf, ExpandLabelContextTooLong)
{
    ...
    auto [ec, out] = psm::crypto::expand_label(
        {secret, "key", std::span<const std::uint8_t>{long_context.data(), long_context.size()}, 16});
