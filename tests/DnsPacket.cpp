...
void TestMakeQuery()
{
    ...
    if (msg.id != 0)
    {
        log_fail("id should be 0 (unassigned)");
        return;
    }
    ...
    if (!msg.rd)
    {
        log_fail("rd should be true (recursion desired)");
        return;
    }
    ...
    if (msg.qr)
    {
        log_fail("qr should be false (query, not response)");
        return;
    }
    ...
    {
        log_fail("questions.size() should be 1");
        return;
    }
    ...
    {
        log_fail("question name should be 'example.com'");
        return;
    }
    ...
    {
        log_fail("question qtype should be A (1)");
        return;
    }
...
{
    log_info("=== Testing pack/unpack round trip ===");
    ...
    // === 测试查询报文的序列化/反序列化往返 ===
    {
        ...
        if (!opt)
        {
            log_fail("unpack query returned nullopt");
            return;
        }
        ...
        {
            log_fail("query: id mismatch after round trip");
            return;
        }

        if (!restored.rd)
        {
            log_fail("query: rd should be true after round trip");
            return;
        }

        if (restored.questions.size() != 1 || restored.questions[0].name != "example.com")
        {
            log_fail("query: question name mismatch after round trip");
            return;
        }

        if (restored.questions[0].qtype != psm::resolve::qtype::a)
        {
            log_fail("query: question qtype mismatch after round trip");
            return;
        }
    }
    ...
    // === 测试带应答记录的响应报文往返 ===
    {
        ...
        if (!opt)
        {
            log_fail("unpack response returned nullopt");
            return;
        }
        ...
        if (!restored.qr)
        {
            log_fail("response: qr should be true");
            return;
        }
        ...
        {
            log_fail("response: answers count mismatch");
            return;
        }
        ...
        {
            log_fail("response: answer name mismatch");
            return;
        }
        ...
        {
            log_fail("response: answer type mismatch");
            return;
        }
        ...
        {
            log_fail("response: answer TTL mismatch");
            return;
        }
        ...
        {
            log_fail("response: answer rdata mismatch");
            return;
        }
    }

    log_pass("pack/unpack round trip");
}
...
void TestExtractIPv4()
{
    ...
    // 测试公网地址 8.8.8.8（Google DNS）
    {
        ...
        if (!result)
        {
            log_fail("extract_ipv4 returned nullopt for 8.8.8.8");
            return;
        }
        ...
        {
            log_fail("extract_ipv4: 8.8.8.8 mismatch");
            return;
        }
    }
    ...
    // 测试私有地址 192.168.1.1
    {
        ...
        if (!result)
        {
            log_fail("extract_ipv4 returned nullopt for 192.168.1.1");
            return;
        }
        ...
        if (result->to_uint() != expected.to_uint())
        {
            log_fail("extract_ipv4: 192.168.1.1 mismatch");
            return;
        }
    }

    log_pass("extract_ipv4");
}
...
void TestExtractIPv6()
{
    ...
    if (!result)
    {
        log_fail("extract_ipv6 returned nullopt for ::1");
        return;
    }
    ...
    {
        log_fail("extract_ipv6: ::1 mismatch");
        return;
    }

    log_pass("extract_ipv6");
}
...
{
    log_info("=== Testing extract_ipv4 with bad lengths ===");
    ...
    // rdata 仅 3 字节，不足 IPv4 所需的 4 字节
    {
        ...
        if (result.has_value())
        {
            log_fail("extract_ipv4 should return nullopt for 3-byte rdata");
            return;
        }
    }
    ...
    // rdata 有 5 字节，超出 IPv4 长度
    {
        ...
        if (result.has_value())
        {
            log_fail("extract_ipv4 should return nullopt for 5-byte rdata");
            return;
        }
    }

    log_pass("extract_ipv4 bad length");
}
...
void TestExtractIPs()
{
    ...
    if (ips.size() != 2)
    {
        log_fail("extract_ips should return 2 addresses");
        return;
    }

    log_pass("extract_ips");
}
...
void TestMinTtl()
{
    ...
    // 三条记录，TTL 分别为 300、600、60，取最小值
    {
        ...
        if (msg.min_ttl() != 60)
        {
            log_fail("min_ttl should be 60, got " + std::to_string(msg.min_ttl()));
            return;
        }
    }
    ...
    // 单条记录，TTL=3600（1 小时）
    {
        ...
        if (msg.min_ttl() != 3600)
        {
            log_fail("min_ttl should be 3600 for single record");
            return;
        }
    }
...
void TestPackUnpackTcp()
{
    ...
    if (!opt)
    {
        log_fail("unpack_tcp returned nullopt");
        return;
    }
    ...
    if (restored.id != original.id)
    {
        log_fail("TCP round trip: id mismatch");
        return;
    }

    if (restored.questions.size() != original.questions.size())
    {
        log_fail("TCP round trip: question count mismatch");
        return;
    }

    if (restored.questions[0].name != "test.org")
    {
        log_fail("TCP round trip: question name mismatch");
        return;
    }

    if (restored.questions[0].qtype != psm::resolve::qtype::aaaa)
    {
        log_fail("TCP round trip: question qtype mismatch");
        return;
    }
...
int main()
{
    ...
    log_info("Starting DNS packet tests...");
    ...
    log_info("DNS packet tests completed.");
