...
namespace
{
    ...
    net::awaitable<void> MultiEchoServer(tcp::acceptor acceptor, const int count)
    {
        for (int i = 0; i < count; ++i)
        {
            ...
            net::co_spawn(
                ...
                [sock = std::move(socket)]() mutable -> net::awaitable<void>
                {
                    std::array<char, 8192> buf{};
                    while (true)
                    {
                                             ...
                                             const std::string_view tag, const std::string_view payload)
    {
        ...
        {
            throw psm::exception::network(std::format("{} method negotiation failed: {:02x} {:02x}", tag,
        ...
        {
            throw psm::exception::network(
                std::format("{} CONNECT failed: {:02x} {:02x}", tag, connect_resp[0], connect_resp[1]));
        }
        ...
        // 读取地址 + 2 字节端口
        co_await net::async_read(socket, net::buffer(connect_resp.data() + 4, addr_len + 2),
                                 net::use_awaitable);
        ...
        if (echo != payload)
        {
            ...
            throw psm::exception::network(
                std::format("{} echo mismatch: expected [{}] got [{}]", tag, expected_hex, got_hex));
        }
                                                 ...
                                                 const std::string_view tag, const std::string_view payload)
    {
        ...
        {
            throw psm::exception::network(std::format("{} auth method negotiation failed: {:02x} {:02x}", tag,
                                                      method_resp[0], method_resp[1]));
        }
        ...
        {
            throw psm::exception::network(
                std::format("{} auth failed: {:02x} {:02x}", tag, auth_resp[0], auth_resp[1]));
        }
    ...
    net::awaitable<void> E2ESocks5EchoImpl(psm::resource::process &server_ctx,
                                           psm::resource::worker &worker_ctx)
    {
        try
        {
    ...
    net::awaitable<void> E2EHttpConnectEchoImpl(psm::resource::process &server_ctx,
                                                psm::resource::worker &worker_ctx)
    {
        try
        {
    ...
    net::awaitable<void> E2ESocks5AuthImpl(psm::resource::process &server_ctx,
                                           psm::resource::worker &worker_ctx)
    {
        try
        {
            ...
            co_await RawSocks5AuthClientEcho(proxy_ep, echo_ep, "testuser", "test_password", "socks5_auth",
                                             "hello_e2e_socks5_auth");
        }
        catch (const std::exception &e)
        {
    ...
    net::awaitable<void> E2EHttpAuth407Impl(psm::resource::process &server_ctx,
                                            psm::resource::worker &worker_ctx)
    {
        try
        {
    ...
    net::awaitable<void> E2EConcurrencyImpl(psm::resource::process &server_ctx,
                                            psm::resource::worker &worker_ctx)
    {
        try
        {
            ...
            net::co_spawn(ioc,
                          MultiProxyAccept(std::move(proxy_acceptor), conn_count, server_ctx, worker_ctx),
                          net::detached);
            ...
            for (int i = 0; i < conn_count; ++i)
            {
                ...
                auto client_task = [proxy_ep, echo_ep, payload = std::move(payload), completed, succeeded,
                                    all_done, i]() -> net::awaitable<void>
                {
                    try
                    {
                        co_await RawSocks5ClientEcho(proxy_ep, echo_ep, std::format("conc_{}", i), payload);
