...
int main(const int argc, char *argv[])
{
    ...
    try
    {
        ...
        std::string host;
        std::uint16_t port = 0;
        if (!split_host_port(server_addr, host, port))
        {
        ...
        const auto handshake = client.handshake(dst, {}, now);
        net::write(sock, net::buffer(handshake));

        // 加密 echo 载荷并发送（chunk）
        const std::string payload = "hello interop ss2022";
        auto key = shadow2022::session_key(psk, client.salt());
        // 服务端握手响应消耗 nonce 0,1，数据从 2 起
        shadow2022::chunk_codec codec(key, 2);
        const auto enc = codec.seal(view(
        ...
        const auto resp_key = shadow2022::session_key(psk, view(resp_head.data(), 16));
        shadow2022::chunk_codec resp_codec(resp_key);
        buffer fixed;
        if (!resp_codec.open_raw(view(resp_head.data() + 16, 43), fixed) ||
            fixed.size() != 27 || fixed[0] != 0x01)
        {
            std::fprintf(stderr, "FAIL: server response fixed header\n");
            return 1;
        }
        // 固定头 paddingLen 字段 = 响应携带的 echo 数据长度
        const auto payload_len = static_cast<std::size_t>((fixed[25] << 8) | fixed[26]);
        if (payload_len == 0)
        {
        ...
        buffer plain;
        if (!resp_codec.open_raw(payload_enc, plain))
        {
