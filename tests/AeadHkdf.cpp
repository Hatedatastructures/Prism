...
namespace
{
    ...
    void TestAes128GcmRoundTrip(TestRunner &runner)
    {
        ...
        runner.Check(ctx.nonce_length() == 12, "aes-128-gcm: nonce_length=12");
        ...
        auto ec = ctx.seal(si);
        runner.Check(ec == psm::fault::code::success, "aes-128-gcm seal: success");
        runner.Check(ciphertext.size() == 5 + 16, "aes-128-gcm seal: ciphertext size");

        std::vector<std::uint8_t> decrypted(5);
        psm::crypto::open_input oi{decrypted, ciphertext, nonce, {}};
        auto ec2 = ctx.open(oi);
        runner.Check(ec2 == psm::fault::code::success, "aes-128-gcm open: success");
        runner.Check(std::memcmp(plaintext, decrypted.data(), 5) == 0, "aes-128-gcm: round-trip match");
    }

    void TestAes256GcmRoundTrip(TestRunner &runner)
    {
        ...
        runner.Check(ctx.nonce_length() == 12, "aes-256-gcm: nonce_length=12");
        ...
        if (ec == psm::fault::code::success)
        {
            ...
            psm::crypto::open_input oi{decrypted, ciphertext, nonce, {}};
            auto ec2 = ctx.open(oi);
            runner.Check(ec2 == psm::fault::code::success, "aes-256-gcm open: success");
            runner.Check(std::memcmp(plaintext, decrypted.data(), len) == 0, "aes-256-gcm: round-trip match");
        }
        else
        {
            runner.Check(true, "aes-256-gcm seal: skipped (no HW AES-256)");
        }
    }

    void TestChaCha20RoundTrip(TestRunner &runner)
    {
        std::array<std::uint8_t, 32> key{};
        key[0] = 0xAA;
        ...
        psm::crypto::seal_input si{ciphertext, plaintext, nonce, {}};
        auto ec = ctx.seal(si);
        runner.Check(ec == psm::fault::code::success, "chacha20 seal: success");

        std::vector<std::uint8_t> decrypted(4);
        psm::crypto::open_input oi{decrypted, ciphertext, nonce, {}};
        auto ec2 = ctx.open(oi);
        runner.Check(ec2 == psm::fault::code::success, "chacha20 open: success");
        runner.Check(std::memcmp(plaintext, decrypted.data(), 4) == 0, "chacha20: round-trip match");
    }

    void TestXChaCha20RoundTrip(TestRunner &runner)
    {
        std::array<std::uint8_t, 32> key{};
        key[31] = 0xFF;
        psm::crypto::aead_context ctx(psm::crypto::aead_cipher::xchacha20_poly1305, key);
        runner.Check(ctx.nonce_length() == 24, "xchacha20: nonce_length=24");
        ...
        psm::crypto::seal_input si{ciphertext, plaintext, nonce, {}};
        auto ec = ctx.seal(si);
        runner.Check(ec == psm::fault::code::success, "xchacha20 seal: success");

        std::vector<std::uint8_t> decrypted(3);
        psm::crypto::open_input oi{decrypted, ciphertext, nonce, {}};
        auto ec2 = ctx.open(oi);
        runner.Check(ec2 == psm::fault::code::success, "xchacha20 open: success");
        runner.Check(std::memcmp(plaintext, decrypted.data(), 3) == 0, "xchacha20: round-trip match");
    }

    void TestExplicitNonce(TestRunner &runner)
    {
        std::array<std::uint8_t, 16> key{};
        for (std::size_t i = 0; i < 16; ++i)
            key[i] = static_cast<std::uint8_t>(i + 1);
        ...
        auto ec1 = ctx.seal(psm::crypto::seal_input{
            ...
            {}});
        runner.Check(ec1 == psm::fault::code::success, "explicit nonce seal 1: success");
        ...
        auto ec2 = ctx.seal(psm::crypto::seal_input{
            ...
            std::span<const std::uint8_t>{nonce1},
            {}});
        runner.Check(ec2 == psm::fault::code::success, "explicit nonce seal 2: success");
        runner.Check(std::memcmp(ct1.data(), ct2.data(), ct1.size()) == 0,
        ...
        auto ec3 = ctx.open(psm::crypto::open_input{
            ...
            std::span<const std::uint8_t>{nonce1},
            {}});
        runner.Check(ec3 == psm::fault::code::success, "explicit nonce open: success");
        runner.Check(std::memcmp(plaintext, decrypted.data(), len) == 0, "explicit nonce: decrypt match");
    }

    void TestOpenTampered(TestRunner &runner)
    {
        std::array<std::uint8_t, 16> key{};
        for (std::size_t i = 0; i < 16; ++i)
            key[i] = static_cast<std::uint8_t>(i + 1);
        ...
        runner.Check(ec != psm::fault::code::success, "open tampered: failure");
    }

    void TestSealOpenSize(TestRunner &runner)
    {
        runner.Check(psm::crypto::aead_context::seal_size(100) == 116, "seal_size(100)=116");
        runner.Check(psm::crypto::aead_context::open_size(116) == 100, "open_size(116)=100");
        runner.Check(psm::crypto::aead_context::tag_length() == 16, "tag_length=16");
        runner.Check(psm::crypto::aead_context::open_size(10) == 0, "open_size(10)=0 (too small)");
    }

    void TestNonceIncrements(TestRunner &runner)
    {
        std::array<std::uint8_t, 16> key{};
        for (std::size_t i = 0; i < 16; ++i)
            key[i] = static_cast<std::uint8_t>(i + 1);
        ...
        runner.Check(starts_at_zero, "nonce starts at 0");
        ...
        runner.Check(nonce_after[0] == 1, "nonce incremented after seal");
    }
    ...
    void TestHmacSha256(TestRunner &runner)
    {
        ...
        runner.Check(std::memcmp(mac.data(), mac2.data(), 32) == 0, "hmac_sha256: deterministic");
    }

    void TestHmacSha512(TestRunner &runner)
    {
        std::array<std::uint8_t, 32> key{};
        const std::uint8_t data[] = "test";

        auto mac = psm::crypto::hmac_sha512(key, data);
        runner.Check(mac.size() == 64, "hmac_sha512: 64 bytes");
    }

    void TestHkdfExtract(TestRunner &runner)
    {
        ...
        runner.Check(std::memcmp(prk.data(), prk2.data(), 32) == 0, "hkdf_extract: deterministic");
    }

    void TestHkdfExpand(TestRunner &runner)
    {
        ...
        runner.Check(out.size() == 48, "hkdf_expand: 48 bytes");

        // Deterministic
        auto [ec2, out2] = psm::crypto::hkdf_expand(prk, info, 48);
        runner.Check(std::memcmp(out.data(), out2.data(), 48) == 0, "hkdf_expand: deterministic");
    }

    void TestSha256Single(TestRunner &runner)
    {
        const std::uint8_t data[] = "hello";
        auto h = psm::crypto::sha256(data);
        runner.Check(h.size() == 32, "sha256: 32 bytes");

        auto h2 = psm::crypto::sha256(data);
        runner.Check(std::memcmp(h.data(), h2.data(), 32) == 0, "sha256: deterministic");
    }

    void TestSha256Multi(TestRunner &runner)
    {
        ...
        runner.Check(std::memcmp(h_ab.data(), h_concat.data(), 32) == 0,
                     "sha256(a,b) == sha256(a||b)");
    }

    void TestSha256Triple(TestRunner &runner)
    {
        const std::uint8_t d1[] = {'a'};
        const std::uint8_t d2[] = {'b'};
        const std::uint8_t d3[] = {'c'};
        auto h = psm::crypto::sha256(d1, d2, d3);
        runner.Check(h.size() == 32, "sha256 triple: 32 bytes");
    ...
    void TestExpandLabel(TestRunner &runner)
    {
        std::array<std::uint8_t, 32> secret{};
        secret[0] = 0x01;

        auto [ec, out] = psm::crypto::expand_label({secret, "key", {}, 16});
        runner.Check(ec == psm::fault::code::success, "expand_label: success");
        runner.Check(out.size() == 16, "expand_label: 16 bytes");

        // Different label → different output
        auto [ec2, out2] = psm::crypto::expand_label({secret, "iv", {}, 12});
        runner.Check(ec2 == psm::fault::code::success, "expand_label iv: success");
        runner.Check(out2.size() == 12, "expand_label iv: 12 bytes");
    }
...
int main()
{
    ...
    TestRunner runner("AeadHkdf");

    TestAes128GcmRoundTrip(runner);
    TestAes256GcmRoundTrip(runner);
    TestChaCha20RoundTrip(runner);
    TestXChaCha20RoundTrip(runner);
    TestExplicitNonce(runner);
    TestOpenTampered(runner);
    TestSealOpenSize(runner);
    TestNonceIncrements(runner);
    TestHmacSha256(runner);
    TestHmacSha512(runner);
    TestHkdfExtract(runner);
    TestHkdfExpand(runner);
    TestSha256Single(runner);
    TestSha256Multi(runner);
    TestSha256Triple(runner);
    TestExpandLabel(runner);
