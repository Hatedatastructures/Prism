...
namespace
{
    ...
    {
        DuctFixture fx;
        runner.Check(fx.duct_obj->stream_id() == 42, "constructor: stream_id = 42");
    }
    ...
    void TestConstructorWithMr(TestRunner &runner)
    {
        ...
        runner.Check(d->stream_id() == 1, "constructor: with mr -> stream_id = 1");
    }

    void TestConstructorSmallBuffer(TestRunner &runner)
    {
        DuctFixture fx{100};
        runner.Check(fx.duct_obj->stream_id() == 42, "constructor: small buffer -> ok");
    }

    void TestConstructorLargeBuffer(TestRunner &runner)
    {
        DuctFixture fx{100000};
        runner.Check(fx.duct_obj->stream_id() == 42, "constructor: large buffer -> ok");
    }

    // ─── 析构函数 ─────────────────────────────

    void TestDestructorNoStart(TestRunner &runner)
    {
        DuctFixture fx;
        fx.duct_obj.reset();
        runner.Check(true, "destructor: no start -> no crash");
    }
    ...
    void TestCloseIdempotent(TestRunner &runner)
    {
        DuctFixture fx;
        fx.duct_obj->close();
        runner.Check(true, "close: first close -> no crash");
        fx.duct_obj->close();
        runner.Check(true, "close: second close -> no crash");
    }

    void TestCloseAccumulatesTraffic(TestRunner &runner)
    {
        DuctFixture fx;
        ...
        fx.duct_obj->close();
        runner.Check(true, "close: with traffic state -> no crash");
    }
    ...
    void TestCloseWithoutTraffic(TestRunner &runner)
    {
        DuctFixture fx;
        fx.duct_obj->close();
        runner.Check(true, "close: without traffic state -> no crash");
    }

    void TestCloseTargetClosed(TestRunner &runner)
    {
        DuctFixture fx;
        runner.Check(!fx.target_transport->is_closed(), "close: target not closed before close");
        fx.duct_obj->close();
        runner.Check(fx.target_transport->is_closed(), "close: target transport closed after close");
    }

    // ─── on_fin ─────────────────────────────

    void TestOnFinNoTargetClosed(TestRunner &runner)
    {
        DuctFixture fx;
        fx.duct_obj->on_fin();
        runner.Check(true, "on_fin: mux fin only -> no crash");
    }

    void TestOnFinThenClose(TestRunner &runner)
    {
        DuctFixture fx;
        fx.duct_obj->on_fin();
        fx.duct_obj->close();
        runner.Check(true, "on_fin: then close -> no crash");
    }

    void TestOnFinAfterClose(TestRunner &runner)
    {
        DuctFixture fx;
        fx.duct_obj->close();
        fx.duct_obj->on_fin();
        runner.Check(true, "on_fin: after close -> no crash");
    }

    // ─── on_data 协程路径 ─────────────────────

    void TestOnDataAfterClose(TestRunner &runner)
    {
        DuctFixture fx;
        fx.duct_obj->close();
        ...
        net::co_spawn(mock_ioc, fx.duct_obj->on_data(psm::memory::vector<std::byte>{}),
            [&](std::exception_ptr) {
                runner.Check(true, "on_data: after close -> co_returns immediately");
            });
        mock_ioc.restart();
        mock_ioc.run_one();
    }
    ...
    void TestOnFinTargetClosedTriggersClose(TestRunner &runner)
    {
        ...
        fx.duct_obj->on_fin();
        fx.duct_obj->close();
        runner.Check(true, "on_fin + close: no crash");
    }
...
int main()
{
    ...
    TestRunner runner("DuctDeep");

    TestConstructorDefault(runner);
    TestConstructorWithMr(runner);
    TestConstructorSmallBuffer(runner);
    TestConstructorLargeBuffer(runner);

    TestDestructorNoStart(runner);
    TestDestructorAfterClose(runner);

    TestCloseIdempotent(runner);
    TestCloseAccumulatesTraffic(runner);
    TestCloseWithoutTraffic(runner);
    TestCloseTargetClosed(runner);

    TestOnFinNoTargetClosed(runner);
    TestOnFinThenClose(runner);
    TestOnFinAfterClose(runner);

    TestOnDataAfterClose(runner);

    TestOnFinTargetClosedTriggersClose(runner);
