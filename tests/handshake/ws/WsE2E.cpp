...
namespace
{
    ...
    net::awaitable<void> DoWsClient(psm::transport::shared_transmission transport, const std::string &payload,
                                    std::shared_ptr<bool> ok)
    {
        try
        {
            ...
            {
                frame.push_back(static_cast<std::byte>(static_cast<std::uint8_t>(payload[i]) ^ mask[i % 4]));
            }
    ...
    net::awaitable<void> DoWsServer(psm::transport::shared_transmission transport, const std::string &payload,
