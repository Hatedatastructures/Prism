...
namespace
{
    ...
    auto make_valid_client_hello(std::string_view password) -> psm::memory::vector<std::byte>
    {
        ...
        raw[1] = 0x03;
        raw[2] = 0x01; // Version TLS 1.0
        raw[3] = 0x00;
        raw[4] = 0x47; // Length = 71

        // Handshake Header
        raw[5] = hs_type_clienthello; // Handshake Type: ClientHello
        raw[6] = 0x00;
        raw[7] = 0x00;
        raw[8] = 0x43; // Length = 67
    ...
    TEST(ShadowtlsAuthDeep, VerifyClientHelloWrongPassword)
    {
        auto buf = make_valid_client_hello("mypassword");
        auto span = std::span<const std::byte>(buf.data(), buf.size());
        EXPECT_TRUE(!verify_client_hello(span, "wrongpassword"))
            << "verify_client_hello: wrong password -> false";
    }
    ...
    TEST(ShadowtlsAuthDeep, VerifyClientHelloBadContentType)
    {
        ...
        auto span = std::span<const std::byte>(buf.data(), buf.size());
        EXPECT_TRUE(!verify_client_hello(span, "password"))
            << "verify_client_hello: bad content type -> false";
    }
    ...
    TEST(ShadowtlsAuthDeep, VerifyClientHelloBadHandshakeType)
    {
        ...
        auto span = std::span<const std::byte>(buf.data(), buf.size());
        EXPECT_TRUE(!verify_client_hello(span, "password"))
            << "verify_client_hello: bad handshake type -> false";
    }
    ...
    TEST(ShadowtlsAuthDeep, VerifyClientHelloBadSessionIdLen)
    {
        ...
        auto span = std::span<const std::byte>(buf.data(), buf.size());
        EXPECT_TRUE(!verify_client_hello(span, "password"))
            << "verify_client_hello: bad session_id len -> false";
    }
