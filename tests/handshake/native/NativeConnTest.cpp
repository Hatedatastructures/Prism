...
#include <common/stealth/native/native.hpp>
...
namespace
{
    ...
    net::awaitable<void> DoNativeServer(shared_transmission raw, ssl::context &server_ctx,
                                        const std::string &payload, std::shared_ptr<bool> ok)
    {
        auto trans = co_await psmtest::native::accept(std::move(raw), server_ctx);
        if (!trans)
        {
            *ok = false;
            co_return;
...
TEST(NativeConn, ClientVerifyFailure)
{
    ...
    auto coro = [&]() -> net::awaitable<void>
    {
        net::co_spawn(
            ...
            [&]() -> net::awaitable<void>
            {
                psmtest::transport::connector conn(sa);
                auto stream = std::make_shared<ssl::stream<psmtest::transport::connector>>(
                    std::move(conn), client_ctx);
                boost::system::error_code ec;
                co_await stream->async_handshake(ssl::stream_base::client,
            ...
            net::detached);
        // 服务端握手应因客户端中止而失败
        auto trans = co_await psmtest::native::accept(sb, server_ctx);
        if (!trans)
        {
            *server_ok = true;
        }
