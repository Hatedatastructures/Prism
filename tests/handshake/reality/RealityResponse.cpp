...
namespace
{
    ...
    TEST(RealityResponse, MakeRecordEmpty)
    {
        ...
        EXPECT_EQ(rec.size(), 5) << "make_record empty: size=5";
        EXPECT_EQ(rec[0], tls::CT_HANDSHAKE) << "make_record empty: content_type";
    ...
    TEST(RealityResponse, MakeRecordChangeCipherSpec)
    {
        ...
        EXPECT_EQ(rec[5], 0x01) << "make_record ccs: payload=0x01";
    }
    ...
    TEST(RealityResponse, EncryptRecordDeterministic)
    {
        ...
        EXPECT_GT(rec1.size(), 20) << "encrypt_record: output size > 20";
        EXPECT_EQ(rec1[0], tls::CT_APPLICATION_DATA) << "encrypt_record: content_type=0x17";
    ...
    TEST(RealityResponse, EncryptRecordEmptyPlaintext)
    {
        ...
        EXPECT_EQ(rec.size(), 5 + 1 + 16) << "encrypt empty: size=22";
    }
    ...
    TEST(RealityResponse, EncryptRecordRecordHeader)
    {
        ...
        EXPECT_EQ(rec[0], 0x17) << "encrypt header: ct=0x17";
        EXPECT_EQ(rec[1], 0x03) << "encrypt header: version hi=0x03";
        EXPECT_EQ(rec[2], 0x03) << "encrypt header: version lo=0x03";

        // length field should match remaining bytes
        const auto len = (static_cast<std::uint16_t>(rec[3]) << 8) | rec[4];
        EXPECT_EQ(len, rec.size() - 5) << "encrypt header: length matches";
    }
    ...
    TEST(RealityResponse, GenerateShelloBasic)
    {
        ...
        EXPECT_EQ(result.shello_record[0], tls::CT_HANDSHAKE) << "generate_shello: record ct=0x16";

        // ccs_record 应为 6 字节: [0x14, 0x03, 0x03, 0x00, 0x01, 0x01]
        EXPECT_EQ(result.ccs_record.size(), 6) << "generate_shello: ccs_record size=6";
        EXPECT_EQ(result.ccs_record[0], tls::CT_CHANGE_CIPHER_SPEC) << "generate_shello: ccs ct=0x14";

        // enc_hs_record 不应为空
        EXPECT_TRUE(!result.enc_hs_record.empty()) << "generate_shello: enc_hs_record not empty";
        EXPECT_EQ(result.enc_hs_record[0], tls::CT_APPLICATION_DATA) << "generate_shello: enc_hs ct=0x17";
