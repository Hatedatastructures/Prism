...
namespace
{
    ...
    TEST(StealthCommonPure, AeadNonceNonZeroSequence)
    {
        ...
        auto nonce = psm::handshake::common::aead_nonce(iv, 1);
        EXPECT_EQ(nonce[11], 0x01) << "aead_nonce: sequence=1 仅影响最后字节";
        for (int i = 0; i < 11; ++i)
        {
            EXPECT_EQ(nonce[i], 0x00) << "aead_nonce: sequence=1 高位不变";
        }
    }
    ...
    {
        const std::uint8_t iv[12] = {};
        auto nonce = psm::handshake::common::aead_nonce(iv, 0x0102030405060708ULL);
        EXPECT_EQ(nonce[4], 0x01) << "aead_nonce: 大序列号 byte 4";
        EXPECT_EQ(nonce[11], 0x08) << "aead_nonce: 大序列号 byte 11";
    }
    ...
    {
        auto ad = psm::handshake::common::record_ad(0);
        EXPECT_EQ(ad[0], 0x17) << "record_ad: content type=0x17";
    ...
    TEST(StealthCommonPure, XorKeyBasic)
    {
        ...
        EXPECT_EQ(data[0], std::byte{0xAB}) << "xor_key: byte 0 = 0xAA^0x01";
        EXPECT_EQ(data[1], std::byte{0xB9}) << "xor_key: byte 1 = 0xBB^0x02";
        EXPECT_EQ(data[2], std::byte{0xCF}) << "xor_key: byte 2 = 0xCC^0x03";
    }
    ...
    TEST(StealthCommonPure, XorKeyShortKeyCycles)
    {
        ...
        psm::handshake::common::xor_key(data, key);
        EXPECT_EQ(data[0], std::byte{0xF0}) << "xor_key: 单字节密钥循环 byte 0";
        EXPECT_EQ(data[1], std::byte{0x0F}) << "xor_key: 单字节密钥循环 byte 1";
        EXPECT_EQ(data[2], std::byte{0xF0}) << "xor_key: 单字节密钥循环 byte 2";
        EXPECT_EQ(data[3], std::byte{0x0F}) << "xor_key: 单字节密钥循环 byte 3";
    }
    ...
    TEST(StealthCommonPure, XorKeyEmptyData)
    {
        ...
        const std::uint8_t key[] = {0xFF};
        psm::handshake::common::xor_key(std::span<std::byte>{}, key);
        EXPECT_EQ(data[0], std::byte{0x42}) << "xor_key: 空数据不修改任何内容";
    }
