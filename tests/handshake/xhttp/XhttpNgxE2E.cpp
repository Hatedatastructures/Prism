 ...
 * @brief XHTTP stream-one 端到端测试（T2-2，psmtest 自包含实现）
 * @details 模拟 h2 客户端（psmtest http2）：
 *          1. TLS 握手（自签证书）
...
#include <common/core/http2/impl.hpp>
#include <common/core/transport/memory_stream.hpp>
#include <common/stealth/xhttp/xhttp.hpp>
...
namespace
{
    ...
    {
        psmtest::transport::connector conn(raw);
        auto stream = std::make_shared<ssl::stream<psmtest::transport::connector>>(
    ...
    net::awaitable<void> DoXhttpServer(shared_transmission raw, ssl::context &server_ctx,
                                       const std::string &payload, std::shared_ptr<bool> ok)
    {
        psmtest::xhttp::config cfg;
        auto trans = co_await psmtest::xhttp::accept(std::move(raw), server_ctx, cfg);
        if (!trans)
        {
            *ok = false;
            co_return;
