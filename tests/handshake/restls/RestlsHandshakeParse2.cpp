...
namespace
{
    namespace restls = psm::handshake::restls;

    using restls::tls_hdrsize;

    // 构造最小 TLS 1.3 ServerHello
    auto make_tls13_hello() -> std::vector<std::byte>
    {
        ...
        // Server version
        raw[9] = 0x03;
        raw[10] = 0x03; // TLS 1.2 legacy
        // random: 32 bytes at offset 11..42 (zeros)
        // session_id_len = 0
        raw[43] = 0x00;
        // cipher suite
        raw[44] = 0x13;
        raw[45] = 0x01; // TLS_AES_128_GCM_SHA256
        // compression
        raw[46] = 0x00;
        ...
        raw[52] = 0x02; // length
        raw[53] = 0x03;
        raw[54] = 0x04; // TLS 1.3 = 0x0304
        return hello;
    }

    // 构造 TLS 1.2 ServerHello
    auto make_tls12_hello() -> std::vector<std::byte>
    {
    ...
    auto is_tls13_local(std::span<const std::byte> server_hello) -> bool
    {
        ...
        const std::uint16_t ext_list_len = (static_cast<std::uint16_t>(raw[offset]) << 8) | raw[offset + 1];
        offset += 2;
        const std::size_t ext_end = offset + ext_list_len;
        while (offset + 4 <= ext_end && offset < server_hello.size())
        {
            const std::uint16_t ext_type = (static_cast<std::uint16_t>(raw[offset]) << 8) | raw[offset + 1];
