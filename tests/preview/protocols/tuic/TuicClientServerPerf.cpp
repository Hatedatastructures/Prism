    ...
    TEST(TuicClientServer, HandshakeAndTransfer)
    {
        ...
        run_coro(ioc,
                 [&]() -> net::awaitable<void>
                 {
                     ...
                     {
                         auto [err, req, conn] =
                             co_await tuic::accept(std::make_shared<memory_stream>(std::move(b)),
                     ...
                     auto [herr, cli] =
                         co_await tuic::connect(std::make_shared<memory_stream>(std::move(a)),
                                                tuic::client_config{make_uuid(), "pw"}, make_dst());
    ...
    TEST(TuicClientServer, UdpDatagramRoundtrip)
    {
        ...
        run_coro(
            ...
            [&]() -> net::awaitable<void>
            {
                ...
                auto [herr, cli] = co_await tuic::connect(std::make_shared<memory_stream>(std::move(a)),
                                                          tuic::client_config{make_uuid(), "pw"}, make_dst());
                if (herr != error::none || !cli)
                {
                    co_return;
