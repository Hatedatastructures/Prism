    ...
    TEST(TrojanClientServer, Transfer100MB)
    {
        ...
        run_coro(ioc,
                 [&]() -> net::awaitable<void>
                 {
                     ...
                     auto [herr, cli] =
                         co_await trojan::connect(std::make_shared<memory_stream>(std::move(a)),
                                                  trojan::client_config{"pw123456"}, make_dst());
            ...
            [&]() -> net::awaitable<void>
            {
                auto server_coro = [&]() -> net::awaitable<void>
                {
                    auto [err, req, conn] = co_await trojan::accept(
                        std::make_shared<memory_stream>(std::move(b)), trojan::server_config{"pw123456"});
                    if (err != error::none)
                    {
                        co_return;
                    }
                ...
                auto [herr, cli] = co_await trojan::connect(std::make_shared<memory_stream>(std::move(a)),
                                                            trojan::client_config{"pw123456"}, make_dst());
                if (herr != error::none || !cli)
                {
                    co_return;
                ...
                {
                    auto [err, req, conn] = co_await trojan::accept(
                        std::make_shared<memory_stream>(std::move(b2)), trojan::server_config{"pw123456"});
                    if (err != error::none)
                    {
                        co_return;
                    }
                ...
                auto [herr2, cli2] = co_await trojan::connect(std::make_shared<memory_stream>(std::move(a2)),
                                                              trojan::client_config{"pw123456"}, make_dst());
