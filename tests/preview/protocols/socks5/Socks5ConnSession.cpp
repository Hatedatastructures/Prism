    ...
    TEST(Socks5ConnSession, UserPassMalformedVersion)
    {
        ...
        run_coro(ioc,
                 [&]() -> net::awaitable<void>
                 {
                     ...
                     co_await a.AsyncWriteSome(AsBytes(std::span<const std::uint8_t>(Greeting)), ec);
                     // 读取方法选择
                     std::array<std::uint8_t, 2> sel{};
                     const auto n = co_await a.AsyncReadSome(AsBytes(std::span<std::uint8_t>(sel)), ec);
                     EXPECT_EQ(n, 2u);
                     EXPECT_EQ(sel[1], 0x02);
                     // 发送非法版本的 userpass 子协商
                     const std::array<std::uint8_t, 3> bad{0x02, 0x01, 0x61}; // 版本 0x02
                     co_await a.AsyncWriteSome(AsBytes(std::span<const std::uint8_t>(bad)), ec);
                     a.Close();
                 });
    }
    ...
    TEST(Socks5ConnSession, VersionMismatchRejected)
    {
        ...
        run_coro(ioc,
                 [&]() -> net::awaitable<void>
                 {
                     ...
                     std::error_code ec;
                     co_await a.AsyncWriteSome(AsBytes(std::span<const std::uint8_t>(bad_greeting)), ec);
                     a.Close();
                 });
    }
        ...
        run_coro(ioc,
                 [&]() -> net::awaitable<void>
                 {
                     ...
                     auto server_coro = [&]() -> net::awaitable<void>
                     {
                         ...
                         std::error_code ec;
                         co_await b.AsyncWriteSome(AsBytes(std::span<const std::uint8_t>(Greeting)), ec);
                         std::array<std::uint8_t, 2> sel{0x05, 0x01}; // gssapi
                         co_await b.AsyncWriteSome(AsBytes(std::span<const std::uint8_t>(sel)), ec);
                     };
                     net::co_spawn(ioc.get_executor(), server_coro(), net::detached);
        ...
        run_coro(ioc,
                 [&]() -> net::awaitable<void>
                 {
                     ...
                     const std::array<std::uint8_t, 10> req{0x05, 0x09, 0x00, 0x03, 0x0B, 'e', 'x',
                                                            'a',  'm',  'p'};
                     co_await a.AsyncWriteSome(AsBytes(std::span<const std::uint8_t>(req)), ec);
                     // 读取方法选择（避免服务端响应写失败）
                     std::array<std::uint8_t, 2> sel{};
                     co_await a.AsyncReadSome(AsBytes(std::span<std::uint8_t>(sel)), ec);
                     a.Close();
                 });
    }
        ...
        run_coro(ioc,
                 [&]() -> net::awaitable<void>
                 {
                     ...
                     auto server_coro = [&]() -> net::awaitable<void>
                     {
                         ...
                         std::error_code ec;
                         const auto n = co_await b.AsyncReadSome(AsBytes(std::span<std::uint8_t>(Greeting)),
                                                                   ec);
                         EXPECT_GT(n, 0u);
                         b.Close();
                     };
