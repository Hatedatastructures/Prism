...
namespace
{
    ...
    TEST(CoreAead, ConstructAllCiphers)
    {
        ...
        const cases Table[] = {
            {aead::AeadCipher::aes_128_gcm, 16, 12},
            {aead::AeadCipher::aes_256_gcm, 32, 12},
            {aead::AeadCipher::chacha20_poly1305, 32, 12},
            {aead::AeadCipher::xchacha20_poly1305, 32, 24},
        };
    ...
    TEST(CoreAead, ConstructWrongKeySize)
    {
        ...
        aead::AeadContext ctx(aead::AeadCipher::aes_128_gcm, make_key(32));
        std::array<std::uint8_t, 64> out{};
        EXPECT_EQ(ctx.Seal(out, g_plain), Preview::Fault::Code::crypto_error);
    }
    ...
    TEST(CoreAead, RoundTripAllCiphers)
    {
        ...
        const aead::AeadCipher ciphers[] = {
            aead::AeadCipher::aes_128_gcm,
            aead::AeadCipher::aes_256_gcm,
            aead::AeadCipher::chacha20_poly1305,
            aead::AeadCipher::xchacha20_poly1305,
        };
        for (const auto cipher : ciphers)
        {
            const auto key_len = cipher == aead::AeadCipher::aes_128_gcm ? 16u : 32u;
    ...
    TEST(CoreAead, SealOpenWithAd)
    {
        ...
        aead::AeadContext enc(aead::AeadCipher::chacha20_poly1305, key);
        aead::AeadContext dec(aead::AeadCipher::chacha20_poly1305, key);
        std::array<std::uint8_t, 128> ciphertext{};
    ...
    TEST(CoreAead, NonceAutoIncrement)
    {
        ...
        aead::AeadContext ctx(aead::AeadCipher::aes_128_gcm, make_key(16));
    ...
    {
        // 密文被篡改 → 认证失败
        aead::AeadContext ctx(aead::AeadCipher::aes_256_gcm, make_key(32));
        std::array<std::uint8_t, 128> ciphertext{};
    ...
    TEST(CoreAead, SealOutputTooSmall)
    {
        ...
        aead::AeadContext ctx(aead::AeadCipher::chacha20_poly1305, make_key(32));
    ...
    TEST(CoreAead, ExplicitNonceXchacha)
    {
        ...
        aead::AeadContext ctx(aead::AeadCipher::xchacha20_poly1305, make_key(32));
    ...
    TEST(CoreAead, ExplicitNonceOpenBadCiphertext)
    {
        ...
        const auto key = make_key(32);
        aead::AeadContext enc(aead::AeadCipher::aes_256_gcm, key);
        aead::AeadContext dec(aead::AeadCipher::aes_256_gcm, key);
        const std::array<std::uint8_t, 12> Nonce{};
        std::array<std::uint8_t, 128> ciphertext{};
        aead::SealInput sin{std::span(ciphertext), g_plain, Nonce, g_ad};
    ...
    TEST(CoreAead, MoveConstructor)
    {
        ...
        aead::AeadContext src(aead::AeadCipher::aes_128_gcm, make_key(16));
        std::array<std::uint8_t, 128> ciphertext{};
    ...
    {
        // 移动赋值：旧资源释放、新状态接管
        aead::AeadContext src(aead::AeadCipher::chacha20_poly1305, make_key(32));
        aead::AeadContext dst(aead::AeadCipher::aes_128_gcm, make_key(16));
    ...
    TEST(CoreAead, IncrementNonceOverflow)
    {
        ...
        aead::AeadContext ctx2(aead::AeadCipher::aes_256_gcm, make_key(32));
        ctx2.nonce_[0] = 0x0A;
        ctx2.IncrementNonce();
