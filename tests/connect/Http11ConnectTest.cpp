...
#include <common/core/http/conn.hpp>
...
TEST(Http11Connect, ParseRequestOk)
{
    const std::string raw = "CONNECT example.com:443 HTTP/1.1\r\n"
                            ...
                            "User-Agent: test\r\n"
                            "\r\n";
    psmtest::http11::http_request req;
    const auto rc = psmtest::http11::parse_request(raw, req);
...
TEST(Http11Connect, ParseMalformed)
{
    psmtest::http11::http_request req;
    // 缺请求行
    EXPECT_EQ(psmtest::http11::parse_request("GET\r\n\r\n", req), psmtest::fault::code::parse_error);
    // 缺头结束
    EXPECT_EQ(psmtest::http11::parse_request("CONNECT h:1 HTTP/1.1\r\nHost: h:1\r\n", req),
              psmtest::fault::code::parse_error);
    // 空数据
    EXPECT_EQ(psmtest::http11::parse_request("", req), psmtest::fault::code::parse_error);
}

TEST(Http11Connect, ParseStatusCode)
{
    EXPECT_EQ(psmtest::http11::parse_status_code("HTTP/1.1 200 Connection Established\r\n\r\n"), 200);
    EXPECT_EQ(psmtest::http11::parse_status_code("HTTP/1.1 407 Proxy Authentication Required\r\n\r\n"), 407);
    EXPECT_EQ(psmtest::http11::parse_status_code("garbage"), 0);
}

TEST(Http11Connect, MakeConnectRequest)
{
    const auto req = psmtest::http11::make_connect_request("example.com", 443);
    ...
    const auto with_auth = psmtest::http11::make_connect_request("h", 80, "Basic dXNlcjpwYXNz");
...
TEST(Http11Connect, CheckBasicAuth)
{
    ...
    const psmtest::static_authenticator auth("user", "pass");

    auto ok = psmtest::http11::check_basic(auth_ok, auth);
    EXPECT_TRUE(ok.ok);
    EXPECT_EQ(ok.identity, "user");

    EXPECT_FALSE(psmtest::http11::check_basic(auth_bad, auth).ok);
    EXPECT_FALSE(psmtest::http11::check_basic("Bearer token", auth).ok);
    EXPECT_FALSE(psmtest::http11::check_basic("", auth).ok);
}
             ...
             [&]() -> net::awaitable<void>
             {
                 net::co_spawn(
                     ...
                     [&]() -> net::awaitable<void>
                     {
                         ...
                         psmtest::http11::server_conn server(transport);
                         psmtest::http11::http_request req;
                         const auto rc = co_await server.read_request(req);
                         if (rc == psmtest::fault::code::success && req.method == "CONNECT")
                         {
                             co_await server.send_response(psmtest::http11::status::ok);
                         }
                     },
                     net::detached);

                 client = co_await psmtest::net_dialer::dialer(ioc.get_executor()).connect("127.0.0.1", port, ec);
                 if (!ec)
                 {
                     co_await psmtest::http11::send_connect(client, "example.com", 443);
                     status = co_await psmtest::http11::read_response(client, ec);
                 }
             });
    EXPECT_FALSE(ec);
...
TEST(Http11Connect, AuthRequired407)
{
    ...
    run_coro(ioc,
             [&]() -> net::awaitable<void>
             {
                 ...
                 net::co_spawn(
                     ...
                     [&]() -> net::awaitable<void>
                     {
                         for (int round = 0; round < 2; ++round)
                         {
                             ...
                             psmtest::http11::http_request req;
                             const auto rc = co_await server.read_request(req);
                             if (rc == psmtest::fault::code::success)
                             {
                                 const auto ar = psmtest::http11::check_basic(req.authorization, auth);
                                 co_await server.send_response(ar.ok ? psmtest::http11::status::ok
                                                                     : psmtest::http11::status::proxy_auth_required);
                             }
                         }
                     },
                     net::detached);
                 ...
                 {
                     co_await psmtest::http11::send_connect(client, "example.com", 443);
                     status_no_auth = co_await psmtest::http11::read_response(client, ec);
                     client->close();
                 }
                 // 连接 2：正确凭据 → 200
                 client = co_await psmtest::net_dialer::dialer(ioc.get_executor()).connect("127.0.0.1", port, ec);
                 if (!ec)
                 {
                     co_await psmtest::http11::send_connect(client, "example.com", 443, "Basic dXNlcjpwYXNz");
                     status_ok = co_await psmtest::http11::read_response(client, ec);
                     client->close();
                 }
             });
             ...
             [&]() -> net::awaitable<void>
             {
                 net::co_spawn(
                     ...
                     [&]() -> net::awaitable<void>
                     {
                         ...
                         psmtest::http11::http_request req;
                         const auto rc = co_await server.read_request(req);
                         co_await server.send_response(rc == psmtest::fault::code::success
                                                           ? psmtest::http11::status::ok
                                                           : psmtest::http11::status::bad_request);
                     },
                     net::detached);
