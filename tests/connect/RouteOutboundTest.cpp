...
{
    net_route::route_table routes;
    routes.add_reverse("internal.example", net_route::endpoint{"10.0.0.1", 8080});
...
{
    net_route::route_table routes;
    routes.set_positive(net_route::endpoint{"127.0.0.1", 443});
    // 未命中反向 → 正向
...
{
    net_route::route_table routes;
    routes.add_reverse("a", net_route::endpoint{"1.1.1.1", 1});
    routes.add_reverse("b", net_route::endpoint{"2.2.2.2", 2});
...
TEST(Outbound, DialViaReverseRoute)
{
    ...
    auto routes = std::make_shared<net_route::route_table>();
    // 反向映射：internal.example → 本机 echo 端口
    routes->add_reverse("internal.example", net_route::endpoint{"127.0.0.1", real_port});
    ...
    run_coro(ioc,
             [&]() -> net::awaitable<void>
             {
                 net::co_spawn(
                     ...
                     net::detached);
                 psmtest::outbound::outbound ob(ioc.get_executor(), routes);
                 conn = co_await ob.dial(psmtest::outbound::target{"internal.example", 9999}, ec);
             });
    ...
    run_coro(ioc,
             [&]() -> net::awaitable<void>
             {
                 net::co_spawn(
                     ...
                     net::detached);
                 psmtest::outbound::outbound ob(ioc.get_executor(), routes);
                 conn = co_await ob.dial(psmtest::outbound::target{"127.0.0.1", port}, ec);
             });
    EXPECT_FALSE(ec);
    EXPECT_NE(conn, nullptr);
}
...
TEST(Outbound, InvalidPortRejected)
{
    ...
    run_coro(ioc,
             ...
             {
                 psmtest::outbound::outbound ob(ioc.get_executor(), routes);
                 conn = co_await ob.dial(psmtest::outbound::target{"127.0.0.1", 0}, ec);
             });
    EXPECT_TRUE(ec);
