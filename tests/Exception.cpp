...
{
    LogInfo("=== Testing construct with error code ===");
    ...
    catch (const psm::exception::network &ex)
    {
        ...
        {
            LogFail(std::format("error_code().value()={}, expected 3", ex.error_code().value()));
            return;
        }
        ...
        {
            LogFail(std::format("what()='{}' does not contain 'eof'", what));
            return;
        }

        LogPass("construct with error code");
        return;
    }

    LogFail("exception not caught as network");
}
...
{
    LogInfo("=== Testing construct with string ===");
    ...
    catch (const psm::exception::network &ex)
    {
        ...
        {
            LogFail(std::format("what()='{}' does not contain 'test error message'", what));
            return;
        }
        ...
        {
            LogFail(std::format("error_code().value()={}, expected {} (generic_error)",
                                 ex.error_code().value(), expected_value));
            return;
        }

        LogPass("construct with string");
        return;
    }
...
{
    LogInfo("=== Testing construct with format string ===");
    ...
    catch (const psm::exception::network &ex)
    {
        ...
        {
            LogFail(std::format("what()='{}' does not contain 'formatted msg 42'", what));
            return;
        }

        LogPass("construct with format string");
        return;
    }
...
{
    LogInfo("=== Testing source location capture ===");
    ...
    catch (const psm::exception::network &ex)
    {
        ...
        {
            LogFail("location().file_name() is empty");
            return;
        }
        ...
        {
            LogFail(std::format("filename()='{}' contains directory separators", fname));
            return;
        }
        if (fname.find("Exception") == std::string::npos)
        {
            LogFail(std::format("filename()='{}' does not contain 'Exception'", fname));
            return;
        }
        ...
        if (loc.line() <= 0)
        {
            LogFail(std::format("location().line()={}, expected > 0", loc.line()));
            return;
        }

        LogPass("source location capture");
        return;
    }
...
void TestDeviantDump()
{
    ...
    catch (const psm::exception::protocol &ex)
    {
        ...
        {
            LogFail(std::format("dump()='{}' does not contain 'PROTOCOL'", dump));
            return;
        }
        ...
        if (!contains(dump, "2"))
        {
            LogFail(std::format("dump()='{}' does not contain '2' (parse_error value)", dump));
            return;
        }
        ...
        {
            LogFail(std::format("dump()='{}' does not contain 'bad request'", dump));
            return;
        }
        ...
        {
            LogFail(std::format("dump()='{}' does not match expected format", dump));
            return;
        }
    ...
    LogFail("exception not caught as protocol");
}
...
{
    LogInfo("=== Testing derived type names ===");
    ...
    // network：继承自 deviant，代表网络 I/O 层异常
    {
        ...
        catch (const psm::exception::network &ex)
        {
            ...
            {
                LogFail(std::format("network dump()='{}' does not contain 'NETWORK'", dump));
                return;
            }
        }
    }
    ...
    // protocol：继承自 deviant，代表协议解析异常
    {
        ...
        catch (const psm::exception::protocol &ex)
        {
            ...
            if (!contains(dump, "PROTOCOL"))
            {
                LogFail(std::format("protocol dump()='{}' does not contain 'PROTOCOL'", dump));
                return;
            }
        }
    }
    ...
    // security：继承自 deviant，代表认证/安全层异常
    {
        ...
        catch (const psm::exception::security &ex)
        {
            ...
            {
                LogFail(std::format("security dump()='{}' does not contain 'SECURITY'", dump));
                return;
            }
        }
    }

    LogPass("derived type names");
}
...
int main()
{
    ...
    LogInfo("Starting exception tests...");
    ...
    LogInfo("Exception tests completed.");
