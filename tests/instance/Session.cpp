...
namespace
{
                                                ...
                                                const std::string_view tag)
    {
        ...
        EXPECT_TRUE(echo == payload) << std::format("{} echo mismatch", tag);
     ...
     * @details 依次执行 SessionEcho、SessionUpstreamClose、SessionClientClose，
    ...
    net::awaitable<void> RunAllTests(psm::context::server &server_ctx, psm::context::worker_ref &worker_ctx)
    {
        // 测试 1: CONNECT + echo 完整往返
        {
            ...
            {
                ADD_FAILURE() << "SessionEcho: " << e.what();
            }
        }
        ...
        // 测试 2: 上游关闭后客户端被关闭
        {
            ...
            {
                ADD_FAILURE() << "SessionUpstreamClose: " << e.what();
            }
        }
        ...
        // 测试 3: 客户端关闭后上游能检测到
        {
            ...
            {
                ADD_FAILURE() << "SessionClientClose: " << e.what();
            }
        }
