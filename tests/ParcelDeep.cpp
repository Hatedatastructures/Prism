...
namespace
{
    ...
    {
        ParcelFixture fx;
        runner.Check(fx.parcel_obj->stream_id() == 42, "constructor: stream_id = 42");
    }
    ...
    void TestConstructorWithMr(TestRunner &runner)
    {
        ...
        runner.Check(p->stream_id() == 1, "constructor: with mr -> stream_id = 1");
        p->close();
    }

    void TestConstructorPacketAddr(TestRunner &runner)
    {
        ParcelFixture fx{4096, multiplex::addr_mode::packet_addr};
        runner.Check(fx.parcel_obj->stream_id() == 42, "constructor: packet_addr mode -> ok");
    }

    void TestConstructorSmallMaxDgram(TestRunner &runner)
    {
        ParcelFixture fx{100};
        runner.Check(fx.parcel_obj->stream_id() == 42, "constructor: small max_dgram -> ok");
    }
    ...
    void TestSetDestination(TestRunner &runner)
    {
        ParcelFixture fx;
        fx.parcel_obj->set_destination("example.com", 443);
        runner.Check(true, "set_destination: no crash");
    }

    void TestSetDestinationAfterClose(TestRunner &runner)
    {
        ParcelFixture fx;
        fx.parcel_obj->close();
        fx.parcel_obj->set_destination("example.com", 443);
        runner.Check(true, "set_destination: after close -> no crash");
    }
    ...
    void TestOnDataAfterClose(TestRunner &runner)
    {
        ...
        ParcelFixture fx;
        fx.parcel_obj->close();
        runner.Check(true, "on_data: after close -> safe without spawn");
    }

    void TestOnDataEmpty(TestRunner &runner)
    {
        // 不 spawn 协程，仅验证 set_destination + close 路径
        ParcelFixture fx;
        fx.parcel_obj->set_destination("127.0.0.1", 80);
        fx.parcel_obj->close();
        runner.Check(true, "on_data: set dest + close -> safe");
    }

    // ─── on_uplink_done ─────────────────────

    void TestOnUplinkDoneNullptr(TestRunner &runner)
    {
        ParcelFixture fx;
        fx.parcel_obj->close();
        runner.Check(true, "on_uplink_done: via close -> no crash");
    }
...
int main()
{
    ...
    TestRunner runner("ParcelDeep");

    TestConstructorDefault(runner);
    TestConstructorWithMr(runner);
    TestConstructorPacketAddr(runner);
    TestConstructorSmallMaxDgram(runner);
    ...
    TestSetDestination(runner);
    TestSetDestinationAfterClose(runner);

    TestOnDataAfterClose(runner);
    TestOnDataEmpty(runner);

    TestOnUplinkDoneNullptr(runner);
