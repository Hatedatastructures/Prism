...
namespace
{
    ...
    void TestParseValidClientHello(TestRunner &runner)
    {
        ...
        runner.Check(psm::fault::succeeded(ec), "valid ClientHello parses successfully");
        runner.Check(ch.sni() == "example.com", "SNI extracted correctly");
        runner.Check(ch.has_x25519(), "X25519 key_share detected");
        runner.Check(!ch.versions().empty(), "versions list not empty");
        runner.Check(ch.session_id().empty(), "empty session_id");
        runner.Check(ch.random()[0] == 0 && ch.random()[31] == 31, "random bytes preserved");
        runner.Check(!ch.raw_record().empty(), "raw_record preserved");
        runner.Check(!ch.raw_msg().empty(), "raw_msg preserved");
    }
    ...
    void TestParseWithSessionId(TestRunner &runner)
    {
        ...
        auto [ec, ch] = psm::tls::client_hello::from_bytes(raw);
        runner.Check(psm::fault::succeeded(ec), "ClientHello with session_id parses");
        runner.Check(ch.session_id().size() == 16, "session_id length is 16");
        runner.Check(ch.session_id()[0] == 0xAB, "session_id content preserved");
    }
    ...
    void TestParseNoExtensions(TestRunner &runner)
    {
        ...
        auto [ec, ch] = psm::tls::client_hello::from_bytes(raw);
        runner.Check(psm::fault::succeeded(ec), "ClientHello without extensions parses");
        runner.Check(ch.sni().empty(), "no SNI when no extensions");
        runner.Check(!ch.has_x25519(), "no X25519 when no extensions");
    }

    void TestParseNoKeyShare(TestRunner &runner)
    {
        client_hello_builder builder;
        ...
        auto [ec, ch] = psm::tls::client_hello::from_bytes(raw);
        runner.Check(psm::fault::succeeded(ec), "ClientHello without key_share parses");
        runner.Check(!ch.has_x25519(), "has_x25519 is false");
        runner.Check(ch.sni() == "nokey.example.com", "SNI still extracted");
    }

    void TestParseNoVersions(TestRunner &runner)
    {
        client_hello_builder builder;
        ...
        auto [ec, ch] = psm::tls::client_hello::from_bytes(raw);
        runner.Check(psm::fault::succeeded(ec), "ClientHello without versions parses");
        runner.Check(ch.versions().empty(), "versions list is empty");
    }
    ...
    void TestParseEmptySni(TestRunner &runner)
    {
        ...
        auto [ec, ch] = psm::tls::client_hello::from_bytes(raw);
        runner.Check(psm::fault::succeeded(ec), "ClientHello with empty SNI parses");
        runner.Check(ch.sni().empty(), "SNI is empty");
    }

    void TestParseBufferTooShort(TestRunner &runner)
    {
        std::vector<std::uint8_t> short_buf(10, 0x16);
        auto [ec, ch] = psm::tls::client_hello::from_bytes(short_buf);
        runner.Check(psm::fault::failed(ec), "buffer < 44 bytes rejected");
        runner.Check(ec == psm::fault::code::recorderr, "error code is recorderr");
    }
    ...
    void TestParseSessionIdTooLong(TestRunner &runner)
    {
        ...
        auto [ec, ch] = psm::tls::client_hello::from_bytes(raw);
        runner.Check(psm::fault::failed(ec), "session_id > 32 bytes rejected");
    }
    ...
    void TestToFeatures(TestRunner &runner)
    {
        client_hello_builder builder;
        ...
        auto [ec, ch] = psm::tls::client_hello::from_bytes(raw);
        runner.Check(psm::fault::succeeded(ec), "parse succeeds for to_features test");

        auto feat = ch.to_features();
        runner.Check(feat.server_name == "features.test", "to_features: server_name");
        runner.Check(feat.has_x25519 == true, "to_features: has_x25519");
        runner.Check(feat.x25519_key == expected_key, "to_features: x25519_key");
        runner.Check(!feat.versions.empty(), "to_features: versions not empty");
        runner.Check(feat.session_id.empty(), "to_features: session_id empty");
        runner.Check(!feat.raw_msg.empty(), "to_features: raw_msg not empty");
        runner.Check(!feat.raw_record.empty(), "to_features: raw_record not empty");
    }
    ...
    void TestParseMultipleVersions(TestRunner &runner)
    {
        ...
        auto [ec, ch] = psm::tls::client_hello::from_bytes(raw);
        runner.Check(psm::fault::succeeded(ec), "multiple versions parses");
        runner.Check(ch.versions().size() == 2, "two versions extracted");
    }
    ...
    void TestParseX25519Mlkem768(TestRunner &runner)
    {
        ...
        runner.Check(psm::fault::succeeded(ec), "X25519MLKEM768 hybrid key_share parses");
        runner.Check(ch.has_x25519() == true, "hybrid key_share sets has_x25519");
        runner.Check(ch.sni() == "hybrid.test", "SNI still extracted with hybrid");
    }
    ...
    void TestParseMaxSessionId(TestRunner &runner)
    {
        ...
        auto [ec, ch] = psm::tls::client_hello::from_bytes(raw);
        runner.Check(psm::fault::succeeded(ec), "session_id = 32 bytes accepted");
        runner.Check(ch.session_id().size() == 32, "session_id length is 32");
    }

    // === 新增：解析边界条件测试 ===

    void TestParseHandshakeLenOverflow(TestRunner &runner)
    {
        ...
        auto [ec, ch] = psm::tls::client_hello::from_bytes(raw);
        runner.Check(psm::fault::failed(ec), "handshake_len overflow → recorderr");
    }

    void TestParseCompLenOverflow(TestRunner &runner)
    {
        ...
        auto [ec, ch] = psm::tls::client_hello::from_bytes(raw);
        runner.Check(psm::fault::failed(ec), "comp_len overflow → recorderr");
    }
    ...
    void TestParseSniNonHostnameType(TestRunner &runner)
    {
        ...
        auto [ec, ch] = psm::tls::client_hello::from_bytes(record);
        runner.Check(psm::fault::succeeded(ec), "SNI non-hostname type: parses");
        runner.Check(ch.sni() == "host.test", "SNI non-hostname type: skips non-hostname, finds hostname");
    }

    void TestParseKeyshareNonX25519Group(TestRunner &runner)
    {
        ...
        auto [ec, ch] = psm::tls::client_hello::from_bytes(record);
        runner.Check(psm::fault::succeeded(ec), "P-256 key_share: parses");
        runner.Check(ch.has_x25519() == false, "P-256 key_share: has_x25519=false");
        runner.Check(ch.sni() == "p256.test", "P-256 key_share: SNI still extracted");
    }

    void TestParseKeyshareTruncated(TestRunner &runner)
    {
        ...
        auto [ec, ch] = psm::tls::client_hello::from_bytes(record);
        runner.Check(psm::fault::succeeded(ec), "truncated key_share: parses without crash");
        runner.Check(ch.has_x25519() == false, "truncated key_share: has_x25519=false");
    }
    ...
    void TestParseVersionsEmpty(TestRunner &runner)
    {
        ...
        auto [ec, ch] = psm::tls::client_hello::from_bytes(record);
        runner.Check(psm::fault::succeeded(ec), "empty supported_versions: parses");
        runner.Check(ch.versions().empty(), "empty supported_versions: versions is empty");
    }

    void TestParseExtensionsTruncated(TestRunner &runner)
    {
        ...
        auto [ec, ch] = psm::tls::client_hello::from_bytes(raw);
        runner.Check(psm::fault::succeeded(ec), "truncated extensions: parses without crash");
...
int main()
{
    ...
    TestRunner runner("TlsHello");

    TestParseValidClientHello(runner);
    TestParseWithSessionId(runner);
    TestParseNoExtensions(runner);
    TestParseNoKeyShare(runner);
    TestParseNoVersions(runner);
    TestParseEmptySni(runner);
    TestParseBufferTooShort(runner);
    ...
    TestToFeatures(runner);
    TestParseMultipleVersions(runner);
    TestParseX25519Mlkem768(runner);
    TestParseMaxSessionId(runner);

    // === 解析边界条件测试 ===
    TestParseHandshakeLenOverflow(runner);
    TestParseCompLenOverflow(runner);
    TestParseSniNonHostnameType(runner);
    TestParseKeyshareNonX25519Group(runner);
    TestParseKeyshareTruncated(runner);
    TestParseVersionsEmpty(runner);
    TestParseExtensionsTruncated(runner);
