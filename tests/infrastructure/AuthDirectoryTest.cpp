...
namespace
{
    ...
    TEST(DirectoryAuthenticator, HitPassesWithLease)
    {
        ...
        const psmtest::account::directory_authenticator auth(&dir);
    ...
    TEST(DirectoryAuthenticator, NotFoundRejected)
    {
        ...
        EXPECT_FALSE(r.ok);
        EXPECT_EQ(r.reason, psmtest::account::auth_reason::not_found);
    }
    ...
    TEST(DirectoryAuthenticator, DisabledRejected)
    {
        ...
        EXPECT_FALSE(r.ok);
        EXPECT_EQ(r.reason, psmtest::account::auth_reason::disabled);
    }
    ...
    TEST(DirectoryAuthenticator, ExpiredRejected)
    {
        ...
        fake_now = 500;
        const psmtest::account::directory_authenticator auth(&dir, fake_clock);
        EXPECT_TRUE(auth.check_directory("user", "old").ok);

        fake_now = 1000; // 过期
        auto r = auth.check_directory("user", "old");
        EXPECT_FALSE(r.ok);
        EXPECT_EQ(r.reason, psmtest::account::auth_reason::expired);
    }
    ...
    TEST(DirectoryAuthenticator, MiddlewareIntegration)
    {
        ...
        auto auth = std::make_shared<psmtest::account::directory_authenticator>(&dir);
        ...
        psmtest::fault::code rc_ok = psmtest::fault::code::success;
        psmtest::fault::code rc_bad = psmtest::fault::code::success;
        ...
        ioc.run();
        ASSERT_FALSE(ep);
        EXPECT_EQ(rc_ok, psmtest::fault::code::success);
        EXPECT_EQ(ctx.identity, "cred-a");
        EXPECT_EQ(rc_bad, psmtest::fault::code::auth_failed);
    }
