...
namespace
{
    ...
    /// 内存传输（极简，供 relay 测试）
    class mem_tx final : public psmtest::transmission
    {
    public:
    ...
    /// 按身份聚合的 fake sink
    class aggregating_sink final : public psmtest::middleware::context::traffic_sink
    {
    public:
    ...
    TEST(TrafficIdentity, RelayReportsWithIdentity)
    {
        ...
        net::co_spawn(ioc, [&]() -> net::awaitable<void>
        {
            ...
            ctx.traffic = &sink;
            auto shared_inbound = std::shared_ptr<psmtest::transmission>(inbound);

            psmtest::middleware::builtin::relay_middleware relay(outbound);
