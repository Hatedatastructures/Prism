...
namespace
{
        ...
        -> net::awaitable<void>
    {
        ...
        {
            net::co_spawn(ex,
                dispatch_coro(stream_pfxs[i], stream_pfxs[i].conn, done),
                net::detached);
...
TEST(ScopedPrefix, SessionReturn_MuxStillRunning)
{
    ...
    {
        session_pfx[i].conn = 400 + i;
        mux_pfx[i].conn = 500 + i;
...
TEST(ScopedPrefix, NestedCoSpawn_AnyTlsMode)
{
    ...
    {
        outer_pfx[i].conn = 600 + i;
        middle_pfx[i].conn = 700 + i;
    }
