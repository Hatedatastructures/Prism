...
TEST(FeatureBitmap, BuildBitmap)
{
    // 测试空特征
    ...
    EXPECT_TRUE(psm::recognition::tls::has_feature(bitmap, fb::has_sni)) << "Should have has_sni bit";
    ...
    EXPECT_TRUE(psm::recognition::tls::has_feature(bitmap, fb::has_x25519)) << "Should have has_x25519 bit";
    ...
    EXPECT_TRUE(psm::recognition::tls::has_feature(bitmap, fb::full_session)) << "Should have has_full_session_id bit";
    ...
    EXPECT_TRUE(psm::recognition::tls::has_feature(bitmap, fb::nonstd_session)) << "Should have session_id_non_standard bit";
}

TEST(FeatureBitmap, RealityMarker)
{
...
TEST(FeatureBitmap, HasAllFeatures)
{
    // 构建包含多个特征的位图
