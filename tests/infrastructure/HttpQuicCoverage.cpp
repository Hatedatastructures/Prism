...
#include <common/core/http2/stream.hpp>
#include <common/core/http3/auth.hpp>
#include <common/core/http3/qpack.hpp>
#include <common/core/http3/server.hpp>
#include <common/core/quic/gateway_common.hpp>
#include <common/core/quic/stream_adapter.hpp>

namespace
{

    namespace h2 = psmtest::http2;
    namespace h3 = psmtest::http3;
    namespace h3a = psmtest::protocol::hysteria2::h3;
    namespace memory = psmtest::memory;
    namespace qp = psmtest::protocol::hysteria2::qpack;
    namespace quic = psmtest::quic;
    ...
    {
        const auto mr = psmtest::memory::current_resource();
        // 空输入
    ...
    TEST(Http3Server, InitFailureAndGuards)
    {
        ...
        EXPECT_EQ(srv->feed(0, {}, false), psmtest::fault::code::protocol_error);
    ...
    TEST(Http3Server, AuthFlowEndToEnd)
    {
        ...
        EXPECT_EQ(srv->feed(2, settings, false), psmtest::fault::code::success);

        // 客户端 QPACK encoder 流（stream 6）：Set Dynamic Table Capacity=0
        const std::array<std::byte, 1> qenc{std::byte{0x20}};
        EXPECT_EQ(srv->feed(6, qenc, false), psmtest::fault::code::success);
        ...
        EXPECT_EQ(srv->submit_auth_response(), psmtest::fault::code::success);
    ...
    TEST(QuicStreamAdapter, CloseAndTypePropagation)
    {
        ...
        // 传输类型与执行器传播
        EXPECT_EQ(adapter->transport_type(), psmtest::transmission::type::tcp);
