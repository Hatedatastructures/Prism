...
namespace
{
    ...
    TEST(SchemeRouteTable, MatchesAny)
    {
        auto cfg = BuildTestConfig();
        auto table = psm::recognition::route_table::build(cfg);

        EXPECT_TRUE(table.matches_any("reality.example.com")) << "Reality SNI should match";
        EXPECT_TRUE(table.matches_any("shadowtls.example.com")) << "ShadowTLS SNI should match";
        EXPECT_TRUE(!table.matches_any("unknown.example.com")) << "Unknown SNI should not match";
        EXPECT_TRUE(!table.matches_any("")) << "Empty SNI should not match";
    }
} // namespace
