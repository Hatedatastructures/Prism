 ...
 * - udp_transmission：往返、未打开写/读错误、close/cancel
...
namespace
{
    ...
    using namespace psmtest::transport;
     ...
     */
    class programmable_mock final : public psmtest::transmission
    {
    public:
    ...
    {
        return psmtest::as_bytes_span(s);
    }
    ...
    {
        psmtest::transport::pad_config def;
        EXPECT_TRUE(def.enabled());

        psmtest::transport::pad_config off;
        off.pad_targets = "";
        ...
        psmtest::transport::pad_transport pad(sink, off);
        ...
        EXPECT_EQ(psmtest::as_str_view(psmtest::as_u8(std::span<const std::byte>(sink->written_))), msg);
    }
    ...
    TEST(PadTransport, MaxBoundaryRange)
    {
        ...
        // 数据前缀完好
        const auto wire = psmtest::as_str_view(psmtest::as_u8(std::span<const std::byte>(sink->written_)));
    ...
    TEST(PadTransport, DataPassthroughOverPipe)
    {
        ...
        run_coro(ioc,
                 [&]() -> net::awaitable<void>
                 {
                     ...
                     psmtest::transport::pad_config cfg;
                     psmtest::transport::pad_transport pad(sa, cfg);
                     ...
                     EXPECT_EQ(psmtest::as_str_view(psmtest::as_u8(std::span<const std::byte>(rbuf))), msg);
                 });
    }
    ...
    TEST(PadTransport, RandomPaddingDeterminism)
    {
        ...
        psmtest::transport::pad_transport pad1(sink1, cfg);
        psmtest::transport::pad_transport pad2(sink2, cfg);
        ...
        const auto w1 = psmtest::as_str_view(psmtest::as_u8(std::span<const std::byte>(sink1->written_)));
        const auto w2 = psmtest::as_str_view(psmtest::as_u8(std::span<const std::byte>(sink2->written_)));
    ...
    TEST(Preview, PrereadReturnedFirst)
    {
        ...
        run_coro(ioc,
                 [&]() -> net::awaitable<void>
                 {
                     ...
                     psmtest::transport::preview pv(sb, sv_bytes(preread));

                     std::array<std::byte, 3> buf{};
                     std::error_code ec;
                     auto n = co_await pv.async_read_some(std::span<std::byte>(buf), ec);
                     EXPECT_EQ(n, 3u);
                     EXPECT_EQ(psmtest::as_str_view(psmtest::as_u8(std::span<std::byte>(buf))), "ABC");
                     n = co_await pv.async_read_some(std::span<std::byte>(buf), ec);
                     EXPECT_EQ(n, 3u);
                     EXPECT_EQ(psmtest::as_str_view(psmtest::as_u8(std::span<std::byte>(buf))), "DEF");
                 });
    }
    ...
    TEST(Preview, ExhaustedDelegatesToInner)
    {
        ...
        run_coro(ioc,
                 [&]() -> net::awaitable<void>
                 {
                     ...
                     auto n = co_await pv.async_read_some(std::span<std::byte>(buf), ec);
                     EXPECT_EQ(n, 4u);
                     EXPECT_EQ(psmtest::as_str_view(psmtest::as_u8(std::span<std::byte>(buf))), "PRE-");

                     // 预读耗尽 → 委托底层
                     n = co_await pv.async_read_some(std::span<std::byte>(buf), ec);
                     EXPECT_EQ(n, 3u);
                     EXPECT_EQ(psmtest::as_str_view(psmtest::as_u8(std::span<std::byte>(buf).first(3))), "XYZ");
                 });
    }
    ...
    TEST(Preview, WrapWithPreviewBehavior)
    {
        ...
        // 空预读 → 不包装
        auto not_wrapped = psmtest::transport::wrap_with_preview(sa, {});
        EXPECT_EQ(not_wrapped.get(), sa.get());

        // 非空预读 → 包装为 preview
        auto wrapped = psmtest::transport::wrap_with_preview(sb, sv_bytes("hello"));
        auto *pv = dynamic_cast<psmtest::transport::preview *>(wrapped.get());
        ASSERT_NE(pv, nullptr);
        EXPECT_EQ(pv->next_layer(), sb.get());
        EXPECT_EQ(wrapped->transport_type(), psmtest::transmission::type::tcp);

        // completion-handler 风格预读路径
        net::io_context ioc2;
        std::promise<std::pair<boost::system::error_code, std::size_t>> done;
        auto fut = done.get_future();
        psmtest::transport::preview pv2(sb, sv_bytes("hi"));
        ...
        EXPECT_FALSE(ec);
        EXPECT_EQ(n, 2u);
        EXPECT_EQ(psmtest::as_str_view(psmtest::as_u8(std::span<std::byte>(buf).first(2))), "hi");
    }
    ...
    TEST(Snapshot, CaptureRewindReplay)
    {
        ...
        run_coro(ioc,
                 [&]() -> net::awaitable<void>
                 {
                     ...
                     auto snap = std::make_shared<psmtest::transport::snapshot>(sb);
                     ...
                     EXPECT_EQ(n, 4u);
                     EXPECT_EQ(psmtest::as_str_view(psmtest::as_u8(std::span<std::byte>(buf))), "0123");
    ...
    TEST(Snapshot, ContinueAfterRewind)
    {
        ...
        run_coro(ioc,
                 [&]() -> net::awaitable<void>
                 {
                     ...
                     EXPECT_EQ(n, 6u);
                     EXPECT_EQ(psmtest::as_str_view(psmtest::as_u8(std::span<std::byte>(buf))), "012345");

                     // 回放完剩余捕获，再读新数据
                     std::array<std::byte, 8> buf2{};
                     n = co_await snap->async_read_some(std::span<std::byte>(buf2), ec);
                     EXPECT_EQ(n, 4u);
                     EXPECT_EQ(psmtest::as_str_view(psmtest::as_u8(std::span<std::byte>(buf2).first(4))),
                               "6789");
    ...
    TEST(Snapshot, WriteDisablesRewind)
    {
        ...
        run_coro(ioc,
                 [&]() -> net::awaitable<void>
                 {
                     ...
                     EXPECT_EQ(r, 3u);
                     EXPECT_EQ(psmtest::as_str_view(psmtest::as_u8(std::span<std::byte>(buf).first(3))),
                               "out");
    ...
    TEST(Encrypted, SslHandshakeNullInbound)
    {
        ...
        std::tuple<psmtest::fault::code, encrypted::shared_stream, psmtest::shared_transmission> result;
        run_coro(ioc,
                 [&]() -> net::awaitable<void>
                 {
        ...
        EXPECT_EQ(code, psmtest::fault::code::io_error);
    ...
    TEST(Encrypted, HandshakeFailureRecoversTransport)
    {
        ...
        auto &[code, stream, recovered] = result;
        EXPECT_NE(code, psmtest::fault::code::success);
        EXPECT_EQ(stream, nullptr);
        ASSERT_NE(recovered, nullptr); // 失败后从 connector 恢复底层传输
        EXPECT_EQ(recovered->transport_type(), psmtest::transmission::type::tcp);
    }
    ...
    TEST(Encrypted, TlsRoundTripOverMemoryPipe)
    {
        ...
        run_coro(ioc,
                 [&]() -> net::awaitable<void>
                 {
                     ...
                     {
                         psmtest::transport::connector c(sa);
                     ...
                     // 装饰器访问器
                     EXPECT_EQ(client_t->transport_type(), psmtest::transmission::type::tcp);
                     EXPECT_EQ(client_t->next_layer(), nullptr);
                     ...
                     EXPECT_EQ(r, 5u);
                     EXPECT_EQ(psmtest::as_str_view(psmtest::as_u8(std::span<std::byte>(buf).first(5))),
                               "hello");
    ...
    TEST(Connector, PrereadInjectionThenDelegate)
    {
        ...
        run_coro(ioc,
                 [&]() -> net::awaitable<void>
                 {
                     ...
                     psmtest::transport::connector conn(sb, sv_bytes("PRE"));
                     std::array<std::byte, 8> buf{};
                     ...
                     EXPECT_FALSE(c_ec);
                     EXPECT_EQ(n, 8u);
                     EXPECT_EQ(psmtest::as_str_view(psmtest::as_u8(std::span<std::byte>(buf))), "inner-da");
                 });
    }
    ...
    TEST(Connector, WriteDelegation)
    {
        ...
        run_coro(ioc,
                 [&]() -> net::awaitable<void>
                 {
                     ...
                     psmtest::transport::connector conn(sb);
                     ...
                     const auto r = co_await sa->async_read_some(std::span<std::byte>(buf), ec);
                     EXPECT_EQ(r, msg.size());
                     EXPECT_EQ(psmtest::as_str_view(psmtest::as_u8(std::span<std::byte>(buf).first(r))), msg);
                 });
    }
    ...
    TEST(Connector, MemberAsyncReadWrite)
    {
        ...
        run_coro(ioc,
                 [&]() -> net::awaitable<void>
                 {
                     ...
                     psmtest::transport::connector conn_reader(sa);
                     std::vector<std::byte> buf(msg.size());
                     const auto r = co_await conn_reader.async_read(std::span<std::byte>(buf), ec);
                     EXPECT_EQ(r, msg.size());
                     EXPECT_FALSE(ec);
                     EXPECT_EQ(psmtest::as_str_view(psmtest::as_u8(std::span<std::byte>(buf))), msg);
                 });
    }
    ...
    TEST(Connector, MoveReleaseAccessors)
    {
        ...
        // 移动构造
        psmtest::transport::connector moved(std::move(conn));
        // 移动赋值
        psmtest::transport::connector assigned(
            std::make_shared<psmtest::memory_stream>(ioc.get_executor()));
    ...
    TEST(Reliable, LoopbackReadWriteFull)
    {
        ...
        run_coro(ioc,
                 [&]() -> net::awaitable<void>
                 {
                     ...
                     std::shared_ptr<psmtest::transport::reliable> server;
                     auto accept_coro = [&]() -> net::awaitable<void>
                     {
                         auto sock = co_await acceptor.async_accept(net::use_awaitable);
                         server = std::make_shared<psmtest::transport::reliable>(std::move(sock));
                     };
                     net::co_spawn(ioc.get_executor(), accept_coro(), net::detached);

                     psmtest::transport::reliable client(ioc.get_executor());
    ...
    {
        net::io_context ioc;
        psmtest::transport::reliable t(ioc.get_executor());

        EXPECT_EQ(t.transport_type(), psmtest::transmission::type::tcp);
        EXPECT_EQ(t.next_layer(), nullptr);
        EXPECT_EQ(t.lowest_layer<psmtest::transport::reliable>(), &t);
        EXPECT_NO_THROW((void)t.executor());
        ...
        psmtest::transport::reliable t2(ioc.get_executor());
    ...
    TEST(Unreliable, DatagramRoundTrip)
    {
        ...
        run_coro(ioc,
                 [&]() -> net::awaitable<void>
                 {
                     psmtest::transport::unreliable server(ioc.get_executor());
                     psmtest::transport::unreliable client(ioc.get_executor());
                     ...
                     EXPECT_EQ(r, 4u);
                     EXPECT_FALSE(ec);
                     EXPECT_EQ(psmtest::as_str_view(psmtest::as_u8(std::span<std::byte>(buf).first(4))),
                               "ping");
    ...
    {
        net::io_context ioc;
        psmtest::transport::unreliable u(ioc.get_executor());
        boost::system::error_code oec;
        ...
        ioc.run();
        ASSERT_TRUE(ec);
        EXPECT_EQ(ec, psmtest::fault::make_error_code(psmtest::fault::code::io_error));
    }
    ...
    TEST(Unreliable, SourceFiltering)
    {
        ...
        run_coro(ioc,
                 ...
                 {
                     psmtest::transport::unreliable server(ioc.get_executor());
                     psmtest::transport::unreliable client_a(ioc.get_executor());
                     psmtest::transport::unreliable client_b(ioc.get_executor());
                     ...
                     const auto r = co_await server.async_read_some(std::span<std::byte>(buf), ec);
                     EXPECT_EQ(r, 7u);
                     EXPECT_EQ(psmtest::as_str_view(psmtest::as_u8(std::span<std::byte>(buf).first(7))),
                               "trusted");
    ...
    TEST(Unreliable, AccessorsCloseCancel)
    {
        net::io_context ioc;
        psmtest::transport::unreliable u(ioc.get_executor());
        EXPECT_EQ(u.transport_type(), psmtest::transmission::type::udp);
    ...
    {
        net::io_context ioc;
        psmtest::udp_transmission u(ioc.get_executor()); // socket 未打开
    ...
    TEST(UdpTransmission, ReadAfterCloseFails)
    {
        ...
        run_coro(ioc,
                 [&]() -> net::awaitable<void>
                 {
                     psmtest::udp_transmission u(ioc.get_executor());
                     boost::system::error_code oec;
                     u.socket().open(net::ip::udp::v4(), oec);
                     if (oec || !u.bind(0))
                     {
