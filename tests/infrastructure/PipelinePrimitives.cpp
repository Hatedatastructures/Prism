...
namespace
{
    ...
    TEST(PipelinePrimitives, IsMuxTarget)
    {
    ...
    TEST(PipelinePrimitives, PreviewConstruction)
    {
        ...
        EXPECT_TRUE(preview->transport_type() == psm::transport::transmission::type::tcp)
    ...
    TEST(PipelinePrimitives, ProbeResultHelpers)
    {
        ...
        // 测试 preload_bytes() 返回正确的字节跨度
        {
            ...
            EXPECT_TRUE(match) << "preload_bytes() returns correct byte content";
        }
    }
    ...
    TEST(PipelinePrimitives, ShutCloseNullptr)
    {
        // 对空指针调用 shut_close 不应崩溃
        psm::connect::shut_close(nullptr);
        // nullptr 安全是函数契约，验证调用后程序状态未受影响
        EXPECT_TRUE(true) << "shut_close(nullptr) is safe by contract";
    }
} // namespace
