...
namespace
{
    ...
    void TestParseTooShort(TestRunner &runner)
    {
        ...
        runner.Check(psm::fault::failed(ec), "parse: too short -> error");
    }

    void TestParseNotHandshake(TestRunner &runner)
    {
        ...
        runner.Check(psm::fault::failed(ec), "parse: not handshake -> error");
    }

    void TestParseNotClientHello(TestRunner &runner)
    {
        auto buf = build_minimal_client_hello();
        buf[5] = 0x02; // ServerHello, not ClientHello
        auto [ec, features] = parse_client_hello({buf.data(), buf.size()});
        runner.Check(psm::fault::failed(ec), "parse: not ClientHello -> error");
    }

    void TestParseTruncatedBody(TestRunner &runner)
    {
        auto buf = build_minimal_client_hello();
        // Set record body length larger than actual data
        buf[3] = 0xFF; buf[4] = 0xFF;
        auto [ec, features] = parse_client_hello({buf.data(), buf.size()});
        runner.Check(psm::fault::failed(ec), "parse: truncated body -> error");
    }

    void TestParseMinimalValid(TestRunner &runner)
    {
        auto buf = build_minimal_client_hello();
        auto [ec, features] = parse_client_hello({buf.data(), buf.size()});
        runner.Check(!psm::fault::failed(ec), "parse: minimal valid -> success");
        runner.Check(features.server_name.empty(), "parse: minimal SNI empty");
        runner.Check(!features.has_x25519, "parse: minimal no x25519");
        runner.Check(features.versions.empty(), "parse: minimal no versions");
        runner.Check(features.random[0] == 0x42, "parse: minimal random[0]=0x42");
        runner.Check(features.session_id_len == 0, "parse: minimal session_id_len=0");
    }

    void TestParseWithSNI(TestRunner &runner)
    {
        auto buf = build_minimal_client_hello();
        ...
        auto [ec, features] = parse_client_hello({buf.data(), buf.size()});
        runner.Check(!psm::fault::failed(ec), "parse: SNI -> success");
        runner.Check(features.server_name == "example.com", "parse: SNI = example.com");
    }

    void TestParseWithX25519(TestRunner &runner)
    {
        ...
        auto [ec, features] = parse_client_hello({buf.data(), buf.size()});
        runner.Check(!psm::fault::failed(ec), "parse: x25519 -> success");
        runner.Check(features.has_x25519, "parse: has_x25519=true");
        runner.Check(features.x25519_key[0] == 0xDE, "parse: x25519_key[0]=0xDE");
        runner.Check(features.x25519_key[31] == 0xAD, "parse: x25519_key[31]=0xAD");
    }

    void TestParseWithVersions(TestRunner &runner)
    {
        auto buf = build_minimal_client_hello();
        auto ext_start = buf.size();
        ...
        auto [ec, features] = parse_client_hello({buf.data(), buf.size()});
        runner.Check(!psm::fault::failed(ec), "parse: versions -> success");
        runner.Check(features.versions.size() == 2, "parse: 2 versions");
        runner.Check(features.versions[0] == tls_proto::VERSION_TLS12, "parse: versions[0]=TLS1.2");
        runner.Check(features.versions[1] == tls_proto::VERSION_TLS13, "parse: versions[1]=TLS1.3");
    }

    void TestParseWithECH(TestRunner &runner)
    {
        auto buf = build_minimal_client_hello();
        auto ext_start = buf.size();
        ...
        auto [ec, features] = parse_client_hello({buf.data(), buf.size()});
        runner.Check(!psm::fault::failed(ec), "parse: ECH -> success");
        runner.Check(features.has_ech, "parse: has_ech=true");
    }
    ...
    void TestParseWithSessionID(TestRunner &runner)
    {
        ...
        auto [ec, features] = parse_client_hello({buf.data(), buf.size()});
        runner.Check(!psm::fault::failed(ec), "parse: session_id -> success");
        runner.Check(features.session_id_len == 32, "parse: session_id_len=32");
        runner.Check(features.session_id.size() == 32, "parse: session_id size=32");
        runner.Check(features.session_id[0] == 0x01, "parse: session_id[0]=0x01 (reality marker)");
        runner.Check(features.session_id[2] == 0x02, "parse: session_id[2]=0x02 (reality marker)");
    }

    void TestParseInvalidSessionIDTooLong(TestRunner &runner)
    {
        auto buf = build_minimal_client_hello();
        std::size_t sid_len_pos = 43;
        ...
        auto [ec, features] = parse_client_hello({buf.data(), buf.size()});
        runner.Check(psm::fault::failed(ec), "parse: session_id too long -> error");
    }

    void TestParseFullClientHello(TestRunner &runner)
    {
        ...
        auto [ec, features] = parse_client_hello({buf.data(), buf.size()});
        runner.Check(!psm::fault::failed(ec), "parse: full CH -> success");
        runner.Check(features.server_name == "test.example.org", "parse: full SNI");
        runner.Check(features.has_x25519, "parse: full has_x25519");
        runner.Check(features.x25519_key[0] == 0xCA, "parse: full key[0]");
        runner.Check(features.versions.size() == 1, "parse: full 1 version");
        runner.Check(features.has_ech, "parse: full has_ech");
        runner.Check(features.random[0] == 0x42, "parse: full random");
        runner.Check(!features.raw_msg.empty(), "parse: full raw_msg not empty");
        runner.Check(!features.raw_record.empty(), "parse: full raw_record not empty");
    }
} // namespace
...
int main()
{
    ...
    TestRunner runner("TlsSignalParse");

    TestParseTooShort(runner);
    TestParseNotHandshake(runner);
    TestParseNotClientHello(runner);
    TestParseTruncatedBody(runner);
    TestParseMinimalValid(runner);
    TestParseWithSNI(runner);
    TestParseWithX25519(runner);
    TestParseWithVersions(runner);
    TestParseWithECH(runner);
    TestParseWithSessionID(runner);
    TestParseInvalidSessionIDTooLong(runner);
    TestParseFullClientHello(runner);
