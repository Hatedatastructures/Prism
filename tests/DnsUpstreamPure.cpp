...
namespace
{
    ...
    void TestSelectBestFastestMode(TestRunner &runner)
    {
        ...
        runner.Check(psm::fault::succeeded(best.error),
                     "select_best fastest: 成功返回");
        runner.Check(best.rtt_ms == 50,
                     "select_best fastest: RTT=50（最低）");
        runner.Check(best.server_addr == "server-b",
    ...
    void TestSelectBestAllFailed(TestRunner &runner)
    {
        ...
        runner.Check(best.server_addr == "server-a",
                     "select_best all failed: 返回第一个");
        runner.Check(best.error == psm::fault::code::timeout,
    ...
    void TestSelectBestEmpty(TestRunner &runner)
    {
        ...
        runner.Check(best.error == psm::fault::code::dns_failed,
                     "select_best empty: 返回 dns_failed");
        runner.Check(best.ips.empty(),
                     "select_best empty: 无 IP 结果");
    }

    // ─── 混合场景：部分成功部分失败 ───────────────────

    void TestSelectBestMixed(TestRunner &runner)
    {
        psm::memory::vector<query_result> results;
        results.push_back(make_result(psm::fault::code::timeout, 300, "server-a"));
        ...
        runner.Check(best.rtt_ms == 80,
                     "select_best mixed: RTT=80（最快成功）");
        runner.Check(best.server_addr == "server-d",
    ...
    void TestSelectBestSingleSuccess(TestRunner &runner)
    {
        ...
        runner.Check(best.rtt_ms == 999,
                     "select_best single: RTT=999");
        runner.Check(best.server_addr == "only-one",
                     "select_best single: only-one");
    }

    // ─── 成功但 ips 为空：视为失败 ────────────────────

    void TestSelectBestSuccessEmptyIps(TestRunner &runner)
    {
        psm::memory::vector<query_result> results;
        ...
        runner.Check(best.server_addr == "empty-ips",
                     "select_best empty ips: 回退到第一个");
    }

    // ─── 同 RTT 多成功：取第一个 ──────────────────────

    void TestSelectBestSameRtt(TestRunner &runner)
    {
        psm::memory::vector<query_result> results;
        ...
        runner.Check(best.server_addr == "first",
...
auto main() -> int
{
    ...
    TestRunner runner("DnsUpstreamPure");

    TestSelectBestFastestMode(runner);
    TestSelectBestAllFailed(runner);
    TestSelectBestEmpty(runner);
    TestSelectBestMixed(runner);
    TestSelectBestSingleSuccess(runner);
    TestSelectBestSuccessEmptyIps(runner);
    TestSelectBestSameRtt(runner);
