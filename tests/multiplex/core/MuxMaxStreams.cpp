 ...
 */
    struct mux_test_context
    {
        ...
        explicit mux_test_context(const std::uint32_t max_streams = 32)
            : router({ioc, psm::dns::config{}}), outbound(router)
        {
 ...
 */
                                           ...
                                           const std::uint32_t target_sid) -> bool
    {
        ...
        while (off + 12 <= n)
        {
            ...
            const auto flags_val = static_cast<std::uint16_t>((static_cast<std::uint8_t>(p[2]) << 8) |
                                                              static_cast<std::uint8_t>(p[3]));
            const auto sid = static_cast<std::uint32_t>(
                (static_cast<std::uint8_t>(p[4]) << 24) | (static_cast<std::uint8_t>(p[5]) << 16) |
                (static_cast<std::uint8_t>(p[6]) << 8) | static_cast<std::uint8_t>(p[7]));
            ...
            const auto len = (static_cast<std::uint32_t>(p[8]) << 24) |
                             ...
                             (static_cast<std::uint32_t>(p[10]) << 8) | static_cast<std::uint32_t>(p[11]);
 ...
 */
    TEST(MuxMaxStreams, YamuxReject)
    {
        ...
        auto coro = [&]() -> net::awaitable<void>
        {
            ...
            for (std::uint32_t i = 1; i <= 3; ++i)
            {
                auto wu_syn = build_yamux_header(yamux::message_type::window_update, yamux::flags::syn, i,
                                                 yamux::default_window);
