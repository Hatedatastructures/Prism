 ...
 */
TEST(YamuxWindow, WindowRecoveryResumesWrite)
{
    net::io_context ioc;
    yamux::stream_window window(ioc.get_executor());
    ...
    EXPECT_TRUE(!try_acquire_window(window, 100)) << "WindowRecovery: acquire fails when window is 0";
 ...
 */
TEST(YamuxWindow, DeltaClamping)
{
    net::io_context ioc;
    yamux::stream_window window(ioc.get_executor());
 ...
 * 单次大量、多轮触发等场景。
 */
TEST(YamuxWindow, RecvConsumedAutoWindowUpdate)
{
    net::io_context ioc;
    constexpr auto initial = yamux::default_window; // 262144
    ...
    // 累积到阈值触发
    {
        ...
        EXPECT_TRUE(!triggered) << "RecvAuto: 130000 bytes does not trigger";
    ...
    {
        yamux::stream_window window(ioc.get_executor());
        EXPECT_TRUE(!check_recv_window_threshold(window, threshold - 1, initial))
            << "RecvAuto: threshold - 1 does not trigger";
        EXPECT_TRUE(check_recv_window_threshold(window, 1, initial))
 ...
 */
TEST(YamuxWindow, WindowSignalNoSpuriousWake)
{
    net::io_context ioc;
    yamux::stream_window window(ioc.get_executor());
 ...
 */
TEST(YamuxWindow, WindowUpdateFrameOverSocketPair)
{
    net::io_context ioc;
    ...
    ASSERT_TRUE(!ec) << std::format("SocketPair: client connect failed: {}", ec.message());

    auto server_sock = acceptor.accept(ec);
    ASSERT_TRUE(!ec) << std::format("SocketPair: server accept failed: {}", ec.message());
    acceptor.close();
    ...
    // --- 场景 2: WindowUpdate(SYN, stream_id=1, delta=256KB) 模拟流打开 ---
    {
        ...
        EXPECT_TRUE(syn_parsed->type == yamux::message_type::window_update)
    ...
    // --- 场景 3: WindowUpdate(ACK, stream_id=1, delta=512KB) 模拟服务端确认 ---
    {
        ...
        EXPECT_TRUE(ack_parsed->type == yamux::message_type::window_update)
