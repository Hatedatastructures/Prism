...
namespace
{
    ...
    void TestConstructAes128(TestRunner &runner)
    {
        ...
        runner.Check(relay != nullptr, "construct aes-128: not null");
        runner.Check(relay->method() == psm::protocol::shadowsocks::cipher_method::aes_128_gcm,
                     "construct aes-128: method=aes_128_gcm");
    }

    void TestConstructAes256(TestRunner &runner)
    {
        auto cfg = make_aes256_config();
        auto tracker = std::make_shared<psm::protocol::shadowsocks::session_tracker>();
        auto relay = psm::protocol::shadowsocks::make_udp_relay(cfg, tracker);
        runner.Check(relay != nullptr, "construct aes-256: not null");
        runner.Check(relay->method() == psm::protocol::shadowsocks::cipher_method::aes_256_gcm,
    ...
    void TestConstructChacha20(TestRunner &runner)
    {
        ...
        auto tracker = std::make_shared<psm::protocol::shadowsocks::session_tracker>();
        auto relay = psm::protocol::shadowsocks::make_udp_relay(cfg, tracker);
        runner.Check(relay != nullptr, "construct chacha20: not null");
        runner.Check(relay->method() == psm::protocol::shadowsocks::cipher_method::chacha20_poly1305,
                     "construct chacha20: method=chacha20_poly1305");
    }

    void TestConstructBadPsk(TestRunner &runner)
    {
        psm::protocol::shadowsocks::config cfg;
        ...
        runner.Check(ec == psm::fault::code::invalid_psk, "bad psk: decrypt returns invalid_psk");

        std::array<std::uint8_t, 8> sid{};
        auto [ec2, enc] = relay->encrypt_out({}, sid, nullptr);
        runner.Check(ec2 == psm::fault::code::invalid_psk, "bad psk: encrypt returns invalid_psk");
    }
    ...
    void TestAes128DecryptInbound(TestRunner &runner)
    {
        ...
        runner.Check(ec == psm::fault::code::bad_message, "aes128 decrypt short: bad_message");
    }

    void TestAes128EncryptOut(TestRunner &runner)
    {
        auto cfg = make_aes128_config();
        auto tracker = std::make_shared<psm::protocol::shadowsocks::session_tracker>();
        auto relay = psm::protocol::shadowsocks::make_udp_relay(cfg, tracker);
        ...
        runner.Check(dec_ec == psm::fault::code::success, "aes128 encrypt: decode psk");

        auto entry = tracker->get_or_create({session_id, sender, psk_bytes,
            psm::protocol::shadowsocks::cipher_method::aes_128_gcm});
        runner.Check(entry != nullptr, "aes128 encrypt: session created");

        const std::byte payload[] = {std::byte{0xDE}, std::byte{0xAD}};
        auto [enc_ec, ciphertext] = relay->encrypt_out(payload, session_id, entry);
        runner.Check(enc_ec == psm::fault::code::success, "aes128 encrypt: success");
        runner.Check(ciphertext.size() > 16 + 16, "aes128 encrypt: ciphertext size > header+tag");
    }

    void TestChacha20DecryptInbound(TestRunner &runner)
    {
        ...
        std::array<std::byte, 31> short_packet{};
        auto [ec, result] = relay->decrypt_inbound(short_packet, sender);
        runner.Check(ec == psm::fault::code::bad_message, "chacha decrypt short: bad_message");
    }

    void TestChacha20EncryptOut(TestRunner &runner)
    {
        auto cfg = make_chacha_config();
        auto tracker = std::make_shared<psm::protocol::shadowsocks::session_tracker>();
        auto relay = psm::protocol::shadowsocks::make_udp_relay(cfg, tracker);
        ...
        auto [dec_ec, psk_bytes] = psm::protocol::shadowsocks::format::decode_psk(cfg.psk);
        runner.Check(dec_ec == psm::fault::code::success, "chacha encrypt: decode psk");
        ...
        auto [enc_ec, ciphertext] = relay->encrypt_out(payload, session_id, entry);
        runner.Check(enc_ec == psm::fault::code::success, "chacha encrypt: success");
        runner.Check(ciphertext.size() > 16 + 16, "chacha encrypt: ciphertext size > header+tag");
    }

    void TestDecryptTooShort(TestRunner &runner)
    {
        auto cfg = make_aes128_config();
        auto tracker = std::make_shared<psm::protocol::shadowsocks::session_tracker>();
        auto relay = psm::protocol::shadowsocks::make_udp_relay(cfg, tracker);
        ...
        runner.Check(ec == psm::fault::code::bad_message, "decrypt too short: bad_message");
    }

    void TestEncryptNullEntry(TestRunner &runner)
    {
        auto cfg = make_aes128_config();
        auto tracker = std::make_shared<psm::protocol::shadowsocks::session_tracker>();
        auto relay = psm::protocol::shadowsocks::make_udp_relay(cfg, tracker);
        ...
        runner.Check(ec != psm::fault::code::success, "encrypt null entry: failure");
    }

    void TestDecryptBadTimestamp(TestRunner &runner)
    {
        auto cfg = make_aes128_config();
        auto tracker = std::make_shared<psm::protocol::shadowsocks::session_tracker>();
        auto relay = psm::protocol::shadowsocks::make_udp_relay(cfg, tracker);
        ...
        auto [dec_ec, psk_bytes] = psm::protocol::shadowsocks::format::decode_psk(cfg.psk);
        runner.Check(dec_ec == psm::fault::code::success, "bad ts: decode psk");
        ...
        auto [ec, result] = relay->decrypt_inbound(packet, sender);
        runner.Check(ec == psm::fault::code::timestamp_expired, "bad timestamp: timestamp_expired");
    }
...
int main()
{
    ...
    TestRunner runner("ShadowsocksDatagram");

    TestConstructAes128(runner);
    TestConstructAes256(runner);
    TestConstructChacha20(runner);
    TestConstructBadPsk(runner);
    TestAes128DecryptInbound(runner);
    TestAes128EncryptOut(runner);
    TestChacha20DecryptInbound(runner);
    TestChacha20EncryptOut(runner);
    TestDecryptTooShort(runner);
    TestEncryptNullEntry(runner);
    TestDecryptBadTimestamp(runner);
