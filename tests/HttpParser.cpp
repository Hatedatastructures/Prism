...
void TestBasicGetRequest()
{
    ...
    {
        LogFail("parse_req should return success for valid GET request");
        return;
    }
    // 验证方法字段被正确提取
    if (req.method != "GET")
    ...
    if (req.line_end == 0)
    {
        LogFail("line_end should not be 0");
        return;
    }
...
void TestReqLineAndHeaderEndOffsets()
{
    ...
    if (req.line_end != expected_line_end)
    {
        LogFail("line_end offset incorrect");
        return;
    }
