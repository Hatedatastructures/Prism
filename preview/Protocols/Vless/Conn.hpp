...
namespace Preview::Vless
{
    ...
    public:
            ...
            -> Net::awaitable<std::pair<Error, RequestHeader>>
        {
            ...
            const auto AddressTypeValue = static_cast<AddressType>(Tail[3]);
            ...
            // 4. 地址体
            RequestHeader RequestValue;
            std::memcpy(RequestValue.Uuid.data(), Prefix.data() + 1, UuidLen);
            RequestValue.Cmd = Cmd;
            RequestValue.Target.Type = AddressTypeValue;
            RequestValue.Target.Port = static_cast<std::uint16_t>(Tail[1]) << 8 | Tail[2];
            ...
            // 5. UUID 校验（常量时间比较，不匹配则静默断开；可注入认证器）
            const std::string_view GotUuid(reinterpret_cast<const char *>(Prefix.data() + 1), UuidLen);
            const std::string_view ExpectedUuid(reinterpret_cast<const char *>(Uuid_.data()), UuidLen);
            bool BadAuth = false;
            if (Auth_)
            {
                auto AuthenticationResult = Auth_->Check("", GotUuid);
            ...
            {
                BadAuth = !Preview::ConstantTimeEqual(GotUuid, ExpectedUuid);
            }
            if (BadAuth)
            {
