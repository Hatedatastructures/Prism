 ...
 * @note 全部纯函数，零状态零分配。
 * @note 具体帧长度、类型、流 ID、NeedMore、over-report 和 wire 编码规则
 *       由各协议策略负责；本文件只定义共享的结构约束。
...
#include <preview/Protocols/Mux/Types.hpp>
...
namespace Preview::Mux
{
    ...
    concept FrameCodec = requires {
        ...
        {
            C::ParsePayload(
                std::declval<typename C::FrameType &>(),
                std::span<const std::uint8_t>{})
        } -> std::same_as<Error>;
        {
            C::FrameEvent(std::declval<const typename C::FrameType &>())
        } -> std::same_as<StreamEvent>;
