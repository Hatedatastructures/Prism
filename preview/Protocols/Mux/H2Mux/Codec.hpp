...
namespace Preview::Mux::H2Mux
{
    ...
    {
        std::array<std::uint8_t, FrameHdrsize + sizeof(std::uint32_t)> out{};
        out[0] = static_cast<std::uint8_t>(FrameType::WindowUpdate);
        out[1] = 0;
        out[2] = 0;
        out[3] = 0;
        out[4] = sizeof(std::uint32_t);
        out[5] = static_cast<std::uint8_t>((StreamId >> 24) & 0xFF);
        out[6] = static_cast<std::uint8_t>((StreamId >> 16) & 0xFF);
        out[7] = static_cast<std::uint8_t>((StreamId >> 8) & 0xFF);
        out[8] = static_cast<std::uint8_t>(StreamId & 0xFF);
        out[9] = static_cast<std::uint8_t>((delta >> 24) & 0xFF);
        out[10] = static_cast<std::uint8_t>((delta >> 16) & 0xFF);
        out[11] = static_cast<std::uint8_t>((delta >> 8) & 0xFF);
        out[12] = static_cast<std::uint8_t>(delta & 0xFF);
        return out;
    }
