...
namespace Preview::Mux
{
    ...
    public:
         ...
         * @param Options 会话选项（Role 强制客户端）
         * @return 是否成功
         */
        auto Connect(SharedTransmission Raw, const SessionOptions &Options = {}) -> bool
        {
            if (!Raw)
            {
                return false;
            ...
            OptionsValue.Role = Preview::Role::Client;
    ...
    [[nodiscard]] inline auto Connect(
        ...
        const SessionOptions &Options = {}) -> Client<C, Memory>
    {
        Client<C, Memory> ClientValue;
        ClientValue.Connect(std::move(Upstream), Options);
        return ClientValue;
    }
