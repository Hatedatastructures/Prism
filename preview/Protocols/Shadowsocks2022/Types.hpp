...
namespace Preview::Shadowsocks2022
{
    ...
    /// UDP SeparateHeader 中 Session ID 的字节数
    inline constexpr std::size_t SessionIdLen = 8;

    /// UDP SeparateHeader 中 Packet ID 的字节数
    inline constexpr std::size_t PacketIdLen = 8;

    /// 数据块最大载荷（SIP022：uint16 最大值）
    inline constexpr std::uint16_t MaxChunkSize = 0xFFFF;
    ...
    {
        std::array<std::uint8_t, SessionIdLen> LocalSessionId{};
        std::array<std::uint8_t, SessionIdLen> PeerSessionId{};
