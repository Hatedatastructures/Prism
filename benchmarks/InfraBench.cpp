 ...
 * @file InfraBench.cpp
 * @brief 统计与账户基础设施基准测试
 * @details 测量 stats::counter 原子操作、user::entry 流量累加、
 *          user::lease RAII 生命周期、user::directory 查找与更新性能。
...
void BM_Entry_AccumulateUplink(benchmark::State &state)
{
    ...
    for (auto _ : state)
    {
        psm::user::accumulate_uplink(&ent, bytes);
...
void BM_Entry_AccumulateDownlink(benchmark::State &state)
{
    ...
    for (auto _ : state)
    {
        psm::user::accumulate_downlink(&ent, bytes);
...
void BM_Entry_AccumulateUplink_Nullptr(benchmark::State &state)
{
    for (auto _ : state)
    {
        psm::user::accumulate_uplink(nullptr, 1024);
    }
}
BENCHMARK(BM_Entry_AccumulateUplink_Nullptr);

// ============================================================
// user::lease RAII 生命周期基准测试
// ============================================================

void BM_Lease_ConstructDestroy(benchmark::State &state)
{
    auto ent = std::make_shared<psm::user::entry>();
...
auto make_directory(std::size_t n) -> std::shared_ptr<psm::user::directory>
{
    auto dir = std::make_shared<psm::user::directory>();
    dir->reserve(n);
...
void BM_Directory_Find_Hit(benchmark::State &state)
{
    ...
    const psm::user::directory *dir = nullptr;
...
void BM_Directory_Contains(benchmark::State &state)
{
    ...
    for (auto _ : state)
    {
        auto result = psm::user::contains(*dir_100, key);
        benchmark::DoNotOptimize(result);
    }
}
...
void BM_Directory_TryAcquire_Success(benchmark::State &state)
{
    for (auto _ : state)
    {
        auto l = psm::user::try_acquire(*dir_100, "credential-50");
...
void BM_Directory_TryAcquire_Miss(benchmark::State &state)
{
    for (auto _ : state)
    {
        auto l = psm::user::try_acquire(*dir_100, "nonexistent-credential");
        benchmark::DoNotOptimize(static_cast<bool>(l));
    }
}
...
void BM_Directory_TryAcquire_Capped(benchmark::State &state)
{
    ...
    // 先占用唯一名额
    auto holder = psm::user::try_acquire(capped_dir, "capped-user");
    for (auto _ : state)
    {
        auto l = psm::user::try_acquire(capped_dir, "capped-user");
        benchmark::DoNotOptimize(static_cast<bool>(l));
    }
}
