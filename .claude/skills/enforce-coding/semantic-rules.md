...
```cpp
// module.hpp — 用前向声明，不 include
namespace psm::module
{
    class impl;
}

// module.cpp — include 完整定义
#include <project/module/impl.hpp>
```
...
// 2. 项目内头文件（统一使用尖括号）
#include <project/module/transport.hpp>
#include <project/module/packet.hpp>
#include <project/trace.hpp>
...
#include <project/module/sub/header.hpp>
```

每个主要模块都有根级聚合头文件。遗漏更新会导致下游编译失败。

## 5. 注释风格

### 5.1 Doxygen 文档（仅 .hpp）

**规则：Doxygen 风格文档只写在 `.hpp` 头文件中。** 声明处的文档即为接口契约。并且禁止markdown语法，禁止英文注释，禁止写在.cpp文件中，使用常见标签。详细注释格式规范参见 `cpp-doxygen-comment` skill。
...
auto impl::on_data(...)
    -> std::int32_t
{
    // 回调可能在单次接收中多次触发，必须立即投递到 channel
    // 否则协程上下文切换后 send_pending 会导致帧重排
...
/**
 * @file server.hpp
 * @brief HTTP/2 多路复用会话服务端
 * @details 继承基类，利用 HTTP/2 库实现帧编解码。
 */
```
...
[[nodiscard]] auto is_active() const noexcept
    -> bool;
[[nodiscard]] auto get_stream_channel(std::int32_t stream_id) const
...
// 正确：类内部别名
class server final : public core
{
public:
...
[](auto &&stream)
{
    co_await std::forward<decltype(stream)>(stream).read();
};
...
auto handshake(handshake_context ctx)
...
**例外**：第三方 C 库同步回调中的 lambda（API 限制）。
...
// 禁止
#ifndef PROJECT_MODULE_SERVER_HPP
#define PROJECT_MODULE_SERVER_HPP
// ...
#endif
```
...
{
public:
    server(transport, router &, config, address_resolver, mr);
    void start();
    [[nodiscard]] auto is_active() const noexcept
        -> bool;
...
private:
    [[nodiscard]] auto frame_loop()
        -> net::awaitable<void>;
    void activate_stream(std::int32_t id, const stream_info &info);
...
```cpp
// impl.cpp — 匿名命名空间封装文件内部函数
namespace
{
    [[nodiscard]] auto verify_basic_auth(std::string_view header, const users &list)
        -> bool
    {
        // ...
    }
...
class server final : public base
{
    ...
    void send_data(std::int32_t id, memory::vector<std::byte> data) override;
};
...
class server final : public base, public std::enable_shared_from_this<server>
{
public:
...
private:
    explicit server(transport t, router &r, config cfg, mr);
};
```
...
```cpp
// 正确
explicit server(transport t, router &r, config cfg);

// 禁止：未标注 explicit
server(transport t, router &r, config cfg);
```
...
{
public:
    void send_data(std::int32_t id, memory::vector<std::byte> data)
    {
        // 不加 this->（成员名本身已足够清晰）
        session_ = nullptr;
...
```cpp
// 正确：参数名加后缀区分
server(transport trans, router &rt, config cfg)
    : transport_(std::move(trans)), route_(rt), config_(cfg)
{
}

// 禁止：初始化列表中写 this->（非法 C++）
server(transport trans, router &route, config cfg)
...
void connect(const memory::string &host, std::uint16_t port, bool use_tls);
