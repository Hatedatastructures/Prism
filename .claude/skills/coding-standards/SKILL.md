...
**规则：lambda 不超过 10 行。** 超过时提取为命名函数。

### 13.1 co_spawn 传 lambda 的方式

**规则：`co_spawn` 中内联 lambda 不超过 10 行。** 超过时将 lambda 提取为命名函数，通过 `std::move` 传入。

```cpp
// OK：短 lambda 内联
co_spawn(executor(),
    [self]() -> net::awaitable<void> { co_await self->handshake(); },
    net::detached);
...
**规则：`co_await` 表达式按 150 行宽自然折行，不强制换行点。**

```cpp
// 短：一行
auto result = co_await handshake(std::move(ctx));

// 超 150 字符：在赋值号或参数处自然断行
auto [ec, protocol] = co_await negotiate(std::move(opts),
                                         remote_endpoint);
```
...
**规则：混合风格。** 函数/类/结构体/命名空间的开括号 `{` 独占一行，控制结构（`if`/`for`/`while`/`switch`）的开括号在同一行。
...
```cpp
auto negotiate(const memory::string &host,
               std::uint16_t port,
               tls_mode mode)
