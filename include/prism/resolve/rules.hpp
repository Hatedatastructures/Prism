 ...
 * 模块组成包括 rule_result 为规则匹配结果，包含静态地址、CNAME 目标
 * 和拦截标记；domain_trie 为反转域名基数树，支持通配符和后缀匹配；
 * rules_engine 为规则引擎，整合地址规则和 CNAME 规则的统一匹配接口。
 * 设计原理为反转存储，域名标签按 "com → example → www" 顺序存储，
 * 使后缀匹配等价于前缀遍历大幅提升匹配效率；通配符折叠，
 * "*.example.com" 在 "example" 节点标记 wildcard，搜索时回溯检查即可
 * 无需枚举子域；PMR 内存管理，所有容器使用 memory:: 命名空间类型，
 * 支持运行时内存资源切换。
 */
#pragma once
...
namespace psm::resolve
{
    ...
    private:
        /**
         * @brief 将域名分割为标签并反转顺序
         * @details 按 "." 分割域名并反转标签顺序，例如 "www.example.com"
         * 分割为 ["com", "example", "www"]。
    ...
    public:
        /**
         * @brief 构造规则引擎
         * @details 使用指定内存资源初始化地址规则树和 CNAME 规则树。
        ...
        /**
         * @brief 添加静态地址规则
         * @details 将域名和对应的 IP 地址列表插入地址规则基数树。
         ...
         * @details 将源域名和目标域名插入 CNAME 规则基数树。
