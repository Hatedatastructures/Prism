 ...
 *
 * 用法：
 *   auto ctx = std::make_shared<trace_context>();
 *   ctx->born = steady_clock::now();
 *   ctx->proto = "trojan";
 *   ctx->bump();
 *   trace::debug(ctx, "msg");  // 前缀: [W0][S3][trojan][... 18:22:48.842]
 */
#pragma once
...
struct trace_context : public std::enable_shared_from_this<trace_context>
{
    ...
    /// @brief 标记脏——字段变化后调用
    auto bump() noexcept -> void { cache_gen++; }
...
private:
    auto render() const noexcept -> void
    {
        ...
        std::snprintf(cached, sizeof(cached),
                      "[W%u][S%u][%s][%s %02d:%02d:%02d.%03d]",
                      worker, stage, proto, target,
                      hh, mm, ss, static_cast<int>(ms % 1000));
    }
