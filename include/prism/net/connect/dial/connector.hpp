...
namespace psm::connect
{
    ...
    struct dial_options
    {
        ...
        flag routing{flag::normal};

        dial_options(std::string_view l, const psm::connect::target &t, flag f,
                     std::shared_ptr<trace::trace_context> tr)
            : label(l), target(t), trace(std::move(tr)), routing(f)
        {
        }

        dial_options(std::string_view l, const psm::connect::target &t, flag f = flag::normal)
