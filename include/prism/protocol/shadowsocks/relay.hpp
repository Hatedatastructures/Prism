...
#include <prism/protocol/shadowsocks/salt_pool.hpp>
...
namespace psm::protocol::shadowsocks
{
    ...
    {
    public:
        explicit relay(channel::transport::shared_transmission next_layer, const config &cfg,
    ...
    private:
        ...
        // 握手中的初始 payload（可能为空）
        std::vector<std::byte> initial_payload_;
        ...
        [[nodiscard]] auto derive_aead_context(const std::vector<std::uint8_t> &salt) const
        ...
        /// 构建并发送服务端响应
        auto send_response(const std::vector<std::uint8_t> &client_salt, std::int64_t server_ts)
            -> net::awaitable<fault::code>;
        ...
        /// 加密并写入一个数据块
        auto send_chunk(std::span<const std::byte> data, std::error_code &ec) const
            -> net::awaitable<std::size_t>;
    };

    using shared_relay = std::shared_ptr<relay>;

    inline auto make_relay(channel::transport::shared_transmission next_layer, const config &cfg,
