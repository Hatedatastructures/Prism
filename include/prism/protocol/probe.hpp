...
namespace psm::protocol
{
     ...
     * 器复用，避免重复读取。提供错误处理，通过 fault::code 提供详细的错误
     ...
     * 中的错误代码，成功时为 fault::code::success。
    ...
    struct detection_result
    {
         ...
         * fault::code::success，且协议类型不是 unknown。
         * @return true 如果检测成功，否则 false
         */
        auto success() const noexcept -> bool
        {
            return ec == fault::code::success && type != protocol_type::unknown;
        }
        ...
        -> net::awaitable<detection_result>
    {
        ...
        result.type = analysis::detect(peek_view);
