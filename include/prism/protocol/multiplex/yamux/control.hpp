...
namespace psm::multiplex::yamux
{
    ...
    private:
         ...
         */
        auto handle_data(const frame_header &hdr, memory::vector<std::byte> payload) -> net::awaitable<void>;

        /**
         * @brief 处理 Data(SYN) 帧，创建流并回复 ACK
         * @details sing-mux 兼容模式：允许 Data 帧携带 SYN 标志和地址数据。
         */
        auto handle_syn(std::uint32_t stream_id, memory::vector<std::byte> payload) -> net::awaitable<void>;
         ...
         */
        auto handle_winupd(const frame_header &hdr) -> net::awaitable<void>;

        /**
         * @brief 处理 Ping 帧
         * @details SYN 为心跳请求，回复 ACK 携带相同 ID；ACK 忽略。
         */
        auto handle_ping(const frame_header &hdr) -> net::awaitable<void>;

        /**
         * @brief 处理 GoAway 帧，关闭整个会话
         */
        auto handle_goaway(const frame_header &hdr) -> net::awaitable<void>;
         ...
         */
        auto update_recv_win(std::uint32_t stream_id, std::uint32_t consumed) -> net::awaitable<void>;
        ...
        memory::unordered_map<std::uint32_t, std::shared_ptr<net::steady_timer>>
            pending_timers_;                                       ///< pending 流超时定时器
