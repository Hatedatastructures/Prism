...
namespace psm::recognition::probe
{
        ...
        -> net::awaitable<probe_result>
    {
        ...
        const auto span = std::span(result.pre_read_data.data(), peek_size);

        std::error_code sys_ec;
        const std::size_t n = co_await transport.async_read_some(span, sys_ec);
        if (sys_ec)
        {
        ...
        const std::string_view peek_view(reinterpret_cast<const char *>(result.pre_read_data.data()), n);
        result.type = detect(peek_view);
