 ...
 * @file stats.hpp
 * @brief Worker 负载统计模块
 * @details 本文件提供单个 worker 线程的运行状态统计功能。
 * 统计数据包括活跃会话数、待处理连接数和事件循环延迟三项
 * 核心指标。这些指标被负载均衡器用于决策新连接应该分发到
 * 哪个 worker，实现基于实际负载的动态调度。
 */
...
namespace psm::agent::worker::stats
{
     ...
     * @details 该类维护三项核心指标：活跃会话数表示当前正在
     * 处理的连接数量，待处理连接数表示已投递但尚未开始处理的
     * 连接数量，事件循环延迟反映 worker 的处理压力。延迟测量
     * 采用 EMA 平滑算法，过滤短期抖动，提供稳定的负载评估
     * 依据。所有计数器均使用原子操作，支持无锁并发访问。
     * @note 活跃会话计数器使用 shared_ptr 包装，允许会话关闭
     * 回调仅捕获计数器而非整个 state 对象
     * @warning observe 协程必须在 worker 的事件循环中运行，
     * 否则延迟测量将不准确
     */
    class state
    {
    ...
    private:
        std::shared_ptr<std::atomic<std::uint32_t>> active_sessions_; // 活跃会话数，共享指针包装支持跨线程访问
        std::atomic<std::uint32_t> pending_handoffs_{0};              // 等待投递到 worker 的 socket 数
        std::atomic<std::uint64_t> event_loop_lag_us_{0};             // 平滑后的事件循环延迟，单位微秒
    };
}
