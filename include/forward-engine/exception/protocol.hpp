...
#include <forward-engine/exception/deviant.hpp>
...
namespace ngx::exception
{
    ...
    public:
        /**
         * @brief 构造函数（协议错误码）
         * @param err 协议错误码，必须为 ngx::fault::code 枚举值
         * @param loc 源码位置，默认自动捕获
         * @details 使用 ngx::fault::code 错误码构造协议异常。此构造函数
         * 将协议错误码转换为 std::error_code 并存储，同时自动捕获
         * 调用点的源码位置。这是推荐的构造函数，保留了完整的错误分类
         * 和源码位置信息。错误码转换通过 ngx::fault::make_error_code()
         * 内联函数完成，无运行时开销。
         * @note 这是推荐的构造函数，保留了完整的错误分类和源码位置信息。
         */
        explicit protocol(ngx::fault::code err,
                          const std::source_location &loc = std::source_location::current())
            : deviant(ngx::fault::make_error_code(err), {}, loc)
        {
        }
         ...
         * @param loc 源码位置，默认自动捕获
         * @details 使用 ngx::fault::code 错误码和额外描述信息构造协议异常。
         ...
         */
        explicit protocol(ngx::fault::code err, std::string_view desc,
                          const std::source_location &loc = std::source_location::current())
            : deviant(ngx::fault::make_error_code(err), desc, loc)
        {
        }
