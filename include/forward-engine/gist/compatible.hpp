 ...
 * @file compatible.hpp
 * @brief 错误码标准库兼容性支持
 * @details 提供 ngx::gist::code 与 std::error_code 和 boost::system::error_code
...
namespace ngx::gist
{
     ...
     * @details 实现 std::error_category 接口，为 ngx::gist::code 提供标准库
     * 错误分类支持。该类是错误码系统与 C++ 标准库错误处理体系之间的桥梁。
     * 核心职责包括分类标识，通过 name() 返回 "ngx::gist" 标识错误来源；
     ...
     * @details 返回 gist_category 的全局单例引用。该函数是标准库错误分类系统
     ...
     * 分类比较，比较错误码是否属于 gist 分类；标准库集成，与 std::system_error
     * 等标准库组件协作。
     ...
     * @param c 自定义错误码枚举值
     * @return 对应的标准错误码对象
     * @details 将 ngx::gist::code 枚举值转换为 std::error_code 对象。该函数
     * 启用自定义错误码与标准库错误处理系统的集成，是错误码兼容性的核心组件。
     * 转换机制包括错误值映射，枚举值安全转换为 int 类型，保持值语义不变；
     * 分类关联，与 gist_category 单例关联，标识错误来源；隐式转换，配合
     * std::is_error_code_enum 特化支持隐式转换，提供零摩擦集成。
     * 设计优势包括类型安全，编译时检查确保只有 ngx::gist::code 枚举可以转换；
...
namespace std
{
     ...
     * @details 标记 ngx::gist::code 为错误码枚举，启用与 std::error_code 的
     * 隐式转换。该特化是 C++ 标准库错误处理系统的关键集成点。作用机制包括
     * 类型标记，继承自 std::true_type，编译时标记 ngx::gist::code 为错误码
     * 枚举类型；构造函数启用，使 std::error_code 构造函数能够接受 ngx::gist::code
     * 参数；隐式转换，启用从 ngx::gist::code 到 std::error_code 的隐式转换，
     * 提供零摩擦集成。
    ...
    /**
     * @brief 特化 hash
     * @details 为 ngx::gist::code 提供 std::hash 特化，使其可用于无序容器，
...
namespace boost::system
{
    /**
     * @brief 特化 is_error_code_enum
     * @details 标记 ngx::gist::code 为 Boost 错误码枚举，启用与
     * boost::system::error_code 的隐式转换。该特化是 Boost.System 错误处理
     * 系统的关键集成点。作用机制包括类型标记，继承自 std::true_type，编译时
     * 标记 ngx::gist::code 为错误码枚举类型；构造函数启用，使 boost::system::
     * error_code 构造函数能够接受 ngx::gist::code 参数；隐式转换，启用从
     * ngx::gist::code 到 boost::system::error_code 的隐式转换，提供零摩擦集成。
     * 兼容性设计包括对称设计，与 std::is_error_code_enum 特化保持对称，提供
     * 一致的开发体验；双向互操作，支持 ngx::gist::code 与 boost::system::
     ...
     * @return gist_category 单例引用，生命周期与程序相同
     * @details 返回 gist_category 的全局单例引用。该函数是 Boost.System 错误
     ...
     * 分类单例；分类比较，比较错误码是否属于 gist 分类；Boost 集成，与 Boost
     * 生态系统的错误处理组件协作。
     ...
     * @details 将 ngx::gist::code 枚举值转换为 boost::system::error_code 对象。
     * 该函数启用自定义错误码与 Boost.System 错误处理系统的集成，是 Boost
     * 兼容性的核心组件。转换机制包括错误值映射，枚举值安全转换为 int 类型，
     * 保持值语义不变；分类关联，与 gist_category 单例关联，标识错误来源；
