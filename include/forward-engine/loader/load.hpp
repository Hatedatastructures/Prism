 ...
 * @file load.hpp
 * @brief 配置加载适配器
 * @details 负责将外部配置格式（如 JSON）转换为内部的 ngx::config 结构。
 ...
 * 内部的 ngx::config 结构。充当外部世界与核心配置之间的防腐层，
 * 确保核心配置结构不受外部格式变化的影响。
 */
namespace ngx::loader
{
     ...
     * @return ngx::config 转换后的配置对象，加载失败时返回空对象
     * @throws exception::security 如果文件打开失败
     * @details 从文件系统加载配置文件，解析 JSON 格式并转换为
     * 内部的 ngx::config 结构。
     * @note 配置文件必须是有效的 JSON 格式，且符合 ngx::config 的结构定义
     */
