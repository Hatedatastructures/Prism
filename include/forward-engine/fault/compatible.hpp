 ...
 * @file compatible.hpp
 * @brief 错误码标准库兼容性支持
 * @details 提供 ngx::fault::code 与 std::error_code 和 boost::system::error_code
...
namespace ngx::fault
{
     ...
     * @details 实现 std::error_category 接口，为 ngx::fault::code 提供标准库
     * 错误分类支持。该类是错误码系统与 C++ 标准库错误处理体系之间的桥梁。
     * 核心职责包括分类标识，通过 name() 返回 "ngx::fault" 标识错误来源；
     ...
     * @param c 自定义错误码枚举值
     * @return 对应的标准错误码对象
     * @details 将 ngx::fault::code 枚举值转换为 std::error_code 对象。该函数
     ...
     * 设计优势包括类型安全，编译时检查确保只有 ngx::fault::code 枚举可以转换；
...
namespace std
{
     ...
     * @details 标记 ngx::fault::code 为错误码枚举，启用与 std::error_code 的
     * 隐式转换。该特化是 C++ 标准库错误处理系统的关键集成点。作用机制包括
     * 类型标记，继承自 std::true_type，编译时标记 ngx::fault::code 为错误码
     * 枚举类型；构造函数启用，使 std::error_code 构造函数能够接受 ngx::fault::code
     * 参数；隐式转换，启用从 ngx::fault::code 到 std::error_code 的隐式转换，
     * 提供零摩擦集成。
     * @note 该特化是 SFINAE 友好的，可用于模板元编程。
     * @warning 修改此特化可能破坏与标准库的兼容性。
     */
    template <>
    struct is_error_code_enum<ngx::fault::code> : std::true_type
    {
    };

    /**
     * @brief 特化 hash
     * @details 为 ngx::fault::code 提供 std::hash 特化，使其可用于无序容器，
     ...
     */
    template <>
    struct hash<ngx::fault::code>
    {
        /**
         * @brief 计算错误码的哈希值
         * @param c 错误码枚举值
         * @return 哈希值
         * @details 将 ngx::fault::code 枚举值转换为哈希值，用于无序容器。
         * 哈希实现委托给 std::hash<int>，保证质量。
         * @note 哈希实现委托给 std::hash<int>，保证质量。
         * @warning 不要依赖哈希值的具体数值，仅用于容器内部组织。
         */
        [[nodiscard]] size_t operator()(const ngx::fault::code c) const noexcept
        {
...
namespace boost::system
{
    /**
     * @brief 特化 is_error_code_enum
     * @details 标记 ngx::fault::code 为 Boost 错误码枚举，启用与
     * boost::system::error_code 的隐式转换。该特化是 Boost.System 错误处理
     * 系统的关键集成点。作用机制包括类型标记，继承自 std::true_type，编译时
     * 标记 ngx::fault::code 为错误码枚举类型；构造函数启用，使 boost::system::
     * error_code 构造函数能够接受 ngx::fault::code 参数；隐式转换，启用从
     * ngx::fault::code 到 boost::system::error_code 的隐式转换，提供零摩擦集成。
     * 兼容性设计包括对称设计，与 std::is_error_code_enum 特化保持对称，提供
     * 一致的开发体验；双向互操作，支持 ngx::fault::code 与 boost::system::
     * error_code 之间的双向转换；生态系统集成，确保项目错误码能够与使用 Boost
     * 的第三方库无缝协作。
     * @note 该特化使 ngx::fault::code 能够与 Boost 生态系统的错误处理组件互操作。
    ...
    public:
        ...
        [[nodiscard]] std::string message(int c) const override
        {
            return ngx::fault::cached_message(static_cast<ngx::fault::code>(c));
        }
    };
     ...
     * @details 将 ngx::fault::code 枚举值转换为 boost::system::error_code 对象。
     ...
     * @warning 转换后的错误码仅包含原始枚举值，不包含额外上下文信息。
     */
    inline error_code make_error_code(const ngx::fault::code c) noexcept
    {
        return {static_cast<int>(c), category()};
    }
}
