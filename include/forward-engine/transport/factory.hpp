 ...
 *
 * 架构说明：
 * - 静态工厂：`factory` 类提供静态工厂方法，无需实例化；
 * - 传输层创建：支持从现有 socket 创建 TCP 和 UDP 传输层；
 * - 协议装饰器：支持创建 Trojan 等协议装饰器；
 * - 智能指针返回：所有工厂方法返回 `transmission_pointer` 智能指针。
 *
 * 设计特性：
 * - 类型安全：使用模板参数和返回类型确保类型正确性；
 * - 所有权管理：所有工厂方法返回智能指针，自动管理对象生命周期；
 * - 扩展性：可轻松添加新的传输层或协议装饰器类型；
 * - 零配置：工厂方法无需额外配置，参数完全通过函数签名传递。
 *
 * @note 该工厂是传输层和协议装饰器的统一创建入口。
 * @warning 工厂方法可能抛出 `std::bad_alloc`（内存分配失败）或 `std::runtime_error`（配置错误）。
 */
...
namespace ngx::transport
{
     ...
     *
     * 核心职责：
     * @details - 传输层创建：支持从现有 socket 创建 TCP 和 UDP 传输层；
     * @details - 协议装饰器：支持创建 Trojan 等协议装饰器；
     * @details - 智能指针管理：所有工厂方法返回智能指针，自动管理生命周期；
     * @details - 错误处理：工厂方法可能抛出异常，调用者应处理异常。
     *
     * 工厂方法：
     * @details - `create_transport(tcp::socket)`：从 TCP socket 创建可靠传输层；
     * @details - `create_transport(udp::socket, endpoint)`：从 UDP socket 创建不可靠传输层；
     * @details - `create_trojan(transmission_pointer, verifier)`：创建 Trojan 协议装饰器。
     *
     * 线程安全性设计：
     * @details - 无状态设计：工厂类不持有任何状态，所有方法都是静态的；
     * @details - 线程安全：静态方法可以在多线程中安全调用；
     * @details - 异常安全：工厂方法可能抛出异常，调用者应正确处理。
     *
    ...
    public:
         ...
         * @details 从现有的 TCP socket 创建可靠传输层实例。传输层接管 socket 的所有权。
         * 该方法是创建 TCP 传输层的标准方式。
         *
         * 创建流程：
         * @details - Socket 接管：接收 TCP socket 的所有权；
         * @details - 传输层包装：将 socket 包装为 `reliable` 传输层；
         * @details - 智能指针返回：返回 `transmission_pointer` 智能指针，自动管理生命周期。
         *
         ...
         * @details 从现有的 UDP socket 创建不可靠传输层实例。传输层接管 socket 的所有权。
         * 该方法是创建 UDP 传输层的标准方式。
         *
         * 创建流程：
         * @details - Socket 接管：接收 UDP socket 的所有权；
         * @details - 远程端点：接收可选的远程端点，用于无连接 UDP；
         * @details - 传输层包装：将 socket 包装为 `unreliable` 传输层；
         * @details - 智能指针返回：返回 `transmission_pointer` 智能指针，自动管理生命周期。
         *
         ...
         * @details 创建 Trojan 协议装饰器，用于加密和认证代理流量。装饰器包装底层传输层，
         * 在数据传输前进行加密和认证处理。
         *
         * 创建流程：
         * @details - 传输层包装：接收底层传输层的所有权；
         * @details - 凭据验证：接收凭据验证回调，用于 Trojan 握手认证；
         * @details - 装饰器构造：将传输层包装为 Trojan 装饰器；
         * @details - 智能指针返回：返回 `transmission_pointer` 智能指针，自动管理生命周期。
         *
         * Trojan 协议特性：
         * @details - TLS 加密：所有流量通过 TLS 加密传输；
         * @details - 凭据认证：使用密码哈希或令牌进行认证；
         * @details - 混淆技术：使用密码派生密钥进行数据混淆；
         * @details - 流量伪装：加密流量伪装成正常的 HTTPS 流量。
         *
