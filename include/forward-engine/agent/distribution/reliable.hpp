 ...
 * TCP 是面向连接的可靠传输协议，因此命名为 reliable_resolver。
 */
#pragma once
...
namespace ngx::agent::distribution
{
     ...
     * @brief 可靠传输解析器。
     * @details 解析器负责解析主机名到 TCP 端点并建立连接，同时缓存
     * 解析结果以提升性能。它实现了两级优化策略：首先检查本地缓存，
     * 若缓存命中且未过期则尝试连接缓存的端点列表；若缓存未命中，
     ...
     */
    class reliable_resolver final
    {
    public:
        using result = std::pair<gist::code, unique_sock>;
         ...
         */
        explicit reliable_resolver(source &pool, const net::any_io_executor &executor, memory::resource_pointer mr = memory::current_resource(),
