 ...
 * @file protocols.hpp
 * @brief 协议处理管道入口
 * @details 定义 HTTP、SOCKS5 和 Trojan 流量的具体会话处理函数。
 * 会话层检测协议后直接分发到这些函数，不再使用单独的注册表
 * 包装或兼容层。每个函数负责完整的协议处理流程，包括请求解析、
 * 上游连接建立以及双向隧道转发。Trojan 协议内部完成 TLS 握手。
 */
 ...
 * TLS 握手函数 tls_handshake()、原始转发函数 tunnel()
 * 以及协议处理函数 http()、socks5()、trojan()。
...
namespace ngx::agent::pipeline
{
     ...
     * 处理流程包括 TLS 握手执行服务器端 TLS 握手、Trojan 握手
