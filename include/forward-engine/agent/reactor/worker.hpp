 ...
 * @brief Worker 线程管理功能。
 * @details 该命名空间包含 worker 类及其相关组件，负责处理实际
 * 的网络连接和数据转发。worker 是代理服务的核心执行单元，每个
 * worker 运行在独立线程中，拥有完整的事件循环和资源上下文。
 */
namespace ngx::agent::reactor
{
    ...
    public:
         ...
         * 异步处理，实现跨线程连接分发。实际处理逻辑由 launch::dispatch
         * 完成，包括 socket 预配置和会话创建。
