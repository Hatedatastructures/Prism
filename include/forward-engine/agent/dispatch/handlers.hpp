 ...
 * @file handlers.hpp
 * @brief 协议处理器分发器
 * @details 定义具体协议处理器实现如 Http、SOCKS5、TLS、Unknown 及其全局注册函数。
 ...
 * 等方法。SOCKS5 支持 SOCKS5 协议标准定义的认证和连接命令。TLS 支持 TLS 握手
 * 和加密代理协议如 Trojan/Obscura。Unknown 支持原始 TCP 透传。设计特性包括
...
namespace ngx::agent::dispatch
{
     ...
     * @class Tls
     * @brief TLS 协议处理器
     * @details 实现 TLS 协议的处理逻辑，将检测到的 TLS 数据流转发到 pipeline::tls
     * 处理流水线。该处理器通过 registry::global 工厂注册，在协议检测阶段被调用。
     * 核心职责包括协议标识，通过 type 方法返回 protocol_type::tls。名称标识，
     * 通过 name 方法返回 tls 用于日志。数据处理，通过 process 方法将 TLS 流
     * 转发到 pipeline::tls。线程安全性设计方面，无状态设计，处理器实例不持有
     * 任何状态，所有处理逻辑在 pipeline::tls 中。单例模式，工厂内部维护单例，
     * 避免多线程创建多个实例。协程安全，process 方法为协程，保证单线程执行。
     * @note 该处理器为单例实例，避免每个连接重复创建。
     * @warning 处理器实例不持有状态，所有处理逻辑委托给 pipeline::tls 协程。
     */
    class Tls : public handler
    {
    public:
        /**
         * @brief 默认构造函数
    ...
    public:
         ...
         * 然后调用 primitives::original_tunnel 建立全双工隧道。
     ...
     * @details 将 Http、SOCKS5、TLS、Unknown 协议处理器注册到全局处理器工厂
     ...
     * 到 protocol_type::socks5。TLS 协议，注册 Tls 到 protocol_type::tls。
