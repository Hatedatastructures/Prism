 ...
 * DNS 解析缓存和请求合并机制，避免重复的 DNS 查询开销。同时提供
...
namespace ngx::agent::resolve
{
     ...
     * @class udpcache
     * @brief UDP 解析缓存。
     * @details 负责解析主机名到 UDP 端点，并缓存解析结果以提升性能。
     * 实现两级优化策略：首先检查本地缓存，若命中且未过期则直接返回；
     * 若未命中，则检查是否有相同目标的解析请求正在进行中，若有则等待
     * 其完成后复用结果；否则发起新的 DNS 解析并将结果存入缓存。
     * 缓存采用 FIFO 淘汰策略，当记录数超过上限时移除最旧的条目。
     * 与 tcpcache 不同，该类仅存储单个端点而非端点列表，
     ...
     */
    class udpcache final
    {
    public:
        using endpoint = net::ip::udp::endpoint;        // UDP 端点类型
        using result = std::pair<fault::code, endpoint>; // 解析结果类型
         ...
         */
        explicit udpcache(net::any_io_executor executor, memory::resource_pointer mr = memory::current_resource(),
