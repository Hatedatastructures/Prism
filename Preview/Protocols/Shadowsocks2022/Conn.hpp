...
namespace Preview::Shadowsocks2022
{
    ...
    public:
         ...
         */
        explicit Conn(std::array<std::uint8_t, 16> psk,
                      std::uint64_t TimeWindow = 90,
                      const Preview::Authenticator *Auth = nullptr,
                      Preview::SharedAuthenticator AuthOwner = {})
            : Psk_(psk),
            ...
            if (Auth_)
            {
                auto AuthenticationResult = Auth_->Authenticate(Preview::AuthenticationRequest{
                    ...
                    .Credential = Preview::Account::CredentialView{
                        Preview::Account::CredentialKind::Psk,
                        std::as_bytes(std::span<const std::uint8_t>(Psk_))},
                    .Rate = {}});
                if (!AuthenticationResult.Accepted)
                {
                    co_return std::pair{Error::BadAuth, ss::Message{}};
                }
    ...
    private:
        ...
        {
            // 识别阶段与 handler 阶段共用同一个增量 Parser，避免两套
            // SS2022 变长头解析逻辑在真实客户端 wire 上产生分叉。
            std::array<std::uint8_t, 16> salt{};
            if (co_await RecvExact(std::span<std::uint8_t>(salt)))
            {
                co_return Error::IoError;
            }
            Parser RequestParser(this->Psk_);
            std::error_code ParseError;
            RequestParser.Put(Net::const_buffer(salt.data(), salt.size()), ParseError);
            ...
            const auto Now = static_cast<std::uint64_t>(
                std::chrono::duration_cast<std::chrono::seconds>(
                    std::chrono::system_clock::now().time_since_epoch())
                    .count());
            const auto Timestamp = RequestParser.TimeSec();
            const auto Difference = Now >= Timestamp ? Now - Timestamp : Timestamp - Now;
            if (Difference > TimeWindow_)
            {
                co_return Error::BadAuth;
            }
            ...
            std::memcpy(this->ClientSalt_.data(), salt.data(), salt.size());
            std::memcpy(this->SessionKey_.data(), Key.data(), Key.size());
            // Parser has consumed the fixed and variable header with nonce 0/1;
