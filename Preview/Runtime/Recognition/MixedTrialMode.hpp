...
#include <Preview/Runtime/Recognition/ConfiguredMode.hpp>
    ...
    private:
        ...
        struct Trial
        {
            ...
            std::vector<CandidateId> Rejected;
    ...
    public:
                                     ...
                                     RecognitionControl Control = {}) -> Net::awaitable<RecognizeResult>
        {
            ...
            while (true)
            {
                ...
                auto Structural = Current.Structural;
                Structural.erase(std::remove_if(Structural.begin(), Structural.end(),
                                                [&Active](const auto Id)
                                                {
                                                    return std::find(Active.begin(), Active.end(), Id) ==
                                                           Active.end();
                                                }),
                                 Structural.end());
                if (Structural.empty())
                ...
                auto TrialRace = co_await PrepareCandidates(
                    PrepareRequest{std::move(Structural), Snapshot, &Control, Deadline, &Result});
                ...
                for (const auto Id : TrialResult.Rejected)
                {
