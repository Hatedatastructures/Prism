...
namespace Preview::Recognition
{
    ...
    namespace detail
    {
        ...
        {
            explicit OperationState(Net::any_io_executor Executor)
        ...
        inline constexpr auto ControlGracePeriod = std::chrono::milliseconds(100);

        template <typename T>
        auto RunOperation(Net::awaitable<T> Operation,
                          const std::shared_ptr<OperationState<T>> &State)
            -> Net::awaitable<void>
        {
            try
            {
                State->Value = co_await std::move(Operation);
            }
            catch (...)
            {
            ...
            -> Net::awaitable<ControlRaceResult<T>>
        {
            ...
            const auto Executor = co_await Net::this_coro::executor;
            auto State = std::make_shared<OperationState<T>>(Executor);
            Net::co_spawn(Executor, RunOperation(std::move(Operation), State), Net::detached);
            ...
            using Net::experimental::awaitable_operators::operator||;
            auto Race = co_await (State->Completion.async_receive(Net::use_awaitable) ||
                                  std::move(Wait));
            if (Race.index() == 0)
            {
                if (State->Failure)
            ...
            Net::steady_timer GraceTimer(Executor);
            GraceTimer.expires_after(ControlGracePeriod);
            auto Grace = co_await (State->Completion.async_receive(Net::use_awaitable) ||
                                  GraceTimer.async_wait(Net::use_awaitable));
            if (Grace.index() == 0 && State->Value)
            {
                Result.Value = std::move(*State->Value);
            }
            co_return Result;
    ...
    namespace detail
    {
        ...
        inline auto SetControlStatus(RecognizeResult &Result, RecognitionControlEvent Event) -> void
        {
            if (Event == RecognitionControlEvent::Cancelled)
            {
                ...
                Result.FaultCode = Preview::Fault::Code::Canceled;
                Result.Cancelled = true;
                return;
            }
            SetResultStatus(ResultStatusRequest{&Result, RecognitionStatus::TimedOut,
                                                std::make_error_code(std::errc::timed_out)});
            Result.FaultCode = Preview::Fault::Code::Timeout;
        }
    ...
    private:
                                    ...
                                    const CandidateHandle &Handle) -> void
        {
            ...
            Result.FaultCode = Committed.FaultCode;
            Result.NativeError = Committed.NativeError;
            Result.Carrier = Committed.Metadata;
        }
