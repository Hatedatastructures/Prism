/**
 * @file ProtocolAdapter.hpp
 * @brief 协议接入缝（唯一桥接层）— 统一适配 ProtocolHandler 到 runtime
 * @details 以 Handler::ProtocolHandler 为唯一协议接口，MakeProtocolAccept
 *          把对象式 handler 适配为 SessionOptions::ProtocolAcceptFn：
 *          统一处理 AcceptResult → Middleware::Context 的装配与错误映射。
 *          协议便捷工厂（make_accept_*）是薄封装，不再重复 ctx 装配逻辑。
 */

#pragma once

#include <boost/asio/awaitable.hpp>

#include <functional>
#include <memory>
#include <utility>

#include <Preview/Foundation/Utility/Diagnose/Log.hpp>
#include <Preview/Foundation/Error.hpp>
#include <Preview/Foundation/Fault/Code.hpp>
#include <Preview/Foundation/Fault/Handling.hpp>
#include <Preview/Composition/Adapters/TypedResult.hpp>
#include <Preview/Runtime/Middleware/Context.hpp>
#include <Preview/Runtime/Session.hpp>
#include <Preview/Transport/Transmission.hpp>
#include <Preview/Runtime/Contract/Handler.hpp>
#include <Preview/Composition/Adapters/Socks5.hpp>
#include <Preview/Composition/Adapters/Vless.hpp>
#include <Preview/Composition/Adapters/Trojan.hpp>
#include <Preview/Composition/Adapters/Vmess.hpp>
#include <Preview/Composition/Adapters/Ss2022.hpp>

namespace Preview::Runtime
{

    namespace Net = boost::asio;
    namespace Adapters = Preview::Composition::Adapters;

    inline auto ResetProtocolContext(Middleware::Context &Context) noexcept -> void
    {
        if (const auto Transport = Context.DataPlane.Transport())
        {
            Transport->Close();
        }
        Context.DataPlane = {};
        Context.AccountLease.Release();
        Context.Target = Preview::Network::Target{};
        Context.AccountId = {};
        Context.identity.clear();
        Context.ProtocolAuthenticated = false;
        Context.IsDgram = false;
        Context.PostDial = {};
    }

    /**
     * @brief 把对象式 ProtocolHandler 适配为 Session 的协议接入回调
     * @param h 协议处理器（共享所有权，随回调存活）
     * @return Session 协议接入回调
     * @details 成功后把 AcceptResult 装配进 Middleware::Context：
     *          Target / identity / ProtocolAuthenticated / AccountLease / IsDgram / PostDial，并把数据面传输
     *          替换到 Inbound。失败或无传输时统一走错误映射，不留下半状态。
     */
    [[nodiscard]] inline auto MakeProtocolAccept(std::shared_ptr<Handler::ProtocolHandler> h)
        -> SessionOptions::ProtocolAcceptFn
    {
        return [h = std::move(h)](SharedTransmission &in, Middleware::Context &ctx)
            -> Net::awaitable<Fault::Code>
        {
            ResetProtocolContext(ctx);
            if (!h)
            {
                co_return Fault::Code::IoError;
            }
            auto R = co_await h->Accept(std::move(in));
            const auto HasDataPlane = R.DataPlane.HasTransport();
            if (R.err != Error::None || (!R.Transmission && !HasDataPlane))
            {
                if (R.err != Error::None)
                {
                    // 消费 handler Name()：错误日志标识失败协议，便于定位
                    Preview::Diagnose::Warn("Protocol handler {} Accept Failed", h->Name());
                }
                if (R.err == Error::None)
                {
                    co_return Fault::Code::IoError;
                }
                co_return Fault::ToCode(make_error_code(R.err));
            }
            if (HasDataPlane)
            {
                ctx.SetDataPlane(std::move(R.DataPlane));
                ctx.AccountLease = std::move(R.AccountLease);
                ctx.PostDial = std::move(R.PostDial);
                ctx.IsDgram = ctx.DataPlane.IsDatagram();
                in = ctx.Inbound;
            }
            else
            {
                ctx.Target = R.Target;
                ctx.identity = R.identity;
                ctx.AccountId = R.AccountId;
                ctx.ProtocolAuthenticated = R.ProtocolAuthenticated;
                ctx.AccountLease = std::move(R.AccountLease);
                ctx.IsDgram = R.IsDgram;
                if (R.PostDial)
                {
                    ctx.PostDial = std::move(R.PostDial);
                }
            }
            if (!HasDataPlane)
            {
                in = std::move(R.Transmission);
            }
            co_return Fault::Code::Success;
        };
    }

    /**
     * @brief 执行一次 handler 接入并返回 typed data-plane 结果。
     * @param h 协议 handler 共享所有权。
     * @param Inbound 已识别的入站传输。
     * @return 保留旧上下文字段且不含协议动态转换的 typed 结果。
     */
    [[nodiscard]] inline auto AcceptTyped(
        std::shared_ptr<Handler::ProtocolHandler> h,
        SharedTransmission Inbound) -> Net::awaitable<Adapters::TypedAcceptResult>
    {
        if (!h)
        {
            Adapters::TypedAcceptResult Result;
            Result.Status = Error::IoError;
            Result.Data = Adapters::DataPlaneResult::Failure(Error::IoError);
            co_return Result;
        }
        co_return Adapters::ToTypedResult(co_await h->Accept(std::move(Inbound)));
    }

    /// SOCKS5 便捷工厂
    [[nodiscard]] inline auto MakeAcceptSocks5(Socks5::ServerConfig cfg = {})
        -> SessionOptions::ProtocolAcceptFn
    {
        return MakeProtocolAccept(std::make_shared<Handler::Socks5>(std::move(cfg)));
    }

    /// VLESS 便捷工厂
    [[nodiscard]] inline auto MakeAcceptVless(Vless::ServerConfig cfg = {})
        -> SessionOptions::ProtocolAcceptFn
    {
        return MakeProtocolAccept(std::make_shared<Handler::Vless>(std::move(cfg)));
    }

    /// Trojan 便捷工厂
    [[nodiscard]] inline auto MakeAcceptTrojan(Trojan::ServerConfig cfg = {})
        -> SessionOptions::ProtocolAcceptFn
    {
        return MakeProtocolAccept(std::make_shared<Handler::Trojan>(std::move(cfg)));
    }

    /// VMess 便捷工厂
    [[nodiscard]] inline auto MakeAcceptVmess(Vmess::ServerConfig cfg = {})
        -> SessionOptions::ProtocolAcceptFn
    {
        return MakeProtocolAccept(std::make_shared<Handler::Vmess>(std::move(cfg)));
    }

    /// VMess typed 账户目录工厂（按握手 Credential 选择账户）
    [[nodiscard]] inline auto MakeAcceptVmess(
        Vmess::ServerConfig cfg,
        std::shared_ptr<const Preview::Account::AccountDirectory> Directory)
        -> SessionOptions::ProtocolAcceptFn
    {
        return MakeProtocolAccept(
            std::make_shared<Handler::Vmess>(std::move(cfg), std::move(Directory)));
    }

    /// Shadowsocks2022 便捷工厂（本缝仅 TCP；SS2022 UDP 为独立 socket 通道）
    [[nodiscard]] inline auto MakeAcceptSs2022(Shadowsocks2022::ServerConfig cfg = {})
        -> SessionOptions::ProtocolAcceptFn
    {
        return MakeProtocolAccept(std::make_shared<Handler::Ss2022>(std::move(cfg)));
    }

    /// SS2022 typed 账户目录工厂（按握手 PSK 选择账户）
    [[nodiscard]] inline auto MakeAcceptSs2022(
        Shadowsocks2022::ServerConfig cfg,
        std::shared_ptr<const Preview::Account::AccountDirectory> Directory)
        -> SessionOptions::ProtocolAcceptFn
    {
        return MakeProtocolAccept(
            std::make_shared<Handler::Ss2022>(std::move(cfg), std::move(Directory)));
    }

} // namespace Preview::Runtime
