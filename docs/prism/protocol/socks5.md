...
检测位于 `psm::recognition::probe::detect()`（analyzer.hpp）：
...
`protocol_type::socks5`（枚举索引 2）在 session::diversion() 中映射到 `protocol::socks5::handle`（session.cpp）。
...
session::diversion()
  ...
  +-- 步骤 2: 伪装管道（SOCKS5 跳过）
  |
  +-- 步骤 3: session::diversion() switch (protocol_type::socks5)
              |
              +-- protocol::socks5::handle(ctx, span)
                  |
                  ...
                  |   |   +-- framing::parse_header() -> header_parse
                  |   |
                  |   +-- [根据 header.cmd 切换]
                  ...
                  |       |          +-- framing::parse_ipv4() + framing::decode_port()
                  |       |
                  |       +-- ipv6:  read_address<16>(parse_ipv6)
                  |       |          +-- async_read_impl(18 字节) -> 地址(16) + 端口(2)
                  |       |          +-- framing::parse_ipv6() + framing::decode_port()
                  |       |
                  |       +-- domain: read_domain_address()
                  |                  +-- async_read_impl(1 字节) -> 域名长度
                  |                  +-- async_read_impl(len + 2 字节) -> 域名 + 端口
                  |                  +-- framing::parse_domain() + framing::decode_port()
                  |
                  +-- [根据 process.cpp 中的 request.cmd 切换]
                      |
                      +-- CONNECT (0x01)
...
```cpp
// 管道入口点
auto protocol::socks5::handle(session_context &ctx, std::span<const std::byte> data)
    -> net::awaitable<void>;
...
auto protocol::socks5::make_relay(
    ...
    const config &cfg = {},
    account::directory *account_dir = nullptr)
    -> shared_relay;
...
// 线路级解析
auto framing::parse_header(std::span<const std::uint8_t> buffer)
    -> std::pair<fault::code, header_parse>;

auto framing::parse_ipv4(std::span<const std::uint8_t> buffer)
    -> std::pair<fault::code, ipv4_address>;

auto framing::parse_domain(std::span<const std::uint8_t> buffer)
    -> std::pair<fault::code, domain_address>;

auto framing::decode_port(std::span<const std::uint8_t> buffer)
    -> std::pair<fault::code, uint16_t>;

auto framing::decode_udp_header(std::span<const std::uint8_t> buffer)
    -> std::pair<fault::code, udp_header_parse>;

auto framing::encode_udp_header(const udp_header &header, memory::vector<std::uint8_t> &out)
    -> fault::code;

auto framing::parse_password_auth(std::span<const std::uint8_t> data)
    -> std::pair<fault::code, password_auth_request>;

auto framing::build_password_auth_response(bool success)
...
|---|---|---|---|
| 协商期间读取错误 | `io_error` | 无（静默关闭） | `conn.hpp` 第 612 行 |
| 协议版本错误 | `protocol_error` | 无 | `conn.hpp` 第 618 行 |
| 无可接受的认证方法 | `not_supported` | `0xFF`（写入后返回错误） | `conn.hpp` 第 681 行 |
| 认证解析错误 | `bad_message` | `0x01 0x01`（认证失败） | `conn.hpp` 第 707-709 行 |
| 认证凭据不匹配 | `success`（认证失败但无错误） | `0x01 0x01`（认证失败） | `conn.hpp` 第 736-739 行 |
| 命令被配置禁用 | `not_supported` | `connection_not_allowed` (0x02) | `conn.hpp` 第 237-238 行 |
| BIND 未启用 | `unsupported_command` | `command_not_supported` (0x07) | `conn.hpp` 第 253-254 行 |
| 未知命令 | `unsupported_command` | `command_not_supported` (0x07) | `conn.hpp` 第 259-260 行 |
| 不支持的地址类型 | `unsupported_address` | 无 | `conn.hpp` 第 299 行 |
| IPv6 禁用 | `ipv6_disabled` | `network_unreachable` (0x03) | `process.cpp` 第 58-60 行 |
| 拨号失败（通用） | `*` | `host_unreachable` (0x04) | `process.cpp` 第 64-65 行 |
| UDP 关联失败 | `*` | 已记录，无响应（已发送成功） | `process.cpp` 第 94-97 行 |
| UDP FRAG != 0 | `not_supported` | 无（数据报丢弃） | `framing.hpp` 第 266-269 行 |
| UDP RSV != 0x0000 | `protocol_error` | 无（数据报丢弃） | `framing.hpp` 第 260-263 行 |

### 8.2 认证边缘情况

| 场景 | 行为 | 源 |
|---|---|---|
| `enable_auth=true` 但客户端仅提供 `0x00`（无需认证） | 服务器响应 `0xFF`（无可接受的方法） | `conn.hpp` 第 675-681 行 |
| `enable_auth=true` 但 `account_directory_` 为空 | 如果客户端支持则回退到 no_auth | `conn.hpp` 第 645 行（条件失败） |
| 认证请求中 ULEN = 0 | 发送认证失败响应 | `conn.hpp` 第 705-709 行 |
| 认证请求中 PLEN = 0 | `framing::parse_password_auth` 返回 `bad_message` | `framing.hpp` 第 418-420 行 |
| 用户名 > 255 字节 | 不可能（ULEN 为 1 字节，最大 255） | 协议限制 |
| 认证请求总大小 > 513 字节 | 缓冲区为 513 字节，不可能溢出 | `conn.hpp` 第 695 行 |
...
| 请求中 VER != 0x05 | `framing::parse_header()` 返回 `protocol_error` | 返回 `protocol_error`，不发送响应 |
...
| 场景 | 行为 | 源 |
|---|---|---|
| 入口数据报无载荷（仅头部） | `parsed.header_size >= ingress_packet.size()` -> 丢弃 | `conn.hpp` 第 513-516 行 |
| 路由回调失败 | 数据报静默丢弃 | `conn.hpp` 第 508-511 行 |
| 出口套接字打开失败 | 数据报静默丢弃 | `conn.hpp` 第 522-525 行 |
| 发送到目标失败 | 数据报静默丢弃 | `conn.hpp` 第 528-532 行 |
| 从目标接收失败 | 数据报静默丢弃 | `conn.hpp` 第 537-540 行 |
| UDP 中继期间控制连接关闭 | `wait_control_close` 取消入口套接字 | `conn.hpp` 第 567-576 行 |
| 空闲超时触发 | `associate_loop` 通过 `co_return` 退出 | `conn.hpp` 第 457-460 行 |
| 操作已中止（取消） | `associate_loop` 通过 `co_return` 退出 | `conn.hpp` 第 465-467 行 |
| UDP 竞态：控制关闭 + 空闲超时 | `||` 运算符：先完成者获胜 | `conn.hpp` 第 191 行 |
...
| 约束 | 值 | 源 |
|---|---|---|
| 方法缓冲区 | 258 字节（栈分配） | `conn.hpp` 第 607 行 |
| 认证缓冲区 | 513 字节（栈分配） | `conn.hpp` 第 695 行 |
| 请求头缓冲区 | 4 字节（栈分配） | `conn.hpp` 第 766 行 |
| IPv4 读取缓冲区 | 6 字节（栈分配） | `conn.hpp` 第 797 行（N=4） |
| IPv6 读取缓冲区 | 18 字节（栈分配） | `conn.hpp` 第 797 行（N=16） |
| 域名读取缓冲区 | 258 字节（栈分配） | `conn.hpp` 第 838 行 |
| 成功响应缓冲区 | 262 字节（栈分配） | `conn.hpp` 第 315 行 |
| 错误响应 | 10 字节（栈分配常量） | `conn.hpp` 第 332 行 |
| UDP 最大数据报 | 可配置，默认 65535 | `config.hpp` 第 43 行 |
| UDP 响应数据报 | PMR 分配自 `memory::current_resource()` | `conn.hpp` 第 547 行 |
