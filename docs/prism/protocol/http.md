...
检测发生在 `psm::recognition::probe::detect()`（analyzer.hpp）。算法按以下顺序检查预读数据：
...
分派通过 **session::diversion() switch/case** 完成：
...
`protocol_type::http` 枚举值直接映射到 `protocol::http::handle`。没有虚函数，没有运行时工厂——只有直接的 switch/case 分发。
...
session::diversion()
  ...
  |
  +-- 步骤 3: session::diversion() switch (protocol_type::http)
              |
              +-- protocol::http::handle(ctx, span)
                  |
...
```cpp
// 管道入口点
auto protocol::http::handle(session_context &ctx, std::span<const std::byte> data)
    -> net::awaitable<void>;
...
```
  客户端                    Prism 代理
    ...
    |                            |  protocol::http::handle co_returns
    |                            |  session 释放资源
```
...
| 参数 | 默认值 | 源 |
|---|---|---|
| 初始读取缓冲区 | 4096 字节 | `src/prism/protocol/http/conn.cpp` 第 22 行 |
| 最大头部大小 | 65536 字节 | `src/prism/protocol/http/conn.cpp` 第 16 行 |
| 缓冲区增长因子 | 2 倍 | `src/prism/protocol/http/conn.cpp` 第 110 行 |
| 预读大小 | 24 字节 | `include/prism/protocol/probe.hpp` 第 97 行 |
| CONNECT 默认端口 | 443 | process.cpp |
| HTTP 默认端口 | 80 | process.cpp |
| HTTPS 默认端口 | 443 | process.cpp |
...
| 场景 | 错误码 | 客户端响应 | 源 |
|---|---|---|---|
| 头部读取期间读取错误 | `fault::code::io_error` | 无（静默关闭） | `conn.cpp` 第 30 行 |
| 解析错误（格式错误的请求） | `fault::code::parse_error` | 无（静默关闭） | `conn.cpp` 第 38 行 |
| 认证失败（无凭据） | `fault::code::auth_failed` | `407 需要代理认证` | `conn.cpp` 第 48 行 |
| 认证失败（密码错误） | `fault::code::auth_failed` | `403 禁止访问` | `conn.cpp` 第 48 行 |
| 上游拨号失败 | `fault::code::*` | `502 网关错误` | `http.cpp` 第 42 行 |
| 响应写入错误 | `fault::code::io_error` | 无（连接已断开） | `conn.cpp` 各处 |

### 8.2 Slowloris 防护

`max_header_size` 常量（65536 字节，位于 `conn.cpp` 第 16 行）防止慢速 HTTP 头部攻击：
...
`analysis::resolve()` 函数检测到此情况（process.cpp）：
...
`analysis::parse()` 函数处理 IPv6 括号语法（process.cpp）：
- 检测 `[` 前缀
- 查找闭合的 `]`
- 提取括号之间的地址
- 检查 `]:` 以找到端口
...
| 方法过短（< 4 字节） | 未被检测为 HTTP | 探测返回 `shadowsocks` |
