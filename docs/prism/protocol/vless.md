...
```
  ...
  [session] -- 预读 24 字节 -> 嗅探协议
    ...
    |
    v
  [session::diversion() switch] -- case protocol_type::vless -> protocol::vless::handle
    |
    v
  [protocol::vless::handle] -- wrap_with_preview (重放预读数据)
    |                     make_relay (创建中继器)
...
这是 `relay::handshake()` 中首次 `read_at_least` 的最小读取量，定义于 `conn.cpp` 第 89 行。
...
握手缓冲区大小为 320 字节 (`conn.cpp` 第 84 行)，留有安全余量。
...
转换逻辑位于 `conn.cpp` 第 23-44 行，使用 `snprintf("%02x")` 逐字节格式化，按 `groups[] = {4, 2, 2, 2, 6}` 分组插入连字符。
...
代码实现 (`conn.cpp` 第 89-179 行) 使用 `read_at_least` 和 `read_remaining` 两个辅助函数，均限制读取的 span 大小以防止过度消费。
...
构建逻辑 (`conn.cpp` 第 280-307 行 `send_vless_udp_response`)：
```cpp
// 从 sender_ep 提取地址
...
framing::build_udp_packet(frame, {response_data}, buf.send);
```
...
`build_udp_packet` 函数 (`framing.cpp` 第 200-240 行) 负责将响应数据封装为 VLESS UDP 帧：
...
```
  relay::handshake()
       ...
       |     +-- fault::failed(read_ec) -> process.cpp 中 trace::warn -> co_return
       |
       ...
       +-- 全部通过 -> {success, request}
             +-- process.cpp 中 switch (req.cmd) 分发
```
...
| 日志级别 | 消息模板 | 触发条件 | 代码位置 |
|----------|----------|----------|----------|
| `warn` | `[Pipeline.Vless] account directory not configured` | account_directory_ptr 为空 | `process.cpp:27` |
| `warn` | `[Pipeline.Vless] credential verification failed` | account::try_acquire 返回空 | `process.cpp:33` |
| `warn` | `[Pipeline.Vless] handshake failed: {error}` | 握手返回失败错误码 | `process.cpp:46` |
| `info` | `[Pipeline.Vless] mux session started` | MUX 会话建立 | `process.cpp:66` |
| `info` | `[Pipeline.Vless] CONNECT -> {host}:{port}` | TCP 连接建立 | `process.cpp:78` |
| `info` | `[Pipeline.Vless] UDP associate started` | UDP 命令开始处理 | `process.cpp:86` |
| `warn` | `[Pipeline.Vless] UDP associate failed: {error}` | UDP 关联失败 | `process.cpp:93` |
| `info` | `[Pipeline.Vless] UDP associate completed` | UDP 关联正常结束 | `process.cpp:97` |
| `warn` | `[Pipeline.Vless] unknown command: {n}` | CMD 值不在枚举范围内 | `process.cpp:102` |
| `debug` | `[Vless.UDP] Idle timeout` | UDP 空闲超时 | `conn.cpp:342` |
| `debug` | `[Vless.UDP] Read error or EOF` | UDP 读取错误或 EOF | `conn.cpp:352` |
| `warn` | `[Vless.UDP] Packet parse failed` | UDP 帧解析失败 | `conn.cpp:359` |
| `debug` | `[Vless.UDP] Route failed` | 目标地址路由失败 | `conn.cpp:371` |
| `debug` | `[Vless.UDP] Write response failed` | UDP 响应写入失败 | `conn.cpp:303` |
