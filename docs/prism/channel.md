...
│   ├── reliable.hpp            # 可靠传输（TCP socket 包装）
│   ├── encrypted.hpp           # 加密传输（TLS stream 装饰器）
│   └── unreliable.hpp          # 不可靠传输（UDP socket 包装）
├── connection/
│   └── pool.hpp                # TCP 连接池（LIFO 缓存 + 健康检查）
└── eyeball/
    └── racer.hpp               # Happy Eyeballs RFC 8305 竞速器

src/prism/channel/
├── transport/
│   ├── reliable.cpp            # TCP 传输实现
│   ├── encrypted.cpp           # TLS 传输实现
│   └── unreliable.cpp          # UDP 传输实现
├── connection/
│   └── pool.cpp                # 连接池实现
└── eyeball/
    └── racer.cpp               # 竞速器实现
