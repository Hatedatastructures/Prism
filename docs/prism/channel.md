# Connect 模块 — 连接管理与传输抽象

> **注意**：代码中目录为 `include/prism/connect/`（连接管理）和 `include/prism/transport/`（传输实现），本文档沿用旧标题 `Channel` 以保持文档链接兼容。

## 1. 模块概述

Connect + Transport 模块是 Prism 的连接管理与传输层抽象基础设施，提供统一的流式传输接口、连接复用池、Happy Eyeballs 并发竞速和双向隧道转发。传输层将具体的传输介质（TCP、UDP、TLS）统一为 `transmission` 抽象，使上层代码无需感知底层传输细节。
...
├── encrypted.hpp               # 加密传输（TLS stream 装饰器）
...
所有具体实现均继承 `transmission` 纯虚基类，通过 `shared_transmission`（即 `shared_ptr<transmission>`）管理生命周期，支持装饰器模式嵌套。隧道转发由 `connect::tunnel::tunnel()` 实现。
...
| 项目 | 详情 |
|------|------|
| 头文件 | `include/prism/transport/transmission.hpp` |
| 命名空间 | `psm::transport` |
...
| 项目 | 详情 |
|------|------|
| 头文件 | `include/prism/connect/pool/pool.hpp` |
| 实现文件 | `src/prism/connect/pool/pool.cpp` |
| 命名空间 | `psm::connect::pool` |
...
struct config                             // 连接池配置 (connect::config)
├── cache_peraddr          : 32           // 单端点最大缓存连接数
├── conn_timeout           : 300          // 连接超时（毫秒）
├── idle_sec               : 30           // 空闲连接最大存活时间（秒）
├── clean_interval         : 10           // 后台清理间隔（秒）
├── recv_bufsz             : 65536        // 接收缓冲区大小
├── send_bufsz             : 65536        // 发送缓冲区大小
...
| 项目 | 详情 |
|------|------|
| 头文件 | `include/prism/connect/dial/racer.hpp` |
| 实现文件 | `src/prism/connect/dial/racer.cpp` |
| 命名空间 | `psm::connect::dial` |
...
### 6.1 Connect/Transport 模块向外依赖

```
connect/transport 模块
├── memory (PMR 分配器, container, pool)
├── fault::code / fault::compatible (错误码体系)
└── boost::asio (网络异步原语)
```

### 6.2 外部模块对 Connect/Transport 的依赖

```
instance::worker ─────────────► connection_pool (每 worker 一个)
instance::session ────────────► shared_transmission (入站/出站)
connect::dial::router ────────► connection_pool (async_acquire)
connect::tunnel ──────────────► transmission (双向转发)
...
| 参数 | 默认值 | 含义 |
|------|--------|------|
| `cache_peraddr` | 32 | 单个目标端点最大缓存连接数 |
| `conn_timeout` | 300 | 新建连接超时（毫秒） |
| `idle_sec` | 30 | 空闲连接最大存活时间（秒） |
| `clean_interval` | 10 | 后台清理间隔（秒） |
| `recv_bufsz` | 65536 | 接收缓冲区大小（字节） |
| `send_bufsz` | 65536 | 发送缓冲区大小（字节） |
