...
### 第一步：下载与安装
**方法一：预编译版本（推荐新手）**
1. 从项目的 [Release 页面](https://github.com/你的用户名/ForwardEngine/releases) 下载对应系统的二进制文件
2. 解压到任意目录
3. 双击 `Forward.exe`（Windows）或 `./forward`（Linux）运行

**方法二：从源码编译（开发者）**
```bash
git clone https://github.com/你的用户名/ForwardEngine.git
cd ForwardEngine
# 具体编译步骤见 README.md
```
...
### 关键模块简介
1. **协议识别**：自动识别 HTTP、SOCKS5、Trojan、Obscura 协议
...
#### Obscura 隧道
- **功能**：基于 WebSocket over TLS 的传输层隧道
- **场景**：穿透严格防火墙，伪装为普通 Web 流量
- **配置**：需要 WebSocket 客户端支持
...
```json
{
  "agent": {
    "connection": {
      ...
      "max_per_endpoint": 10,         // 每个目标最大缓存连接数
      "zombie_check_interval": 30     // 僵尸连接检查间隔（秒）
    }
  }
}
```
...
```json
{
  "trace": {
    ...
    "file": {
      "enable": true,
      "path": "./logs/forward_engine.log",  // 日志文件路径（相对路径）
...
#### 连接池调优
- **高并发场景**：增加 `max_per_endpoint`，缩短 `max_idle_time`
- **低内存场景**：减少 `max_per_endpoint`，延长检查间隔
- **稳定场景**：保持默认值，平衡性能和内存
...
### 当前开发状态
项目的详细开发进度和待办事项见 [progress.md](progress.md)。

#### 已完成功能
- ✅ HTTP/HTTPS 代理（正向代理 + CONNECT 隧道）
- ✅ SOCKS5 代理（无认证，TCP Connect）
- ✅ Trojan 代理（密码验证，TLS 传输）
- ✅ Obscura 隧道（WebSocket over TLS）
- ✅ 智能连接池（TCP 连接复用）
- ✅ 协程驱动的异步架构
- ✅ 完整的日志系统
...
### 开发环境搭建
1. **编译器**：支持 C++23（GCC 13+、Clang 16+、MSVC 2022+）
2. **构建系统**：CMake 3.20+
3. **依赖库**：Boost 1.82+、OpenSSL 3.0+、spdlog、glaze
4. **Windows**：使用 MinGW 工具链，依赖库安装在 `c:/bin`
...
**Q：连接经常断开？**
A：调整连接池的 `max_idle_time`，增加 `max_per_endpoint`。
...
### 学习资源
- [README.md](../README.md)：项目总览和构建指南
- [progress.md](progress.md)：开发进度和规划
- 源码注释：代码中包含详细中文注释
...
### 致谢
感谢以下开源项目的支持：
- [Boost.Asio/Beast](https://www.boost.org/)：异步 I/O 和网络库
