...
- 关键实现：
  ...
  - 不直接依赖 `reactor::worker` 类型，而是依赖 `worker_binding` 回调绑定
...
## 2. reactor 模块
位置：`include/forward-engine/agent/reactor/`、`src/forward-engine/agent/reactor/`

### worker
- 职责：工作线程核心，管理事件循环和资源
- 内部资源组合：
  - `io_context`：单线程事件循环
  - `ngx::channel::source`：TCP 连接池
  - `distribution::router`：路由表
...
- 源码：[worker.hpp](include/forward-engine/agent/reactor/worker.hpp)、[worker.cpp](src/forward-engine/agent/reactor/worker.cpp)
...
- 关键实现：
  ...
  - 会话关闭时通过回调递减活跃会话计数
- 源码：[launch.hpp](include/forward-engine/agent/reactor/launch.hpp)、[launch.cpp](src/forward-engine/agent/reactor/launch.cpp)
...
- 源码：[stats.hpp](include/forward-engine/agent/reactor/stats.hpp)、[stats.cpp](src/forward-engine/agent/reactor/stats.cpp)
...
- 关键实现：
  ...
  - 若未配置证书则返回空指针，运行明文模式
- 源码：[tls.hpp](include/forward-engine/agent/reactor/tls.hpp)、[tls.cpp](src/forward-engine/agent/reactor/tls.cpp)

## 3. connection 模块
位置：`include/forward-engine/agent/connection/`、`src/forward-engine/agent/connection/`

### session
- 职责：单个连接的完整生命周期管理
- 关键实现：
  - 持有 `inbound`/`outbound` transmission
  - 通过 `protocol::sniff::probe` 检测协议（预读 24 字节）
  - 从 `dispatch::registry` 获取 handler
  - 通过 `shared_from_this` 实现异步生命周期保活
  - 支持设置凭证验证器和账户目录
  - 关闭时触发 `on_closed` 回调
- 源码：[session.hpp](include/forward-engine/agent/connection/session.hpp)、[session.cpp](src/forward-engine/agent/connection/session.cpp)
...
## 6. distribution 模块
位置：`include/forward-engine/agent/distribution/`、`src/forward-engine/agent/distribution/`
...
- 源码：[router.hpp](include/forward-engine/agent/distribution/router.hpp)、[router.cpp](src/forward-engine/agent/distribution/router.cpp)
...
- 源码：[arbiter.hpp](include/forward-engine/agent/distribution/arbiter.hpp)
...
- 关键实现：
  ...
  - FIFO 淘汰策略
  - 连接失败时清除对应缓存记录
- 源码：[reliable.hpp](include/forward-engine/agent/distribution/reliable.hpp)、[reliable.cpp](src/forward-engine/agent/distribution/reliable.cpp)

### datagram_resolver
- 职责：UDP DNS 解析、缓存
- 关键实现：
  - 与 `reliable_resolver` 类似的缓存和请求合并机制
  - 仅存储单个端点（UDP 场景通常不需要尝试多个地址）
  - 缓存 TTL 默认 120 秒，最大条目 4096
- 源码：[datagram.hpp](include/forward-engine/agent/distribution/datagram.hpp)、[datagram.cpp](src/forward-engine/agent/distribution/datagram.cpp)
...
- 源码：[coalescer.hpp](include/forward-engine/agent/distribution/coalescer.hpp)
