...
| command::udp | 0x02 | UDP 代理 | ⚠ (命令已解析，中继未实现) |
...
部分实现:
- VLESS UDP 代理 (command::udp)：relay 层已正确解析 UDP 命令并设置 form::datagram，
  但 pipeline 层当前仅记录警告日志，未实现实际的 UDP 数据报中继
```
