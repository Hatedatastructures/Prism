...
`pipeline::http` 使用手动读取循环从 `connector` 中累积原始字节，直到找到 HTTP 头部结束标记 `\r\n\r\n`，然后调用轻量解析器提取代理转发所需信息：

```cpp
// 循环读取直到 \r\n\r\n（HTTP 头部结束标记）
while (true)
{
    const auto sv = std::string_view(buffer.data(), used);
...
// 解析 HTTP 请求头（零分配，结果为 string_view 指向原始缓冲区）
protocol::http::proxy_request req;
protocol::http::parse_proxy_request(raw, req);
```

**解析流程**（[parser.hpp](../../include/prism/protocol/http/parser.hpp)）：
1. **手动读取**：使用 PMR `vector<char>` 缓冲区循环读取，直到找到 `\r\n\r\n`。
...
auto tunnel(shared_transmission inbound, shared_transmission outbound,
            const session_context &ctx) -> net::awaitable<void>
{
  // 使用双缓冲区实现双向转发
  // 任一方向断开即终止隧道
}
...
worker.accept -> session::diversion
  -> pipeline::http
      -> 循环读取原始字节直到 \r\n\r\n (请求头读取)
      -> protocol::http::parse_proxy_request (零分配解析)
      ...
      -> else:
          -> extract_relative_path (绝对 URI → 相对路径)
          -> 分段写入上游: 新请求行 + 原始剩余数据
