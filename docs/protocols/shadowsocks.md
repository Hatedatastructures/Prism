...
- **AES-GCM**：仅支持 AES-128-GCM（16 字节 PSK）和 AES-256-GCM（32 字节 PSK）
- **持续加密**：relay 在整个会话生命周期内保持活跃，所有数据持续加解密

**协议参数**：
- **加密算法**：2022-blake3-aes-128-gcm / 2022-blake3-aes-256-gcm
- **密钥长度**：16 字节（AES-128）或 32 字节（AES-256）
- **认证方式**：预共享密钥（PSK），Base64 编码
- **Salt 长度**：与 PSK 长度一致（16 或 32 字节）
- **AEAD Tag**：16 字节（AES-GCM 固定）
...
SS2022 的 UDP 代理需要独立的 AEAD 分帧和重放保护机制，当前 Prism 仅实现了 TCP 代理。
...
```
当前不支持:
- UDP 代理（SS2022 的 UDP 分帧和重放保护未实现）
- 多路复用（SS2022 是加密传输层，无法叠加 mux）
- Shadowsocks 旧版（stream cipher / AEAD 2022 之前的版本）
- 仅支持 AES-128-GCM 和 AES-256-GCM 两种加密算法
- 不支持 chacha20-poly1305
