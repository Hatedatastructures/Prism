...
2. **Trojan 处理器调用**：已建立的 SSL 流被传递给 `handler::trojan` 函数进行处理。
   入口位置：[handler.hpp](../../include/forward-engine/agent/dispatch/handler.hpp)，命名空间 `ngx::agent::dispatch` 的 `trojan` 模板函数。

3. **Trojan 流对象创建**：创建 `protocol::trojan::relay` 对象，封装 SSL 流和协议处理逻辑。
   位置：[stream.hpp](../../include/forward-engine/protocol/trojan/stream.hpp)，类 `ngx::protocol::trojan::relay`。
