...
2. **Trojan 处理器调用**：已建立的 SSL 流被传递给 `handler::trojan` 函数进行处理。
   入口位置：[handler.hpp](../../include/prism/agent/dispatch/handler.hpp)，命名空间 `ngx::agent::dispatch` 的 `trojan` 模板函数。

3. **Trojan 流对象创建**：创建 `protocol::trojan::relay` 对象，封装 SSL 流和协议处理逻辑。
   位置：[stream.hpp](../../include/prism/protocol/trojan/stream.hpp)，类 `ngx::protocol::trojan::relay`。
...
if (ec) 
{
    co_return std::pair{fault::code::tls_handshake_failed, request{}};
}
```
...
**关键代码位置**：`include/prism/protocol/trojan/stream.hpp` 第 291-331 行。
...
if (verifier_) 
{
    ...
    {
        co_return std::pair<fault::code, header_information>{fault::code::auth_failed, header_information{}};
    }
}
```
...
Trojan 协议支持两种命令（定义在 `include/prism/protocol/trojan/constants.hpp`）：
...
- `[Trojan] TLS handshake started`
  位置：[protocols.cpp](../../src/prism/agent/pipeline/protocols.cpp) 的 `ngx::agent::pipeline::trojan`。
...
关键枚举定义在 `include/prism/protocol/trojan/constants.hpp`：
