...
2. **协议识别**：`session::diversion` 预读并识别协议，然后分流到 HTTP 处理器  
   位置：[session.hpp](../../include/forward-engine/agent/session.hpp)，类 `ngx::agent::session` 的 `diversion`。通过检查请求行前几个字节判断是否为 HTTP 协议（`GET `、`POST `、`CONNECT ` 等）。

3. **HTTP 处理器调用**：`handler::http` 读取并解析 HTTP 请求，确定目标与路由方向  
   位置：[handler.hpp](../../include/forward-engine/agent/handler.hpp)，命名空间 `ngx::agent::handler` 的 `http` 模板函数。
...
5. **上游连接建立**：`handler::connect_upstream` 根据 `forward_proxy` 选择 `route_forward` 或 `route_reverse`  
...
**关键点**：
- 使用 `transport::adaptation::async_write` 保证完整写入。
- 写入后调用 `consume` 清空缓冲区。
...
- `[Handler] HTTP request received: {method} {target}`  
  位置：[handler.hpp](../../include/forward-engine/agent/handler.hpp) 的 `ngx::agent::handler::http`。

- `[Handler] HTTP upstream resolving: forward_proxy={value} host={host} port={port}`  
  位置：[handler.hpp](../../include/forward-engine/agent/handler.hpp) 的 `ngx::agent::handler::http`。

- `[Handler] Sent 200 Connection Established.`  
  位置：[handler.hpp](../../include/forward-engine/agent/handler.hpp) 的 `ngx::agent::handler::http`（CONNECT 请求成功响应后）。

- `[Handler] Forwarding {n} bytes of prefetched data.`  
  位置：[handler.hpp](../../include/forward-engine/agent/handler.hpp) 的 `ngx::agent::handler::http`（预读数据转发时）。
...
worker.accept -> session::diversion
  -> handler::http
      ...
      -> handler::connect_upstream
          -> distributor::route_forward | route_reverse (路由决策与连接建立)
      -> if CONNECT:
