...
`config` 是代理服务的全局配置入口，聚合所有子模块配置。该结构用于初始化 worker、`account::directory` 和 `distribution::router`。
...
**定义位置**: [distribution/router.hpp](include/forward-engine/agent/distribution/router.hpp)
...
**类型定义**: [transport/transmission.hpp](include/forward-engine/transport/transmission.hpp)
