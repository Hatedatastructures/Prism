...
- 功能：轻量级代理协议，通过 UUID 认证，可裸跑或搭配 TLS
- 要求：配置 UUID 凭证，推荐搭配 TLS 使用
- 优势：协议简洁，支持 mux 多路复用，可搭配 REALITY 增强抗探测
...
| 字段 | 默认值 | 说明 |
|------|--------|------|
| `mode` | `fastest` | 解析模式：`first`（首个成功响应）、`fastest`（最低延迟）、`fallback`（顺序尝试） |
...
| 字段 | 说明 |
|------|------|
| `users[].password` | 用户密码（明文，内部转为 SHA224），用于 Trojan/HTTP/SOCKS5 认证 |
| `users[].uuid` | VLESS UUID 字符串，标准格式 `xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx` |
