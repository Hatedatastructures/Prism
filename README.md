...
# 配置 (Release)
cmake -B build_release -DCMAKE_BUILD_TYPE=Release -G "MinGW Makefiles"

# 编译 (首次会自动下载 Boost/BoringSSL/spdlog/glaze，约 2-5 分钟)
...
| **Shadowsocks 2022** | 计划中 | AEAD 2022 加密，抗探测 |
| **VLESS** | 计划中 | 轻量 TLS 内层协议 |
| **Reality** | 计划中 | TLS 指纹伪装 |
...
| **Google Benchmark** | latest | 性能测试（自动拉取） |
...
- SOCKS5 认证模式默认关闭，需配置 `socks5.enable_auth` 开启
- 反向代理路由表暂不支持热更新
