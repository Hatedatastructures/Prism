...
ForwardEngine 是一个基于 **C++23 协程** 与 **Boost.Asio** 构建的高性能服务端代理，支持 `Clash` 连接。

它采用了现代 C++ 的最新特性——**纯协程架构**与**PMR 内存模型**，专为低延迟、高并发的网络转发场景打造。无论是作为代理网关、网络中间件，还是转发节点，ForwardEngine 都能提供极致的性能。
...
| **透明代理** | 握手后双向透传 TCP 字节流，零协议感知开销 |
| **动态识别** | 首包嗅探 HTTP/SOCKS5/Trogan，自动分流处理 |
| **并发解析** | 内部实现 DNS 解析器，多服务器并发查询，取最快响应；支持缓存、负面缓存、IPv6 过滤 |
| **负载均衡** | 基于评分的 Worker 选择，过载检测与滞后机制 |
...
| smux 多路复用 | ✅ | 兼容 Mihomo/xtaci/smux v1，单连接多流复用 |
